...
Future<void> registerAdminServices(GetIt sl) async {
  ...
  if (!sl.isRegistered<PlaceClaimService>()) {
    sl.registerLazySingleton<PlaceClaimService>(
      () => PlaceClaimService(
        supabaseService: sl<SupabaseService>(),
        businessAuthService: sl<BusinessAuthService>(),
      ),
    );
  }
  ...
  sl.registerLazySingleton(() => const AdminIdentityRedactionService());
  ...
  sl.registerLazySingleton(
    () => BhamAdminOperationsService(
      adminAuthService: sl<AdminAuthService>(),
      prefs: sl<SharedPreferencesCompat>(),
      redactionService: sl<AdminIdentityRedactionService>(),
      communityService:
          sl.isRegistered<CommunityService>() ? sl<CommunityService>() : null,
      clubService: sl.isRegistered<ClubService>() ? sl<ClubService>() : null,
      expertiseEventService: sl.isRegistered<ExpertiseEventService>()
          ? sl<ExpertiseEventService>()
          : null,
      listsRepository:
          sl.isRegistered<ListsRepository>() ? sl<ListsRepository>() : null,
      supabaseService: sl<SupabaseService>(),
    ),
  );
  ...
  sl.registerLazySingleton<SignatureHealthAdminService>(
    () => SignatureHealthAdminService(
      ...
      replaySimulationAdminService:
          ReplaySimulationAdminService(intakeRepository: sl()),
  ...
  if (sl.isRegistered<EventLearningSignalService>()) {
    sl.registerLazySingleton<PlanningTruthSurfaceAdminService>(
  ...
  if (sl.isRegistered<SandboxOrchestratorService>() &&
      sl.isRegistered<ImmuneMemoryLedger>() &&
      sl.isRegistered<SecurityCampaignRegistry>() &&
      sl.isRegistered<SecurityCampaignScheduler>() &&
      sl.isRegistered<SecurityScoutCoordinator>() &&
      !sl.isRegistered<SecurityImmuneSystemAdminService>()) {
    sl.registerLazySingleton<SecurityImmuneSystemAdminService>(
      () => SecurityImmuneSystemAdminService(
        ...
        scoutCoordinator: sl<SecurityScoutCoordinator>(),
        governedRunKernel: sl.isRegistered<GovernedRunKernelService>()
            ? sl<GovernedRunKernelService>()
            : null,
        releaseGateService: sl.isRegistered<SecurityKernelReleaseGateService>()
