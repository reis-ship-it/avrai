...
class _UserDetailPageState extends State<UserDetailPage>
  ...
  late final AdminIdentityRedactionService _redactionService =
  ...
  Future<void> _loadUserData() async {
    final service = widget.godModeService ??
        (GetIt.instance.isRegistered<AdminGodModeService>()
            ? GetIt.instance<AdminGodModeService>()
            : null);
    ...
    try {
      final progress = await service.getUserProgress(widget.userId);
      final predictions = await service.getUserPredictions(widget.userId);
  ...
  void _startRealTimeUpdates() {
    ...
    _dataSubscription = service.watchUserData(widget.userId).listen(
      (snapshot) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    final redactedUser = _redactionService.redactActor(widget.userId);
    return AppFlowScaffold(
      title: 'User Detail',
      titleWidget: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('User: ${redactedUser.displayLabel}'),
          Text(
            'AI Data Only',
