...
import 'package:avrai_admin_app/ui/widgets/visualization/three_js_visualization_widget.dart';
...
class _Network3DVisualizationWidgetState
  ...
  Future<void> highlightNode(String userId) async {
    ...
    await _bridge!
        .evaluateJS('window.networkRenderer?.highlightNode($safeUserId)');
  }
...
class _NetworkFallbackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final edge in edges) {
      ...
      final fromPos = Offset(center.dx + from.position.x * scale,
          center.dy + from.position.y * scale);
      final toPos = Offset(
          center.dx + to.position.x * scale, center.dy + to.position.y * scale);
    ...
    for (final node in nodes) {
      final pos = Offset(center.dx + node.position.x * scale,
          center.dy + node.position.y * scale);
