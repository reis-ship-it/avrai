...
class MeshTrustDiagnosticsPanel extends StatelessWidget {
  const MeshTrustDiagnosticsPanel({
    ...
    required this.transportRetentionSnapshot,
    required this.lifecycleVisibilitySnapshot,
  ...
  final Ai2AiTransportRetentionSnapshot transportRetentionSnapshot;
  final ArtifactLifecycleVisibilitySnapshot lifecycleVisibilitySnapshot;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Wrap(
                  ...
                  children: [
                    ...
                    _TrustMetricChip(
                      label: 'DM consumed',
                      value:
                          transportRetentionSnapshot.dmConsumedCount.toString(),
                      color: AppColors.success,
                    ),
                    _TrustMetricChip(
                      label: 'DM failures',
                      value:
                          transportRetentionSnapshot.dmFailureCount.toString(),
                      color: transportRetentionSnapshot.dmFailureCount > 0
                          ? AppColors.error
                          : AppColors.textSecondary,
                    ),
                    _TrustMetricChip(
                      label: 'Community consumed',
                      value: transportRetentionSnapshot.communityConsumedCount
                          .toString(),
                      color: AppColors.success,
                    ),
                    _TrustMetricChip(
                      label: 'Community failures',
                      value: transportRetentionSnapshot.communityFailureCount
                          .toString(),
                      color:
                          transportRetentionSnapshot.communityFailureCount > 0
                              ? AppColors.error
                              : AppColors.textSecondary,
                    ),
                    _TrustMetricChip(
                    ...
                    _TrustMetricChip(
                      ...
                      value: ambientSocialSnapshot.crowdUpgradeCount.toString(),
                    ...
                    _TrustMetricChip(
                      label: 'DNA feedback',
                      value: ambientSocialSnapshot.personalDnaAppliedCount
                          .toString(),
                ...
                const SizedBox(height: 8),
                if (transportRetentionSnapshot.recentEvents.isEmpty)
                  const Text(
                    'No consume telemetry recorded yet.',
                    style: TextStyle(color: AppColors.textSecondary),
                  )
                else
                  Column(
                    children: transportRetentionSnapshot.recentEvents
                        .take(5)
                        .map((event) {
                      final parts = <String>[
                        event.channel.name,
                        event.outcome.name,
                        'deleted ${event.deletedTransportCount}',
                        'remaining ${event.remainingTransportCount}',
                      ];
                      if (event.recipientDeviceId != null &&
                          event.recipientDeviceId!.isNotEmpty) {
                        parts.add('device ${event.recipientDeviceId}');
                      }
                      if (event.errorSummary != null &&
                          event.errorSummary!.isNotEmpty) {
                        parts.add(event.errorSummary!);
                      }
                      return ListTile(
                        dense: true,
                        ...
                        leading: Icon(
                          event.outcome ==
                                  Ai2AiTransportRetentionOutcome.consumed
                        ...
                        title: Text(event.messageId),
                        subtitle: Text(parts.join(' | ')),
                        trailing: Text(
                          '${event.occurredAtUtc.hour.toString().padLeft(2, '0')}:${event.occurredAtUtc.minute.toString().padLeft(2, '0')}',
                        ),
                      );
                    }).toList(growable: false),
                  ),
                if (transportRetentionSnapshot.latestFailureSummary !=
                    null) ...[
                  const SizedBox(height: 8),
                  Text(
                    'Latest failure: ${transportRetentionSnapshot.latestFailureSummary}',
                    style: const TextStyle(color: AppColors.textSecondary),
                  ),
                ],
                const SizedBox(height: 16),
                ...
                Text(
                  [
                    'canonical ${lifecycleVisibilitySnapshot.canonicalEntryCount}',
                    'staging ${lifecycleVisibilitySnapshot.stagingEntryCount}',
                    'archive ${lifecycleVisibilitySnapshot.archiveEntryCount}',
                    'temporary ${lifecycleVisibilitySnapshot.temporaryEntryCount}',
                    lifecycleVisibilitySnapshot.replayUsesDedicatedProject
                        ? 'replay dedicated project'
                        : 'replay isolated namespace',
                  ].join(' | '),
                  style: const TextStyle(color: AppColors.textSecondary),
                ),
                const SizedBox(height: 12),
                if (lifecycleVisibilitySnapshot.entries.isEmpty)
                  const Text(
                    'No lifecycle visibility entries registered yet.',
                    style: TextStyle(color: AppColors.textSecondary),
                  )
                else
                  Column(
                    children: lifecycleVisibilitySnapshot.entries
                        .map(
                          (entry) => ListTile(
                            dense: true,
                            contentPadding: EdgeInsets.zero,
                            title: Text(entry.label),
                            subtitle: Text(_lifecycleEntrySummary(entry)),
                            trailing: Icon(
                              _iconForLifecycleClass(entry.artifactClass),
                              color:
                                  _colorForLifecycleClass(entry.artifactClass),
                            ),
                          ),
                        )
                        .toList(growable: false),
                    ...
                    children: [
                      Text(
                        [
                          ...
                          ambientSocialSnapshot
                              .lastPromotionTrace!.placeVibeLabel,
                          ambientSocialSnapshot
                              .lastPromotionTrace!.socialContext,
                        ].join(' | '),
                      ),
  ...
  String _lifecycleEntrySummary(ArtifactLifecycleVisibilityEntry entry) {
    final parts = <String>[
      entry.familyId,
      '${entry.artifactClass}/${entry.artifactState}',
      entry.retentionMode,
      if (entry.ttlSeconds != null) 'ttl ${_formatDuration(entry.ttlSeconds!)}',
      if (entry.cleanupGraceSeconds != null)
        'grace ${_formatDuration(entry.cleanupGraceSeconds!)}',
      if (entry.deleteOnConsume) 'consume-delete',
      if (entry.cleanupAfterSuccessfulUpload) 'cleanup-on-upload',
      if (entry.deleteWhenSuperseded) 'delete-when-superseded',
      if (entry.trainingEligible) 'training-eligible',
      if (entry.containsRawPersonalPayload) 'raw-personal-bounded',
      if (entry.containsMessageContent) 'message-content',
      if (entry.sourceScope != null && entry.sourceScope!.isNotEmpty)
  ...
  IconData _iconForLifecycleClass(String artifactClass) {
  ...
  Color _colorForLifecycleClass(String artifactClass) {
    switch (artifactClass) {
      case 'canonical':
