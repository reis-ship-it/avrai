  ...
  group('Partnership Profile Flow Integration Tests', () {
    ...
    group('Scenario 1: Get User Partnerships Flow', () {
      test('should get all partnerships for a user', () {
        ...
        expect(
            allPartnerships
                .any((p) => p.type == ProfilePartnershipType.business),
            isTrue);
        expect(
            allPartnerships.any((p) => p.type == ProfilePartnershipType.brand),
            isTrue);
        expect(
            allPartnerships
                .any((p) => p.type == ProfilePartnershipType.company),
            isTrue);
      });
    ...
    group('Scenario 2: Filter Partnerships by Type Flow', () {
      test('should filter partnerships by business type', () {
        ...
        expect(
            businessPartnerships
                .every((p) => p.type == ProfilePartnershipType.business),
            isTrue);
      });
      ...
      test('should filter partnerships by brand type', () {
        ...
        expect(
            brandPartnerships.first.type, equals(ProfilePartnershipType.brand));
      });
      ...
      test('should filter partnerships by company type', () {
        ...
        expect(companyPartnerships.first.type,
    ...
    group('Scenario 4: Profile Visibility Controls Flow', () {
      test('should filter public partnerships only', () {
        ...
        // Act
        final publicPartnerships =
            partnerships.where((p) => p.isPublic).toList();
      ...
      test('should filter active partnerships for profile display', () {
        ...
        final activePublicPartnerships =
            partnerships.where((p) => p.isActive && p.isPublic).toList();

        // Assert
        expect(activePublicPartnerships.length, equals(2));
        expect(activePublicPartnerships.every((p) => p.isActive && p.isPublic),
            isTrue);
      });
    });
  });
