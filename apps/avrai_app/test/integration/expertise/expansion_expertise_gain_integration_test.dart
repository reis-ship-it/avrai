  ...
  group('Expansion Expertise Gain Integration Tests', () {
    ...
    group('End-to-End Expansion Flow', () {
      test('should track event expansion and grant expertise', () async {
        ...
        final updatedUser =
            await expertiseGainService.grantExpertiseFromExpansion(
          user: user,
          expansion: expansion,
          category: category,
        );
    ...
    group('Club Leader Expertise Recognition', () {
      test(
          'should grant expertise to club leaders in all localities where club hosts events',
          () async {
        const clubId = 'club-1';
        const category = 'Coffee';
        final localities = [
    ...
    group('75% Coverage Rule', () {
      test('should grant city expertise when 75% city coverage reached',
          () async {
        const clubId = 'club-1';
        const category = 'Coffee';
        const city = 'Brooklyn';
        ...
        if (expansion != null) {
          final hasReached =
              expansionService.hasReachedCityThreshold(expansion, city);

          if (hasReached) {
            // Grant city expertise
            final cityExpertise =
                await expertiseGainService.checkAndGrantCityExpertise(
              user: user,
              expansion: expansion,
              category: category,
            );
