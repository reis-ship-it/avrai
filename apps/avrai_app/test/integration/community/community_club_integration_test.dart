...
void main() {
  group('Community/Club Integration Tests', () {
    ...
    group('End-to-End: Event → Community → Club', () {
      test('should create community from event, then upgrade to club',
          () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          currentCommunity =
              await communityService.getCommunityById(community.id) ??
                  currentCommunity;
        }
      ...
      test('should preserve all community data when upgrading to club',
          () async {
        // Create community
        final community = await communityService.createCommunityFromEvent(
          event: successfulEvent,
        ...
        expect(club.eventCount, equals(updatedCommunity.eventCount));
        expect(
            club.memberGrowthRate, equals(updatedCommunity.memberGrowthRate));
        expect(club.eventGrowthRate, equals(updatedCommunity.eventGrowthRate));
        expect(
            club.originalLocality, equals(updatedCommunity.originalLocality));
      });
    });
    ...
    group('End-to-End: Organizational Structure Management', () {
      ...
      test('should manage organizational structure: add leaders and admins',
          () async {
    ...
    group('End-to-End: Member Roles and Permissions Flow', () {
      ...
      test(
          'should promote member through roles: Member → Moderator → Admin → Leader',
          () async {
