  ...
  group('Neighborhood Boundary Integration Tests', () {
    ...
    group('End-to-End Border Refinement Flow', () {
      test('should refine soft border based on user behavior', () async {
        ...
        final shouldRefine =
            await boundaryService.shouldRefineBorder('Nolita', 'East Village');
        expect(shouldRefine, isTrue);

        // Step 4: Calculate refinement
        final refinement = await boundaryService.calculateBorderRefinement(
            'Nolita', 'East Village');
        expect(refinement, isNotNull);
        expect(refinement['spotsToAssociate'], isNotEmpty);

        // Step 5: Apply refinement
        await boundaryService.applyBoundaryRefinement(
            'Nolita', 'East Village', refinement);

        // Step 6: Verify boundary was updated
        final updated =
            await boundaryService.getBoundary('Nolita', 'East Village');
      ...
      test('should track complete refinement lifecycle', () async {
        ...
        final initial =
            await boundaryService.getBoundary('Greenpoint', 'Williamsburg');
    ...
    group('Soft Border Spot Association', () {
      test('should associate spots with localities based on visit patterns',
          () async {
        ...
        // Verify associations
        expect(await boundaryService.getSpotLocalityAssociation('spot-1'),
            equals('Nolita'));
        expect(await boundaryService.getSpotLocalityAssociation('spot-2'),
            equals('East Village'));
      });
      ...
      test('should handle spots shared between localities', () async {
        ...
        final visitCounts =
            await boundaryService.getSpotVisitCounts('spot-shared');
    ...
    group('Boundary Updates from User Behavior', () {
      test('should update boundary coordinates based on user movement',
          () async {
        final boundary = NeighborhoodBoundary(
        ...
        for (int i = 0; i < 50; i++) {
          await boundaryService.trackUserMovement(
              'user-${i % 10}', 'spot-1', 'Nolita');
        }
        for (int i = 0; i < 10; i++) {
          await boundaryService.trackUserMovement(
              'user-${i % 5}', 'spot-1', 'East Village');
        }

        // Analyze movement patterns
        final analysis = await boundaryService.analyzeMovementPatterns(
            'Nolita', 'East Village');
        expect(analysis, isNotNull);

        // Update boundary from behavior
        await boundaryService.updateBoundaryFromBehavior(
            'Nolita', 'East Village');
      ...
      test('should track movement patterns for locality', () async {
        ...
        for (int i = 0; i < 30; i++) {
          await boundaryService.trackUserMovement(
              'user-${i % 10}', 'spot-1', 'Nolita');
          await boundaryService.trackUserMovement(
              'user-${i % 10}', 'spot-2', 'Nolita');
        }

        // Get movement patterns
        final patterns =
            await boundaryService.getUserMovementPatterns('Nolita');
      ...
      test('should calculate boundary changes from behavior', () async {
        ...
        // Calculate changes
        final changes = await boundaryService.calculateBoundaryChanges(
            'Nolita', 'East Village');
        ...
        final spotIds =
            spotsToAssociate.map((a) => a['spotId'] as String).toList();
    ...
    group('Integration with Geographic Hierarchy', () {
      test('should integrate boundaries with geographic hierarchy', () async {
        ...
        final boundaries =
            await boundaryService.getBoundariesForLocality('Greenpoint');
        ...
        expect(
          boundaryKeys.any(
              (key) => key == 'Greenpoint_DUMBO' || key == 'DUMBO_Greenpoint'),
          isTrue,
        );
      });
    ...
    group('Integration with Large City Detection', () {
      test('should work with large city neighborhoods', () async {
        ...
        expect(
            largeCityService.getParentCity('Greenpoint'), equals('Brooklyn'));
        expect(
            largeCityService.getParentCity('Williamsburg'), equals('Brooklyn'));
      ...
      test('should handle boundaries in multiple large cities', () async {
        ...
        if (largeCityService.isLargeCity('Los Angeles')) {
          ...
          expect(
              largeCityService.isNeighborhoodLocality('Santa Monica'), isTrue);
        }
    ...
    group('Complete Workflow Integration', () {
      test('should handle complete boundary lifecycle', () async {
        ...
        final boundaries =
            await boundaryService.loadBoundariesFromGoogleMaps('Brooklyn');
        ...
        final visitCount =
            finalBoundary?.userVisitCounts['spot-1']?['Nolita'] ?? 0;
        expect(visitCount, greaterThanOrEqualTo(10),
            reason: 'At least 10 visits should be tracked');

        final association =
            await boundaryService.getSpotLocalityAssociation('spot-1');
