...
void main() {
  group('Event Hosting Integration Tests', () {
    ...
    group('Scenario 6: Event Hosting Flow', () {
      ...
      test('should fail to create event when host lacks Local level expertise',
          () async {
        // Arrange
        final userWithoutExpertise =
            IntegrationTestHelpers.createUserWithoutHosting();
    ...
    group('Expertise Unlock Check Tests', () {
      test('should unlock event hosting at Local level', () {
        ...
        final hasLocalLevel = userAtLocalLevel.expertiseMap.values
            .any((level) => level == ExpertiseLevel.local.name);
        expect(hasLocalLevel, isTrue);
      });

      test('should unlock event hosting at City level (expanded scope)', () {
        // Arrange
        final userAtCityLevel =
            IntegrationTestHelpers.createUserWithCityExpertise();

        // Assert
        expect(userAtCityLevel.canHostEvents(), isTrue);

        // Verify City level expertise exists
        final hasCityLevel = userAtCityLevel.expertiseMap.values
            .any((level) => level == ExpertiseLevel.city.name);
        expect(hasCityLevel, isTrue);
      });

      test('should block event hosting without Local level expertise', () {
        // Arrange
        final userWithoutHosting =
            IntegrationTestHelpers.createUserWithoutHosting();
      ...
      test(
          'should display unlock indicator for users without Local level expertise',
          () {
        // Arrange
        final userWithoutHosting =
            IntegrationTestHelpers.createUserWithoutHosting();
