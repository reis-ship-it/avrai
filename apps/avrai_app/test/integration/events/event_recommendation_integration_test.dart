...
void main() {
  group('Event Recommendation Integration Tests', () {
    ...
    group('End-to-End Recommendation Flow', () {
      ...
      test('should prioritize local experts for users who prefer them',
          () async {
        ...
        final cityEvent = await eventService.createEvent(
          ...
          endTime: DateTime.now().add(const Duration(days: 7, hours: 2)),
          location:
              'San Francisco', // City experts can only host in city, not specific localities
        );
    ...
    group('Tab-Based Filtering', () {
      test('should provide recommendations per tab scope', () async {
        ...
        // Act
        final recommendations =
            await recommendationService.getRecommendationsForScope(
          user: user,
          scope: 'locality',
          maxResults: 20,
        );
