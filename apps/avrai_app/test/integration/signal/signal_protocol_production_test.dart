...
void main() {
  group('Signal Protocol Production Integration', () {
    ...
    test('should initialize Signal Protocol in production-like scenario',
        () async {
    ...
    test(
        'should perform multiple operations without disposal (production-like)',
        () async {
      if (!librariesAvailable) {
        return; // Skip if libraries not available
      }
    ...
    test('should handle errors gracefully without crashing (production-like)',
        () async {
      if (!librariesAvailable) {
        return; // Skip if libraries not available
      }
      ...
      try {
        ...
        expect(
            e.toString(),
            anyOf(
                contains('session'), contains('error'), contains('Exception')));
      }
