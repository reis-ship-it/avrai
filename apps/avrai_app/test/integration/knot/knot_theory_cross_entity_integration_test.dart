...
void main() {
  ...
  group('Knot Theory Cross-Entity Integration Tests', () {
    ...
    group('End-to-End: Person Discovers Event', () {
      test(
          'should generate knots and calculate compatibility for person-event match',
          () async {
        ...
        final compatibility =
            await compatibilityService.calculateIntegratedCompatibility(
          entityA: personKnot,
          entityB: eventKnot,
        );
        ...
        expect(
            personKnot.knot.invariants.crossingNumber, greaterThanOrEqualTo(0));
        expect(
            eventKnot.knot.invariants.crossingNumber, greaterThanOrEqualTo(0));
      });

      test('should discover events through network cross-pollination',
          () async {
        final personProfile = PersonalityProfile.initial(
          'person-2',
          userId: 'user-2',
        ...
        final discoveredPaths =
            await networkService.findCrossEntityDiscoveryPaths(
    ...
    group('End-to-End: Person Discovers Place', () {
      test(
          'should generate knots and calculate compatibility for person-place match',
          () async {
        final personProfile = PersonalityProfile.initial(
          'person-3',
          userId: 'user-3',
    ...
    group('End-to-End: Person Discovers Company', () {
      test(
          'should generate knots and calculate compatibility for person-company match',
          () async {
        final personProfile = PersonalityProfile.initial(
          'person-4',
          userId: 'user-4',
    ...
    group('End-to-End: Multi-Entity Weave Compatibility', () {
      test('should calculate compatibility for person-event-place combination',
          () async {
        final personProfile = PersonalityProfile.initial(
          'person-5',
          userId: 'user-5',
        ...
        final weaveCompatibility =
            await compatibilityService.calculateMultiEntityWeaveCompatibility(
      ...
      test('should calculate path compatibility for discovery path', () async {
        ...
        final pathCompatibility =
            await networkService.calculatePathCompatibility([
          personKnot1,
          personKnot2,
          eventKnot,
        ]);
    ...
    group('End-to-End: Cross-Entity Discovery Workflow', () {
      test('should complete full discovery workflow: person → event → place',
          () async {
        ...
        final personEventCompatibility =
            await compatibilityService.calculateIntegratedCompatibility(
          entityA: personKnot,
          entityB: eventKnot,
        );
        ...
        final personPlaceCompatibility =
            await compatibilityService.calculateIntegratedCompatibility(
          entityA: personKnot,
          entityB: placeKnot,
        );
    ...
    group('End-to-End: Entity Type Coverage', () {
      test('should handle all supported entity types in single workflow',
          () async {
        ...
        final allEntitiesCompatibility =
            await compatibilityService.calculateMultiEntityWeaveCompatibility(
        ...
        expect(
            eventKnot.knot.invariants.crossingNumber, greaterThanOrEqualTo(0));
        expect(
            placeKnot.knot.invariants.crossingNumber, greaterThanOrEqualTo(0));
        expect(companyKnot.knot.invariants.crossingNumber,
    ...
    group('End-to-End: Error Handling and Edge Cases', () {
      test('should handle empty entity list in multi-entity compatibility',
          () async {
        final compatibility =
            await compatibilityService.calculateMultiEntityWeaveCompatibility(
          entities: [],
      ...
      test('should handle single entity in path compatibility', () async {
        ...
        final pathCompatibility =
            await networkService.calculatePathCompatibility([personKnot]);

        expect(pathCompatibility, equals(1.0));
      });

      test(
          'should handle network discovery with unsupported entity type gracefully',
          () async {
        final personProfile = PersonalityProfile.initial(
          'person-11',
          userId: 'user-11',
