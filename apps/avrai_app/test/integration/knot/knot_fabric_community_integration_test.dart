  ...
  group('Knot Fabric Community Integration Tests', () {
    ...
    setUpAll(() async {
      try {
        ...
        geographicExpansionService =
            GeographicExpansionService(); // Create directly
    ...
    group('Knot Fabric Generation from Community', () {
      test('should generate fabric from community member knots', () async {
        ...
        // ignore: unused_local_variable - May be used in callback or assertion
        final community = await _createTestCommunity(
            memberKnots.map((k) => k.agentId).toList());
      ...
      test('should calculate fabric invariants for community', () async {
        ...
        final invariants =
            await knotFabricService.calculateFabricInvariants(fabric);
    ...
    group('Community Health Metrics from Fabric', () {
      test('should get community health metrics from fabric', () async {
        ...
        final event =
            _createTestEvent(memberKnots.map((k) => k.agentId).toList());
        final createdCommunity =
            await communityService.createCommunityFromEvent(
          event: event,
          minAttendees:
              memberKnots.length, // Adjust to match actual member count
        );

        // Act: Get community health from fabric
        final health =
            await communityService.getCommunityHealth(createdCommunity.id);
    ...
    group('Community Discovery from Fabric', () {
      test('should discover communities from fabric clusters', () async {
        ...
        final discoveredCommunities =
            await communityService.discoverCommunitiesFromFabric(
    ...
    group('Fabric Evolution Tracking', () {
      test('should track fabric changes when community members change',
          () async {
        // Arrange: Create initial fabric
        final initialKnots = await _createTestMemberKnots(4);
        final initialFabric =
            await knotFabricService.generateMultiStrandBraidFabric(
    ...
    group('Fabric-Based Community Insights', () {
      ...
      test('should identify community cohesion from fabric stability',
          () async {
    ...
    group('Error Handling', () {
      ...
      test('should handle empty community gracefully', () async {
        ...
        final testAttendees =
            List.generate(5, (i) => 'attendee_$i'.padRight(10, '_'));
