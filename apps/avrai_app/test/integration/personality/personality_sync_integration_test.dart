  ...
  group('Personality Sync Integration Tests', () {
    ...
    group('Password Update Flow', () {
      test('should re-encrypt profile when password changes', () async {
        final userId =
            'test_user_pwd_change_${DateTime.now().millisecondsSinceEpoch}';
        final oldPassword =
            'old_password_${DateTime.now().millisecondsSinceEpoch}';
        final newPassword =
            'new_password_${DateTime.now().millisecondsSinceEpoch + 1}';
    ...
    group('Conflict Resolution Integration', () {
      test('should detect when cloud profile is newer', () async {
        final userId =
            'test_user_cloud_newer_${DateTime.now().millisecondsSinceEpoch}';
        ...
        final cloudProfile =
            PersonalityProfile.initial(agentId, userId: userId).evolve(
      ...
      test('should detect when local profile is newer', () async {
        final userId =
            'test_user_local_newer_${DateTime.now().millisecondsSinceEpoch}';
    ...
    group('End-to-End Sync Flow', () {
      test('should sync profile after evolution', () async {
        final userId =
            'test_user_sync_evolve_${DateTime.now().millisecondsSinceEpoch}';
    ...
    group('Cross-Device Scenario', () {
      test('should load profile from cloud on new device', () async {
        final userId =
            'test_user_cloud_load_${DateTime.now().millisecondsSinceEpoch}';
        final password =
            'test_password_${DateTime.now().millisecondsSinceEpoch}';
      ...
      test('should handle profile evolution on multiple devices', () async {
        final userId =
            'test_user_multi_device_${DateTime.now().millisecondsSinceEpoch}';
        final password =
            'test_password_${DateTime.now().millisecondsSinceEpoch}';
