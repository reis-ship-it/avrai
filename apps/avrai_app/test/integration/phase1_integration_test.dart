...
void main() {
  group('Phase 1.3 Integration Tests', () {
    ...
    testWidgets('Streaming Response Widget displays text stream',
        (tester) async {
    ...
    test('AI Thinking Stage enum has all expected values', () {
      ...
      expect(AIThinkingStage.values,
          contains(AIThinkingStage.analyzingPersonality));
      expect(
          AIThinkingStage.values, contains(AIThinkingStage.consultingNetwork));
      expect(
          AIThinkingStage.values, contains(AIThinkingStage.generatingResponse));
    });
  });
