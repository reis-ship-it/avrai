  ...
  group('AgentInitializationController Integration Tests', () {
    ...
    setUpAll(() async {
      ...
      await onboardingService.saveOnboardingData(
          testUserId, onboardingDataWithAgentId);
    });
    ...
    test('should continue even if social media data collection fails',
        () async {
      // Act
      final result = await controller.initializeAgent(
        userId: testUserId,
    ...
    test('should continue even if preferences profile initialization fails',
        () async {
    ...
    test('should return error if agentId cannot be retrieved', () async {
      ...
      final invalidUserId =
          'invalid_user_${DateTime.now().millisecondsSinceEpoch}';
    ...
    group('AVRAI Core System Integration', () {
      test(
          'should create 4D quantum location state when location timing service available',
          () async {
      ...
      test(
        ...
        expect(result.personalityProfile, isNotNull,
            reason: 'Personality profile should be created');
        expect(result.preferencesProfile, isNotNull,
            reason: 'Preferences profile should be created');
