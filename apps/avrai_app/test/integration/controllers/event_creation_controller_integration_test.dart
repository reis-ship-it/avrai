...
void main() {
  group('EventCreationController Integration Tests', () {
    ...
    group('createEvent', () {
      test(
          'should successfully create event with valid data and Local expertise',
          () async {
        // Arrange
      ...
      test('should return failure for invalid dates (past start time)',
          () async {
        // Arrange
        final host = IntegrationTestHelpers.createUserWithLocalExpertise(
          id: 'host-5',
        ...
        expect(result.validationErrors!.generalErrors,
            contains('Start time must be in the future'));
        expect(result.event, isNull);
      });

      test('should return failure for invalid dates (end before start)',
          () async {
        // Arrange
        final host = IntegrationTestHelpers.createUserWithLocalExpertise(
          id: 'host-6',
        ...
        final formData = EventFormData(
          ...
          startTime: DateTime.now().add(const Duration(days: 2)),
          endTime:
              DateTime.now().add(const Duration(days: 1)), // End before start
          location: 'Greenpoint, Brooklyn',
          locality: 'Greenpoint',
        );
      ...
      test('should return failure for invalid form data (empty title)',
          () async {
        // Arrange
        final host = IntegrationTestHelpers.createUserWithLocalExpertise(
          id: 'host-7',
        ...
        expect(result.validationErrors, isNotNull);
        expect(result.validationErrors!.fieldErrors['title'],
            equals('Title is required'));
        expect(result.event, isNull);
      });

      test(
          'should successfully create event with City level expertise in same city',
          () async {
    ...
    group('AVRAI Core System Integration', () {
      test(
          'should create 4D quantum states when location timing service available',
          () async {
        // Arrange
        final host = IntegrationTestHelpers.createUserWithLocalExpertise(
          id: 'host-avrai-1',
      ...
      test('should handle 4D quantum state creation failure gracefully',
          () async {
        ...
        expect(result.isSuccess, isTrue,
            reason: 'Should succeed even if 4D quantum creation fails');
        // The controller should handle quantum state creation failures gracefully
      });

      test('should handle fabric creation for group events gracefully',
          () async {
        ...
        // Assert
        expect(result.isSuccess, isTrue,
            reason: 'Should succeed for group events');
        expect(result.event, isNotNull, reason: 'Event should be created');
