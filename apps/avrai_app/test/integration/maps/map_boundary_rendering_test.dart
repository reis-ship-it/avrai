...
void main() {
  ...
  group('Map Boundary Rendering Integration Tests', () {
    ...
    group('Boundary Loading Workflow', () {
      test(
          'should load locality polygon boundaries and convert to both map types correctly',
          () async {
        ...
        if (poly != null && poly.rings.isNotEmpty) {
          ...
          final googlePolygon =
              MapBoundaryConverter.toGoogleMapsPolygon(boundary);

          // Step 4: Convert to flutter_map type
          final flutterPolygon =
              MapBoundaryConverter.toFlutterMapPolygon(boundary);
      ...
      test(
          'should load city geohash3 tile boundaries and convert to both map types correctly',
          () async {
        ...
        if (tiles.isNotEmpty) {
          ...
          for (int i = 0; i < boundaries.length; i++) {
            expect(googlePolygons[i].points.length,
                boundaries[i].outerRing.length);
            expect(flutterPolygons[i].points.length,
                boundaries[i].outerRing.length);
          }
        } else {
      ...
      test(
          'should load neighborhood boundaries and convert to both map types correctly',
          () async {
        ...
        if (boundaries.isNotEmpty) {
          ...
          final unifiedBoundaries = boundaries
              .map((b) {
                ...
                return MapBoundary.polyline(
                  ...
                  points: points,
                  strokeWidth:
                      b.boundaryType == BoundaryType.hardBorder ? 4 : 3,
                  strokeColor: '0000FF',
                  opacity:
                      b.boundaryType == BoundaryType.hardBorder ? 1.0 : 0.7,
                );
              })
              .whereType<MapBoundary>()
              .toList();
          ...
          for (int i = 0; i < unifiedBoundaries.length; i++) {
            expect(googlePolylines[i].points.length,
                unifiedBoundaries[i].outerRing.length);
            expect(flutterPolylines[i].points.length,
                unifiedBoundaries[i].outerRing.length);
          }
        } else {
    ...
    group('Boundary Priority Hierarchy', () {
      test(
          'should prioritize locality polygons over city tiles when both are available',
          () async {
      ...
      test(
          'should prioritize city tiles over geohash overlays when locality polygon unavailable',
          () async {
      ...
      test(
          'should fallback through boundary loading hierarchy correctly when services fail',
          () async {
    ...
    group('Error Handling', () {
      test('should handle service errors gracefully and clear boundaries',
          () async {
