  ...
  group('ContinuousLearningProgressWidget Widget Tests', () {
    ...
    group('Rendering', () {
      testWidgets('widget displays correctly', (WidgetTester tester) async {
        ...
        await tester
            .pump(const Duration(milliseconds: 500)); // Wait for data load
      ...
      testWidgets('displays progress for all 10 learning dimensions',
        ...
        final emptyStateFinder =
            find.textContaining('No progress data available');
        final headerFinder = find.textContaining('Learning Progress');
        expect(
            emptyStateFinder.evaluate().isNotEmpty ||
                headerFinder.evaluate().isNotEmpty,
            isTrue,
            reason:
                'Widget should display either empty state or progress header');
      });
    });

    group('Progress Bars', () {
      testWidgets('shows widget displays correctly without active learning',
          (WidgetTester tester) async {
    ...
    group('Improvement Metrics', () {
      testWidgets('displays improvement metrics', (WidgetTester tester) async {
        ...
        final emptyStateFinder2 =
            find.textContaining('No progress data available');
        final averageProgressFinder = find.textContaining('Average Progress');
        expect(
            emptyStateFinder2.evaluate().isNotEmpty ||
                averageProgressFinder.evaluate().isNotEmpty,
            isTrue,
            reason:
                'Widget should display either empty state or average progress');
      });
    });
    ...
    group('Loading States', () {
      testWidgets('shows loading indicator while fetching data',
        ...
        expect(find.byType(CircularProgressIndicator), findsNothing,
            reason:
                'Loading should complete after getLearningProgress() returns');
      });
    });

    group('Error Handling', () {
      testWidgets('displays error message when backend fails',
          (WidgetTester tester) async {
    ...
    group('Backend Integration', () {
      testWidgets('calls ContinuousLearningSystem.getLearningProgress()',
          (WidgetTester tester) async {
        ...
        expect(find.byType(CircularProgressIndicator), findsNothing,
            reason:
                'Loading should complete after getLearningProgress() is called');
      });
    });
  });
}
