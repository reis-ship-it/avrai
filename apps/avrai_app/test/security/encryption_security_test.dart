...
void main() {
  group('Encryption Security Tests', () {
    ...
    test('Same plaintext produces different ciphertexts (non-deterministic)',
        () async {
      const userId = 'test-user';
      const agentId = 'agent_test_abc123';
      ...
      expect(encrypted1.encryptedBlob, isNot(equals(encrypted2.encryptedBlob)),
          reason:
              'Same plaintext should produce different ciphertexts (non-deterministic)');
    });
    ...
    test('Key ID is unique per user', () async {
      ...
      expect(
          encrypted1.encryptionKeyId, isNot(equals(encrypted2.encryptionKeyId)),
