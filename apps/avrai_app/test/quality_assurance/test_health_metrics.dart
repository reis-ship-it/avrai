...
class TestHealthMetrics {
  ...
  static Future<TestHealthScore> calculateHealthScore() async {
    ...
    return TestHealthScore(
      ...
      overallScore:
          _calculateOverallScore(structure, coverage, quality, maintenance),
    );
  }
  ...
  static Future<double> _analyzeTestStructure() async {
    ...
    final structureAlignment =
        _calculateStructureAlignment(libStructure, testStructure);
  ...
  static Future<double> _analyzeCoverage() async {
    try {
      ...
      for (final module in criticalModules) {
        ...
        criticalScore +=
            (moduleCoverage >= 95.0) ? 0.75 : (moduleCoverage / 95.0) * 0.75;
      }
      score += criticalScore;
      ...
      score +=
          (integrationCoverage >= 85.0) ? 1.0 : (integrationCoverage / 85.0);
  ...
  static Future<double> _analyzeTestReliability() async {
    try {
      ...
      for (int i = 0; i < runs; i++) {
        final result =
            await Process.run('flutter', ['test', '--reporter=json']);
  ...
  static double _calculateOverallScore(
      double structure, double coverage, double quality, double maintenance) {
    // Weighted scoring for deployment readiness
    return (structure * 0.25 +
        coverage * 0.30 +
        quality * 0.30 +
        maintenance * 0.15);
  }
  ...
  static double _calculateStructureAlignment(
      List<String> libStructure, List<String> testStructure) {
  ...
  static Future<double> _analyzeEdgeCaseCoverage() async {
    ...
    for (final file in testFiles) {
      ...
      if (content.contains('null') || content.contains('empty')) {
        edgeCaseScore += 0.1;
      }
      if (content.contains('error') || content.contains('exception')) {
        edgeCaseScore += 0.1;
      }
      if (content.contains('boundary') || content.contains('limit')) {
        edgeCaseScore += 0.1;
      }
      if (content.contains('invalid') || content.contains('malformed')) {
        edgeCaseScore += 0.1;
      }
    }
  ...
  static Future<double> _analyzeTestEvolution() async {
    ...
    try {
      final result =
          await Process.run('git', ['log', '--oneline', '--', 'test/']);
  ...
  static double _analyzeSingleFileMockUsage(String content) {
    ...
    if (content.contains('import') &&
        (content.contains('mockito') || content.contains('mocktail'))) {
      score += 0.3;
    }
