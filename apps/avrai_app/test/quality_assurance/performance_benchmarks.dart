...
class TestPerformanceBenchmarks {
  ...
  static Future<FullSuitePerformance> _benchmarkFullSuite() async {
    ...
    return FullSuitePerformance(
      ...
      optimizationSuggestions:
          _generateOptimizationSuggestions(stopwatch.elapsedMilliseconds),
    );
  }
  ...
  static Future<MemoryUsageAnalysis> _analyzeMemoryUsage() async {
    ...
    return MemoryUsageAnalysis(
      ...
      efficiencyScore:
          _calculateMemoryEfficiency(baseline, peakUsage, averageUsage),
  ...
  static Future<ConcurrencyMetrics> _analyzeConcurrencyEfficiency() async {
    ...
    return ConcurrencyMetrics(
      ...
      recommendations: _generateConcurrencyRecommendations(
          currentEfficiency, concurrencyBottlenecks),
    );
  }
  ...
  static String _gradeIntegrationTestPerformance(
      List<TestFilePerformance> results) {
    final avgTime = _calculateAverageTimePerTest(results);
  ...
  static List<String> _generateOptimizationSuggestions(int totalTimeMs) {
    ...
    if (totalTimeMs > MAX_TOTAL_SUITE_MINUTES * 60 * 1000) {
      suggestions
          .add('Consider parallel test execution to reduce total suite time');
      suggestions.add('Profile slow tests and optimize or split them');
      suggestions.add('Implement test sharding for CI/CD pipeline');
    }

    suggestions
        .add('Use test groups to organize related tests for better execution');
  ...
  static List<String> _generateMemoryRecommendations(
      double peakUsage, List<String> leaks) {
    final recommendations = <String>[];

    if (peakUsage > MAX_TOTAL_MEMORY_MB) {
      recommendations
          .add('Optimize memory usage in tests - peak usage too high');
  ...
  static List<String> _generateConcurrencyRecommendations(
      double efficiency, List<String> bottlenecks) {
    final recommendations = <String>[];

    if (efficiency < MIN_CPU_EFFICIENCY) {
      recommendations.add('Improve parallel test execution efficiency');
      recommendations
          .add('Reduce test dependencies that prevent parallelization');
    }

    if (bottlenecks.isNotEmpty) {
      recommendations
          .add('Address concurrency bottlenecks: ${bottlenecks.join(', ')}');
    }

    recommendations
        .add('Use isolated test data to enable better parallelization');
  ...
  static double _calculateAverageTimePerTest(
      List<TestFilePerformance> results) {
    if (results.isEmpty) return 0.0;

    final totalTests = results.fold(0, (sum, file) => sum + file.testCount);
    final totalTime =
        results.fold(0, (sum, file) => sum + file.executionTimeMs);
  ...
  static Future<double> _getBaselineMemoryUsage() async =>
      15.0; // MB (placeholder)
  static Future<double> _getPeakMemoryUsage() async => 75.0; // MB (placeholder)
  static Future<double> _getAverageMemoryUsage() async =>
      35.0; // MB (placeholder)
  ...
  static double _calculateMemoryEfficiency(
      double baseline, double peak, double average) {
  ...
  static Future<int> _countParallelizableTests() async {
    ...
    final totalTests = _countTotalTests();
    return (totalTests * 0.8)
        .round(); // 80% parallelizable (placeholder estimate)
  }
...
class TestCategoryPerformance {
  ...
  Map<String, dynamic> toJson() => {
        ...
        'slowestFile': files.isNotEmpty
            ? files
                .reduce((a, b) => a.executionTimeMs > b.executionTimeMs ? a : b)
                .filePath
            : null,
      };
}
