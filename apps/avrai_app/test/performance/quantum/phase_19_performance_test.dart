  ...
  group('Phase 19 Performance Tests', () {
    ...
    group('Entanglement Calculation Performance', () {
      test('should calculate entanglement for ≤10 entities in < 200ms',
        ...
        final result = await entanglementService.createEntangledState(
      ...
      test('should handle 100+ entities with dimensionality reduction',
        ...
        expect(stopwatch.elapsedMilliseconds, lessThan(2000),
            reason:
                'Entanglement calculation for 100 entities took ${stopwatch.elapsedMilliseconds}ms');
      });
    });
    ...
    group('Matching Controller Performance', () {
      test('should perform matching with multiple entities efficiently',
          () async {
        ...
        expect(stopwatch.elapsedMilliseconds, lessThan(500),
            reason:
                'Matching took ${stopwatch.elapsedMilliseconds}ms, target: < 500ms');
      });
      ...
      test('should handle concurrent matching requests', () async {
        ...
        final users = List.generate(
            10,
            (i) => IntegrationTestHelpers.createUserWithLocalExpertise(
        ...
        final events = users
            .map((user) => IntegrationTestHelpers.createTestEvent(
                  host: user,
        ...
        expect(stopwatch.elapsedMilliseconds, lessThan(2000),
            reason:
                '10 concurrent matches took ${stopwatch.elapsedMilliseconds}ms');
      });
    });

    group('Memory Usage', () {
      test('should not leak memory during repeated matching operations',
          () async {
        // Arrange
        final user = IntegrationTestHelpers.createUserWithLocalExpertise(
          id: 'user-memory-1',
    ...
    group('Scalability Tests', () {
      test('should handle N-way matching with increasing entity count',
          () async {
        ...
        expect(lastResult.duration.inMilliseconds, lessThan(1000),
            reason:
                '20-entity matching took ${lastResult.duration.inMilliseconds}ms');
      });
    });
  });
}
