...
void main() {
  group('Signal Framework Loading Tests', () {
    test(
        'should load SignalFFI framework using DynamicLibrary.process() on macOS',
        () {
      // Only run on macOS
      if (!Platform.isMacOS) {
        // ignore: avoid_print
        print(
            '⏭️  Skipping macOS framework test on ${Platform.operatingSystem}');
        return;
      }
      ...
      try {
        ...
        try {
          ...
          final symbol =
              lib.lookup<NativeFunction<Void Function()>>('signal_init_logger');
          ...
          // ignore: avoid_print
          print(
              '   This is normal - framework needs to be embedded in app bundle for symbols to be available');
        }
    ...
    test('should verify framework structure exists', () {
      ...
      expect(signalFFI.existsSync(), isTrue,
          reason: 'SignalFFI binary should exist');
      expect(headers.existsSync(), isTrue,
          reason: 'Headers directory should exist');
      expect(modules.existsSync(), isTrue,
          reason: 'Modules directory should exist');
      ...
      expect(headerFile.existsSync(), isTrue,
          reason: 'SignalFFI.h header should exist');

      // Check for module map
      final moduleMap = File('$frameworkPath/Modules/module.modulemap');
      expect(moduleMap.existsSync(), isTrue,
          reason: 'module.modulemap should exist');

      // Verify library type
      final libType = Process.runSync('file', [signalFFI.path]);
      expect(libType.exitCode, equals(0));
      expect(libType.stdout.toString(),
          contains('dynamically linked shared library'));
