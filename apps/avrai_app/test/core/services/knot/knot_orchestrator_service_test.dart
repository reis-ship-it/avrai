...
class MockPersonalityKnotService extends Mock
    implements PersonalityKnotService {}

class MockKnotStorageService extends Mock implements KnotStorageService {}

class MockKnotEvolutionCoordinatorService extends Mock
    implements KnotEvolutionCoordinatorService {}
...
void main() {
  group('KnotOrchestratorService', () {
        ...
        braid: MultiStrandBraid(
          ...
          userToStrandIndex: Map.fromEntries(
            userKnots
                .asMap()
                .entries
                .map((e) => MapEntry(e.value.agentId, e.key)),
          ),
        ),
    ...
    group('Individual Knot Operations', () {
      test(
          'should generate and save knot, get or generate knot (load from storage or generate if missing), and handle profile evolution',
          () async {
        ...
        verify(() => mockStorageService.saveKnot('agent-1', testKnot))
            .called(1);
        ...
        when(() => mockCoordinator.handleProfileEvolution(
            'user-1', evolvedProfile)).thenAnswer((_) async => Future.value());

        await service.handleProfileEvolution('user-1', evolvedProfile);

        verify(() => mockCoordinator.handleProfileEvolution(
            'user-1', evolvedProfile)).called(1);
      });
    });

    group('String Operations', () {
      test(
          'should create user string and get knot at specific time from string',
          () async {
        ...
        final knotAtTime = await service.getUserKnotAtTime(
            'agent-1', DateTime(2025, 1, 1, 12));
      ...
      test('should return null when string cannot be created', () async {
        ...
        final string = await service.createUserString('agent-1');
        final knotAtTime =
            await service.getUserKnotAtTime('agent-1', DateTime(2025, 1, 1));
    ...
    group('Group/Fabric Operations', () {
      test(
          'should create fabric for group, get or create fabric (load from storage or create if missing), and create fabric snapshot',
          () async {
        ...
        verify(() => mockStorageService.saveFabric('group-1', testFabric))
            .called(1);
        ...
        verify(() => mockStorageService.saveFabricSnapshot('group-1', any()))
            .called(1);
      });

      test('should throw error when creating fabric with no knots found',
          () async {
        // Test business logic: error handling
