...
import 'package:avrai_runtime_os/data/datasources/remote/mapkit_places_datasource.dart';
...
void main() {
  group('MapKitPlacesDataSource', () {
    test(
        'searchPlaces maps MapKit items to Spot with apple_places source and null googlePlaceId',
        () async {
    ...
    test(
        'searchNearbyPlaces maps MapKit items to Spot with apple_places metadata',
        () async {
      final channel = _FakeMapKitSearchChannel(
        searchNearbyResult: [
          {
      ...
      final ds = MapKitPlacesDataSource(channel: channel, cache: null);
      final spots =
          await ds.searchNearbyPlaces(latitude: 40.71, longitude: -74.0);
    ...
    test('searchNearbyPlaces returns empty list when channel throws', () async {
      ...
      final ds = MapKitPlacesDataSource(channel: channel, cache: null);
      final spots =
          await ds.searchNearbyPlaces(latitude: 40.0, longitude: -74.0);

      expect(spots, isEmpty);
    });

    test('item without identifier uses fallback id from lat_lon_name hash',
        () async {
      final channel = _FakeMapKitSearchChannel(
        searchResult: [
          {
