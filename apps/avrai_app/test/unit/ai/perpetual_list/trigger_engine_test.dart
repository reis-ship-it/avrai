...
import 'package:avrai_runtime_os/ai/perpetual_list/engines/trigger_engine.dart';
import 'package:avrai_runtime_os/ai/perpetual_list/models/trigger_decision.dart';
...
void main() {
  group('TriggerEngine', () {
    ...
    group('safeguards', () {
      test('should respect 8-hour minimum interval between generations',
          () async {
    ...
    group('cold start detection', () {
      ...
      test('should detect cold start when very long time since last generation',
          () async {
    ...
    group('time-based check-in', () {
      test('should trigger in morning window (8-10am) with no recent lists',
          () async {
      ...
      test('should trigger in evening window (6-8pm) with no recent lists',
          () async {
      ...
      test('should NOT trigger outside time windows with no other triggers',
          () async {
    ...
    group('location change triggers', () {
      test('should trigger on significant location change (5+ km)', () async {
        ...
        expect(decision.shouldGenerate, isTrue);
        expect(decision.hasReason(TriggerReason.significantLocationChange),
            isTrue);
    ...
    group('AI2AI insight triggers', () {
      test('should trigger on 3+ high-quality AI2AI insights', () async {
        ...
        final context = _createTriggerContext(
          ...
          ai2aiInsights: [
            AI2AIInsightSummary(
                quality: 0.8, type: 'test', receivedAt: DateTime.now()),
            AI2AIInsightSummary(
                quality: 0.75, type: 'test', receivedAt: DateTime.now()),
            AI2AIInsightSummary(
                quality: 0.9, type: 'test', receivedAt: DateTime.now()),
          ],
        );
      ...
      test('should NOT trigger on low-quality AI2AI insights', () async {
        ...
        final context = _createTriggerContext(
          ...
          ai2aiInsights: [
            AI2AIInsightSummary(
                quality: 0.5, type: 'test', receivedAt: DateTime.now()),
            AI2AIInsightSummary(
                quality: 0.4, type: 'test', receivedAt: DateTime.now()),
            AI2AIInsightSummary(
                quality: 0.6, type: 'test', receivedAt: DateTime.now()),
          ],
        );
    ...
    group('poor engagement correction', () {
      test('should trigger on 80%+ dismiss rate', () async {
        ...
        expect(decision.shouldGenerate, isTrue);
        expect(
            decision.hasReason(TriggerReason.poorEngagementCorrection), isTrue);
        expect(decision.priority, equals(TriggerPriority.critical));
      });
    });

    group('priority calculation', () {
      test('should assign critical priority for engagement correction',
          () async {
