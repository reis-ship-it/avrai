  ...
  group('ListsBloc', () {
    ...
    group('LoadLists Event', () {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        expect: () => [
          isA<ListsLoading>(),
          isA<ListsLoaded>()
              .having((state) => state.lists.length, 'lists length',
                  testLists.length)
              .having((state) => state.filteredLists.length,
                  'filtered lists length', testLists.length),
        ],
        verify: (_) {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        expect: () => [
          isA<ListsLoading>(),
          isA<ListsError>().having((state) => state.message, 'message',
              contains('Failed to load lists')),
        ],
      );
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        build: () {
          when(() => mockGetListsUseCase()).thenAnswer((_) => Future.delayed(
    ...
    group('CreateList Event', () {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        expect: () => [
          isA<ListsError>().having((state) => state.message, 'message',
              contains('Failed to create list')),
        ],
        verify: (_) {
          verify(() => mockCreateListUseCase(testList)).called(1);
    ...
    group('UpdateList Event', () {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        expect: () => [
          isA<ListsError>().having((state) => state.message, 'message',
              contains('Failed to update list')),
        ],
        verify: (_) {
          verify(() => mockUpdateListUseCase(testList)).called(1);
          verifyNever(() => mockGetListsUseCase());
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        act: (bloc) => bloc.add(UpdateList(testList)),
        expect: () => [
          isA<ListsError>().having(
              (state) => state.message, 'message', contains('Only curator')),
        ],
      );
    ...
    group('DeleteList Event', () {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        expect: () => [
          isA<ListsError>().having(
              (state) => state.message, 'message', contains('List not found')),
        ],
      );
    ...
    group('SearchLists Event', () {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        expect: () => [
          isA<ListsLoaded>()
              .having((state) => state.lists.length, 'original lists length',
                  testLists.length)
              .having((state) => state.filteredLists.length,
                  'filtered lists length', 2)
              .having(
                  (state) => state.filteredLists.every(
                      (list) => list.title.toLowerCase().contains('coffee')),
                  'all filtered lists contain coffee in title',
                  true),
        ],
      );
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        expect: () => [
          isA<ListsLoaded>()
              .having((state) => state.filteredLists.length,
                  'filtered lists length', 1)
              .having((state) => state.filteredLists.first.title,
                  'first filtered list title', 'Pizza Places'),
        ],
      );
        ...
        expect: () => [
          isA<ListsLoaded>()
              .having((state) => state.filteredLists.length,
                  'filtered lists length', 2)
              .having(
                  (state) => state.filteredLists
                      .every((list) => list.category == 'food'),
                  'all filtered lists are food category',
                  true),
        ],
      );
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        act: (bloc) => bloc.add(SearchLists('COFFEE')),
        expect: () => [
          isA<ListsLoaded>().having((state) => state.filteredLists.length,
              'filtered lists length', 2),
        ],
      );
    ...
    group('Role-Based Operations Testing', () {
      blocTest<ListsBloc, ListsState>(
        ...
        build: () {
          ...
          when(() => mockUpdateListUseCase(any()))
              .thenAnswer((_) async => curatorList);
          when(() => mockGetListsUseCase())
              .thenAnswer((_) async => [curatorList]);
          return listsBloc;
        },
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        build: () {
          when(() => mockUpdateListUseCase(any())).thenThrow(
              Exception('Collaborators cannot modify list settings'));
          return listsBloc;
        },
        act: (bloc) => bloc.add(UpdateList(TestDataFactory.createTestList())),
        expect: () => [
          isA<ListsError>().having((state) => state.message, 'message',
              contains('Collaborators cannot')),
        ],
      );
    ...
    group('Collaborative Editing Scenarios', () {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        build: () {
          ...
          when(() => mockGetListsUseCase()).thenAnswer((_) async {
            callCount++;
            return TestDataFactory.createTestLists(
                callCount + 2); // Simulate new lists being added
          });
          return listsBloc;
        },
        act: (bloc) async {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        build: () {
          ...
          when(() => mockGetListsUseCase())
              .thenAnswer((_) async => TestDataFactory.createTestLists(3));
          return listsBloc;
        },
        act: (bloc) {
    ...
    group('Performance and Edge Cases', () {
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        build: () {
          ...
          when(() => mockGetListsUseCase())
              .thenAnswer((_) async => complexLists);
          return listsBloc;
        },
        act: (bloc) => bloc.add(LoadLists()),
        expect: () => [
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        build: () {
          when(() => mockCreateListUseCase(any()))
              .thenAnswer((_) async => TestDataFactory.createTestList());
          when(() => mockGetListsUseCase())
              .thenAnswer((_) async => TestDataFactory.createTestLists(6));
          return listsBloc;
        },
    ...
    group('Privacy and Security Validation', () {
      blocTest<ListsBloc, ListsState>(
        ...
        build: () {
          ...
          when(() => mockCreateListUseCase(any()))
              .thenAnswer((_) async => publicList);
          when(() => mockGetListsUseCase())
              .thenAnswer((_) async => [publicList]);
          return listsBloc;
        },
        act: (bloc) => bloc
            .add(CreateList(TestDataFactory.createTestList(isPublic: true))),
        expect: () => [
          isA<ListsLoading>(),
          isA<ListsLoaded>(),
        ],
      ...
      blocTest<ListsBloc, ListsState>(
        ...
        act: (bloc) => bloc
            .add(CreateList(TestDataFactory.createTestList(isPublic: false))),
        expect: () => [
          isA<ListsLoading>(),
          isA<ListsLoaded>(),
        ],
