...
void main() {
  group('SpotsBloc', () {
    ...
    late MockGetSpotsFromRespectedListsUseCase
    ...
    setUp(() {
      ...
      mockGetSpotsFromRespectedListsUseCase =
          BlocMockFactory.getSpotsFromRespectedListsUseCase;
    ...
    group('LoadSpots Event', () {
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        expect: () => [
          isA<SpotsLoading>(),
          isA<SpotsLoaded>()
              .having((state) => state.spots.length, 'spots length',
                  testSpots.length)
              .having((state) => state.filteredSpots.length,
                  'filtered spots length', testSpots.length)
              .having((state) => state.searchQuery, 'search query', isNull)
              .having((state) => state.respectedSpots.length,
                  'respected spots length', 0),
        ],
        verify: (_) {
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        expect: () => [
          isA<SpotsLoading>(),
          isA<SpotsError>().having((state) => state.message, 'message',
              contains('Failed to load spots')),
        ],
      );
        ...
        expect: () => [
          ...
          isA<SpotsLoaded>()
              .having((state) => state.spots.length, 'spots length',
                  testSpots.length)
              .having((state) => state.respectedSpots.length,
                  'respected spots length', respectedSpots.length),
        ],
        verify: (_) {
          verify(() => mockGetSpotsUseCase()).called(1);
    ...
    group('SearchSpots Event', () {
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        expect: () => [
          isA<SpotsLoaded>()
              .having((state) => state.spots.length, 'original spots length',
                  testSpots.length)
              .having((state) => state.filteredSpots.length,
                  'filtered spots length', 2)
              .having((state) => state.searchQuery, 'search query', 'coffee')
              .having(
                  (state) => state.filteredSpots.every((spot) =>
                  ...
                  'all filtered spots contain query',
                  true),
        ],
      );
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        expect: () => [
          isA<SpotsLoaded>()
              .having((state) => state.filteredSpots.length,
                  'filtered spots length', 1)
              .having((state) => state.filteredSpots.first.name,
                  'first filtered spot name', 'Pizza Restaurant'),
        ],
      );
        ...
        expect: () => [
          isA<SpotsLoaded>()
              ...
              .having(
                  (state) => state.filteredSpots.every((spot) =>
                      spot.category == 'cafe' ||
                      spot.name.toLowerCase().contains('cafe')),
                  'all filtered spots are cafes or contain cafe',
                  true),
        ],
      );
        ...
        expect: () => [
          isA<SpotsLoaded>()
              .having((state) => state.filteredSpots.length,
                  'filtered spots length', 0)
              .having(
                  (state) => state.searchQuery, 'search query', 'nonexistent'),
        ],
      );
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        act: (bloc) => bloc.add(SearchSpots(query: 'COFFEE')),
        expect: () => [
          isA<SpotsLoaded>().having((state) => state.filteredSpots.length,
              'filtered spots length', 2),
        ],
      );
    ...
    group('CreateSpot Event', () {
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        build: () {
          ...
          when(() => mockGetSpotsUseCase())
              .thenAnswer((_) async => existingSpots);
          return spotsBloc;
        },
    ...
    group('UpdateSpot Event', () {
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        expect: () => [
          isA<SpotsError>().having((state) => state.message, 'message',
              contains('Failed to update spot')),
        ],
        verify: (_) {
          verify(() => mockUpdateSpotUseCase(testSpot)).called(1);
          verifyNever(() => mockGetSpotsUseCase());
    ...
    group('DeleteSpot Event', () {
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        act: (bloc) => bloc.add(DeleteSpot(testSpotId)),
        expect: () => [
          isA<SpotsError>().having(
              (state) => state.message, 'message', contains('Spot not found')),
        ],
      );
    });
    ...
    group('State Transitions and Complex Scenarios', () {
      blocTest<SpotsBloc, SpotsState>(
        ...
        build: () {
          ...
          when(() => mockCreateSpotUseCase(any()))
              .thenAnswer((_) async => spotWithCoffee);
          when(() => mockGetSpotsUseCase())
              .thenAnswer((_) async => updatedSpots);
          return spotsBloc;
        },
        act: (bloc) async {
    ...
    group('Performance and Reliability', () {
      ...
      test('State props equality works correctly', () {
        ...
        final spots2 =
            List<Spot>.from(spots1); // same elements, new list instance
    ...
    group('Edge Cases and Error Scenarios', () {
      blocTest<SpotsBloc, SpotsState>(
        ...
        build: () {
          ...
          when(() => mockGetSpotsUseCase())
              .thenAnswer((_) async => [spotWithEmptyProps]);
          return spotsBloc;
        },
        act: (bloc) => bloc.add(LoadSpots()),
        expect: () => [
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        build: () {
          when(() => mockGetSpotsUseCase()).thenAnswer((_) => Future.delayed(
    ...
    group('Integration with Respected Lists', () {
      blocTest<SpotsBloc, SpotsState>(
        ...
        build: () {
          ...
          when(() => mockGetSpotsUseCase())
              .thenAnswer((_) async => regularSpots);
          when(() => mockGetSpotsFromRespectedListsUseCase())
              .thenAnswer((_) async => respectedSpots);
          return spotsBloc;
        },
        ...
        expect: () => [
          ...
          isA<SpotsLoaded>()
              ...
              .having(
                  (state) => state.respectedSpots.length, 'respected spots', 3),
        ],
      );
      ...
      blocTest<SpotsBloc, SpotsState>(
        ...
        expect: () => [
          isA<SpotsLoading>(),
          isA<SpotsLoaded>().having(
              (state) => state.respectedSpots.length, 'respected spots', 0),
        ],
      );
    });
  });
