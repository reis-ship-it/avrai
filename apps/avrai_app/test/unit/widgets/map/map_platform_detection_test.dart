...
void main() {
  group('Map Platform Detection Tests', () {
      ...
      test('should return correct map type based on current platform', () {
        ...
        if (Platform.isAndroid) {
          expect(useGoogleMaps, isTrue,
              reason: 'Android should use Google Maps');
        } else if (Platform.isMacOS ||
            Platform.isWindows ||
            Platform.isLinux ||
            kIsWeb) {
          expect(useGoogleMaps, isFalse,
              reason: 'Desktop/Web should use flutter_map');
        }
    ...
    group('iOS Platform Detection', () {
      test(
          'should respect ENABLE_IOS_GOOGLE_MAPS environment variable when on iOS',
          () {
        ...
        if (Platform.isIOS) {
          ...
          final useGoogleMaps =
              MapPlatformDetectionHelper.shouldUseGoogleMaps();
    ...
    group('Platform Detection Documentation', () {
      test('should document expected map selection behavior for each platform',
        ...
        if (Platform.isAndroid) {
          ...
          expect(useGoogleMaps, isFalse,
              reason:
                  'macOS should use flutter_map (Google Maps not supported)');
        } else if (Platform.isWindows) {
          expect(useGoogleMaps, isFalse,
              reason:
                  'Windows should use flutter_map (Google Maps not supported)');
        } else if (Platform.isLinux) {
          expect(useGoogleMaps, isFalse,
              reason:
                  'Linux should use flutter_map (Google Maps not supported)');
        } else if (kIsWeb) {
          expect(useGoogleMaps, isFalse,
