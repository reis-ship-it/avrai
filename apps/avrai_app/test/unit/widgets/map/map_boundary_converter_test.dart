...
void main() {
  group('MapBoundaryConverter Tests', () {
    group('Google Maps Conversion', () {
      test(
          'should convert polyline boundary to Google Maps Polyline with correct coordinates and styling',
          () {
      ...
      test(
          'should convert polygon boundary to Google Maps Polygon with outer ring and holes',
          () {
        ...
        expect(
            polygon.strokeColor, isNotNull); // Google Maps uses int color value
        expect(
            polygon.fillColor, isNotNull); // Google Maps uses int color value
      });

      test(
          'should throw ArgumentError when converting wrong boundary type (polyline vs polygon)',
          () {
    ...
    group('flutter_map Conversion', () {
      test(
          'should convert polyline boundary to flutter_map Polyline with correct coordinates and styling',
          () {
        // Test behavior: polyline conversion
        final points = [
          const LatLng(40.7128, -74.0060),
      ...
      test(
          'should convert polygon boundary to flutter_map Polygon with outer ring and holes',
          () {
        // Test behavior: polygon conversion with holes
        final outerRing = [
          const LatLng(40.7128, -74.0060),
      ...
      test(
          'should convert polygon without holes to flutter_map Polygon with null holePointsList',
          () {
    ...
    group('Color Parsing', () {
      test(
          'should parse hex colors correctly and apply opacity for 6-digit and 8-digit hex',
          () {
      ...
      test(
          'should parse AppTheme color names (primary, secondary, error) and apply opacity',
          () {
        ...
        final primaryPolyline =
            MapBoundaryConverter.toFlutterMapPolyline(primaryBoundary);
        final secondaryPolyline =
            MapBoundaryConverter.toFlutterMapPolyline(secondaryBoundary);
        final errorPolyline =
            MapBoundaryConverter.toFlutterMapPolyline(errorBoundary);
        ...
        expect(primaryPolyline.color,
            equals(AppTheme.primaryColor.withValues(alpha: 0.8)));
        expect(secondaryPolyline.color,
            equals(AppColors.secondary.withValues(alpha: 0.8)));
        expect(errorPolyline.color,
            equals(AppColors.error.withValues(alpha: 0.8)));
      });
    ...
    group('Round-Trip Conversion', () {
      test(
          'should maintain boundary data integrity when converting between map types',
          () {
        ...
        final googlePolyline =
            MapBoundaryConverter.toGoogleMapsPolyline(boundary);
        final flutterPolyline =
            MapBoundaryConverter.toFlutterMapPolyline(boundary);
        ...
        expect(
            flutterPolyline.points[0].longitude, originalPoints[0].longitude);
      });
    });
  });
}
