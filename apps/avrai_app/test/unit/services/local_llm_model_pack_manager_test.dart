...
import 'package:avrai_runtime_os/services/local_llm/model_pack_manifest.dart';
...
void main() {
  ...
  test('downloadAndActivate extracts zip safely and sets active pointers',
      () async {
    // Build a tiny zip artifact with a single file.
    final archive = Archive()
      ..addFile(
          ArchiveFile('bundle/model.mlmodelc/placeholder.txt', 3, [1, 2, 3]));
    ...
    final extracted =
        File('${status.activeDir}/bundle/model.mlmodelc/placeholder.txt');
    expect(await extracted.exists(), isTrue);
  });

  test('zip slip is rejected', () async {
    final archive = Archive()..addFile(ArchiveFile('../evil.txt', 1, [1]));
    final zipBytes = ZipEncoder().encode(archive);
    final zipSha = sha256.convert(zipBytes).toString();
  ...
  test('skips artifact download when requested pack is already active',
      () async {
    ...
    final artifactUrl =
        Uri.parse('https://example.com/should_not_download.zip');
    final manifest = {
      'model_id': 'llama3.1-8b-instruct',
      'version': 'test',
      'family': 'llama3.1-8b-instruct',
