...
import 'package:avrai_runtime_os/services/partnerships/partnership_profile_service.dart';
import 'package:avrai_core/models/misc/platform_phase.dart'
    show PlatformPhase, PhaseName;
import 'package:avrai_core/models/misc/platform_phase.dart' as platform_phase
    show SaturationFactors;
...
final String _agentRunId =
    'expertise_partnership_boost_test_${DateTime.now().microsecondsSinceEpoch}';
...
ExplorationExpertise _dummyExplorationExpertise(
...
ProfessionalExpertise _dummyProfessionalExpertise(
    String userId, String category) {
  final t = DateTime.fromMillisecondsSinceEpoch(0);
...
class MockSaturationAlgorithmService extends Mock
    implements SaturationAlgorithmService {
  @override
  Future<SaturationMetrics> analyzeCategorySaturation({
    ...
    _agentLog(
        'A',
        'expertise_calculation_partnership_boost_test.dart:MockSaturationAlgorithmService',
        'analyzeCategorySaturation invoked', {
      'category': category,
...
class MockMultiPathExpertiseService extends Mock
    implements MultiPathExpertiseService {
  @override
  Future<ExplorationExpertise> calculateExplorationExpertise({
    ...
    _agentLog(
        'A',
        'expertise_calculation_partnership_boost_test.dart:MockMultiPathExpertiseService',
        'calculateExplorationExpertise invoked', {
      'userId': userId,
      'category': category,
  ...
  Future<CommunityExpertise> calculateCommunityExpertise({
    ...
    _agentLog(
        'A',
        'expertise_calculation_partnership_boost_test.dart:MockMultiPathExpertiseService',
        'calculateCommunityExpertise invoked', {
      'userId': userId,
      'category': category,
    });
    // #endregion
  ...
  Future<ProfessionalExpertise> calculateProfessionalExpertise({
    ...
    _agentLog(
        'A',
        'expertise_calculation_partnership_boost_test.dart:MockMultiPathExpertiseService',
        'calculateProfessionalExpertise invoked', {
      'userId': userId,
      'category': category,
  ...
  Future<InfluenceExpertise> calculateInfluenceExpertise({
    ...
    _agentLog(
        'A',
        'expertise_calculation_partnership_boost_test.dart:MockMultiPathExpertiseService',
        'calculateInfluenceExpertise invoked', {
      'userId': userId,
      'category': category,
...
class MockPartnershipProfileService extends Mock
    implements PartnershipProfileService {
  @override
  Future<PartnershipExpertiseBoost> getPartnershipExpertiseBoost(
    ...
    _agentLog(
        'A',
        'expertise_calculation_partnership_boost_test.dart:MockPartnershipProfileService',
        'getPartnershipExpertiseBoost invoked', {
      'userId': userId,
      'category': category,
    });
    // #endregion
    return super.noSuchMethod(
      ...
      returnValue:
          Future.value(const PartnershipExpertiseBoost(totalBoost: 0.0)),
  ...
  group('ExpertiseCalculationService Partnership Boost Tests', () {
    ...
    group('calculatePartnershipBoost', () {
      ...
      test('should return zero boost when service is not available', () async {
        ...
        // Act
        final result =
            await serviceWithoutPartnership.calculatePartnershipBoost(
          userId: 'user-123',
          category: 'Coffee',
        );
    ...
    group('calculateExpertise with partnership boost', () {
      test('should integrate partnership boost into expertise calculation',
          () async {
        // Arrange
