...
void main() {
  group('EventRecommendationService Tests', () {
    ...
    group('getPersonalizedRecommendations', () {
      ...
      test(
          'records governed learning usage receipts when a learning record boosts a surfaced event recommendation',
          () async {
        ...
        final intakeResult = await upwardService.stagePersonalAgentHumanIntake(
          ownerUserId: user.id,
          actorAgentId: 'agent_user_1',
          chatId: 'chat_123',
          messageId: 'msg_nightlife_123',
          ...
          airGapArtifact: const UpwardAirGapService().issueArtifact(
            ...
            sanitizedPayload: <String, dynamic>{
              'chatId': 'chat_123',
              'messageId': 'msg_nightlife_123',
              'sourceOccurredAtUtc': '2026-04-04T05:00:00.000Z',
            },
        ...
        await observationService.recordReceipts([
          GovernedLearningChatObservationReceipt(
            ...
            recordedAtUtc: DateTime.utc(2026, 4, 4, 5, 30),
            chatId: 'chat_123',
        ...
        when(mockMatchingService.calculateMatchingScore(
          ...
          category: anyNamed('category'),
          locality: anyNamed('locality'),
        )).thenAnswer((_) async => 0.45);

        final nightlifeEvent = IntegrationTestHelpers.createTestEvent(
          id: 'event-nightlife-1',
        ...
        final coffeeEvent = IntegrationTestHelpers.createTestEvent(
          id: 'event-coffee-1',
          title: 'Morning Coffee Crawl',
          host: IntegrationTestHelpers.createUserWithLocalExpertise(
            id: 'expert-coffee',
            category: 'coffee',
          ),
        ...
        when(mockEventService.searchEvents(
          ...
          maxResults: anyNamed('maxResults'),
            ...
            (_) async => <ExpertiseEvent>[coffeeEvent, nightlifeEvent]);
        ...
        expect(recommendations, isNotEmpty);
        expect(recommendations.first.event.id, 'event-nightlife-1');

        final receipts = await usageService.listReceiptsForEnvelope(
          ownerUserId: user.id,
          envelopeId: intakeResult.envelope.id,
        );
        ...
        final observationReceipts =
            await observationService.listReceiptsForEnvelope(
          ownerUserId: user.id,
          envelopeId: intakeResult.envelope.id,
        );
        expect(receipts, isNotEmpty);
        expect(receipts.first.targetEntityId, 'event-nightlife-1');
        expect(receipts.first.targetEntityTitle, 'Austin After Dark');
        expect(receipts.first.domainLabel, 'Nightlife');
        expect(receipts.first.influenceScoreDelta, greaterThan(0));
        expect(
          adoptionReceipts
              .where(
                (entry) =>
        ...
        expect(
          observationReceipts.single.validatedSurface,
          'events_personalized',
        );
        expect(
      ...
      test(
          'records first-surfaced adoption receipts for explore event recommendations when that surface is requested',
          () async {
        final repository = UniversalIntakeRepository();
        final adoptionService = UserGovernedLearningAdoptionService(
          storageService: StorageService.instance,
        ...
        final intakeResult = await upwardService.stagePersonalAgentHumanIntake(
          ...
          actorAgentId: 'agent_user_1',
          chatId: 'chat_456',
          messageId: 'msg_music_456',
          occurredAtUtc: DateTime.utc(2026, 4, 4, 6),
          airGapArtifact: const UpwardAirGapService().issueArtifact(
            originPlane: 'personal_device',
            sourceKind: 'personal_agent_human_intake',
            sourceScope: 'human',
            ...
            sanitizedPayload: <String, dynamic>{
              'chatId': 'chat_456',
              'messageId': 'msg_music_456',
              'sourceOccurredAtUtc': '2026-04-04T06:00:00.000Z',
            },
            pseudonymousActorRef: 'anon_user',
          ),
          boundaryMetadata: const <String, dynamic>{
            'summary': 'The user wants live music events downtown.',
            'sanitized_summary': 'The user wants live music events downtown.',
            'referenced_entities': <String>['Downtown Live Set', 'music'],
            'accepted': true,
            'learning_allowed': true,
          },
          cityCode: 'atx',
        ...
        when(mockMatchingService.calculateMatchingScore(
          ...
          category: anyNamed('category'),
          locality: anyNamed('locality'),
        )).thenAnswer((_) async => 0.46);

        final musicEvent = IntegrationTestHelpers.createTestEvent(
          id: 'event-music-1',
          title: 'Downtown Live Set',
          host: IntegrationTestHelpers.createUserWithLocalExpertise(
            id: 'expert-music',
            category: 'music',
          ),
        ...
        when(mockEventService.searchEvents(
          ...
          location: anyNamed('location'),
          maxResults: anyNamed('maxResults'),
        )).thenAnswer((_) async => <ExpertiseEvent>[musicEvent]);
      ...
      test('boosts events covered by governed downstream consumer state',
          () async {
        await governedStateService.upsertState(
          GovernedDomainConsumerState(
            ...
            summary: 'Event priors are ready for Austin.',
            boundedUse: 'Bounded only.',
            targetedSystems: const <String>['event_priors'],
            requestCount: 2,
        ...
        await governedStateService.upsertState(
          GovernedDomainConsumerState(
            ...
            generatedAt: DateTime.utc(2026, 4, 1, 12),
            status: 'executed_local_governed_domain_consumer_refresh',
            summary: 'Locality priors are ready for Austin.',
            boundedUse: 'Bounded only.',
        ...
        when(mockMatchingService.calculateMatchingScore(
          ...
          final locality =
              invocation.namedArguments[#locality]?.toString().toLowerCase();
          if (locality == 'austin') {
            return 0.72;
          }
          return 0.45;
        ...
        final atxEvent = IntegrationTestHelpers.createTestEvent(
          id: 'event-atx',
          host: IntegrationTestHelpers.createUserWithLocalExpertise(
            id: 'expert-atx',
            category: 'music',
        ...
        final seaEvent = IntegrationTestHelpers.createTestEvent(
          id: 'event-sea',
          host: IntegrationTestHelpers.createUserWithLocalExpertise(
            id: 'expert-sea',
            category: 'music',
        ...
        when(mockEventService.searchEvents(
          ...
          location: anyNamed('location'),
          maxResults: anyNamed('maxResults'),
        )).thenAnswer((_) async => <ExpertiseEvent>[seaEvent, atxEvent]);
        ...
        expect(recommendations, isNotEmpty);
        expect(recommendations.first.event.id, 'event-atx');
      });

      test(
          'uses place community and venue governed state as bounded event ranking inputs',
          () async {
        await governedStateService.upsertState(
          GovernedDomainConsumerState(
            sourceId: 'simulation_training_source_atx',
            ...
            environmentId: 'atx-replay-world-2024',
            cityCode: 'atx',
            generatedAt: DateTime.utc(2026, 4, 1, 13),
            status: 'executed_local_governed_domain_consumer_refresh',
        ...
        await governedStateService.upsertState(
          GovernedDomainConsumerState(
            ...
            generatedAt: DateTime.utc(2026, 4, 1, 13),
            status: 'executed_local_governed_domain_consumer_refresh',
            summary: 'Community coordination is ready for Austin.',
            boundedUse: 'Bounded only.',
            targetedSystems: const <String>['community_priors'],
            requestCount: 3,
        ...
        await governedStateService.upsertState(
          GovernedDomainConsumerState(
            ...
            boundedUse: 'Bounded only.',
            targetedSystems: const <String>['venue_priors'],
            requestCount: 3,
            averageConfidence: 0.9,
          ),
        );
        ...
        final atxEvent = IntegrationTestHelpers.createTestEvent(
          id: 'event-atx-place-community-venue',
          host: IntegrationTestHelpers.createUserWithLocalExpertise(
            id: 'expert-atx-2',
            category: 'music',
          ),
        ...
        final seaEvent = IntegrationTestHelpers.createTestEvent(
          id: 'event-sea-place-community-venue',
          host: IntegrationTestHelpers.createUserWithLocalExpertise(
            id: 'expert-sea-2',
            category: 'music',
          ),
        ...
        expect(recommendations, isNotEmpty);
        expect(
            recommendations.first.event.id, 'event-atx-place-community-venue');
      });
    });
