...
void main() {
  group('Signature layer', () {
    ...
    test('keeps Birmingham shaping in pheromones rather than DNA', () {
      ...
      final birminghamSignature = spotSignatureBuilder.build(
        spot: birminghamSpot,
      );
    ...
    test(
      ...
      () async {
        ...
        final analyticsStorage = MockGetStorage.getInstance(
        ...
        final rawLatest = userSignatureBuilder.build(
          ...
          userVibe: await userVibeAnalyzer.compileUserVibe(
            user.id,
            evolvedPersonality,
          ),
        );
        ...
        expect(
          seeded.summary,
          contains('Explorer baseline seeded from onboarding in Birmingham.'),
        );
        expect(
    ...
    test(
      ...
      () async {
        ...
        expect(learned.signatureId, contains('user:user-2'));
        expect(
          learned.sourceTrace.any(
            (trace) => trace.sourceId == 'spot_view',
          ),
          isTrue,
        );
        expect(
    ...
    test(
      ...
      () async {
        ...
        final vibeKernel = TestVibeKernel(trajectoryKernel: trajectoryKernel);
        final serviceLocator = GetIt.instance;
        final hadRegisteredVibeKernel =
            serviceLocator.isRegistered<VibeKernel>();
        final previousVibeKernel =
            hadRegisteredVibeKernel ? serviceLocator<VibeKernel>() : null;
        if (hadRegisteredVibeKernel) {
          await serviceLocator.unregister<VibeKernel>();
        }
        serviceLocator.registerSingleton<VibeKernel>(vibeKernel);
        addTearDown(() async {
          if (serviceLocator.isRegistered<VibeKernel>()) {
            await serviceLocator.unregister<VibeKernel>();
          }
          if (previousVibeKernel != null) {
            serviceLocator.registerSingleton<VibeKernel>(previousVibeKernel);
          }
        });
    ...
    test(
      'chat reflection signals can nudge the persisted signature baseline',
      () async {
        final defaultStorage = MockGetStorage.getInstance(
          boxName: 'spots_default',
        );
        ...
        expect(
          reflected.sourceTrace.any(
            (trace) => trace.sourceId == 'chat_reflection',
          ),
          isTrue,
        );
      },
    ...
    test(
      'browse selection and strong intent negative signals persist learning',
      () async {
        final defaultStorage = MockGetStorage.getInstance(
          boxName: 'spots_default',
        );
        ...
        final afterCommunity =
            await service.recordCommunityBrowseSelectionSignal(
          userId: 'user-browse',
          community: community,
          personality: personality,
        );
        ...
        expect(afterCommunity.signatureId, contains('user:user-browse'));
        expect(
        ...
        expect(
          afterSoftIgnore.sourceTrace.any(
            (trace) => trace.sourceId == 'soft_ignore',
          ),
          isTrue,
        );
        expect(
