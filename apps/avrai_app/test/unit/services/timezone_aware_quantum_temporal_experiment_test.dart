...
void main() {
  group('Experiment 7: Timezone-Aware Quantum Temporal Matching', () {
    ...
    test('should match same local time across 10+ different timezones',
        () async {
      ...
      expect(highCompatibilityCount,
          greaterThan(0)); // At least some pairs have compatibility
    ...
    test(
        'should have higher compatibility for same local time than different local times',
        () async {
      ...
      final sfDifferentState =
          QuantumTemporalStateGenerator.generate(sfDifferentTimestamp);

      final sameLocalTimeCompatibility =
          QuantumTemporalStateGenerator.calculateTemporalCompatibility(
              tokyoState, sfState);
      final differentLocalTimeCompatibility =
          QuantumTemporalStateGenerator.calculateTemporalCompatibility(
              tokyoState, sfDifferentState);
    ...
    test(
        'should validate timezone-aware matching for 100+ pairs across timezones',
        () async {
      final timezones = [
        ('Asia/Tokyo', const Duration(hours: 9)),
        ('America/Los_Angeles', const Duration(hours: -8)),
    ...
    test(
        'should match morning preferences across timezones (use case validation)',
        () async {
    ...
    test(
        'should validate cross-timezone quantum temporal state generation for multiple hours',
        () async {
      ...
      for (final hour in hours) {
        ...
        for (int i = 0; i < sameHourStates.length; i++) {
          for (int j = i + 1; j < sameHourStates.length; j++) {
            final compatibility =
                QuantumTemporalStateGenerator.calculateTemporalCompatibility(
                    sameHourStates[i], sameHourStates[j]);
        ...
        for (final otherHour in hours) {
          if (otherHour != hour) {
            ...
            final sameHourCompatibility =
                QuantumTemporalStateGenerator.calculateTemporalCompatibility(
                    sameHourStates[0], sameHourStates[1]);
            final differentHourCompatibility =
                QuantumTemporalStateGenerator.calculateTemporalCompatibility(
                    sameHourStates[0], differentHourStates[0]);
    ...
    test('should validate timezone-aware matching accuracy for 500+ pairs',
        () async {
      final timezones = [
        ('Asia/Tokyo', const Duration(hours: 9)),
        ('America/Los_Angeles', const Duration(hours: -8)),
