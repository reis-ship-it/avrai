...
void main() {
  group('SignatureHealthAdminService', () {
    ...
    test(
      'maps sources into health categories and review queue counts',
      () async {
    ...
    test(
      'merges remote records and emits live updates when local data changes',
      () async {
        ...
        when(
          () => remoteService.watchRows(),
        ).thenAnswer((_) => const Stream<List<SignatureHealthRecord>>.empty());
        ...
        final updated = await stream.firstWhere(
          (snapshot) => snapshot.records.any(
            (record) => record.sourceId == 'local-source',
          ),
        );
    ...
    test(
      'builds feedback trend rows by entity type and recency window',
      () async {
        final remoteService = MockRemoteSourceHealthService();
        when(() => remoteService.fetchRows()).thenAnswer(
