...
void main() {
  group('BusinessAccountService Tests', () {
    ...
    late BusinessOperatorFollowUpPromptPlannerService
    ...
    setUp(() async {
      ...
      final prefs = await SharedPreferencesCompat.getInstance(
        storage: getTestStorage(
          boxName: 'business_account_service_follow_up_prefs',
        ),
      );
      businessFollowUpPlannerService =
      ...
      service = BusinessAccountService(
        ...
        businessFollowUpPlannerService: businessFollowUpPlannerService,
      );
    ...
    group('createBusinessAccount', () {
      test(
        ...
        expect(
          reviews.last.payload['sourceKind'],
          'business_operator_input_intake',
        );
        expect(
          reviews.last.payload['convictionTier'],
          'business_operator_profile_seed',
        );
        final followUpPlans = await businessFollowUpPlannerService.listPlans(
          creator.id,
        );
        expect(followUpPlans, hasLength(3));
        expect(followUpPlans.first.businessName, 'Test Business 3');
        expect(
          followUpPlans.first.promptQuestion,
          contains(
              'should AVRAI remember before it broadens business learning'),
        );
      });
    });
    ...
    group('updateBusinessAccount', () {
      test('should update business account fields, or update categories',
        ...
        final updateReviews = reviews
            .where(
              (review) =>
                  review.payload['sourceKind'] ==
                      'business_operator_input_intake' &&
                  review.payload['channel'] == 'update',
            )
            .toList();
        expect(updateReviews, isNotEmpty);
        final categoryUpdateReview = updateReviews.firstWhere(
          (review) =>
              (review.payload['changedFields'] as List).contains('categories'),
        );
        expect(
          categoryUpdateReview.payload['changedFields'],
          contains('categories'),
        );
        final followUpPlans = await businessFollowUpPlannerService.listPlans(
          creator.id,
        ...
        expect(
          followUpPlans.where((plan) => plan.action == 'update'),
          isNotEmpty,
        );
      });
    });
