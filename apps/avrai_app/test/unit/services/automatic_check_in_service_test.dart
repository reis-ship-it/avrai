...
void main() {
  group('AutomaticCheckInService Tests', () {
    ...
    group('checkOut', () {
      ...
      test('stages completed visits into governed upward learning', () async {
        ...
        await service.checkOut(
          userId: 'user-1',
          checkOutTime: checkIn.checkInTime.add(const Duration(minutes: 18)),
        );

        final reviews = await repository.getAllReviewItems();
        expect(reviews, hasLength(1));
        expect(
            reviews.single.payload['sourceKind'], 'visit_observation_intake');
        expect(
      ...
      test('creates bounded visit follow-up plans after completed visits',
          () async {
        final prefs = await SharedPreferencesCompat.getInstance(
          storage:
              getTestStorage(boxName: 'automatic_check_in_follow_up_prefs'),
        );
        ...
        await service.checkOut(
          userId: 'user-visit-follow-up',
          checkOutTime: checkIn.checkInTime.add(const Duration(minutes: 34)),
        );

        final plans = await planner.listPlans('user-visit-follow-up');
        expect(plans, hasLength(1));
        expect(plans.single.observationKind, 'visit');
        expect(plans.single.targetLabel, 'night-owl-cafe');
      });
    });
