...
import 'package:reality_engine/vibe/vibe_kernel.dart' as native_vibe;

import '../../mocks/mock_storage_service.dart';
import '../../../../../engine/reality_engine/lib/reality_engine.dart';

class _ThrowingExportVibeKernel extends native_vibe.VibeKernel {
  @override
...
void main() {
  group('CanonicalVibeMigrationService', () {
    ...
    test('imports legacy personality, signature, and locality state once',
        () async {
      ...
      final persistenceService = VibeKernelPersistenceService(storage: storage);
    ...
    test('runIfNeeded tolerates string-key iteration from storage service',
        () async {
      final storage = StorageService.instance;
      await storage.setString(
    ...
    test('skips canonical export when no legacy state exists', () async {
      ...
      expect(
        manifest?.metadata['canonical_persist_skipped'],
        isTrue,
      );
      expect(
        manifest?.metadata['skip_reason'],
        'no_legacy_state',
      );
    });
  });
