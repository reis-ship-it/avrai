  ...
  group('SourceIntakeOrchestrator', () {
    ...
    test('routes structured dated source into event path automatically',
      ...
      final importedSource =
          sources.firstWhere((candidate) => candidate.id == 'source-event');
      expect(
        importedSource.metadata['signatureConfidence'],
        isA<double>(),
      );
      ...
      expect(
        reviews.any((review) =>
            ...
            review.payload['entityType'] == 'event'),
        isTrue,
      );
    });
    ...
    test('routes recurring group with dated happening into linked bundle',
      ...
      expect(
        reviews.any((review) =>
            review.payload['sourceKind'] ==
                'external_confirmation_import_intake' &&
            review.payload['entityType'] == 'community'),
        isTrue,
      );
    });
    ...
    test('imported spots surface through hybrid search with external metadata',
      ...
      expect(
        reviews.any((review) =>
            review.payload['sourceKind'] ==
                'external_confirmation_import_intake' &&
            review.payload['entityType'] == 'spot'),
        isTrue,
      );
    });

    test('links existing external events into upward confirmation intake',
        () async {
      final event = await eventService.importExternalEvent(
        ownerUserId: 'owner-5',
        title: 'Imported Existing Event',
        description: 'External event already present in AVRAI.',
        category: 'Music',
        cityCode: 'us-bhm',
        localityCode: 'us-bhm-downtown',
        metadata: ExternalSyncMetadata(
          sourceProvider: 'calendar',
          sourceUrl: 'https://example.com/seed.ics',
          externalId: 'seed-event-1',
          connectionMode: ExternalConnectionMode.feed,
          importedAt: DateTime(2026, 3, 10),
      ...
      final source = ExternalSourceDescriptor(
        id: 'source-existing-event',
        ownerUserId: 'owner-5',
        sourceProvider: 'eventbrite',
        sourceUrl: 'https://example.com/eventbrite/existing',
        connectionMode: ExternalConnectionMode.api,
        entityHint: IntakeEntityType.event,
        createdAt: DateTime(2026, 3, 10),
        updatedAt: DateTime(2026, 3, 10),
      );

      await orchestrator.registerSourceForExistingEntity(
        source: source,
        entityType: IntakeEntityType.event,
        entityId: event.id,
        rawPayload: <String, dynamic>{
          'title': 'Imported Existing Event',
          'description': 'Confirmed by external source',
          'category': 'Music',
          'location': 'Birmingham, AL',
      ...
      expect(
        reviews.any((review) =>
            review.payload['sourceKind'] ==
                'external_confirmation_import_intake' &&
            review.payload['action'] == 'linked_existing_entity' &&
            review.payload['entityType'] == 'event'),
        isTrue,
      );
    });
