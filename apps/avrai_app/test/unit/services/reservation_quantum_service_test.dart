...
class MockLocationTimingQuantumStateService extends Mock
    implements LocationTimingQuantumStateService {}
...
void main() {
  ...
  group('ReservationQuantumService', () {
    ...
    group('createReservationQuantumState', () {
      test(
          'should create reservation quantum state with user personality and vibe',
          () async {
        // Arrange
        ...
        // Verify interactions
        verify(() => mockPersonalityLearning.getCurrentPersonality(userId))
            .called(1);
        verify(() =>
                mockVibeAnalyzer.compileUserVibe(userId, personalityProfile))
            .called(1);
        verify(() => mockAtomicClock.getAtomicTimestamp()).called(1);
      });

      test('should throw exception when personality profile not found',
          () async {
        // Arrange
        const userId = 'user-123';
        final reservationTime = DateTime.now().add(const Duration(days: 7));
    ...
    group('calculateReservationCompatibility', () {
      ...
      test('should return lower compatibility for dissimilar states', () async {
        ...
        expect(compatibility,
            lessThan(0.8)); // Should be lower than similar states
      });
    });
  });
}
