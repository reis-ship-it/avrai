...
void main() {
  group('LocalityAgentEngineV1', () {
    ...
    test(
        'updateFromVisit increases community-related delta when ai2ai exchange happened',
        () async {
      const key = LocalityAgentKeyV1(geohashPrefix: 'dr5regw', precision: 7);
      final localStore =
          LocalityAgentLocalStoreV1(storage: StorageService.instance);
      final globalRepo = _FakeGlobalRepo(byKey: {});
      final engine =
          LocalityAgentEngineV1(globalRepo: globalRepo, localStore: localStore);
      ...
      final idxCommunity =
          LocalityAgentEngineV1.dimensions.indexOf('community_orientation');
      final idxTrust =
          LocalityAgentEngineV1.dimensions.indexOf('trust_network_reliance');
