...
void main() {
  group('TrustNetworkManager', () {
    ...
    group('Trust Relationship Establishment', () {
      ...
      test('should reject trust establishment with user data present',
          () async {
        ...
        expect(
          () => trustManager.establishTrust(
              'agent-with-user-data', invalidContext),
          throwsA(isA<TrustNetworkException>()),
        );
      });

      test('should store anonymous factors without exposing identity',
          () async {
        ...
        final relationship =
            await trustManager.establishTrust('anon-agent', context);
    ...
    group('Trust Score Updates', () {
      test('should update trust score based on positive interactions',
          () async {
        ...
        final initial =
            await trustManager.establishTrust('update-agent', initialContext);
        ...
        final updated = await trustManager.updateTrustScore(
            'update-agent', positiveInteraction);

        expect(updated.trustScore, greaterThan(initialScore));
        expect(updated.interactionCount, equals(initial.interactionCount + 1));
        expect(
            updated.lastInteraction.isAfter(initial.lastInteraction), isTrue);
      });
      ...
      test('should enforce trust score bounds during updates', () async {
        ...
        final initial =
            await trustManager.establishTrust('bounds-agent', context);
    ...
    group('Trusted Agent Discovery', () {
      test('should find trusted agents meeting minimum trust level', () async {
        ...
        for (final agent in trustedAgents) {
          expect(agent.trustLevel.index,
              greaterThanOrEqualTo(TrustLevel.verified.index));
        }
      });
    ...
    group('Agent Reputation Verification', () {
      test('should verify reputation for existing agent', () async {
        ...
        final reputation =
            await trustManager.verifyAgentReputation('reputation-agent');

        expect(reputation.agentId, equals('reputation-agent'));
        expect(reputation.overallScore, greaterThan(0.0));
        expect(
            reputation.reputationLevel, isNot(equals(ReputationLevel.unknown)));
      ...
      test('should return unknown reputation for non-existent agent', () async {
        final reputation =
            await trustManager.verifyAgentReputation('unknown-agent');
    ...
    group('Privacy and Security Validation', () {
      test('should never expose user-identifying information in trust data',
          () async {
        final context = TrustContext(
          hasUserData: false,
          hasValidatedBehavior: true,
        ...
        final relationship =
            await trustManager.establishTrust('privacy-agent', context);
