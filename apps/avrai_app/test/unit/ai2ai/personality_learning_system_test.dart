...
void main() {
  group('PersonalityProfile Core Functionality', () {
    ...
    group('Personality Evolution', () {
      test('should evolve personality with bounded dimension updates', () {
        ...
        expect(
            evolved.dimensionConfidence['exploration_eagerness'], equals(0.7));
      ...
      test('should add evolution milestone during evolution', () {
        ...
        final newMilestones =
            evolved.learningHistory['evolution_milestones'] as List<DateTime>;
      ...
      test('should merge additional learning data correctly', () {
        ...
        expect(evolved.learningHistory['learning_sources'],
            contains('ai2ai_network'));
    ...
    group('Compatibility Calculations', () {
      test('should calculate compatibility between similar personalities', () {
        // Phase 8.3: Use agentId for privacy protection
        final profile1 =
            PersonalityProfile.initial('agent_user1', userId: 'user1').evolve(
          newDimensions: {
            'exploration_eagerness': 0.8,
        ...
        // Phase 8.3: Use agentId for privacy protection
        final profile2 =
            PersonalityProfile.initial('agent_user2', userId: 'user2').evolve(
          newDimensions: {
      ...
      test('should calculate compatibility between dissimilar personalities',
        ...
        final compatibility = profile1.calculateCompatibility(profile2);
        expect(compatibility,
            lessThan(VibeConstants.mediumCompatibilityThreshold));
      });
    ...
    group('Learning Potential Calculations', () {
      test(
          'should calculate high learning potential for compatible personalities',
          () {
        ...
        final compatibleProfile =
            PersonalityProfile.initial('agent_compatible', userId: 'compatible')
                .evolve(
        ...
        final learningPotential =
            confidentProfile.calculateLearningPotential(compatibleProfile);
        expect(learningPotential, greaterThanOrEqualTo(0.6));
      });

      test(
          'should calculate minimal learning potential for incompatible personalities',
          () {
        // Phase 8.3: Use agentId for privacy protection
        final incompatibleProfile = PersonalityProfile.initial(
                'agent_incompatible',
                userId: 'incompatible')
            .evolve(
          newDimensions: {
        ...
        final learningPotential =
            profile.calculateLearningPotential(incompatibleProfile);
      ...
      test('should ensure minimum learning potential for any personality pair',
        ...
        // Phase 8.3: Use agentId for privacy protection
        final randomProfile =
            PersonalityProfile.initial('agent_random', userId: 'random').evolve(
          newDimensions: {
            'exploration_eagerness': 0.1,
            'community_orientation': 0.9,
          },
        ...
        final learningPotential =
            profile.calculateLearningPotential(randomProfile);
    ...
    group('Serialization and Persistence', () {
      ...
      test('should deserialize from JSON correctly', () {
        ...
        expect(deserialized.evolutionGeneration,
    ...
    group('Privacy and Security Validation', () {
      ...
      test('should maintain consistent hash for same profile state', () {
        ...
        final profile1 =
            PersonalityProfile.initial(agentId, userId: testUserId);
        final profile2 =
            PersonalityProfile.initial(agentId, userId: testUserId);
  ...
  group('AI2AI Learning Network Integration', () {
    test(
        'should support network effect calculations without exposing user data',
        () {
      final profiles = List.generate(
        5,
        (i) =>
            // Phase 8.3: Use agentId for privacy protection
            PersonalityProfile.initial('agent_anonymous-$i',
                    userId: 'anonymous-$i')
                .evolve(
          newDimensions: {
    ...
    test('should support ecosystem evolution tracking', () {
      // Phase 8.3: Use agentId for privacy protection
      final baseProfile = PersonalityProfile.initial('agent_ecosystem-node',
          userId: 'ecosystem-node');
      ...
      for (int gen = 0; gen < 5; gen++) {
        current = current.evolve(
          newDimensions: {
            'exploration_eagerness':
                (current.dimensions['exploration_eagerness']! + 0.1)
                    .clamp(0.0, 1.0),
          },
          additionalLearning: {
      ...
      final finalInteractions =
          generations.last.learningHistory['total_interactions'] as int;
      expect(finalInteractions, equals(50)); // 10 * 5 generations

      // Verify dimension evolution
      final finalEagerness =
          generations.last.dimensions['exploration_eagerness']!;
      expect(finalEagerness,
          greaterThan(baseProfile.dimensions['exploration_eagerness']!));
    });
  });
  ...
  group('Performance and Optimization Tests', () {
    test('should handle large-scale compatibility calculations efficiently',
      ...
      final profiles = List.generate(
          100,
          (i) => PersonalityProfile.initial('agent_perf-test-$i',
              userId: 'perf-test-$i'));
    ...
    test('should maintain memory efficiency during evolution chains', () {
      // Phase 8.3: Use agentId for privacy protection
      var current = PersonalityProfile.initial('agent_memory-test',
          userId: 'memory-test');
      ...
      expect(
          current.learningHistory['evolution_milestones']?.length, isNotNull);
    });
    ...
    test('should validate dimension value bounds consistently', () {
      ...
      // Phase 8.3: Use agentId for privacy protection
      final testProfile = PersonalityProfile.initial('agent_test-user-bounds',
          userId: 'test-user-bounds');
      for (final value in testCases) {
        ...
        expect(
            actualValue, greaterThanOrEqualTo(VibeConstants.minDimensionValue));
