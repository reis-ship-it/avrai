...
void main() {
  group('DimensionalityReductionService', () {
    ...
    group('PCA Reduction', () {
      test('should reduce dimensionality using variance-based selection',
          () async {
        ...
        expect(
            reduced.length, lessThanOrEqualTo(8)); // Original has 8 dimensions
      });

      test('should return original vector if target dimensions >= original',
          () async {
        final state = QuantumEntityState(
          entityId: 'test_entity',
          entityType: QuantumEntityType.user,
    ...
    group('Schmidt Decomposition', () {
      test('should perform Schmidt decomposition', () async {
        ...
        expect(decomposition.leftBasis.length,
            equals(decomposition.coefficients.length));
        expect(decomposition.rightBasis.length,
            equals(decomposition.coefficients.length));
      });
      ...
      test('should calculate Schmidt rank correctly', () async {
        ...
        expect(decomposition.schmidtRank,
            lessThanOrEqualTo(decomposition.coefficients.length));
      });
    });
    ...
    group('Integration Tests', () {
      test('should work with full quantum entity state', () async {
        ...
        expect(pcaReduced.length,
            lessThanOrEqualTo(16)); // Original has ~16 dimensions
