...
class WorldsheetEvolutionDynamics {
  ...
  EvolutionRate calculateEvolutionRate(
    ...
    final stabilityRate = (fabric2.invariants.stability - 
                          fabric1.invariants.stability) / timeSeconds;
    
    final densityRate = (fabric2.invariants.density - 
                        fabric1.invariants.density) / timeSeconds;
    
    final crossingRate = (fabric2.invariants.crossingNumber - 
                         fabric1.invariants.crossingNumber) / timeSeconds;
    ...
    final alexanderRate = _calculatePolynomialRate(alexander1, alexander2, timeSeconds);
  ...
  KnotFabric predictFabricState({
    ...
    final idealStability = idealFabric?.invariants.stability ?? currentFabric.invariants.stability;
    final idealDensity = idealFabric?.invariants.density ?? currentFabric.invariants.density;
    
    // Evolution equation: dF/dt = α·(F_ideal - F) + rate
    final stabilityChange = relaxationRate * (idealStability - currentFabric.invariants.stability) * timeSeconds +
                           rate.stabilityRate * timeSeconds;
    
    final densityChange = relaxationRate * (idealDensity - currentFabric.invariants.density) * timeSeconds +
    ...
    final newStability = (currentFabric.invariants.stability + stabilityChange)
        .clamp(0.0, 1.0);
    final newDensity = (currentFabric.invariants.density + densityChange)
        .clamp(0.0, double.infinity);
    final newCrossingNumber = (currentFabric.invariants.crossingNumber + crossingChange.round())
        .clamp(0, 100);
    
    // Evolve polynomial coefficients
    final currentJones = currentFabric.invariants.jonesPolynomial.coefficients;
    final newJones = _evolvePolynomial(currentJones, rate.jonesRate, timeSeconds);
    
    final currentAlexander = currentFabric.invariants.alexanderPolynomial.coefficients;
    final newAlexander = _evolvePolynomial(currentAlexander, rate.alexanderRate, timeSeconds);
  ...
  List<double> _evolvePolynomial(
    ...
    final maxLength = current.length > rate.length ? current.length : rate.length;
