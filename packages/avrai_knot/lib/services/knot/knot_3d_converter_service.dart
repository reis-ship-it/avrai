...
class Knot3DConverterService {
  ...
  List<Vector3> _generateTorusKnot({
    ...
    final r = 0.5 + (crossingNumber / 20.0).clamp(0.0, 1.0); // Minor radius (complexity-based)
  ...
  List<Vector3> smoothCoordinates(List<Vector3> coordinates, {int iterations = 1}) {
  ...
  List<Vector3> applyInvariantsToShape({
    ...
    final complexityFactor = 1.0 + (invariants.crossingNumber / 20.0).clamp(0.0, 0.5);
    
    // Apply writhe (chirality)
    final chiralityFactor = invariants.writhe.sign == -1 ? -1 : 1;
    
    // Apply signature (overall shape)
    final signatureFactor = 1.0 + (invariants.signature.abs() / 10.0).clamp(0.0, 0.2);
