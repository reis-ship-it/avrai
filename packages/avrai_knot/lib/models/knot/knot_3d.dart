...
class Knot3D {
  ...
  factory Knot3D.fromBraidData({
    ...
    while (i < braidData.length) {
      ...
      crossings.add(Crossing3D(
        index: crossingIndex,
        strand: strand,
        isOver: isOver,
        position: Vector3.zero(), // Will be calculated during coordinate generation
      ));
  ...
  static List<Vector3> _generate3DCoordinates({
    ...
    for (int i = 0; i < numPoints; i++) {
      ...
      final x = (R + r * (1.0 + 0.2 * writhe.sign) * (writhe.abs() / 10.0).clamp(0.0, 1.0) * 
                 math.cos(n * t)) * math.cos(m * t);
      final y = (R + r * (1.0 + 0.2 * writhe.sign) * (writhe.abs() / 10.0).clamp(0.0, 1.0) * 
                 math.cos(n * t)) * math.sin(m * t);
      final z = r * (1.0 + 0.2 * writhe.sign) * (writhe.abs() / 10.0).clamp(0.0, 1.0) * 
                math.sin(n * t);
  ...
  static List<Crossing3D> _updateCrossingPositions({
    ...
    for (final crossing in crossings) {
      ...
      final positionIndex = ((crossing.index / crossings.length) * coordinates.length)
          .round()
          .clamp(0, coordinates.length - 1);
      
      updated.add(Crossing3D(
  ...
  factory Knot3DMesh.generateMesh({
    ...
    for (int i = 0; i < knot3d.coordinates.length; i++) {
      ...
      for (int j = 0; j < tubeSegments; j++) {
        ...
        final offset = normal * (math.cos(angle) * tubeRadius) + 
...
class Knot3DExport {
  ...
  static String toOBJ(Knot3DMesh mesh, {String? objectName}) {
    ...
    for (final face in mesh.faces) {
      ...
      buffer.writeln('f ${face.v0 + 1}//${face.v0 + 1} '
          '${face.v1 + 1}//${face.v1 + 1} '
          '${face.v2 + 1}//${face.v2 + 1}');
    }
  ...
  static Map<String, dynamic> toGLTF(Knot3DMesh mesh, {String? name}) {
    ...
    return {
      'asset': {
        'version': '2.0',
        'generator': 'AVRAI Knot 3D Export',
      },
      'scene': 0,
      'scenes': [
        {
      ...
      'buffers': [
        {
          ...
          'uri': 'data:application/octet-stream;base64,', // Would need base64 encoding
        }
      ],
    };
  }
