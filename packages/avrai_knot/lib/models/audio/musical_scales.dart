...
class ChordBuilder {
  ...
  List<double> buildChord(int voiceCount) {
    ...
    for (final degree in degreesToUse) {
      ...
      final freq =
          baseFrequency * MusicalScales._semitoneToRatio(semitones + octave * 12);
  ...
  List<double> buildDeterministicChord(int voiceCount) {
    ...
    switch (voiceCount) {
      case 2:
        ...
        return MusicalScales.getChordRatios(mode: mode, chordType: 'power')
            .map((r) => baseFrequency * r)
            .toList();

      case 3:
        // Triad: root + third + fifth
        return MusicalScales.getChordRatios(mode: mode, chordType: 'triad')
            .map((r) => baseFrequency * r)
            .toList();

      case 4:
        // Seventh chord
        return MusicalScales.getChordRatios(mode: mode, chordType: 'seventh')
            .map((r) => baseFrequency * r)
            .toList();
      ...
      default:
        ...
        final extended =
            MusicalScales.getChordRatios(mode: mode, chordType: 'extended')
                .map((r) => baseFrequency * r)
                .toList();
