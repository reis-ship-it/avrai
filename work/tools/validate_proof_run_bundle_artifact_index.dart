...
Future<void> main(List<String> args) async {
  if (args.isEmpty) {
    stderr.writeln(
      'usage: dart run work/tools/validate_proof_run_bundle_artifact_index.dart '
      '<artifact_root>',
    );
    exitCode = 64;
    return;
...
Map<String, Object?> _validateArtifactRoot(Directory artifactRoot) {
  ...
  if (!indexJsonFile.existsSync()) {
    errors.add('Missing proof_run_bundle_index.json');
  }
  if (!indexMarkdownFile.existsSync()) {
    errors.add('Missing proof_run_bundle_index.md');
  }
  ...
  if (indexJson['artifact_kind'] != 'proof_run_bundle_index') {
    errors.add(
      'proof_run_bundle_index.json artifact_kind must be '
      'proof_run_bundle_index',
    );
  }
  ...
  for (final entry in indexEntries) {
    ...
    if (!indexMarkdown.contains('`$artifactDir`')) {
      errors.add(
        'proof_run_bundle_index.md is missing artifact dir row for $artifactDir',
      );
    }
  }
  ...
  for (final artifact in discoveredArtifacts) {
    if (!indexedDirs.contains(artifact.artifactDir)) {
      errors.add(
        'proof_run_bundle_index.json is missing artifact dir: '
  ...
  if (!_listsEqual(expectedOrder, actualOrder)) {
    errors.add(
      'proof_run_bundle_index.json entries are not in deterministic '
      'descending order',
    );
  }
...
List<_DiscoveredArtifact> _discoverArtifacts(
  ...
  for (final child in children) {
    ...
    if (manifest['artifact_kind'] != 'proof_run_bundle') {
      continue;
    }
    artifacts.add(
      _DiscoveredArtifact(
...
void _validateIndexEntry(
  ...
  _expectEqual(
      entry['artifact_dir'], discovered.artifactDir, 'artifact_dir', errors);
  _expectEqual(
  ...
  _expectEqual(entry['bundle_id'], manifest['bundle_id'], 'bundle_id', errors);
