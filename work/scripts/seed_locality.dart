...
void main(List<String> args) {
  ...
  print('Step 1: Mock fetching OpenStreetMap (OSM) POI dump for bounding box...');
  
  // Simulated OSM Data
  final rawOSMData = [
    {'name': 'Saturn Birmingham', 'type': 'music_venue', 'tags': ['indie', 'coffee', 'games', 'bars']},
    {'name': 'Good Dog Cafe', 'type': 'cafe', 'tags': ['dog_friendly', 'chill', 'studying']},
    {'name': 'Railroad Park', 'type': 'park', 'tags': ['outdoor', 'fitness', 'events', 'community']},
    {'name': 'Atomic Lounge', 'type': 'bar', 'tags': ['cocktails', 'vintage', 'costumes']},
  ];
  ...
  for (final poi in rawOSMData) {
    ...
    final Map<String, double> vibeVector = {
      'energy_preference': poi['tags'].contains('indie') || poi['tags'].contains('cocktails') ? 0.8 : 0.4,
      'community_orientation': poi['tags'].contains('community') || poi['tags'].contains('bars') ? 0.9 : 0.5,
      'authenticity_preference': poi['tags'].contains('vintage') || poi['tags'].contains('indie') ? 0.9 : 0.6,
    ...
    seededSpots.add({
      ...
      'confidence': 0.3, // Initial confidence is low until real humans visit and validate
    });
  }
  ...
  final outputFile = File('${outputDir.path}/${globalState['locality_id']}_seed.json');
