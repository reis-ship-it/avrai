...
Future<void> main() async {
  ...
  final credsFilePath = Platform.environment['SMOKE_CREDS_PATH'] ??
      'tmp/supabase_smoke_users.json';
  ...
  {
    final creds = await _loadOrCreateCreds(
        clientA: clientA, clientB: clientB, runId: runId, path: credsFilePath);
...
Future<bool> _tryAwaitMessageId(
  ...
  try {
    await stream.firstWhere((id) => id == expected).timeout(timeout,
        onTimeout: () {
...
Future<void> _insertKeyState({
  ...
  await client.from('community_sender_key_state').insert({
...
Future<void> _insertDummyShare({
  ...
  await client.from('community_sender_key_shares').insert({
    'community_id': communityId,
    'key_id': keyId,
    'to_user_id': toUserId,
...
Future<void> _upsertMembership({
  ...
  await client.from('community_memberships').upsert({
    'community_id': communityId,
    'user_id': userId,
    'key_id': keyId,
  }).timeout(const Duration(seconds: 15));
...
Future<void> _insertMessageBlob({
  ...
  await client.from('community_message_blobs').insert({
...
Future<_SmokeCreds> _loadOrCreateCreds({
  ...
  final respA = await clientA.auth.signUp(
      email: emailA,
      password: passwordA,
      emailRedirectTo: 'https://example.com');
  final respB = await clientB.auth.signUp(
      email: emailB,
      password: passwordB,
      emailRedirectTo: 'https://example.com');
