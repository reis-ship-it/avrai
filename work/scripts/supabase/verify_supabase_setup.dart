...
void main() async {
  ...
  final String bucketsCsv =
      env['BUCKETS'] ?? 'user-avatars,spot-images,list-images';
  final List<String> requiredBuckets = bucketsCsv
      .split(',')
      .map((s) => s.trim())
      .where((s) => s.isNotEmpty)
      .toList();
  final bool requireCanary =
      (env['REQUIRE_CANARY'] ?? 'false').toLowerCase() == 'true';
  ...
  try {
    ...
    if (requireCanary) {
      final canaryOk =
          await _checkCanaries(url, anonKey, canaryBuckets, canaryPath);
...
Future<bool> _checkStorageBuckets(
    String url, String anonKey, List<String> requiredBuckets) async {
  ...
  try {
    ...
    if (listResp.statusCode == 200) {
      ...
      print(
          'ℹ️  Bucket list (anon-visible): ${parsed is List ? parsed.length : 0}');
    }
  } catch (_) {}
...
Future<bool> _checkCanaries(
    String url, String anonKey, List<String> buckets, String canaryPath) async {
