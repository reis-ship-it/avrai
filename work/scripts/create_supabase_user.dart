...
import 'package:avrai_runtime_os/config/supabase_config.dart' as config;
...
void main(List<String> args) async {
  if (args.length < 2) {
    print(
        'Usage: dart run scripts/create_supabase_user.dart <email> <password> [name]');
    print('');
    print('Example:');
    print(
        '  dart run scripts/create_supabase_user.dart reis@avrai.org avrai "Reis Gordon"');
  ...
  if (!config.SupabaseConfig.isValid) {
    ...
    print(
        '   Current Key: ${config.SupabaseConfig.anonKey.isEmpty ? "NOT SET" : "SET"}');
    exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      if (existingUser.user != null) {
        ...
        print(
            '   Email Confirmed: ${existingUser.user!.emailConfirmedAt != null}');
        print('');
      ...
      print(
          'ℹ️  User does not exist or password is incorrect, creating new user...');
      print('');
    }
