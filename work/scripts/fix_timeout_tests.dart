  ...
  for (final line in lines) {
    ...
    if (line.contains('testWidgets(') && line.contains('tester')) {
      ...
      for (int i = lineNumber - 1;
          i < lines.length && i < lineNumber + 100;
          i++) {
        final currentLine = lines[i];
        if (currentLine.contains('testWidgets(')) {
          inTest = true;
          braceCount =
              currentLine.split('{').length - currentLine.split('}').length;
        } else if (inTest) {
      ...
      final usesTester = bodyText.contains(RegExp(
          r'\btester\.(pump|tap|drag|enterText|find|getCenter|getSize|takeException|allWidgets|allElements|widget|element|binding)'));
      final usesFind = bodyText.contains(RegExp(r'\bfind\.'));
      final usesPumpWidget =
          bodyText.contains('pumpWidget') || bodyText.contains('pumpAndSettle');

      if (!usesTester && !usesFind && !usesPumpWidget) {
        // Check if it only calls helper functions (business logic)
        final hasHelperCalls =
            bodyText.contains(RegExp(r'await\s+_[a-zA-Z]+\('));
        final hasDirectWidgetOps = bodyText.contains(RegExp(
            r'(pumpWidget|pumpAndSettle|tester\.tap|tester\.drag|tester\.enterText)'));
        ...
        if (hasHelperCalls && !hasDirectWidgetOps) {
          issues.add(TestIssue(
            ...
            description:
                'testWidgets declared but tester parameter never used - only calls helper functions',
            severity: '⚠️',
            fix: () => convertTestWidgetsToTest(
                file, lineNumber, testName ?? 'Unknown'),
          ));
        }
      }
    }
