...
class TestQualityChecker {
  ...
  void _checkTestConsolidation() {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      final match =
          RegExp(r'test(Widgets)?\([' r'"' r"'" r'](.*?)[' r'"' r"'" r']\)')
              .firstMatch(line);
    ...
    if (similarTests.length > 2) {
      issues.add(QualityIssue(
        ...
        suggestion:
            'Consider consolidating similar tests into comprehensive test blocks',
  ...
  QualityReport generateReport() {
    return QualityReport(
      ...
      testCount: lines
          .where((l) => l.contains(RegExp(r'^\s*test(Widgets)?\(')))
          .length,
    );
  }
}
...
void main(List<String> args) {
  ...
  print(
      'Average quality score: ${(reports.map((r) => r.qualityScore).reduce((a, b) => a + b) / reports.length).toStringAsFixed(1)}/10.0');
  print(
      'Files with issues: ${reports.where((r) => r.issues.isNotEmpty).length}');
  print(
      'Total issues: ${reports.map((r) => r.issues.length).reduce((a, b) => a + b)}');
  print('');
}
