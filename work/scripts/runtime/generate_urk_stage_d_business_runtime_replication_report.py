#!/usr/bin/env python3
"""Generate/check URK Stage D business runtime replication report."""
...
CONFIG_PATH = Path("configs/runtime/urk_stage_d_business_runtime_replication_controls.json")
OUT_JSON = Path(
...
def _build_summary(cfg: dict) -> dict:
    ...
    required_policy_gate = _require_non_negative_number(
        pipeline.get("required_policy_gate_coverage_pct"),
        "runtime_request_pipeline.required_policy_gate_coverage_pct",
    )
    observed_policy_gate = _require_non_negative_number(
        pipeline.get("observed_policy_gate_coverage_pct"),
        "runtime_request_pipeline.observed_policy_gate_coverage_pct",
    )
    ...
    lineage = cfg.get("lineage_and_audit")
    if not isinstance(lineage, dict):
        _fail("lineage_and_audit must be object")
    required_lineage = _require_non_negative_number(
        lineage.get("required_lineage_coverage_pct"),
        "lineage_and_audit.required_lineage_coverage_pct",
    )
    observed_lineage = _require_non_negative_number(
        lineage.get("observed_lineage_coverage_pct"),
        "lineage_and_audit.observed_lineage_coverage_pct",
    )
    max_unattributed = _require_non_negative_int(
        lineage.get("max_unattributed_actions"),
        "lineage_and_audit.max_unattributed_actions",
    )
    observed_unattributed = _require_non_negative_int(
        lineage.get("observed_unattributed_actions"),
        "lineage_and_audit.observed_unattributed_actions",
    )
    lineage_checks = _require_positive_int(
        lineage.get("checks_executed"),
        "lineage_and_audit.checks_executed",
    )
    ...
    max_unreviewed = _require_non_negative_int(
        safety.get("max_unreviewed_high_impact_commits"),
        "commit_safety.max_unreviewed_high_impact_commits",
    )
    observed_unreviewed = _require_non_negative_int(
        safety.get("observed_unreviewed_high_impact_commits"),
        "commit_safety.observed_unreviewed_high_impact_commits",
    )
    ...
    lineage_ok = observed_lineage >= required_lineage and observed_unattributed <= max_unattributed
    safety_ok = observed_review >= required_review and observed_unreviewed <= max_unreviewed

    errors: list[str] = []
    if not pipeline_ok:
        errors.append("runtime pipeline/policy gate threshold failed")
    if not lineage_ok:
        errors.append("lineage/audit threshold failed")
    if not safety_ok:
        errors.append("high-impact review safety threshold failed")
    ...
    return {
        ...
        "lineage_and_audit": {
            ...
            "max_unattributed_actions": max_unattributed,
            "observed_unattributed_actions": observed_unattributed,
        ...
        "commit_safety": {
            ...
            "max_unreviewed_high_impact_commits": max_unreviewed,
            "observed_unreviewed_high_impact_commits": observed_unreviewed,
...
def _render_md(summary: dict) -> str:
    ...
    lineage = summary.get("lineage_and_audit") or {}
    safety = summary.get("commit_safety") or {}
    window = summary.get("evaluation_window") or {}

    return (
        "# URK Stage D Business Runtime Replication Report\n\n"
        ...
        f"**Lineage/audit check:** {lineage.get('within_threshold')}  \n"
...
def main() -> int:
    ...
    if args.check:
      if not OUT_JSON.exists() or not OUT_MD.exists():
          _fail("URK Stage D business report artifacts missing; run generator")
      if OUT_JSON.read_text(encoding="utf-8") != out_json:
          _fail("URK Stage D business JSON report out of date")
      if OUT_MD.read_text(encoding="utf-8") != out_md:
          _fail("URK Stage D business markdown report out of date")
      print("OK: URK Stage D business runtime replication report is in sync.")
      return 0
