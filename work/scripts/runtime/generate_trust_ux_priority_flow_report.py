#!/usr/bin/env python3
"""Generate/check trust UX priority-flow coverage report."""
...
CONFIG_PATH = Path("configs/runtime/trust_ux_priority_flows.json")
OUT_JSON = Path("docs/plans/methodology/MASTER_PLAN_3_PRONG_TRUST_UX_PRIORITY_FLOW_REPORT.json")
OUT_MD = Path("docs/plans/methodology/MASTER_PLAN_3_PRONG_TRUST_UX_PRIORITY_FLOW_REPORT.md")
...
def _load_config() -> dict:
    if not CONFIG_PATH.exists():
        _fail(f"missing trust UX config: {CONFIG_PATH}")
    try:
        data = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
    except Exception as exc:
        _fail(f"invalid JSON in trust UX config: {exc}")
    if not isinstance(data, dict):
        _fail("trust UX config root must be object")
    return data


def _build_summary(data: dict) -> dict:
    if str(data.get("version", "")).strip() != "v1":
        _fail("trust UX config version must be 'v1'")
    ...
    for idx, raw in enumerate(flows, start=1):
        ...
        req_conf = raw.get("requires_confidence") is True
        req_prov = raw.get("requires_provenance") is True
        req_last = raw.get("requires_last_validated_at") is True
        if req_conf and req_prov and req_last:
            fully_covered += 1
        else:
            errors.append(
                f"flow[{idx}] must require confidence+provenance+last_validated_at"
            )
...
def _render_md(summary: dict) -> str:
    return (
        "# 3-Prong Trust UX Priority Flow Report\n\n"
...
def main() -> int:
    ...
    if args.check:
      if not OUT_JSON.exists() or not OUT_MD.exists():
        _fail("trust UX report artifacts missing; run generator")
      if OUT_JSON.read_text(encoding="utf-8") != out_json:
        _fail("trust UX JSON report out of date")
      if OUT_MD.read_text(encoding="utf-8") != out_md:
        _fail("trust UX markdown report out of date")
      print("OK: trust UX priority flow report is in sync.")
      return 0
