...
bool checkLibrariesAvailable() {
  if (!Platform.isMacOS) {
    logStep(
        '⚠️  Platform not macOS - Signal Protocol tests currently support macOS only');
    return false;
  }
  ...
  final pathsToTry = [
    ...
    findProjectRoot() != null
        ? '${findProjectRoot()}/runtime/avrai_network/native/signal_ffi/macos/libsignal_ffi.dylib'
        : null,
  ...
  logStep('❌ Signal Protocol library not found',
      data: {'pathsChecked': pathsToTry.length});
  return false;
}
...
void main() async {
  ...
  try {
    ...
    logStep('✅ Alice initialized',
        data: {'isInitialized': aliceProtocol.isInitialized});
    ...
    logStep('✅ Bob initialized',
        data: {'isInitialized': bobProtocol.isInitialized});
    ...
    logStep('Session verification', data: {
      ...
      'sessionJsonLength':
          session == null ? 0 : session.toJson().toString().length,
    });
    ...
    const plaintext =
        'Hello from Alice! This is a test message for Signal Protocol end-to-end testing.';
    ...
    print(
        '${match ? "✅" : "❌"} Message decryption: ${match ? "PASSED" : "FAILED"}');
    print(
        '${match ? "✅" : "❌"} End-to-end test: ${match ? "PASSED" : "FAILED"}');
    print('');
    ...
    print('Stack trace:');
    print(stackTrace);
    developer.log('Test failed',
        error: e, stackTrace: stackTrace, name: 'SignalE2ETest');
    exit(1);
  }
}
