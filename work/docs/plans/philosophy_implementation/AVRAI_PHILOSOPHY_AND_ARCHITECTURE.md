...
### 5.4 Personal Egress Air-Gap Invariant

For reverse learning, the Air Gap is not only an ingestion law for raw external data. It is also the mandatory privacy membrane for any signal that leaves a personal device or exits a personal agent on its way to a higher hierarchy tier.

Non-negotiable rules:
- Any signal crossing off a personal device or out of a personal agent into locality, world, universal, admin, supervisor, or reality-model surfaces must pass through an air-gap-governed export path first.
- Governed upward learning intake may consume only air-gap-issued artifacts, not raw personal-runtime payloads or direct service-owned domain objects.
- Personal-origin exports must be minimized, pseudonymized where possible, scope-bound, receipt-backed, temporally lineage-aware, and signed or attested by the issuing boundary.
- The air-gap export path must fail closed. If a valid receipt or attestation is missing, the signal does not enter the higher learning loop.
- Rich local memory may remain on-device, but the exported upward artifact is a bounded abstraction, not a raw replay of personal state.

This rule exists for both privacy and security. Without it, the reverse learning loop becomes a covert exfiltration and poisoning channel.

### 5.5 Reverse Loop Security Boundary

The reverse loop must remain a governed evidence plane, not a privileged control plane.

That means:
- Personal-origin upward artifacts are evidence candidates only. They may support, challenge, or refine convictions, but they do not directly mutate broader truth.
- The reality model and higher governance tiers must never treat personal-origin traffic as self-authorizing policy or executable instruction.
- Downward artifacts must be centrally re-issued after review, simulation, and approval; the system must not simply echo personal-origin content back downward as authority.
- Downward artifacts accepted by personal agents must be signed, scoped, replay-protected, and bounded to their permitted destination and effect surface.

The philosophical rule is simple: AVRAI may learn from the person, but it must not allow raw personal-origin traffic to teach itself authority.

### 5.6 Reverse-Loop Non-Human Observation Emitters

The reverse loop also accepts bounded non-human observations, but only through explicit governed emitters. These emitters are not UI widgets and they are not generic "system thoughts." They are specific service-owned runtime/admin lanes that already hold bounded operational evidence and can export it upward safely.

Canonical emitter families:
- `signature_health_admin_service.dart` is the first operational supervisor emitter. It should emit bounded supervisor observations when the runtime already forms supervisor-facing conclusions such as reality-model update supervisor briefs, validation-simulation interpretations, and downstream re-propagation gate conclusions.
- `replay_simulation_admin_service.dart` is the second simulation-memory supervisor emitter. It should emit bounded supervisor observations when repeated simulation outcomes, rejection patterns, or replay-lab evidence produce durable supervisory learning value.
- An assistant service/orchestration lane is the third emitter family. It may emit bounded assistant observations later, but only from backend/service logic, never directly from admin widget pages.
- Kernel-owned offline evidence should use the separate kernel/offline receipt lane, not the supervisor/assistant observation lane. Future emitters there include `urk_kernel_control_plane_service.dart` and `urk_runtime_activation_receipt_dispatcher.dart`.

Non-emitter rule:
- Admin pages such as command center, oversight, world-model, and simulation-lab surfaces may render these observations, but they must not originate them directly.
