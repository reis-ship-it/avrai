...
11. **Any personal-device or personal-agent signal moving upward must cross the air-gap first.** The security posture must assume that direct personal-runtime egress into higher learning tiers is both a privacy breach path and a poisoning path.
12. **The reverse learning loop is evidence-only until re-issued by trusted governance.** Personal-origin artifacts may not arrive as executable authority, and approved downward artifacts must be centrally re-issued, signed, scoped, and replay-protected before any device or lower tier accepts them.

### 5.1 Reverse-Loop Threat Model

The upward and downward learning loop creates a distinct attack class beyond ordinary perimeter abuse.

Primary risks:
- air-gap bypass from a compromised or malicious client
- conviction poisoning through crafted personal-origin signals
- replay of stale artifacts as if fresh evidence
- scope escape, where a local signal is silently amplified into unrelated domains
- echo attacks, where user-origin content returns downward as if it were trusted system authority
- recommendation or propagation manipulation that causes the loop to reward the attacker's preferred outcomes

Required defensive posture:
- mandatory air-gap normalization for personal-origin upward crossings
- receipt, attestation, hashing, expiry, and replay defense on every crossing artifact
- separation of evidence artifacts from executable or policy-bearing artifacts
- simulation and human approval before broad downstream re-propagation
- signature verification on bounded downward artifacts before lower-tier acceptance
- bounded non-human observation emitters must stay service-owned and explicit: `signature_health_admin_service.dart` first, `replay_simulation_admin_service.dart` second, assistant backend/service logic later, and kernel-native receipt emitters on their separate receipt lane rather than via ad hoc page logic
