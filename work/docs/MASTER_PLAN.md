...
**KernelGraph internal workflow IR companion:** `docs/plans/architecture/KERNELGRAPH_INTERNAL_WORKFLOW_IR_AND_GOVERNED_SELF_IMPROVEMENT_ARCHITECTURE_2026-04-03.md`  
...
- **Quantum-ready**: build pure intelligence functions (no side effects) on classical compute with clean abstraction layers. When quantum hardware arrives, the migration is a backend swap, not an architecture rewrite. The future-notice tracker for any true quantum execution backend, including reality-model simulation/training kernels, lives in `plans/quantum_computing/QUANTUM_COMPUTING_RESEARCH_AND_INTEGRATION_TRACKER.md`.
...
- `docs/plans/architecture/KERNELGRAPH_INTERNAL_WORKFLOW_IR_AND_GOVERNED_SELF_IMPROVEMENT_ARCHITECTURE_2026-04-03.md`
...
**Execution-substrate note:** Phase `7.9` should converge on `KernelGraph` as the internal typed workflow IR for experiment planning, replay/shadow/canary execution, research checkpoints, admin digests, and bounded patch/test/config proposals. This section does not authorize freeform repo mutation. It authorizes governed proposals executed through compiled plans, existing promotion gates, and simulation-first validation before any bounded live inoculation or production promotion.
...
| 7.9.51 | **Structured feedback-prompt planner + proactive surveying gate.** Build a bounded planner that turns governed-learning gaps, recommendation feedback artifacts, contradiction records, and stale preference evidence into scoped clarifying questions with explicit information-gain scoring, user-burden limits, cadence/fatigue guards, and simulation-first validation before broader rollout. Survey prompts must remain grounded in structured reality-intake artifacts (`what`, `why`, `how`, `when`, `where`, `who`) rather than freeform chat-only heuristics, and every accepted response must re-enter the existing governed upward-learning lane instead of opening a parallel intake path |

> **Required companion specs:** `docs/plans/architecture/AUTONOMOUS_RESEARCH_EXPERIMENTATION_ENGINE.md`, `docs/plans/architecture/DREAM_TRAINING_CONVICTION_GOVERNANCE.md`, `docs/plans/architecture/KERNELGRAPH_INTERNAL_WORKFLOW_IR_AND_GOVERNED_SELF_IMPROVEMENT_ARCHITECTURE_2026-04-03.md`
>
...
> **Execution substrate:** `docs/plans/architecture/KERNELGRAPH_INTERNAL_WORKFLOW_IR_AND_GOVERNED_SELF_IMPROVEMENT_ARCHITECTURE_2026-04-03.md` is the preferred implementation path for this section. Break classification, attack manifests, replay/shadow/canary gates, rollback descriptors, and admin truth-surface outputs should converge on compiled internal workflow plans instead of one-off orchestration services. Novel attack families and self-healing candidates are simulation-first: replay/simulation -> shadow -> bounded canary/live inoculation only after earlier lanes succeed.
...
> **Wave-31 follow-on note (2026-04-01):** After `M31-P10-8`, `M31-P10-9` tracks the living city-pack refresh loop. The intended target state is not a silently mutating city pack, but a versioned city-pack identity (`cityPackStructuralRef`) whose realism sidecars and latest-state simulation basis refresh from the newest governed app, runtime/OS, and reality-model evidence for any supported place while preserving lineage, freshness/atomic-time validity windows, and rollback-safe history.

> **Wave-31 follow-on note (2026-04-02):** After `M31-P10-9`, `M31-P10-10` tracks the simulation-to-training temporal backbone. The intended target state is that simulation snapshots, lab outcomes, rerun requests/jobs, bounded-review handoffs, and training exports all carry additive temporal receipts rooted in `AtomicClockService -> AtomicClockTemporalAdapter -> TemporalKernel -> When/native kernel`, with legacy UTC `DateTime` fields preserved only as migration scaffolding until temporal lineage becomes authoritative end to end.
...
Phase 12 also absorbs the internal `KernelGraph` workflow substrate into the eventual headless kernel. `KernelGraph` is not the cognitive OS itself and does not replace the Air Gap, world model, or governance law. It is the typed plan/compiler/executor bridge that lets today's runtime-managed governed workflows become kernel-owned run artifacts later. Read Phase 12 extraction work as gradually moving that substrate from runtime-supervised compiled plans to kernel-owned compiled plans without widening autonomous authority beyond existing governance gates.
...
**Build baseline (2026-03-31):** `work/docs/plans/methodology/REALITY_MODEL_TRAINING_AND_SIMULATION_BUILD_BASELINE_2026-03-31.md` records the current built state across simulation/training control-plane authority, stage-2 executor-parity sequencing, and compressed training acceptance hardening.
**Receipt-binding baseline (2026-04-05):** `work/docs/plans/methodology/M31_P12_5_GOVERNED_CROSSING_RECEIPT_BINDING_MATRIX_BASELINE.md` turns the remaining `12.4B.8` work into an explicit receipt-family and hierarchy-binding contract so upward intake, replay/training promotion, and downward rollout can share one fail-closed crossing model.
**Lifecycle-visibility baseline (2026-04-05):** `work/docs/plans/methodology/M31_P12_6_LIFECYCLE_VISIBILITY_AND_PROVING_TESTS_BASELINE.md` records the post-implementation broad-pass outcome for `12.4B.10`, keeps the verified DM/community/admin lane explicit, and separates the first real request-binding proving follow-up from unrelated native-kernel/mock-drift package failures.
...
| 12.4B.7 | Add a governed artifact lifecycle contract across replay/training/intake surfaces: every durable artifact must declare lifecycle class (`canonical`, `archive`, `staging`, `temporary`, `quarantine`), lifecycle state (`candidate`, `accepted`, `rejected`, `superseded`, `expired`), and retention policy before it can become a stable learning or review input |
| 12.4B.8 | Enforce fail-closed upward durability rules: personal-origin and AI2AI-origin learning artifacts must carry valid air-gap receipts with expiry, destination ceiling, next-stage allowance, and content-hash consistency, and higher tiers must reject raw message/personal payloads as direct training authority |
| 12.4B.9 | Add retention automation and supersession policy: replay staging/partition artifacts expire after successful upload/index, AI2AI delivery artifacts remain short-lived unless user-local policy says otherwise, and stateful AI2AI artifacts such as locality updates retire or supersede older hot-path state instead of accumulating indefinitely |
| 12.4B.10 | Add lifecycle visibility and proving tests: admin/operator surfaces must show artifact class/state/retention truth, and CI/runtime tests must prove deletion, expiry, quarantine non-reuse, and raw-content non-promotion behavior end to end |
...
- **Autonomous research/experimentation tasks:** 51 (Phase 7.9.1-7.9.51: hypothesis mining, interdisciplinary retrieval, self-expanding taxonomy, staged experiments, deterministic journaling, cross-reference scoring, rollback governance, profile-gated systems optimization, bounded-space simulation/model policies, formal invariant oversight, delegation-control governance, adaptive-depth runtime + compute-optimal governance, kernel-bound autonomy + recursive meta-audit + first-occurrence/dwell controls + storm suppression + high-impact oversight caps + downstream scaling regime/sensitivity governance, plus dream-governance, intervention-efficacy, and structured feedback-prompt surveying lanes executed through the emerging KernelGraph workflow substrate)
...
**Last Updated:** April 5, 2026 (v39 -- made the bidirectional governed-learning chat/hierarchy slice operational: governance outcomes now write bounded `attention satisfied` / `attention cleared` state back onto chat-observation receipts, `SignatureHealth` upward-learning handoffs only treat unresolved follow-up/correction pressure as active attention, and the hierarchy/admin lane now prioritizes records with still-open chat pressure instead of leaving every historical follow-up permanently hot. Previous: v38 closed the bidirectional governed-learning chat/hierarchy slice: `SignatureHealth` upward-learning handoffs now aggregate per-envelope chat-observation summaries, so the admin/hierarchy side can see whether explanations were acknowledged, challenged, corrected, validated by surfaced adoption, and later reinforced/constrained/overruled by governance instead of leaving those outcomes trapped in the user-facing ledger/composer path. Previous: v37 extended the self-observing governed-learning chat layer into runtime validation too: when a previously explained record later lands a real first-surfaced adoption receipt in `events_personalized`, `explore_events`, or `explore_spots`, the latest unvalidated chat explanation receipt for that record is now marked as `validatedBySurfacedAdoption`, and the composer can explicitly say that a prior explanation was later borne out by surfaced adoption instead of only tracking conversational reactions. Previous: v36 extended the bounded upward-loop transparency lane into a first self-observing chat layer: governed-learning explanations now emit local chat-observation receipts, follow-up trace questions resolve the prior explanation as `requestedFollowUp`, direct `correct` / `forget` / `stop using` chat actions resolve the latest pending explanation outcome instead of leaving it ambiguous, and lightweight acknowledgements like `thanks, that helps` close the pending explanation as `acknowledged`. The composer now reads those outcomes too, so repeated lineage questions can shift into a more step-by-step trail mode instead of staying static. Previous: v35 extended the surfaced-lane map with a third real recommendation lane: `explore_events` now records first-surfaced adoption receipts through `EventRecommendationService`, `ExploreDiscoveryService` explicitly requests that lane instead of silently reusing `events_personalized`, and user-facing propagation language now recognizes `explore event recommendations` alongside personalized events and explore spots. Previous: v34 tightened the governed-learning adoption/progression slice into a surfaced-lane map: chat and the Data Center now enumerate which recommendation lanes are already active, which are queued, and which are not active yet, using the bounded `events_personalized` + `explore_spots` receipts instead of implying propagation from generic usage alone. Previous: v33 extended the governed-learning adoption/progression slice beyond personalized events: `explore_spots` now emits first-surfaced adoption receipts on the same bounded ledger as `events_personalized`, chat can explicitly name `explore spot recommendations` as an active propagation lane when a governed-learning record first surfaces there, and the Data Center's adoption labels are now surface-aware instead of hardcoded to event-only wording. Previous: v32 extended the decision-grounded upward-loop transparency slice into cross-surface lineage handoffs: chat can now answer trail-oriented governed-learning questions such as `show me what this came from` with explicit Data Center ledger guidance plus latest visible recommendation context, the Data Center can deep-link-focus the best related governed-learning record by record/entity, and surfaced explore/feed recommendation cards now expose `Learning context` actions that route the user straight into the same bounded ledger trail instead of leaving lineage trapped in one UI. Previous: v31 extended the decision-grounded upward-loop transparency slice into navigable ledger history: recent recommendation feedback entries in the Data Center can now open the original recommendation and jump back to the best related governed-learning record on the page, while the preserved evidence-backed explanation wording remains aligned across surfaced recommendation cards, stored feedback history, and the user ledger. Previous: v30 extended the decision-grounded upward-loop transparency slice into the user ledger's feedback history: the Data Center now shows recent recommendation feedback with the same preserved evidence-backed explanation wording the user saw on the recommendation, and `RecommendationFeedbackService` keeps negative-signal history aligned with that surfaced explanation instead of generic `whyDetails` text. Previous: v29 extended the decision-grounded upward-loop transparency slice into stored recommendation-feedback history too: `RecommendationFeedbackService` now preserves surfaced evidence-backed explanation wording when it records negative preference signals, so persisted feedback/signature context stays aligned with the explanation the user actually saw instead of falling back to generic `whyDetails` text. Previous: v28 extended the decision-grounded upward-loop transparency slice through bounded clarification prompts: receipt-backed governed-learning explanation wording now survives into recommendation-feedback `whyDidYouShowThis` follow-ups and saved-discovery follow-up rationale/context, so clarification surfaces keep citing the actual learned signal that boosted a recommendation instead of flattening back into generic runtime language. Previous: v27 extended the decision-grounded upward-loop transparency slice across surfaced recommendation explanations: receipt-backed governed-learning evidence now reaches both user-safe event why copy and live explore-spot attribution copy, so surfaced recommendation cards can cite the actual learned signal that boosted a recommendation instead of falling back only to generic ranking mechanics. Previous: v26 extended the decision-grounded upward-loop transparency slice into surfaced recommendation why explanations: receipt-backed governed-learning evidence now reaches user-safe recommendation why copy, so explanation cards can cite the actual learned signal that boosted a recommendation instead of falling back only to generic ranking mechanics. Previous: v25 expanded the decision-grounded upward-loop transparency slice: surfaced event recommendations and explore spot discovery now emit governed-learning usage receipts, user-facing governed-learning projections carry usage count/applied-domain/latest-use state, and chat can answer with concrete `I saw X so I recommended Y` language plus explicit domain-scope statements like `this affected nightlife, not coffee`. Previous: v24 landed the first decision-grounded upward-loop transparency slice: surfaced event recommendations now emit governed-learning usage receipts, user-facing governed-learning projections carry usage count/applied-domain/latest-use state, chat can answer with concrete `I saw X so I recommended Y` language, and the Data Center ledger can distinguish used-vs-not-yet-used records. Previous: v23 seeded explicit Phase `7.9.51` follow-on scope for a structured feedback-prompt planner and proactive surveying gate so governed-learning clarifying questions can be widened later under information-gain scoring, user-burden limits, cadence/fatigue guards, and simulation-first validation without losing the current upward-loop execution order. Also updated the autonomous-research task-count summary to match the new `7.9.51` endpoint. Previous: v22 aligned the master plan with the new KernelGraph internal workflow IR authority. The plan now explicitly treats KernelGraph as AVRAI's internal typed workflow/compiler/executor substrate for Phase `7.9` autonomous research, Phase `10.9` self-healing + immune-system execution, and the eventual Phase `12` kernel-owned workflow runtime, while preserving simulation-first validation, bounded live inoculation, and the distinction between workflow substrate vs. Air Gap/world-model/governance law. Also corrected the stale autonomous-research task-count summary to match the existing `7.9.50` endpoint. Previous: v21 autonomous security immune system milestones added. Added explicit `10.9.30-10.9.34` future milestones for sandbox red-teaming, immune memory, admin HiTL cockpit, mesh immunization, and bounded security-AI governance, plus Phase 12 implementation tasks (`12.1.13-12.1.16`, `12.3.11-12.3.14`, `12.6.11-12.6.14`) for kernel enforcement, governance endpoints, and headless Pi/sandbox worker deployment)  
