# Multi-Path Dynamic Expertise System with Economic Enablement — Claims Draft (BROAD)
...
(b) Combining path scores using weighted formula: `score = (exploration × a threshold value) + (credentials × a threshold value) + (influence × a threshold value) + (professional × a threshold value) + (community × a threshold value) + (local × weight)`;
(c) Dynamically scaling thresholds based on platform phase, category saturation, and geographic hierarchy;
(d) Enforcing geographic hierarchy  for expertise scope.
