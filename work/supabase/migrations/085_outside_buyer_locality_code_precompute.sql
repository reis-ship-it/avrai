-- Migration: Outside Buyer Insights - locality_code in precompute (v1)
-- Purpose: Extend outside_buyer_precompute_spots_insights_v1_bucket to support
--   geo_bucket_type = 'locality_code' (neighborhood/borough) with k_min >= 200.
...
-- Replace precompute bucket function to allow locality_code and use point lookup.
...
BEGIN
  ...
  IF p_geo_bucket_type NOT IN ('geohash3','city_code','locality_code') THEN
    RAISE EXCEPTION 'unsupported geo_bucket_type for v1 cache: %', p_geo_bucket_type;
  END IF;

  IF p_geo_bucket_type = 'locality_code' AND (p_k_min IS NULL OR p_k_min < 200) THEN
    RAISE EXCEPTION 'for locality_code, k_min must be >= 200';
  END IF;
  ...
  WITH base_events AS (
    SELECT
      ...
      END AS geohash3_id,
      CASE
        WHEN ie.context ? 'location' AND (ie.context->'location') ? 'latitude'
        THEN (ie.context->'location'->>'latitude')::DOUBLE PRECISION
        ELSE NULL
      END AS latitude,
      CASE
        WHEN ie.context ? 'location' AND (ie.context->'location') ? 'longitude'
        THEN (ie.context->'location'->>'longitude')::DOUBLE PRECISION
        ELSE NULL
      END AS longitude
  ...
  normalized AS (
    SELECT
      ...
      CASE
        ...
        WHEN p_geo_bucket_type = 'locality_code' THEN COALESCE(
          CASE
            WHEN be.latitude IS NOT NULL AND be.longitude IS NOT NULL
            THEN (SELECT loc.locality_code FROM public.geo_lookup_locality_by_point_v1(be.latitude, be.longitude) loc LIMIT 1)
            ELSE NULL
          END,
          'unknown'
        )
...
COMMENT ON FUNCTION public.outside_buyer_precompute_spots_insights_v1_bucket(
  UUID, TIMESTAMPTZ, TEXT, INTEGER, TEXT, INTEGER, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, INTEGER, DOUBLE PRECISION, DOUBLE PRECISION
) IS 'Precompute+cache v1 insights for a single UTC bucket (hour/day/week). Supports geohash3, city_code, locality_code (k_min>=200). Uses geo_lookup_locality_by_point_v1 for locality_code.';
