...
// verify_jwt is disabled because this flow starts from an unauthenticated client.
...
async function verifyBusinessCredentials(
  ...
  username: string,
  password: string
): Promise<{ success: boolean; businessId?: string; sessionId?: string; error?: string }> {
  try {
    ...
    if (sessionError || !sessionRow?.id) {
      console.error('Error creating business session:', sessionError)
      return { success: false, error: 'Failed to create business session' }
    }
