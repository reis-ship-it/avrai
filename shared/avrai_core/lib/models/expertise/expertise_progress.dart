...
class ExpertiseProgress extends Equatable {
  ...
  String getContributionSummary() {
    ...
    if (contributionBreakdown.containsKey('lists')) {
      parts.add(
          '${contributionBreakdown['lists']} ${contributionBreakdown['lists'] == 1 ? 'list' : 'lists'}');
    }

    if (contributionBreakdown.containsKey('reviews')) {
      parts.add(
          '${contributionBreakdown['reviews']} ${contributionBreakdown['reviews'] == 1 ? 'review' : 'reviews'}');
    }

    if (contributionBreakdown.containsKey('spots')) {
      parts.add(
          '${contributionBreakdown['spots']} ${contributionBreakdown['spots'] == 1 ? 'spot' : 'spots'}');
    }
  ...
  factory ExpertiseProgress.fromJson(Map<String, dynamic> json) {
    return ExpertiseProgress(
      ...
      currentLevel:
          ExpertiseLevel.fromString(json['currentLevel'] as String?) ??
      ...
      contributionBreakdown:
          Map<String, int>.from(json['contributionBreakdown'] as Map? ?? {}),
  ...
  ExpertiseProgress copyWith({
    ...
    return ExpertiseProgress(
      ...
      contributionBreakdown:
          contributionBreakdown ?? this.contributionBreakdown,
      totalContributions: totalContributions ?? this.totalContributions,
      requiredContributions:
          requiredContributions ?? this.requiredContributions,
