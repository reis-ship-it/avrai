...
import '../utils/vibe_constants.dart';
...
class UserVibe extends Equatable {
  ...
  factory UserVibe.fromPersonalityProfile(
    ...
    for (final dimension in VibeConstants.coreDimensions) {
      ...
      final noise =
          (random.nextDouble() - 0.5) * VibeConstants.privacyNoiseLevel;
  ...
  ///
  /// This is the canonical compatibility score used by offline AI2AI discovery.
  double calculateVibeCompatibility(UserVibe other) {
    ...
    final energyCompatibility =
        1.0 - (overallEnergy - other.overallEnergy).abs();
    final explorationCompatibility =
        1.0 - (explorationTendency - other.explorationTendency).abs();

    return (dimensionCompatibility * 0.6 +
  ...
  /// Back-compat alias.
  double compatibilityWith(UserVibe other) => calculateVibeCompatibility(other);
  ...
  double calculateAIPleasurePotential(UserVibe other) {
    ...
    if (compatibility >= VibeConstants.highCompatibilityThreshold) return 0.9;
    if (compatibility >= VibeConstants.mediumCompatibilityThreshold) return 0.7;
    if (compatibility >= VibeConstants.lowCompatibilityThreshold) return 0.4;
    return 0.2;
  }
  ...
  Map<String, dynamic> getAnonymizedSummary() {
    return {
      'signature_hash': hashedSignature.substring(0, 8),
  ...
  bool verifyIntegrity() {
    ...
    final recalculatedExploration =
        _calculateExplorationTendency(anonymizedDimensions);
  ...
  // Private helper methods (shared between app + packages)
  ...
  static String _createHashedSignature(
    ...
    final input =
        '$userId:$dimensionString:$salt:${timestamp.millisecondsSinceEpoch}';
  ...
  static double _calculateOverallEnergy(Map<String, double> dims) {
    ...
    return (explorationEagerness +
            temporalFlexibility +
            locationAdventurousness) /
        3.0;
  }
  ...
  static double _calculateSocialPreference(Map<String, double> dims) {
    ...
    return (communityOrientation +
            socialDiscoveryStyle +
            trustNetworkReliance) /
        3.0;
  }
  ...
  static double _calculateExplorationTendency(Map<String, double> dims) {
    ...
    return (explorationEagerness +
            locationAdventurousness +
            (1.0 - authenticityPreference)) /
        3.0;
  }

  static String _getTemporalContext(DateTime timestamp) {
