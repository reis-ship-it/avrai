...
class LanguageProfile {
  ...
  LanguageProfile copyWith({
    ...
    if (vocabulary != null) {
      vocabulary.forEach((word, newScore) {
        ...
        updatedVocabulary[word] =
            (existingScore * 0.7 + newScore * 0.3).clamp(0.0, 1.0);
      });
    }
    ...
    final updatedSentenceStructure =
        Map<String, dynamic>.from(this.sentenceStructure);
    if (sentenceStructure != null) {
      sentenceStructure.forEach((key, value) {
        if (key == 'averageLength' && value is num) {
          final existing =
              (updatedSentenceStructure['averageLength'] as num?)?.toDouble() ??
                  0.0;
          updatedSentenceStructure[key] =
              (existing * 0.7 + value.toDouble() * 0.3);
        } else if (key == 'punctuationUsage' && value is Map) {
          final existingPunct = Map<String, double>.from(
            (updatedSentenceStructure['punctuationUsage'] as Map?)
                    ?.cast<String, double>() ??
                <String, double>{},
          );
          value.forEach((punct, freq) {
            ...
            existingPunct[punct] =
                (existingFreq * 0.7 + (freq as num).toDouble() * 0.3)
                    .clamp(0.0, 1.0);
          });
    ...
    if (tone != null) {
      tone.forEach((indicator, newValue) {
        ...
        updatedTone[indicator] =
            (existingValue * 0.7 + newValue * 0.3).clamp(0.0, 1.0);
      });
    }
    ...
    if (metadata != null) {
      metadata.forEach((key, value) {
        if (key == 'messageCount' && value is int) {
          ...
          updatedMetadata[key] =
              (existing * 0.7 + value.toDouble() * 0.3).clamp(0.0, 1.0);
        } else {
  ...
  double get learningConfidence =>
      (metadata['learningConfidence'] as num?)?.toDouble() ?? 0.0;
  ...
  bool get isReadyForAdaptation =>
      messageCount >= 50 && learningConfidence > 0.3;
  ...
  factory LanguageProfile.fromJson(Map<String, dynamic> json) {
    ...
    final sentenceStructure =
        Map<String, dynamic>.from(json['sentenceStructure'] ?? {});
    if (sentenceStructure.containsKey('punctuationUsage')) {
      final punctData = sentenceStructure['punctuationUsage'];
      if (punctData is Map) {
        sentenceStructure['punctuationUsage'] = Map<String, double>.from(
            punctData
    ...
    return LanguageProfile(
      ...
      vocabulary: Map<String, double>.from(
        (json['vocabulary'] as Map?)?.map(
                (k, v) => MapEntry(k.toString(), (v as num).toDouble())) ??
            {},
      ),
      phrases: Map<String, double>.from(
        (json['phrases'] as Map?)?.map(
                (k, v) => MapEntry(k.toString(), (v as num).toDouble())) ??
            {},
      ),
      sentenceStructure: sentenceStructure,
