...
class CommunityValidation {
  ...
  static double _calculateExpertConfidence(List<ValidationCriteria> criteria) {
    ...
    if (criteria.contains(ValidationCriteria.informationCompleteness)) {
      score += 0.05;
    }
  ...
  static double _calculateCommunityConfidence(
      List<ValidationCriteria> criteria) {
...
class SpotValidationSummary {
  ...
  factory SpotValidationSummary.fromValidations(
    ...
    final expertCount =
        validations.where((v) => v.level == ValidationLevel.expert).length;
    final communityCount =
        validations.where((v) => v.level == ValidationLevel.community).length;
  ...
  static List<String> _identifyValidationIssues(
    ...
    final recentValidations =
        validations.where((v) => v.isCurrentlyValid).toList();
    ...
    final hasExpertValidation =
        recentValidations.any((v) => v.level == ValidationLevel.expert);

    if (!hasExpertValidation && recentValidations.length < 2) {
      issues.add(
          'Limited community validation - could benefit from expert review');
    }
