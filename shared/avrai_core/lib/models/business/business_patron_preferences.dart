...
class BusinessPatronPreferences extends Equatable {
  ...
  final List<String>?
      preferredInterests; // e.g., ["Food", "Art", "Music", "Outdoor"]
  final List<String>?
      preferredLifestyleTraits; // e.g., ["Health-conscious", "Eco-friendly", "Social"]
  final List<String>?
      preferredActivities; // e.g., ["Dining", "Socializing", "Working"]

  // Personality & Behavior Preferences
  final List<String>?
      preferredPersonalityTraits; // e.g., ["Outgoing", "Quiet", "Adventurous"]
  final List<String>?
      preferredSocialStyles; // e.g., ["Group-oriented", "Solo", "Family-friendly"]
  final List<String>?
      preferredVibePreferences; // e.g., ["Casual", "Upscale", "Trendy", "Cozy"]

  // Spending & Engagement Preferences
  final SpendingLevel? preferredSpendingLevel; // Budget, Mid-range, Premium
  final List<String>?
      preferredVisitFrequency; // e.g., ["Regular", "Occasional", "One-time"]
  final bool preferLoyalCustomers;
  final bool preferNewCustomers;

  // Expertise & Knowledge Preferences
  final List<String>?
      preferredExpertiseLevels; // e.g., ["Novice", "Intermediate", "Expert"]
  final bool preferEducatedPatrons; // Patrons who appreciate quality/knowledge
  final List<String>?
      preferredKnowledgeAreas; // e.g., ["Coffee", "Wine", "Food"]
  ...
  final List<String>?
      preferredVisitTimes; // e.g., ["Morning", "Lunch", "Evening", "Late Night"]
  ...
  factory BusinessPatronPreferences.fromJson(Map<String, dynamic> json) {
    return BusinessPatronPreferences(
      ...
      preferredSpendingLevel: json['preferredSpendingLevel'] != null
          ? SpendingLevelExtension.fromString(
              json['preferredSpendingLevel'] as String)
          : null,
      ...
      preferAccessibilityNeeds:
          json['preferAccessibilityNeeds'] as bool? ?? false,
  ...
  BusinessPatronPreferences copyWith({
    ...
    return BusinessPatronPreferences(
      ...
      preferredLifestyleTraits:
          preferredLifestyleTraits ?? this.preferredLifestyleTraits,
      preferredActivities: preferredActivities ?? this.preferredActivities,
      preferredPersonalityTraits:
          preferredPersonalityTraits ?? this.preferredPersonalityTraits,
      preferredSocialStyles:
          preferredSocialStyles ?? this.preferredSocialStyles,
      preferredVibePreferences:
          preferredVibePreferences ?? this.preferredVibePreferences,
      preferredSpendingLevel:
          preferredSpendingLevel ?? this.preferredSpendingLevel,
      preferredVisitFrequency:
          preferredVisitFrequency ?? this.preferredVisitFrequency,
      ...
      preferredExpertiseLevels:
          preferredExpertiseLevels ?? this.preferredExpertiseLevels,
      preferEducatedPatrons:
          preferEducatedPatrons ?? this.preferEducatedPatrons,
      preferredKnowledgeAreas:
          preferredKnowledgeAreas ?? this.preferredKnowledgeAreas,
      preferCommunityMembers:
          preferCommunityMembers ?? this.preferCommunityMembers,
      ...
      preferAccessibilityNeeds:
          preferAccessibilityNeeds ?? this.preferAccessibilityNeeds,
      preferredSpecialNeeds:
          preferredSpecialNeeds ?? this.preferredSpecialNeeds,
      ...
      excludedPersonalityTraits:
          excludedPersonalityTraits ?? this.excludedPersonalityTraits,
