...
class HierarchicalAIMonitoring {
  ...
  Future<List<UserAIMetrics>> getUserAIMetrics() async {
    try {
      ...
      return byAgent.entries.map((e) {
        final list = e.value;
        final compat = list.map((s) => s.currentMetrics.currentCompatibility).reduce((a, b) => a + b) / list.length;
        final learning = list.map((s) => s.currentMetrics.learningEffectiveness).reduce((a, b) => a + b) / list.length;
        final pleasure = list.map((s) => s.currentMetrics.aiPleasureScore).reduce((a, b) => a + b) / list.length;
      ...
      developer.log('getUserAIMetrics failed', name: _logName, error: e, stackTrace: st);
      return [];
    }
  }
  ...
  Future<List<AreaAIMetrics>> getAreaAIMetrics() async {
    try {
      ...
      return byArea.entries.map((e) {
        final list = e.value;
        final n = list.length;
        final compat = list.map((u) => u.averageCompatibility).reduce((a, b) => a + b) / n;
        final learning = list.map((u) => u.averageLearningEffectiveness).reduce((a, b) => a + b) / n;
        final pleasure = list.map((u) => u.averagePleasureScore).reduce((a, b) => a + b) / n;
      ...
      developer.log('getAreaAIMetrics failed', name: _logName, error: e, stackTrace: st);
      return [];
    }
  }
  ...
  Future<List<RegionalAIMetrics>> getRegionalAIMetrics() async {
    try {
      ...
      return byRegion.entries.map((e) {
        final list = e.value;
        final n = list.length;
        final compat = list.map((a) => a.averageCompatibility).reduce((a, b) => a + b) / n;
        final learning = list.map((a) => a.averageLearningEffectiveness).reduce((a, b) => a + b) / n;
        final pleasure = list.map((a) => a.averagePleasureScore).reduce((a, b) => a + b) / n;
        final health = (compat * 0.35 + learning * 0.35 + pleasure * 0.30).clamp(0.0, 1.0);
      ...
      developer.log('getRegionalAIMetrics failed', name: _logName, error: e, stackTrace: st);
      return [];
    }
  }
  ...
  Future<UniversalAIMetrics?> getUniversalAIMetrics() async {
    try {
      ...
      final compat = regional.map((r) => r.averageCompatibility).reduce((a, b) => a + b) / n;
      final learning = regional.map((r) => r.averageLearningEffectiveness).reduce((a, b) => a + b) / n;
      final pleasure = regional.map((r) => r.averagePleasureScore).reduce((a, b) => a + b) / n;
      final health = regional.map((r) => r.networkHealthScore).reduce((a, b) => a + b) / n;
      ...
      developer.log('getUniversalAIMetrics failed', name: _logName, error: e, stackTrace: st);
      return null;
    }
  }
  ...
  Future<List<CrossLevelPattern>> analyzeCrossLevelPatterns() async {
    try {
      ...
      if (userMetrics.length >= 2 && areaMetrics.isNotEmpty) {
        final avgPleasureUser = userMetrics.map((u) => u.averagePleasureScore).reduce((a, b) => a + b) / userMetrics.length;
        final avgPleasureArea = areaMetrics.map((a) => a.averagePleasureScore).reduce((a, b) => a + b) / areaMetrics.length;
        final correlation = (avgPleasureUser - avgPleasureArea).abs() < 0.2 ? 0.8 : 0.5;
      ...
      if (regionalMetrics.length >= 2) {
        final scores = regionalMetrics.map((r) => r.networkHealthScore).toList();
      ...
      developer.log('analyzeCrossLevelPatterns failed', name: _logName, error: e, stackTrace: st);
      return [];
    }
  }
  ...
  Future<NetworkFlow> visualizeNetworkFlow() async {
    try {
      ...
      developer.log('visualizeNetworkFlow failed', name: _logName, error: e, stackTrace: st);
