...
class ActionHistoryService {
  ...
  Future<void> recordAction(ActionResult result) async {
    try {
      ...
      _logger.info('Recorded action: ${result.intent?.type ?? "unknown"}',
          tag: _logName);
    } catch (e, stackTrace) {
      _logger.error(
  ...
  Future<void> addAction({
    ...
    try {
      ...
      await _recordContrastiveTupleIfPresent(
        intent: intent,
  ...
  Future<void> _recordContrastiveTupleIfPresent({
    required ActionIntent intent,
    required ActionResult result,
    String? userId,
    Map<String, dynamic>? context,
    ...
    try {
      ...
      final recommendedActionType = _extractRecommendedActionType(context);
      if (recommendedActionType == null || recommendedActionType.isEmpty) {
        return;
      }

      final actualActionType = intent.type;
      if (recommendedActionType == actualActionType) {
        return;
      }

      final resolvedUserId = userId ?? _extractUserId(intent);
      final agentId = _extractAgentId(context, resolvedUserId);
      final contextState = Map<String, dynamic>.from(
          context?['state_before'] as Map? ?? const {});
      final contextMetadata =
          Map<String, dynamic>.from(context?['metadata'] as Map? ?? const {});

      final outcomeEvent =
          result.success ? 'actual_action_succeeded' : 'actual_action_failed';
      ...
      final tuple = EpisodicTuple(
        ...
        actionType: 'contrastive_action_choice',
        actionPayload: {
          'recommended_action': {
          ...
          'actual_action': {
            ...
            'metadata': _extractIntentMetadata(intent),
          },
        },
        nextState: {
      ...
      _logger.error(
        'Error recording contrastive action tuple',
        error: e,
        stackTrace: stackTrace,
        tag: _logName,
      );
  ...
  String? _extractRecommendedActionType(Map<String, dynamic>? context) {
    if (context == null) return null;
    final direct = context['recommended_action_type'] as String?;
    if (direct != null && direct.isNotEmpty) {
      return direct;
    }
    final nested = context['recommended_action'] as Map?;
    if (nested == null) return null;
    final nestedType = nested['type'] as String?;
    if (nestedType == null || nestedType.isEmpty) return null;
    return nestedType;
  }

  String _extractAgentId(Map<String, dynamic>? context, String? userId) {
    final contextAgentId = context?['agent_id'] as String?;
    if (contextAgentId != null && contextAgentId.isNotEmpty) {
  ...
  String _extractUserId(ActionIntent intent) {
    if (intent is CreateSpotIntent) return intent.userId;
    if (intent is CreateListIntent) return intent.userId;
    if (intent is AddSpotToListIntent) return intent.userId;
    if (intent is CreateEventIntent) return intent.userId;
    return '';
  }

  Map<String, dynamic> _extractIntentMetadata(ActionIntent intent) {
    ...
    try {
      ...
      _storage.write(_storageKey, data).catchError((e) {
        ...
        if (e.toString().contains('MissingPluginException') ||
          ...
          _logger.debug('Storage write skipped in test environment',
              tag: _logName);
          return;
        }
  ...
  Future<UndoResult> _performUndo(
      ActionIntent intent, ActionResult result) async {
    try {
  ...
  Future<UndoResult> _undoCreateSpot(
      CreateSpotIntent intent, ActionResult result) async {
    try {
      ...
      return UndoResult(
        success: false,
        message:
            'Spot deletion not yet implemented. Please delete spot "$spotId" manually.',
        data: {'spotId': spotId},
      );
    } catch (e, stackTrace) {
      _logger.error(
  ...
  /// Undo list creation
  /// Phase 7 Week 33: Enhanced with ActionExecutor integration
  Future<UndoResult> _undoCreateList(
      CreateListIntent intent, ActionResult result) async {
    try {
      ...
      return UndoResult(
        success: false,
        message:
            'List deletion not yet implemented. Please delete list "$listId" manually.',
        data: {'listId': listId},
      );
    } catch (e, stackTrace) {
      _logger.error(
  ...
  /// Phase 7 Week 33: Enhanced with ActionExecutor integration
  Future<UndoResult> _undoAddSpotToList(
      AddSpotToListIntent intent, ActionResult result) async {
    try {
      _logger.info(
          'Undo add spot to list: ${intent.spotId} -> ${intent.listId}',
          tag: _logName);
      ...
      return UndoResult(
        success: false,
        message:
            'Spot removal not yet implemented. Please remove spot "$spotId" from list "$listId" manually.',
