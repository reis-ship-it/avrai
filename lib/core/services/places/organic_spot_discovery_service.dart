...
import 'package:avrai/core/models/geographic/discovered_spot_candidate.dart';
import 'package:avrai/core/services/places/geohash_service.dart';
import 'package:avrai/core/ai/perpetual_list/models/visit_pattern.dart';
...
class OrganicSpotDiscoveryService {
  ...
  Future<DiscoveredSpotCandidate?> processUnmatchedVisit({
    ...
    if (candidate != null) {
      ...
      candidate = DiscoveredSpotCandidate(
        ...
        weekendHeavy: dayOfWeek == DayOfWeek.saturday ||
  ...
  DiscoveredSpotCandidate _updateCandidateWithVisit({
    ...
    final newLat = candidate.centroidLatitude * (1 - weight) +
        latitude * weight;
    final newLon = candidate.centroidLongitude * (1 - weight) +
        longitude * weight;

    // Update running average dwell time
    final newAvgDwell =
        ((candidate.averageDwellMinutes * candidate.visitCount) + dwellMinutes) ~/
            newVisitCount;
