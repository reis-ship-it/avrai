...
import 'package:avrai/core/models/visualization_style.dart';
...
class ThreeJsBridgeService {
  ...
  Future<void> _waitForReady() async {
    ...
    try {
      await onReady.first.timeout(
        _readyTimeout,
        onTimeout: () {
          developer.log('WebView ready timeout after ${_readyTimeout.inSeconds}s',
              name: _logName);
  ...
  Future<void> renderKnot({
    ...
    final data = {
      'coordinates':
          knot.coordinates.map((v) => [v.x, v.y, v.z]).toList(),
  ...
  Future<void> renderWorldsheet({
    ...
    final data = {
      'timePoints': worldsheet.timePoints
      ...
      'fabricData': worldsheet.fabricData
          .map((f) => <String, dynamic>{
                'strandPositions': f.strandPositions
                    .map((strand) =>
                        strand.map((v) => [v.x, v.y, v.z]).toList())
                    .toList(),
    ...
    await _evaluateJS(
        'window.worldsheetRenderer.render(${jsonEncode(data)})');
  }
  ...
  Future<void> startBirthExperience({
    ...
    await _evaluateJS(
        'window.birthExperience.start(${jsonEncode(data)})');
  }
