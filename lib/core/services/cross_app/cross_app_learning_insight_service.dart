...
class CrossAppLearningInsightService {
  ...
  Future<bool> recordInsight(CrossAppLearningInsight insight) async {
    ...
    try {
      ...
      if (_insights[insight.source]!.length > _maxInsightsPerSource) {
        _insights[insight.source] = _insights[insight.source]!
            .take(_maxInsightsPerSource)
            .toList();
      }
      ...
      // Update source stats
      _updateSourceStats(insight.source, (stats) => stats.copyWith(
  ...
  Future<void> clearInsightsForSource(CrossAppDataSource source) async {
    ...
    try {
      _insights[source] = [];
      _updateSourceStats(source, (stats) => stats.copyWith(
        insightCount: 0,
      ));
      await _saveToStorage();
  ...
  /// Returns the number of insights that were cleared.
  Future<int> clearInsightsInDateRange(DateTime startDate, DateTime endDate) async {
    if (!_isInitialized) await initialize();
    ...
    try {
      ...
      if (insightsJson != null) {
        ...
        for (final source in CrossAppDataSource.values) {
          ...
          _insights[source] = sourceInsights
              .map((i) => CrossAppLearningInsight.fromJson(i as Map<String, dynamic>))
              .toList();
        }
      } else {
        // Initialize empty
  ...
  Future<void> _saveToStorage() async {
    ...
    try {
      ...
      if (_lastLearningAt != null) {
        await _prefs!.setString(_keyLastLearning, _lastLearningAt!.toIso8601String());
      }
