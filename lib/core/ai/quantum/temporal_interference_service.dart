...
class TemporalInterferenceService {
  ...
  Future<TemporalInterferencePattern> detectInterferencePattern({
    ...
    try {
      ...
      final interferences = [dailyInterference, weeklyInterference, seasonalInterference];
      final constructiveCount = interferences.where((i) => i['type'] == 'constructive').length;
      final destructiveCount = interferences.where((i) => i['type'] == 'destructive').length;
  ...
  Future<double> calculateInterferenceCorrectedCompatibility({
    ...
    switch (interference.type) {
      case TemporalInterferenceType.constructive:
        ...
        correctedCompatibility = baseCompatibility + (interference.strength * 0.1);
        break;
