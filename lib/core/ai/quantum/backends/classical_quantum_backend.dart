...
import 'package:avrai/core/ai/quantum/quantum_entanglement_ml_service.dart';
  ...
  Future<EntangledQuantumState> createEntangledState(
    ...
    for (int i = 0; i < entityStates.length; i++) {
      for (int j = i + 1; j < entityStates.length; j++) {
        ...
        final key =
            '${entityStates[i].entityId}_${entityStates[j].entityId}';
    ...
    if (pairwiseFidelities.isEmpty) {
      ...
      entanglementStrength = math.pow(product, 1.0 / pairwiseFidelities.length)
          .toDouble()
          .clamp(0.0, 1.0);
    }
