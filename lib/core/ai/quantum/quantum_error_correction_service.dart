...
class QuantumErrorCorrectionService {
  ...
  Future<Map<String, double>> decodeAndCorrect({
    ...
    try {
      ...
      for (int i = 0; i < correctedData.length && i < originalKeys.length; i++) {
      ...
      for (int i = correctedData.length; i < originalKeys.length; i++) {
        correctedState[originalKeys[i]] = encoded.originalState[originalKeys[i]] ?? 0.0;
      }
  ...
  List<double> _decodeRepetition3(
    ...
    for (int i = 0; i < encoded.length; i += 3) {
      ...
      double corrected;
      if ((v1 - v2).abs() < (v1 - v3).abs() && (v1 - v2).abs() < (v2 - v3).abs()) {
        // v1 and v2 agree
        corrected = (v1 + v2) / 2.0;
  ...
  List<DetectedError> _detectShorErrors(EncodedQuantumState encoded) {
    ...
    for (int i = 0; i < encoded.encodedData.length; i += 9) {
      ...
      final groups = [
        [encoded.encodedData[i], encoded.encodedData[i + 1], encoded.encodedData[i + 2]],
        [encoded.encodedData[i + 3], encoded.encodedData[i + 4], encoded.encodedData[i + 5]],
        [encoded.encodedData[i + 6], encoded.encodedData[i + 7], encoded.encodedData[i + 8]],
      ];
