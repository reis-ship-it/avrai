...
class AI2AIListLearningIntegration {
  ...
  bool _isRateLimitAllowed(String userId) {
    ...
    // Check hourly limit
    final eventsInLastHour = timestamps.where((t) => t.isAfter(oneHourAgo)).length;
    ...
    // Check daily limit
    final eventsInLastDay = timestamps.where((t) => t.isAfter(oneDayAgo)).length;
  ...
  Future<bool> learnFromListInteraction({
    ...
    final scaledUpdates = _scaleLearningUpdates(collapseResult.dimensionUpdates);
  ...
  Map<String, double> _scaleLearningUpdates(Map<String, double> updates) {
    ...
    return updates.map(
      (key, value) => MapEntry(key, value * (listInteractionLearningRate / VibeConstants.ai2aiLearningRate)),
    );
  }
