...
class StringTheoryPossibilityEngine {
  ...
  Future<List<PossibilityState>> generatePossibilitySpace({
    ...
    while (possibilities.length < branchCount) {
      possibilities.add(_createVariationState(currentDimensions, possibilities.length));
    }
    if (possibilities.length > branchCount) {
  ...
  Future<PossibilityCollapseResult> collapseFromObservation({
    ...
    final observationDimensions = _interactionToObservationDimensions(interaction);
  ...
  void _normalizeProbabilities(List<PossibilityState> possibilities) {
    final totalProbability = possibilities
        .map((p) => p.probability)
        .fold(0.0, (a, b) => a + b);

    for (final possibility in possibilities) {
      possibility.normalizedProbability = 
          totalProbability > 0 ? possibility.probability / totalProbability : 0.0;
    }
  }
