...
import 'package:avrai/data/datasources/remote/places_datasource.dart';
...
class GenerationEngine {
  ...
  List<SuggestedList> groupIntoLists({
    ...
    for (var i = 0; i < listCount && i < sortedCategories.length; i++) {
      ...
      final avgNovelty = categoryPlaces
              .map((c) => c.noveltyScore)
              .fold(0.0, (a, b) => a + b) /
          categoryPlaces.length;
  ...
  Future<List<Spot>> _getNearbyPlaces(ListGenerationContext context) async {
    ...
    for (final category in categories.take(5)) {
      try {
        final categoryPlaces = await _placeListGenerator.searchPlacesForCategory(
          category: category,
          query: category,
          latitude: context.currentLocation.latitude,
          longitude: context.currentLocation.longitude,
  ...
  Map<String, double> _placeToDimensions(Spot place, ListGenerationContext context) {
  ...
  double _calculateNovelty({
    ...
    final categoryVisits = context.visitPatterns
  ...
  double _calculateTimeliness({
    ...
    final slotActivity = context.preferenceSignals.activeTimeSlots[currentSlot] ?? 0.5;
  ...
  String _extractCategoryFromTitle(String title) {
    ...
    for (final word in words) {
      if (!['the', 'a', 'an', 'in', 'at', 'for', 'your', 'my'].contains(word.toLowerCase())) {
