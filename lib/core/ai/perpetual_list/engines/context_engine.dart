...
class ContextEngine {
  ...
  Future<List<AI2AIInsightSummary>> _getRecentAI2AIInsights(String userId) async {
  ...
  Future<ListHistory> _getListHistory(String userId) async {
    try {
      ...
      final createdLists = allLists.where((l) => l.curatorId == userId).toList();
  ...
  bool _isColdStart(List<VisitPattern> patterns, PersonalityProfile personality) {
    // Cold start if:
