...
class TriggerEngine {
  ...
  Future<TriggerDecision> shouldGenerateLists({
    ...
    if (_shouldTriggerTimeBasedCheckin(context)) {
      ...
      metadata['timeWindow'] = context.isInMorningWindow ? 'morning' : 'evening';
    }
    ...
    if (_shouldTriggerAI2AILearning(context)) {
      ...
      metadata['highQualityInsights'] = context.recentAI2AIInsights
          .where((i) => i.isHighQuality)
          .length;
    }
  ...
  bool _shouldTriggerAI2AILearning(TriggerContext context) {
    ...
    final highQualityCount = context.recentAI2AIInsights
        .where((i) => i.isHighQuality)
        .length;
  ...
  int _calculateListCount(List<TriggerReason> triggers, TriggerContext context) {
