...
void main() {
  ...
  test('builds human-readable replay action explanations with kernel lanes', () {
    final explanations = const BhamReplayActionExplanationService().buildExplanations(
    ...
    expect(explanations.first.kernelLanes, containsAll(<String>['when', 'where', 'who', 'how']));
    expect(explanations.first.supportingSourceRefs, containsAll(<String>['Source A', 'Source B']));
    expect(explanations.first.explanation, contains('Birmingham-local circuit'));
  });

  test('includes sampled daily actor explanations when daily behavior is provided', () {
    final explanations = const BhamReplayActionExplanationService().buildExplanations(
      environment: buildEnvironment(),
      populationProfile: buildPopulationProfile(),
      placeGraph: buildPlaceGraph(),
