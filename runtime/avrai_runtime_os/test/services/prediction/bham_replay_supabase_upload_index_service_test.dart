...
ReplayStoragePartitionManifest _buildPartitionManifest({
  ...
  Map<String, dynamic> metadata = const <String, dynamic>{
    'storagePartitionManifestRef':
        'replay_storage_partition_manifest:bham-replay-world-2023:2023:1000',
    'sourceStorageExportManifestRef':
        'replay_storage_export_manifest:bham-replay-world-2023:2023',
    'sourceTrainingManifestRef':
        'replay_training_export_manifest:bham-replay-world-2023:2023:run-1',
    'sourceTrainingRunRef': 'run-1',
...
void main() {
  test('buildUploadManifest uses partitions for large replay artifacts',
    ...
    final exportManifest = ReplayStorageExportManifest(
      ...
      metadata: const <String, dynamic>{
        ...
        'sourceTrainingRunRef': 'run-1',
        'cityCode': 'bham',
        'cityPackId': 'bham_core_2023',
        'cityPackManifestRef': 'city_packs/bham/2023_manifest.json',
    ...
    expect(
      manifest.metadata['simulationEnvironmentId'],
      'bham-simulation-environment-2023',
    );
    expect(
      manifest.metadata['simulationEnvironmentNamespace'],
      'simulation-world/bham/2023',
    );
    expect(
      manifest.metadata['simulationLabel'],
      'Bham Simulation Environment 2023',
    );
    expect(manifest.metadata['cityPackId'], 'bham_core_2023');
    expect(
      manifest.metadata['cityPackStructuralRef'],
      'city_pack:bham_core_2023',
    );
    expect(
      manifest.metadata['storageUploadManifestRef'],
      'replay_storage_upload_manifest:bham-replay-world-2023:2023:dry',
    );
    expect(
      manifest.metadata['sourceStorageExportManifestRef'],
      'replay_storage_export_manifest:bham-replay-world-2023:2023',
    );
    expect(
      manifest.metadata['sourceStoragePartitionManifestRef'],
      'replay_storage_partition_manifest:bham-replay-world-2023:2023:1000',
    );
    expect(
      manifest.metadata['sourceTrainingManifestRef'],
      'replay_training_export_manifest:bham-replay-world-2023:2023:run-1',
    );
    expect(manifest.metadata['sourceTrainingRunRef'], 'run-1');
    final lifecycle = ArtifactLifecycleMetadata.fromJson(
      Map<String, dynamic>.from(
        manifest.metadata['artifactLifecycle'] as Map,
      ),
    );
    expect(lifecycle.artifactClass, ArtifactLifecycleClass.archive);
    expect(lifecycle.artifactState, ArtifactLifecycleState.accepted);
    expect(lifecycle.retentionPolicy.mode, ArtifactRetentionMode.archive);
  });
  ...
  test('buildUploadManifest fails closed on replay binding drift', () {
    const service = BhamReplaySupabaseUploadIndexService();
    ...
    final partitionManifest = _buildPartitionManifest(
      metadata: const <String, dynamic>{
        ...
        'sourceStorageExportManifestRef':
            'replay_storage_export_manifest:bham-replay-world-2023:2023',
        'sourceTrainingManifestRef':
            'replay_training_export_manifest:bham-replay-world-2023:2023:run-2',
        'sourceTrainingRunRef': 'run-1',
      },
    );
    ...
    expect(
      ...
      throwsA(
        isA<StateError>().having(
          (error) => error.message,
          'message',
          contains('same source training manifest'),
        ),
      ),
    );
  });
  ...
  test('uploadAndIndex writes replay-only uploads and index rows', () async {
    ...
    final replayRunRow = fakeGateway.upserts['replay_runs']!.single['metadata']
        as Map<String, dynamic>;
    expect(
      replayRunRow['simulationEnvironmentId'],
      'bham-simulation-environment-2023',
    );
    expect(
      replayRunRow['simulationEnvironmentNamespace'],
      'simulation-world/bham/2023',
    );
    final lineageRow = fakeGateway.upserts['replay_lineage']!.single['metadata']
        as Map<String, dynamic>;
    expect(
      lineageRow['simulationEnvironmentId'],
      'bham-simulation-environment-2023',
    );
    expect(result.metadata['retentionCleanupAttempted'], isTrue);
    expect(result.metadata['retentionCleanupSucceeded'], isTrue);
    expect(
      (result.metadata['deletedSourceRoots'] as List<dynamic>).length,
      2,
    );
    expect(
      (result.metadata['sourceLifecycleStates']
          as Map<String, dynamic>)['replay_storage_staging'],
      'expired',
    );
    expect(
      (result.metadata['sourceLifecycleStates']
          as Map<String, dynamic>)['replay_storage_partitions'],
      'expired',
    );
  ...
  test('uploadAndIndex fails closed on malformed report artifact payloads',
      () async {
    final tempDir =
        await Directory.systemTemp.createTemp('bham_replay_upload_contract');
    addTearDown(() async {
      if (tempDir.existsSync()) {
        await tempDir.delete(recursive: true);
      }
    ...
    calibrationFile.writeAsStringSync(jsonEncode(<dynamic>['not-an-object']));
    ...
    expect(
      ...
      throwsA(
        isA<ReplayAuthorityEndpointContractViolation>().having(
          (error) => error.message,
          'message',
          contains(
            '57_BHAM_REPLAY_CALIBRATION_REPORT_2023.json must decode to an object',
          ),
        ),
      ),
    );
