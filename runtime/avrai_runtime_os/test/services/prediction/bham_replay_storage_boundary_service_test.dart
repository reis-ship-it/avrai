...
void main() {
  ...
  test('passes when replay uses isolated schema and replay-prefixed buckets',
    ...
    final report = service.buildReport(
      ...
      replayYear: 2024,
      artifactMetadata: const <String, dynamic>{
        'cityCode': 'atx',
    ...
    expect(
      report.metadata['simulationEnvironmentId'],
      'atx-simulation-environment-2024',
    );
    expect(
      report.metadata['simulationEnvironmentNamespace'],
      'simulation-world/atx/2024',
    );
    expect(
      report.metadata['simulationLabel'],
      'Atx Simulation Environment 2024',
    );
    expect(report.metadata['cityPackId'], 'austin_core_2024');
    expect(
      report.metadata['cityPackStructuralRef'],
  ...
  test('fails when replay reuses app schema or bucket names', () {
    ...
    expect(
      report.violations
          .any((entry) => entry.contains('Replay buckets overlap')),
      isTrue,
    );
    expect(
      report.violations.any(
        (entry) => entry
            .contains('Replay metadata tables must use the replay_ prefix'),
      ),
      isTrue,
    );
