...
void main() {
  ...
  test('accepts replay as Monte Carlo base year only when all gates pass', () {
    final manifest =
        const BhamReplayTrainingExportManifestService().buildManifest(
    ...
    expect(
      manifest.metadata['legacyEnvironmentId'],
      'env-1',
    );
    expect(
      manifest.metadata['legacyEnvironmentNamespace'],
      'replay-world/bham/2023/run-1',
    );
    expect(
    ...
    expect(manifest.metadata['cityPackId'], 'austin_core_2024');
    expect(
      manifest.metadata['cityPackStructuralRef'],
      'city_pack:austin_core_2024',
    );
    expect(
      manifest.metadata['trainingManifestRef'],
      'replay_training_export_manifest:env-1:2023:run-1',
    );
    expect(manifest.metadata['replayRunRef'], 'run-1');
    final lifecycle = ArtifactLifecycleMetadata.fromJson(
      Map<String, dynamic>.from(
        manifest.metadata['artifactLifecycle'] as Map,
      ),
    );
    expect(lifecycle.artifactClass, ArtifactLifecycleClass.canonical);
    expect(lifecycle.artifactState, ArtifactLifecycleState.accepted);
    expect(lifecycle.retentionPolicy.mode, ArtifactRetentionMode.keepForever);
  ...
  test('marks replay as not ready when realism or isolation fails', () {
    ...
    expect(
      ...
      contains('2023 Austin single-year simulation still has realism'),
    );
    expect(manifest.metadata['cityDisplayName'], 'Austin');
    expect(
      manifest.metadata['cityPackManifestRef'],
