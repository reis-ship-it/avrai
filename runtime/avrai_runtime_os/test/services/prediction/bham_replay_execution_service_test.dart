...
void main() {
  ...
  test('builds an ordered replay execution plan from consolidated artifact',
    ...
    expect(plan.metadata['citySlug'], 'atx');
    expect(plan.metadata['cityDisplayName'], 'Austin');
    expect(plan.metadata['cityPackId'], 'austin_core_2024');
    expect(plan.metadata['cityPackManifestRef'],
        'city_packs/atx/2024_manifest.json');
    expect(
        plan.metadata['cityPackStructuralRef'], 'city_pack:austin_core_2024');
  });
  ...
  test('executes replay plan into ordered runtime events', () async {
    ...
    final stored = await kernel.queryRuntimeEvents(
      const RuntimeTemporalEventQuery(source: 'replay_execution', limit: 10),
    );
