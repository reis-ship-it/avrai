...
void main() {
  test('fallback response includes active reality-model artifacts', () async {
    final port = _FakeRealityModelPort();
    final service = RealityModelCheckInService(realityModelPort: port);
    ...
    expect(port.lastRequest?.cityCode, 'simulation');
    expect(port.lastRequest?.localityCode, 'simulation_reality');
  });
...
class _FakeRealityModelPort implements RealityModelPort {
  RealityModelEvaluationRequest? lastRequest;
