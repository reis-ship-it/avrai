...
void main() {
  group('MeshRuntimeGovernanceOrchestrationLane', () {
    test('records governed mesh plan and outcome with route receipts',
        () async {
      ...
      final plan =
        ...
        destinationId: 'locality:agent-1',
        ...
        logName: 'MeshRuntimeGovernanceOrchestrationLaneTest',
      );

      expect(plan.recordId, 'record-1');
      expect(plan.routeReceipt.plannedRoutes, hasLength(2));
      expect(plan.routeReceipt.status, 'planned');
      expect(fakeKernelOs.lastEnvelope?.eventType, 'mesh_forward_plan');
      ...
      final outcome =
        ...
        destinationId: 'locality:agent-1',
        payloadKind: 'locality_agent_update',
        forwardedPeerIds: const <String>['peer-a'],
        failedPeerIds: const <String>['peer-b'],
        geographicScope: 'locality',
        logger: const AppLogger(defaultTag: 'test'),
        logName: 'MeshRuntimeGovernanceOrchestrationLaneTest',
      );

      expect(outcome.recordId, 'record-2');
      expect(outcome.routeReceipt.status, 'forwarded');
      expect(outcome.routeReceipt.attemptedRoutes, hasLength(2));
      expect(outcome.routeReceipt.winningRoute, isNotNull);
      expect(outcome.routeReceipt.winningRoute?.metadata['peer_id'], 'peer-a');
      expect(outcome.routeReceipt.custodyAcceptedAtUtc, isNotNull);
      expect(outcome.routeReceipt.deliveredAtUtc, isNull);
      expect(fakeKernelOs.lastEnvelope?.eventType, 'mesh_forward_observation');
      expect(fakeKernelOs.lastWhyRequest?.actualOutcome, 'mesh_forwarded');
    });
  });
}
