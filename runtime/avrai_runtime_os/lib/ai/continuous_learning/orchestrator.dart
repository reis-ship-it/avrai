...
import 'package:avrai_runtime_os/ai/continuous_learning/base/learning_dimension_engine.dart';
import 'package:avrai_runtime_os/ai/continuous_learning/engines/personality_learning_engine.dart';
import 'package:avrai_runtime_os/ai/continuous_learning/engines/behavior_learning_engine.dart';
import 'package:avrai_runtime_os/ai/continuous_learning/engines/preference_learning_engine.dart';
import 'package:avrai_runtime_os/ai/continuous_learning/engines/interaction_learning_engine.dart';
import 'package:avrai_runtime_os/ai/continuous_learning/engines/location_intelligence_engine.dart';
...
class ContinuousLearningOrchestrator {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      developer.log('Continuous learning orchestrator initialized',
          name: _logName);
    } catch (e) {
  ...
  Future<void> _learnDimension(String dimension, LearningData data) async {
    try {
      ...
      final engine = _engines.firstWhere(
        ...
        orElse: () =>
            throw Exception('No engine found for dimension: $dimension'),
      );
      ...
      for (final entry in results.entries) {
        ...
        final improvement =
            entry.value - (_currentLearningState[entry.key] ?? 0.5);
  ...
  Map<String, double> _analyzeDataSourceEffectiveness() {
    ...
    final totalEvents =
        sourceCounts.values.fold(0, (sum, count) => sum + count);
    if (totalEvents > 0) {
