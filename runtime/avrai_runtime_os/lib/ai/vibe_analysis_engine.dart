...
class UserVibeAnalyzer {
  ...
  Future<UserVibe> compileUserVibe(
    ...
    try {
      ...
      developer.log('User vibe compiled: ${userVibe.getVibeArchetype()}',
          name: _logName);
      developer.log(
          'Vibe energy: ${(userVibe.overallEnergy * 100).round()}%, social: ${(userVibe.socialPreference * 100).round()}%',
          name: _logName);
  ...
  Future<VibeCompatibilityResult> analyzeVibeCompatibility(
    ...
    try {
      developer.log(
          'Analyzing vibe compatibility between ${localVibe.getVibeArchetype()} and ${remoteVibe.getVibeArchetype()}',
          name: _logName);

      // Calculate basic compatibility (personality-based)
      final basicCompatibility =
          localVibe.calculateVibeCompatibility(remoteVibe);
      ...
      if (localUser != null && remoteUser != null) {
        ...
        final allowOverride = recentActions != null
            ? ageFilter.shouldAllowAgeOverride(
                localUser, remoteUser, recentActions)
            : false;
      ...
      final connectionStrength =
          await _calculateConnectionStrength(localVibe, remoteVibe);

      // Determine optimal interaction style
      final interactionStyle =
          await _determineOptimalInteractionStyle(localVibe, remoteVibe);

      // Calculate trust building potential
      final trustBuildingPotential =
          await _calculateTrustBuildingPotential(localVibe, remoteVibe);

      final result = VibeCompatibilityResult(
        basicCompatibility:
            finalCompatibility, // Use age-adjusted compatibility
        ...
        recommendedConnectionDuration:
            _calculateRecommendedDuration(finalCompatibility),
        connectionPriority: _calculateConnectionPriority(
            finalCompatibility, aiPleasurePotential),
      );

      developer.log(
          'Vibe compatibility: ${(finalCompatibility * 100).round()}% (age-adjusted), AI pleasure: ${(aiPleasurePotential * 100).round()}%',
          name: _logName);
  ...
  Future<UserVibe> refreshUserVibe(
      String userId, PersonalityProfile updatedPersonality) async {
    developer.log('Refreshing user vibe after personality evolution',
        name: _logName);
  ...
  Future<List<VibeEvolutionEvent>> getVibeEvolutionHistory(
      String userId) async {
    try {
  ...
  Future<CommunityVibeInsights> analyzeCommunityVibePatterns() async {
    try {
      ...
      final archetypeDistribution =
          await _analyzeArchetypeDistribution(communityVibes);
      ...
      final emergingPatterns =
          await _identifyEmergingVibePatterns(communityVibes);
      ...
      developer.log('Error analyzing community vibe patterns: $e',
          name: _logName);
  ...
  Future<VibeAuthenticityResult> validateVibeAuthenticity(
      UserVibe vibe, PersonalityProfile personality) async {
    try {
      ...
      final consistencyScore =
          await _checkPersonalityConsistency(vibe, personality);
  ...
  Future<PersonalityVibeInsights> _analyzePersonalityState(
    ...
    final insights = PersonalityVibeInsights(
      ...
      confidenceLevel: personality.dimensionConfidence.values.isNotEmpty
          ? personality.dimensionConfidence.values.reduce((a, b) => a + b) /
              personality.dimensionConfidence.length
          : 0.5,
    );
  ...
  Future<SocialVibeInsights> _analyzeSocialDynamics(
    ...
    final communityOrientation =
        personality.dimensions['community_orientation'] ?? 0.5;
    final socialDiscoveryStyle =
        personality.dimensions['social_discovery_style'] ?? 0.5;
    ...
    return SocialVibeInsights(
      ...
      trustNetworkStrength:
          personality.dimensions['trust_network_reliance'] ?? 0.6,
    );
  }
  ...
  Future<Map<String, double>> _compileVibeDimensions(
    ...
    vibeDimensions['social_discovery_style'] = (social.socialPreference * 0.5 +
            social.collaborationStyle * 0.3 +
    ...
    vibeDimensions['temporal_flexibility'] =
        (behavioral.spontaneityIndex * 0.6 + temporal.contextualModifier * 0.4)
            .clamp(0.0, 1.0);
    ...
    vibeDimensions['curation_tendency'] =
        (social.leadershipTendency * 0.6 + relationship.givingTendency * 0.4)
            .clamp(0.0, 1.0);
    ...
    vibeDimensions.forEach((dimension, value) {
      vibeDimensions[dimension] =
          (value * temporal.contextualModifier).clamp(0.0, 1.0);
    });
  ...
  Future<void> _storeVibeAnalytics(
      String userId, UserVibe vibe, PersonalityProfile personality) async {
    try {
      final analytics = {
      ...
      await _prefs.setString(
          '${_vibeAnalyticsKey}_$userId', analytics.toString());
    } catch (e) {
  ...
  Future<List<LearningOpportunity>> _analyzeLearningOpportunities(
      UserVibe localVibe, UserVibe remoteVibe) async {
    ...
    localVibe.anonymizedDimensions.forEach((dimension, localValue) {
      ...
      if (difference >= 0.3 && difference <= 0.7) {
        ...
        opportunities.add(LearningOpportunity(
          ...
          learningType:
              _determineLearningType(dimension, localValue, remoteValue),
        ));
      }
    });
  ...
  Future<double> _calculateConnectionStrength(
    ...
    final energyAlignment =
        1.0 - (localVibe.overallEnergy - remoteVibe.overallEnergy).abs();
    final socialAlignment =
        1.0 - (localVibe.socialPreference - remoteVibe.socialPreference).abs();
  ...
  Future<AI2AIInteractionStyle> _determineOptimalInteractionStyle(
      UserVibe localVibe, UserVibe remoteVibe) async {
    final compatibility = localVibe.calculateVibeCompatibility(remoteVibe);
    final energyDiff =
        (localVibe.overallEnergy - remoteVibe.overallEnergy).abs();
    final socialDiff =
        (localVibe.socialPreference - remoteVibe.socialPreference).abs();
  ...
  Future<double> _calculateTrustBuildingPotential(
      UserVibe localVibe, UserVibe remoteVibe) async {
    final localTrustDimension =
        localVibe.anonymizedDimensions['trust_network_reliance'] ?? 0.5;
    final remoteTrustDimension =
        remoteVibe.anonymizedDimensions['trust_network_reliance'] ?? 0.5;

    final trustCompatibility =
        1.0 - (localTrustDimension - remoteTrustDimension).abs();
    final overallCompatibility =
        localVibe.calculateVibeCompatibility(remoteVibe);

    return (trustCompatibility * 0.6 + overallCompatibility * 0.4)
        .clamp(0.0, 1.0);
  }
  ...
  double _calculateEvolutionMomentum(PersonalityProfile personality) {
    ...
    final recentEvolutions = personality.learningHistory['evolution_milestones']
            as List<DateTime>? ??
        [];
  ...
  Duration _calculateRecommendedDuration(double compatibility) {
    if (compatibility >= VibeConstants.highCompatibilityThreshold) {
      return const Duration(
          seconds: VibeConstants.maxInteractionDurationSeconds);
    } else if (compatibility >= VibeConstants.mediumCompatibilityThreshold) {
      return Duration(
          seconds: (VibeConstants.maxInteractionDurationSeconds * 0.7).round());
    } else {
      return const Duration(
          seconds: VibeConstants.minInteractionDurationSeconds);
    }
  }

  ConnectionPriority _calculateConnectionPriority(
      double compatibility, double aiPleasure) {
  ...
  LearningType _determineLearningType(
      String dimension, double localValue, double remoteValue) {
  ...
  Future<List<UserVibe>> _getAnonymizedCommunityVibes() async {
    try {
      ...
      developer.log('Error getting anonymized community vibes: $e',
          name: _logName);
      return [];
    }
  }

  Future<Map<String, double>> _analyzeArchetypeDistribution(
      List<UserVibe> vibes) async {
  ...
  Future<Map<String, double>> _analyzeEnergyPatterns(
      List<UserVibe> vibes) async {
    if (vibes.isEmpty) return {};
    ...
    final avgEnergy =
        vibes.map((v) => v.overallEnergy).reduce((a, b) => a + b) / total;
    patterns['average'] = avgEnergy;

    return patterns;
  }

  Future<Map<String, double>> _analyzeSocialPreferenceTrends(
      List<UserVibe> vibes) async {
    if (vibes.isEmpty) return {};
    ...
    final avgSocial =
        vibes.map((v) => v.socialPreference).reduce((a, b) => a + b) / total;
    trends['average'] = avgSocial;

    return trends;
  }

  Future<List<String>> _identifyEmergingVibePatterns(
      List<UserVibe> vibes) async {
    ...
    final highExplorationCount =
        vibes.where((v) => v.explorationTendency >= 0.7).length;
    if (highExplorationCount / vibes.length > 0.3) {
      patterns.add('exploration_focused_community');
    }

    // Identify social-focused patterns
    final highSocialCount =
        vibes.where((v) => v.socialPreference >= 0.7).length;
  ...
  Future<double> _checkPersonalityConsistency(
      UserVibe vibe, PersonalityProfile personality) async {
    try {
      ...
      for (final dimension in VibeConstants.coreDimensions) {
        ...
        if (vibeValue != null && personalityValue != null) {
          ...
          final dimensionConsistency =
              1.0 - (difference / 0.05).clamp(0.0, 1.0);
      ...
      if (validDimensions == 0) {
        return 0.5; // Neutral if no dimensions to compare
      }

      return (consistencyScore / validDimensions).clamp(0.0, 1.0);
    } catch (e) {
      developer.log('Error checking personality consistency: $e',
          name: _logName);
  ...
  Future<double> _detectArtificialPatterns(UserVibe vibe) async {
    try {
      ...
      final exploration =
          vibe.anonymizedDimensions['exploration_eagerness'] ?? 0.5;
      final energy = vibe.overallEnergy;
  ...
  Future<double> _checkTemporalConsistency(UserVibe vibe) async {
    try {
      ...
      final ageConsistency = 1.0 -
          (age.inDays / VibeConstants.vibeSignatureExpiryDays).clamp(0.0, 1.0);
...
class VibeCompatibilityResult {
  ...
  static VibeCompatibilityResult fallback() {
    return VibeCompatibilityResult(
      ...
      recommendedConnectionDuration:
          const Duration(seconds: VibeConstants.minInteractionDurationSeconds),
