...
class AnswerLayerOrchestrator {
  ...
  List<String> _claimsFromContext(LanguageRuntimeContext context) {
    ...
    if (conversationPreferences != null && conversationPreferences.isNotEmpty) {
      ...
      final timingSummary =
          conversationPreferences['governed_knowledge_timing_summary']
              ?.toString()
              .trim();
      if (timingSummary != null && timingSummary.isNotEmpty) {
        claims.add('local context timing: $timingSummary');
      }
      final freshness =
          _conversationKnowledgeFreshness(conversationPreferences);
      if (freshness == _ConversationKnowledgeFreshness.stale) {
        claims.add(
          'local governed knowledge is stale enough that AVRAI should answer more cautiously.',
        );
      } else if (freshness == _ConversationKnowledgeFreshness.fresh) {
        claims.add(
          'local governed knowledge is fresh enough to support grounded locality-sensitive phrasing.',
        );
      }
    }
  ...
  List<String> _evidenceRefsFromContext(LanguageRuntimeContext context) {
    ...
    if (context.conversationPreferences != null &&
      ...
      if (_hasConversationKnowledgeTiming(context.conversationPreferences)) {
        refs.add('conversation_preferences_timing');
      }
    }
    return refs;
  }
  ...
  String _confidenceBandFromContext(LanguageRuntimeContext context) {
    ...
    if (_conversationKnowledgeFreshness(context.conversationPreferences) ==
        _ConversationKnowledgeFreshness.fresh) {
      evidenceCount++;
    }
    return evidenceCount >= 3 ? 'medium' : 'low';
  }

  bool _hasConversationKnowledgeTiming(Map<String, dynamic>? preferences) {
    if (preferences == null || preferences.isEmpty) {
      return false;
    }
    return preferences['governed_knowledge_timing_summary'] != null ||
        preferences['governed_knowledge_occurred_at'] != null ||
        preferences['governed_knowledge_captured_at'] != null ||
        preferences['governed_knowledge_integrated_at'] != null ||
        preferences['governed_knowledge_synced_at'] != null;
  }

  _ConversationKnowledgeFreshness _conversationKnowledgeFreshness(
    Map<String, dynamic>? preferences,
  ) {
    if (!_hasConversationKnowledgeTiming(preferences)) {
      return _ConversationKnowledgeFreshness.none;
    }
    final effectiveAt = _parseKnowledgeTime(
    ...
    final age = DateTime.now().toUtc().difference(effectiveAt.toUtc());
  ...
  DateTime? _parseKnowledgeTime(Object? raw) {
    if (raw == null) {
      return null;
    }
...
enum _ConversationKnowledgeFreshness {
  none,
  presentUnknownAge,
  fresh,
  stale,
