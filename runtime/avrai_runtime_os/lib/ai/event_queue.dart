...
class EventQueue {
    ...
    try {
      ...
      _connectivitySubscription = _connectivity.onConnectivityChanged.listen(
        (List<ConnectivityResult> results) {
          ...
          if (hasConnection) {
            developer.log('Connectivity restored, processing queue',
                name: _logName);
            _processQueue();
          }
        },
      ...
      developer.log(
          '✅ EventQueue initialized with ${_queue.length} queued events',
          name: _logName);
    } catch (e, stackTrace) {
      developer.log(
  ...
  Future<void> _loadQueueFromStorage() async {
    try {
      ...
      _queue.addAll(
        queueList.map(
            (json) => InteractionEvent.fromJson(json as Map<String, dynamic>)),
      );
