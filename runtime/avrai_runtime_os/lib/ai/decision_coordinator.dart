...
class DecisionCoordinator {
  ...
  Future<InferenceResult> coordinate({
    ...
    try {
      ...
      if (isOffline) {
        ...
        decisionReason =
            'Offline: Using device-first strategy (ONNX + rules + AI2AI mesh)';
        ...
        chosenStrategy =
            InferenceStrategy.deviceFirst; // Cached uses device-first
        ...
        chosenStrategy = InferenceStrategy.deviceFirst;
        decisionReason =
            'Default: Using device-first strategy with Gemini fallback';
        developer.log(decisionReason, name: _logName);
  ...
  Future<InferenceResult> _getCachedRecommendations(
      Map<String, dynamic> input) async {
    developer.log('Getting cached recommendations for low latency',
        name: _logName);
