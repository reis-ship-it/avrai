...
class LearningRecommendationsGenerator {
  ...
  Future<List<OptimalPartner>> identifyOptimalLearningPartners(
    ...
    try {
      ...
      final baseline = currentPersonality.dimensions.isNotEmpty
          ...
          : (VibeConstants
                  .personalityArchetypes[currentPersonality.archetype] ??
              const <String, double>{});
  ...
  Future<List<LearningTopic>> generateLearningTopics(
    ...
    try {
      developer.log('Generating learning topics from patterns', name: _logName);
    ...
    try {
      ...
      if (areas.isEmpty) {
        ...
        for (final entry in currentPersonality.dimensions.entries) {
          final v = entry.value;
          final confidence =
              currentPersonality.dimensionConfidence[entry.key] ?? 0.8;
          final isExtreme = v <= 0.2 || v >= 0.8;
          final isLowConfidence = confidence < 0.5;
          if (isExtreme || isLowConfidence) {
            final priority =
                max(1.0 - confidence, ((v - 0.5).abs() * 2.0)).clamp(0.0, 1.0);
      ...
      developer.log('Error recommending development areas: $e', name: _logName);
      return [];
    }
  }
    ...
    try {
      ...
      if (partners.isNotEmpty && topics.isNotEmpty) {
        final avgCompatibility =
            partners.map((p) => p.compatibility).reduce((a, b) => a + b) /
                partners.length;

        final avgTopicPotential =
            topics.map((t) => t.potential).reduce((a, b) => a + b) /
                topics.length;
