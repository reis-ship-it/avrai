...
class LearningEffectivenessAnalyzer {
  ...
  Future<double> calculatePersonalityEvolutionRate(
    ...
    try {
      ...
      final avgDepth = recentChats.isEmpty
          ? 0.0
          : recentChats.map((c) => c.messages.length).reduce((a, b) => a + b) /
              recentChats.length;
      ...
      final avgInsightQuality = learningInsights.isEmpty
          ? 0.0
          : learningInsights.map((i) => i.reliability).reduce((a, b) => a + b) /
      ...
      final normalizedRate =
          min(1.0, evolutionIndicators / 30.0 / timeWindow * 7.0); // Per week
  ...
  Future<double> measureKnowledgeAcquisition(
    ...
    try {
      ...
      final acquisitionScore =
          (insightsPerChat * 0.6 + min(1.0, avgDepth / 10.0) * 0.4);

      return acquisitionScore.clamp(0.0, 1.0);
    } catch (e) {
      developer.log('Error measuring knowledge acquisition: $e',
          name: _logName);
      return 0.0;
    }
  }
  ...
  Future<double> assessInsightQuality(
    ...
    try {
      ...
      final highQualityCount =
          insights.where((i) => i.reliability >= 0.7).length;
      ...
      final qualityScore =
          (avgReliability * 0.5 + consistencyScore * 0.3 + diversityScore * 0.2)
              .clamp(0.0, 1.0);
  ...
  Future<double> measureCollectiveContribution(
    ...
    final totalMessages =
        chats.map((c) => c.messages.length).reduce((a, b) => a + b);
    ...
    final contributionRatio =
        totalMessages > 0 ? userMessages / totalMessages : 0.0;
