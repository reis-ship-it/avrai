...
class CommunityTrendsAnalyzer {
  ...
  Future<List<EmergingPattern>> identifyEmergingPatterns(
    ...
    for (final chat in chats) {
      ...
      if (chat.messages.length >= 10) {
        patterns['deep_conversation'] =
            (patterns['deep_conversation'] ?? 0) + 1;
      }

      // Pattern: multi-participant
      if (chat.participants.length >= 3) {
        patterns['group_interaction'] =
            (patterns['group_interaction'] ?? 0) + 1;
      }
    }
  ...
  Future<List<CommunityTrend>> analyzeCommunityTrends(
    ...
    final avgParticipants =
        chats.map((c) => c.participants.length).reduce((a, b) => a + b) /
            chats.length;
  ...
  Future<double> calculatePersonalityEvolutionRate(
    ...
    final normalizedRate =
        min(1.0, (interactionRate * 0.5 + messageRate * 0.5));
