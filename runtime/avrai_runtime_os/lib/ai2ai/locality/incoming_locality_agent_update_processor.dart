    ...
    if (!stored && sl.isRegistered<LocalityMemory>()) {
      try {
        ...
        await sl<LocalityMemory>().saveMeshUpdate(
          ...
          originOccurredAtUtc: DateTime.tryParse(
            (payload['origin_occurred_at'] ?? '').toString(),
          )?.toUtc(),
          localStateCapturedAtUtc: DateTime.tryParse(
            (payload['local_state_captured_at'] ?? '').toString(),
          )?.toUtc(),
          exchangeCreatedAtUtc: DateTime.tryParse(
            (payload['created_at'] ?? '').toString(),
          )?.toUtc(),
          kernelExchangePhase:
              (payload['kernel_exchange_phase'] as String?)?.trim(),
        );
        logger.debug(
