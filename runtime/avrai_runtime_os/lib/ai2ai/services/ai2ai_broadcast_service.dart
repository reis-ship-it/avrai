...
class AI2AIBroadcastService {
  ...
  StreamSubscription<RealtimeMessage>? subscribeToVibeLearning({
    ...
    try {
      _logger.debug('Subscribing to vibe learning channel for agent: $agentId');
  ...
  Future<int?> measureRealtimeLatency({
    ...
    try {
      sub = listenToAI2AINetwork().listen((message) {
        ...
        completer
            .complete(message.timestamp.difference(sendTime).inMilliseconds);
      });
  ...
  Future<void> unsubscribeAll() async {
    try {
      _logger
          .debug('Unsubscribing from all channels (${_activeChannels.length})');
