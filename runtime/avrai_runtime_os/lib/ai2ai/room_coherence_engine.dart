...
class RoomCoherenceEngine {
  ...
  RoomCoherenceWindowResult observeWindowFrames({
    ...
    final overlapRatio =
        prev == null ? null : _jaccard(prev.nodeKeys, nodeKeys);
    ...
    final newNodeRatePerSec = (newNodesCount == null || windowSeconds == null)
        ? null
  ...
  bool _computeLinger({
    ...
    final recent =
        _samples.where((s) => !s.observedAt.isBefore(cutoff)).toList();
    ...
    if (overlapRatioMedian == null ||
        overlapRatioMedian < overlapRatioMedianMin) {
      return false;
    }
...
class RoomCoherenceFrame {
  ...
  List<double> psiVector() {
    final amps =
        List<double>.filled(SpotsBroadcastFrameV1.dimensionsCount, 0.0);
