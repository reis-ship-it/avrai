...
class CallingScoreNeuralModel {
  ...
  static const int inputSize =
      39; // User vibe (12) + Spot vibe (12) + Context (10) + Timing (5)
  ...
  static const String defaultModelPath =
      'assets/models/calling_score_model.onnx';
  ...
  Future<bool> loadModel({String? version, String? modelPath}) async {
    try {
      ...
      if (modelPath != null) {
        ...
        final activeVersionInfo =
            ModelVersionRegistry.getActiveCallingScoreVersion();
      ...
      try {
        ...
        developer.log(
            'Model file loaded from assets: $actualModelPath (version: $actualVersion)',
            name: _logName);
      } catch (e) {
        developer.log(
  ...
  double _ruleBasedPrediction(List<double> features) {
    ...
    final contextFactor =
        contextFeatures.reduce((a, b) => a + b) / contextFeatures.length;

    // Calculate timing factor (average of timing features)
    final timingFactor =
        timingFeatures.reduce((a, b) => a + b) / timingFeatures.length;

    // Weighted combination (similar to formula-based approach)
    final score =
        (vibeCompatibility * 0.50 + contextFactor * 0.30 + timingFactor * 0.20)
            .clamp(0.0, 1.0);
  ...
  List<double> prepareFeatures(Map<String, dynamic> record) {
    ...
    final userVibeDimensions =
        record['user_vibe_dimensions'] as Map<String, dynamic>? ?? {};
    ...
    final spotVibeDimensions =
        record['spot_vibe_dimensions'] as Map<String, dynamic>? ?? {};
    for (final dimension in [
      'exploration_eagerness',
      'community_orientation',
      'location_adventurousness',
    ...
    final contextFeatures =
        record['context_features'] as Map<String, dynamic>? ?? {};
    features.add(
        (contextFeatures['location_proximity'] as num?)?.toDouble() ?? 0.5);
    features
        .add((contextFeatures['journey_alignment'] as num?)?.toDouble() ?? 0.5);
    features
        .add((contextFeatures['user_receptivity'] as num?)?.toDouble() ?? 0.5);
    features.add(
        (contextFeatures['opportunity_availability'] as num?)?.toDouble() ??
            0.5);
    features
        .add((contextFeatures['network_effects'] as num?)?.toDouble() ?? 0.5);
    features.add(
        (contextFeatures['community_patterns'] as num?)?.toDouble() ?? 0.5);
    ...
    final timingFeatures =
        record['timing_features'] as Map<String, dynamic>? ?? {};
    features.add(
        (timingFeatures['optimal_time_of_day'] as num?)?.toDouble() ?? 0.5);
    features.add(
        (timingFeatures['optimal_day_of_week'] as num?)?.toDouble() ?? 0.5);
    features.add((timingFeatures['user_patterns'] as num?)?.toDouble() ?? 0.5);
    features
        .add((timingFeatures['opportunity_timing'] as num?)?.toDouble() ?? 0.5);
