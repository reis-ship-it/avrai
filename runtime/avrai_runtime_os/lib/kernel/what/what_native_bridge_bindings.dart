...
abstract class WhatNativeInvocationBridge {
...
class WhatNativeBridgeBindings implements WhatNativeInvocationBridge {
  static const String _logName = 'WhatNativeBridgeBindings';

  final WhatLibraryManager _libraryManager;
  ...
  WhatNativeBridgeBindings({
    WhatLibraryManager? libraryManager,
  }) : _libraryManager = libraryManager ?? WhatLibraryManager();
    ...
    try {
      ...
      _invokeJson = _lib!
          .lookup<NativeFunction<_NativeInvokeJson>>(
            'avrai_what_kernel_invoke_json',
          )
          .asFunction<_InvokeJson>();
      _freeJsonString = _lib!
          .lookup<NativeFunction<_NativeFreeJsonString>>(
            'avrai_what_kernel_free_string',
          )
          .asFunction<_FreeJsonString>();
      _available = true;
      developer.log('What native bridge initialized', name: _logName);
    } catch (error, stackTrace) {
      _available = false;
      developer.log(
        'What native bridge unavailable; using fallback transport: $error',
        name: _logName,
        error: error,
  ...
  Map<String, dynamic> invoke({
    ...
    if (!_available || _invokeJson == null) {
      throw StateError('What native bridge is not available.');
    }
    ...
    try {
      ...
      if (responsePtr.address == 0) {
        throw StateError('What native bridge returned a null response.');
      }
      final decoded = jsonDecode(responsePtr.toDartString());
      if (decoded is! Map<String, dynamic>) {
        throw StateError('What native bridge returned a non-map response.');
      }
      if (decoded['ok'] == false) {
        throw StateError(
