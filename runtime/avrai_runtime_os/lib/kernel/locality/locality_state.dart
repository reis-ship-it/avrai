...
class LocalityState extends Equatable {
  ...
  factory LocalityState.zero({
    ...
    return LocalityState(
      ...
      originOccurredAtUtc: DateTime.fromMillisecondsSinceEpoch(0, isUtc: true),
      localStateCapturedAtUtc:
          DateTime.fromMillisecondsSinceEpoch(0, isUtc: true),
      lastSyncAtUtc: DateTime.fromMillisecondsSinceEpoch(0, isUtc: true),
      kernelExchangePhase: 'locality_unresolved',
      topAlias: alias,
    );
  }

  DateTime get effectiveTemporalFreshnessAt =>
      localStateCapturedAtUtc ??
      lastSyncAtUtc ??
      originOccurredAtUtc ??
      freshness;
  ...
  LocalityState copyWith({
    ...
    return LocalityState(
      ...
      originOccurredAtUtc: originOccurredAtUtc ?? this.originOccurredAtUtc,
      localStateCapturedAtUtc:
          localStateCapturedAtUtc ?? this.localStateCapturedAtUtc,
      lastSyncAtUtc: lastSyncAtUtc ?? this.lastSyncAtUtc,
      kernelExchangePhase: kernelExchangePhase ?? this.kernelExchangePhase,
    );
  }
  ...
  Map<String, dynamic> toJson() => {
        ...
        'lastSyncAtUtc': lastSyncAtUtc?.toIso8601String(),
  ...
  factory LocalityState.fromJson(Map<String, dynamic> json) {
    ...
    return LocalityState(
      ...
      lastSyncAtUtc:
          DateTime.tryParse((json['lastSyncAtUtc'] ?? '').toString()),
...
class LocalityObservation extends Equatable {
  ...
  Map<String, dynamic> toJson() => {
        ...
        'key': key.toJson(),
        'occurredAtUtc': occurredAtUtc.toIso8601String(),
        if (localStateCapturedAtUtc != null)
          'localStateCapturedAtUtc': localStateCapturedAtUtc!.toIso8601String(),
        if (emittedAtUtc != null)
          'emittedAtUtc': emittedAtUtc!.toIso8601String(),
        'source': source,
  ...
  factory LocalityObservation.fromJson(Map<String, dynamic> json) {
    return LocalityObservation(
      ...
      localStateCapturedAtUtc:
          DateTime.tryParse((json['localStateCapturedAtUtc'] ?? '').toString()),
      emittedAtUtc: DateTime.tryParse((json['emittedAtUtc'] ?? '').toString()),
  ...
  LocalityAgentUpdateEventV1 toUpdateEvent({
    ...
    return LocalityAgentUpdateEventV1(
      ...
      emittedAtUtc: emittedAtUtc ?? this.emittedAtUtc,
      source: source,
