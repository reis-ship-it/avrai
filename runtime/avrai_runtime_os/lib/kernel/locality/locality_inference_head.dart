...
class LocalityInferenceHead {
  ...
  Future<LocalityState> resolveFromKey({
    ...
    final effectiveFreshness = _latestUtc(
      self.updatedAtUtc,
      delta.localStateCapturedAtUtc,
      delta.originOccurredAtUtc,
      delta.updatedAtUtc,
      null,
    );
    ...
    return LocalityState(
      ...
      originOccurredAtUtc: delta.originOccurredAtUtc,
      localStateCapturedAtUtc: delta.localStateCapturedAtUtc,
      lastSyncAtUtc: self.syncedAtUtc ?? self.updatedAtUtc,
      kernelExchangePhase: delta.kernelExchangePhase,
  ...
  Future<LocalityVisitComputation> observeVisit({
    ...
    final localStateCapturedAtUtc = DateTime.now().toUtc();
    ...
    final delta = LocalityAgentPersonalDeltaV1(
      ...
      updatedAtUtc: localStateCapturedAtUtc,
      originOccurredAtUtc: visit.checkInTime.toUtc(),
  ...
  DateTime _latestUtc(
    ...
    final values = <DateTime>[
      first.toUtc(),
      if (second != null) second.toUtc(),
      if (third != null) third.toUtc(),
      if (fourth != null) fourth.toUtc(),
      if (fifth != null) fifth.toUtc(),
    ];
    values.sort();
    return values.last;
  }
