...
class UrkStageECrossRuntimeConformanceValidator {
  ...
  UrkStageECrossRuntimeConformanceValidationResult validate({
    ...
    if (policy.requiredRuntimeCount <= 0 ||
        policy.maxPolicyDivergenceEvents < 0) {
      failures.add(
          UrkStageECrossRuntimeConformanceFailure.invalidRuntimeCountThreshold);
    }
    if (policy.requiredContractCoveragePct < 0 ||
        policy.requiredReplayDeterminismPct < 0) {
      failures.add(
          UrkStageECrossRuntimeConformanceFailure.invalidCoverageThreshold);
    }
    ...
    if (snapshot.observedContractCoveragePct <
        policy.requiredContractCoveragePct) {
      failures.add(UrkStageECrossRuntimeConformanceFailure
          .contractCoverageBelowThreshold);
    }
    ...
    if (snapshot.observedPolicyDivergenceEvents >
        policy.maxPolicyDivergenceEvents) {
      failures.add(
