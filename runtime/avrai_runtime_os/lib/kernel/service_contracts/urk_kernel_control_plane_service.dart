...
class UrkKernelControlPlaneService {
  ...
  Future<void> _stageKernelOfflineEvidenceReceipt({
    ...
    final normalizedKernelId = kernelId.trim();
    final normalizedRequestId = requestId.trim();
    final metadata = _parseReasonMetadata(reason);
    final signalTags = <String>{
      'event_type:activation_receipt',
      'source:urk_kernel_control_plane',
      if (metadata['privacy_mode'] case final privacyMode?)
        'privacy_mode:$privacyMode',
      if (metadata['runtime_lane'] case final runtimeLane?)
        'runtime_lane:$runtimeLane',
      if (metadata['reason_code'] case final reasonCode?)
        'reason_code:$reasonCode',
    }.toList()
      ..sort();
    final receipt = KernelOfflineEvidenceReceipt(
      receiptId:
          'urk_activation_${normalizedKernelId}_${observedAtUtc.microsecondsSinceEpoch}',
      receiptKind: 'activation_receipt',
      sourceSystem: 'urk_kernel_control_plane_service',
      sourcePlane: 'kernel_control_plane',
      observedAtUtc: observedAtUtc,
      kernelId: normalizedKernelId.isEmpty ? null : normalizedKernelId,
      requestId: normalizedRequestId.isEmpty ? null : normalizedRequestId,
      lineageRef: normalizedRequestId.isEmpty
          ? 'activation:$normalizedKernelId'
          : 'activation:$normalizedRequestId',
      actorScope: 'kernel_control_plane',
      boundedEvidence: <String, dynamic>{
        'activated': true,
        'actor': actor,
        'reason': reason,
        if (metadata.isNotEmpty) 'reasonMetadata': metadata,
      },
      temporalLineage: <String, dynamic>{
        'originOccurredAtUtc': observedAtUtc.toIso8601String(),
        'kernelRecordedAtUtc': observedAtUtc.toIso8601String(),
      },
      signalTags: signalTags,
    );
    final airGapArtifact = const UpwardAirGapService().issueArtifact(
      originPlane: 'kernel_control_plane',
      sourceKind: 'kernel_offline_evidence_receipt_intake',
      sourceScope: 'kernel',
      destinationCeiling: 'reality_model_agent',
      issuedAtUtc: DateTime.now().toUtc(),
      pseudonymousActorRef: normalizedKernelId.isEmpty
          ? 'kernel:unknown'
          : 'kernel:$normalizedKernelId',
      sanitizedPayload: <String, dynamic>{
        'receiptId': receipt.receiptId,
        'receiptKind': receipt.receiptKind,
        'sourceSystem': receipt.sourceSystem,
        if (receipt.kernelId case final value?) 'kernelId': value,
        if (receipt.requestId case final value?) 'requestId': value,
        if (metadata.isNotEmpty) 'reasonMetadata': metadata,
      },
    );
    ...
    try {
      await governedService.stageKernelOfflineEvidenceReceiptIntake(
        ownerUserId: normalizedKernelId.isEmpty
            ? 'kernel_control_plane'
            : 'kernel_control_plane_$normalizedKernelId',
        receipt: receipt,
        airGapArtifact: airGapArtifact,
      );
    } catch (error, stackTrace) {
      developer.log(
        'Failed to stage URK activation receipt into governed upward learning: $error',
        name: 'UrkKernelControlPlaneService',
        error: error,
