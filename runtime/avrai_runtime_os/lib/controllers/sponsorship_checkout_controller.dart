...
class SponsorshipCheckoutController
    implements
        WorkflowController<SponsorshipCheckoutInput,
    ...
    try {
      ...
      if (type == SponsorshipType.financial || type == SponsorshipType.hybrid) {
        if (contributionAmount == null || contributionAmount <= 0) {
          return SponsorshipCheckoutResult.failure(
            error:
                'Financial contribution amount is required for ${type.name} sponsorship',
  ...
  @override
  Future<SponsorshipCheckoutResult> execute(
      SponsorshipCheckoutInput input) async {
  ...
  ValidationResult validate(SponsorshipCheckoutInput input) {
    ...
    switch (input.type) {
      case SponsorshipType.financial:
        if (input.contributionAmount == null ||
            input.contributionAmount! <= 0) {
          errors['contributionAmount'] =
              'Financial sponsorship requires contribution amount > 0';
        }
        break;
      case SponsorshipType.product:
        if (input.productValue == null || input.productValue! <= 0) {
          errors['productValue'] =
              'Product sponsorship requires product value > 0';
        }
        break;
      case SponsorshipType.hybrid:
        if (input.contributionAmount == null ||
            input.contributionAmount! <= 0) {
          errors['contributionAmount'] =
              'Hybrid sponsorship requires contribution amount > 0';
        }
        if (input.productValue == null || input.productValue! <= 0) {
          errors['productValue'] =
              'Hybrid sponsorship requires product value > 0';
        }
        break;
    }
    ...
    if (input.existingSponsorship != null) {
      if (input.existingSponsorship!.eventId != input.event.id) {
        errors['existingSponsorship'] =
            'Existing sponsorship event ID does not match event ID';
      }
      if (input.existingSponsorship!.brandId != input.brandId) {
        errors['existingSponsorship'] =
            'Existing sponsorship brand ID does not match brand ID';
      }
    }
