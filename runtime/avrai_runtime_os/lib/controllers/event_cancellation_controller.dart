...
class EventCancellationController
  ...
  EventCancellationController({
    ...
    QuantumMatchingAILearningService? aiLearningService,
        ...
        _paymentService = paymentService ?? GetIt.instance<PaymentService>(),
  ...
  Future<RefundCalculation> calculateRefund({
    ...
    try {
      ...
      if (payment == null) {
        throw Exception(
            'Payment not found for user $userId and event $eventId');
      }
  ...
  Future<void> rollback(CancellationResult result) async {
    ...
    if (result.success &&
      ...
      try {
        final event =
            await _eventService.getEventById(result.cancellation!.eventId);
