...
class SocialMediaDataCollectionController
    implements WorkflowController<String, SocialMediaDataResult> {
  ...
  SocialMediaDataCollectionController({
    SocialMediaConnectionService? socialMediaService,
            ...
            GetIt.instance<SocialMediaConnectionService>();
  ...
  Future<SocialMediaDataResult> collectAllData({
    ...
    try {
      _logger.info('🎯 Starting social media data collection for user: $userId',
          tag: _logName);
      ...
      try {
        ...
        _logger.debug(
            '📱 Found ${connections.length} active social media connections',
            tag: _logName);
      } catch (e) {
        _logger.error('❌ Failed to get active connections: $e',
      ...
      if (connections.isEmpty) {
        _logger.info('ℹ️ No active social media connections found',
            tag: _logName);
      ...
      for (final connection in connections) {
        try {
          _logger.debug('📥 Processing ${connection.platform} connection...',
              tag: _logName);
          ...
          try {
            ...
            if (connection.platform == 'google') {
              try {
                final placesData =
                    await _socialMediaService.fetchGooglePlacesData(connection);
                profile['savedPlaces'] = placesData['places'] ?? [];
                profile['reviews'] = placesData['reviews'] ?? [];
                profile['photos'] = placesData['photos'] ?? [];
                _logger.debug(
                    '📍 Fetched Google Places data for ${connection.platform}',
                    tag: _logName);
              } catch (e) {
                _logger.warn(
                    '⚠️ Could not fetch Google Places data for ${connection.platform}: $e',
                    tag: _logName);
            ...
            _logger.debug('✅ Fetched profile data from ${connection.platform}',
                tag: _logName);
          } catch (e) {
            _logger.warn(
                '⚠️ Failed to fetch profile from ${connection.platform}: $e',
                tag: _logName);
          ...
          try {
            ...
            if (follows.isNotEmpty) {
              _logger.debug(
                  '✅ Fetched ${follows.length} follows from ${connection.platform}',
                  tag: _logName);
            }
          } catch (e) {
            _logger.warn(
                '⚠️ Failed to fetch follows from ${connection.platform}: $e',
                tag: _logName);
          ...
          if (primaryPlatform == null &&
              allProfileData.containsKey(connection.platform)) {
            primaryPlatform = connection.platform;
          }
        } catch (e) {
          _logger.warn(
              '⚠️ Error processing ${connection.platform} connection: $e',
              tag: _logName);
      ...
      if (allProfileData.isNotEmpty) {
        ...
        return SocialMediaDataResult.failure(
          error:
              'All platforms failed to return data. Errors: ${platformErrors.values.join(", ")}',
  ...
  Future<Map<String, dynamic>> fetchPlatformProfile(
    ...
    try {
      _logger.debug('📥 Fetching profile from ${connection.platform}...',
          tag: _logName);
      return await _socialMediaService.fetchProfileData(connection);
    } catch (e) {
      _logger.error('❌ Failed to fetch profile from ${connection.platform}: $e',
          error: e, tag: _logName);
      rethrow;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> fetchPlatformFollows(
    ...
    try {
      _logger.debug('📥 Fetching follows from ${connection.platform}...',
          tag: _logName);
      return await _socialMediaService.fetchFollows(connection);
    } catch (e) {
      _logger.error('❌ Failed to fetch follows from ${connection.platform}: $e',
          error: e, tag: _logName);
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> fetchPlatformSpecificData(
    ...
    try {
      ...
      _logger.warn(
          '⚠️ Failed to fetch platform-specific data from ${connection.platform}: $e',
          tag: _logName);
      return {};
    }
  }
