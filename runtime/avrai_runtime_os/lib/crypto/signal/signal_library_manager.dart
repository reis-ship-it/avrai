...
class SignalLibraryManager {
  ...
  // Singleton instance
  static final SignalLibraryManager _instance =
  ...
  DynamicLibrary getMainLibrary() {
    ...
    try {
      ...
      throw SignalProtocolException(
          'Failed to load main Signal Protocol library: $e');
    }
  }
  ...
  DynamicLibrary getWrapperLibrary() {
    ...
    try {
      if (Platform.isIOS) {
        ...
        try {
          _wrapperLib = DynamicLibrary.open(
              'SignalFfiWrapper.framework/SignalFfiWrapper');
        } catch (e) {
          developer.log(
        ...
        if (_isFlutterTest) {
          ...
          try {
            final currentDir = Directory.current.path;
            final libPath =
                '$currentDir/runtime/avrai_network/native/signal_ffi/macos/libsignal_ffi_wrapper.dylib';
            final libFile = File(libPath);
            if (libFile.existsSync()) {
      ...
      if (Platform.isIOS) {
        ...
        if (_isFlutterTest) {
          ...
          try {
            final currentDir = Directory.current.path;
            final libPath =
                '$currentDir/runtime/avrai_network/native/signal_ffi/macos/libsignal_callback_bridge.dylib';
            final libFile = File(libPath);
            if (libFile.existsSync()) {
              _bridgeLib = DynamicLibrary.open(libPath);
            } else {
              _bridgeLib =
                  DynamicLibrary.open('libsignal_callback_bridge.dylib');
            }
          } catch (e) {
            developer.log(
