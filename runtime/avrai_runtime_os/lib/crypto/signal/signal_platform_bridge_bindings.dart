...
class SignalPlatformBridgeBindings {
  ...
  Future<void> initialize() async {
    if (_initialized) {
      developer.log('Platform bridge bindings already initialized',
          name: _logName);
      return;
    }
    ...
    try {
      ...
      _registerDispatchCallbackByName = _lib!
          .lookup<NativeFunction<_NativeRegisterDispatchCallbackByName>>(
              'signal_register_dispatch_callback_by_name')
          .asFunction<_RegisterDispatchCallbackByName>();

      // Bind function pointer getter
      _getDispatchFunctionPtr = _lib!
          .lookup<NativeFunction<_NativeGetDispatchFunctionPtr>>(
              'signal_get_dispatch_function_ptr')
          .asFunction<_GetDispatchFunctionPtr>();

      _initialized = true;
      developer.log('Platform bridge bindings initialized successfully',
          name: _logName);
    } on ArgumentError catch (e) {
      // Convert ArgumentError (library not found) to SignalProtocolException
      final error = SignalProtocolException(
          'Failed to load platform bridge library: ${e.message}');
      developer.log('Failed to initialize platform bridge bindings: $error',
          error: error, name: _logName);
      throw error;
    } catch (e, st) {
      developer.log('Failed to initialize platform bridge bindings: $e',
  ...
  void registerDispatchCallbackByName(String functionName) {
    if (!_initialized) {
      throw SignalProtocolException(
          'Platform bridge bindings not initialized. Call initialize() first.');
    }
