...
class NetworkFlow {
  ...
  factory NetworkFlow.fromHierarchicalView(HierarchicalNetworkView view) {
    ...
    if (userCount > 0 && areaCount > 0) {
      final avgPleasureUser = view.userMetrics
              .map((u) => u.averagePleasureScore)
              .reduce((a, b) => a + b) /
          userCount;
      final avgPleasureArea = view.areaMetrics
              .map((a) => a.averagePleasureScore)
              .reduce((a, b) => a + b) /
          areaCount;
      learningPropagation =
          (0.5 + (1.0 - (avgPleasureUser - avgPleasureArea).abs()) / 2)
              .clamp(0.0, 1.0);
    }
    if (areaCount > 0 && regionCount > 0) {
      final avgLearningArea = view.areaMetrics
              .map((a) => a.averageLearningEffectiveness)
              .reduce((a, b) => a + b) /
          areaCount;
      final avgLearningRegion = view.regionalMetrics
              .map((r) => r.averageLearningEffectiveness)
              .reduce((a, b) => a + b) /
          regionCount;
      patternEmergence =
          (0.5 + (avgLearningArea + avgLearningRegion) / 4).clamp(0.0, 1.0);
    }
