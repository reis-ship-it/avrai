...
class UsagePatternTracker {
  ...
  Future<void> trackSpotVisit(
    ...
    final newRecommendationFocus =
        (pattern.recommendationFocus + focusShift).clamp(0.0, 1.0);
    final newCommunityFocus =
        (pattern.communityFocus - focusShift / 2).clamp(0.0, 1.0);

    // Update receptivity by context
    final newReceptivity =
        Map<String, double>.from(pattern.receptivityByContext);
    if (context != null) {
      newReceptivity[context] =
          ((newReceptivity[context] ?? 0.5) + 0.05).clamp(0.0, 1.0);
    }

    // Update receptivity by time
    final timeOfDay = _getTimeOfDay();
    final newTimeReceptivity =
        Map<String, double>.from(pattern.receptivityByTime);
    newTimeReceptivity[timeOfDay] =
        ((newTimeReceptivity[timeOfDay] ?? 0.5) + 0.05).clamp(0.0, 1.0);
  ...
  Future<void> trackEventAttendance(
    ...
    final newEngagement =
        (newEventsAttended / (pattern.daysActive + 1)).clamp(0.0, 1.0);

    // Update community focus (events indicate community engagement)
    const focusShift = 0.02;
    final newCommunityFocus =
        (pattern.communityFocus + focusShift).clamp(0.0, 1.0);
    final newRecommendationFocus =
        (pattern.recommendationFocus - focusShift / 2).clamp(0.0, 1.0);
  ...
  Future<void> trackCommunityJoin(
    ...
    final newDoorFrequency = Map<String, int>.from(pattern.doorTypeFrequency);
    newDoorFrequency['communities'] =
        (newDoorFrequency['communities'] ?? 0) + 1;
  ...
  Future<void> trackQuickRecommendation(
    ...
    if (context != null) {
      newReceptivity[context] =
          ((newReceptivity[context] ?? 0.5) + 0.03).clamp(0.0, 1.0);
    }
    ...
    final newTimeReceptivity =
        Map<String, double>.from(pattern.receptivityByTime);
    newTimeReceptivity[timeOfDay] =
        ((newTimeReceptivity[timeOfDay] ?? 0.5) + 0.03).clamp(0.0, 1.0);
