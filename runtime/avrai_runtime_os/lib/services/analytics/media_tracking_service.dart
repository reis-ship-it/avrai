...
class MediaTrackingService {
  ...
  String _inferGenreFromMetadata(NowPlayingTrack track) {
    ...
    if (_containsAny(
        combined, ['classical', 'orchestra', 'symphony', 'piano sonata'])) {
      return 'classical';
    }
  ...
  String _inferMoodFromMetadata(NowPlayingTrack track) {
    ...
    if (_containsAny(
        combined, ['chill', 'relax', 'calm', 'peaceful', 'serene', 'quiet'])) {
      return 'calm';
    }
    if (_containsAny(
        combined, ['party', 'dance', 'club', 'energy', 'pump', 'workout'])) {
      return 'energetic';
    }
    if (_containsAny(
        combined, ['focus', 'study', 'concentrate', 'work', 'productive'])) {
      return 'focus';
    }
    ...
    if (_containsAny(
        combined, ['sad', 'melancholy', 'heartbreak', 'emotional'])) {
      return 'melancholy';
    }
  ...
  List<Map<String, dynamic>> get trackHistory =>
      List.unmodifiable(_trackHistory);
