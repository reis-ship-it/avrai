...
export 'package:avrai_runtime_os/services/quantum/real_time_user_calling_models.dart';
...
class RealTimeUserCallingService {
  ...
  Future<void> _processUserBatch({
    ...
    final futures = batch.map((user) async {
      try {
        ...
        final wasPreviouslyCalled =
            await _wasUserCalledToEvent(user.id, eventId);
  ...
  Future<double> _calculateUserCompatibility({
    ...
    double stringEvolutionCompatibility =
        0.5; // Default: neutral if not available
    if (eventStartTime != null) {
      try {
        final agentId =
            await _agentIdService.getUserAgentId(userState.entityId);
    ...
    if (userState.timing != null) {
      ...
      if (eventTiming != null) {
        ...
        try {
          final meaningfulScore = await _meaningfulExperienceCalculator
              .calculateMeaningfulExperienceScore(
            userState: userState,
            entangledState: entangledState,
            eventEntities: eventEntities,
          );
          timingFlexibilityFactor = await _meaningfulExperienceCalculator
              .calculateTimingFlexibilityFactor(
            userState: userState,
            eventEntities: eventEntities,
        ...
        timingCompatibility =
            (baseTiming * timingFlexibilityFactor).clamp(0.0, 1.0);
      }
    }
  ...
  double _calculateKnotEvolutionScore({
    ...
    try {
      ...
      final crossingDiff = (futureKnot.invariants.crossingNumber -
          ...
          .abs();
      final writheDiff =
          (futureKnot.invariants.writhe - currentKnot.invariants.writhe).abs();
      ...
      if (_supabaseService.isAvailable) {
        try {
          ...
          final response = await client
              .from('users')
              .select(
                  'id, email, display_name, photo_url, location, created_at, updated_at, is_online, has_completed_onboarding')
              .limit(limit);
          ...
          final users = (response as List).map((json) {
            return User(
              ...
              role: UserRole
                  .follower, // Default role - could be parsed from json if available
  ...
  Future<EntityTimingQuantumState?> _getUserTimingPreferences(
      String userId) async {
    try {
      ...
      if (preferencesProfile != null) {
        ...
        return await _locationTimingService
  ...
  Future<double> _calculateKnotCompatibilityBonus({
    ...
    try {
      ...
      final personalityProfile =
          await _personalityLearning.getCurrentPersonality(userState.entityId);
      if (personalityProfile == null) {
        return 0.0;
      }
  ...
  Future<void> _persistEventCallToDatabase({
    ...
    try {
      final client = _supabaseService.client;
      await client.from('event_user_calls').upsert({
        'event_id': eventId,
        'user_id': userId,
    ...
    if (_supabaseService.isAvailable) {
      try {
        ...
        await client.from('notifications').insert({
          ...
          'body':
              'You have a ${(compatibility * 100).round()}% match with an event!',
          'data': {
            'eventId': eventId,
            'compatibility': compatibility,
          },
