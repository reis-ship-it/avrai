...
class UserJourneyTrackingService {
    ...
    try {
      ...
      _journeyCache[journeyKey] = UserJourney(
        ...
        journeyStartTimestamp:
            existingJourney?.journeyStartTimestamp ?? tAtomic,
  ...
  UserJourneyEvolution _calculateJourneyEvolution({
    ...
    final allDimensions = {
      ...preEventState.vibeDimensions.keys,
      ...postEventState.vibeDimensions.keys
    };
    ...
    final newConnections = math.max(
        0, postEventState.connectionCount - preEventState.connectionCount);

    // 4. Engagement evolution
    final engagementChange =
        postEventState.engagementLevel - preEventState.engagementLevel;
    ...
    final interestExpansionScore =
        newInterests.length / math.max(postEventState.interests.length, 1);
    ...
    final modifierScore =
        0.6 * connectionGrowthScore + 0.4 * engagementEvolutionScore;
