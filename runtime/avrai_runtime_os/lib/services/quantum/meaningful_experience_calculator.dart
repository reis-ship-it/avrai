...
class MeaningfulExperienceCalculator {
        ...
        if (eventTiming != null) {
          ...
          try {
            ...
            final userTemporalState =
                QuantumTemporalStateGenerator.generate(userTimestamp);
            final eventTemporalState =
                QuantumTemporalStateGenerator.generate(eventTimestamp);

            // Calculate quantum temporal compatibility
            quantumTemporalCompat =
                userTemporalState.temporalCompatibility(eventTemporalState);
          } catch (e) {
            developer.log(
          ...
          timingCompatibility =
              (coreScore * (1.0 + 0.3 * modifierScore)).clamp(0.0, 1.0);
        }
      }
  ...
  Future<double> calculateMeaningfulExperienceScore({
    ...
    try {
      ...
      final interestAlignment =
          _calculateInterestAlignment(userState, eventEntities);
      ...
      final modifierScore =
          0.6 * interestAlignment + 0.4 * transformativePotential;
  ...
  Future<double> _calculateTransformativePotential({
    ...
    try {
      ...
      final userGrowthPotential =
          _calculateUserGrowthPotential(userState, eventEntities);

      // 3. Connection opportunity (potential for meaningful connections)
      final connectionOpportunity =
          _calculateConnectionOpportunity(eventEntities);

      // 4. Vibe expansion potential (how much can user's vibe expand)
      final vibeExpansionPotential =
          _calculateVibeExpansionPotential(userState, eventEntities);
      ...
      final modifierScore =
          0.5 * connectionOpportunity + 0.5 * vibeExpansionPotential;
  ...
  double _calculateInterestAlignment(
    ...
    final matchingInterests = userInterests
        .where((interest) => eventCategories.contains(interest))
        .length;
    final alignment = matchingInterests /
        math.max(userInterests.length, eventCategories.length);
  ...
  double _calculateEventNovelty(
    ...
    final userTypicalCategories = userState
            .entityCharacteristics['typical_categories'] as List<String>? ??
        [];
    ...
    final novelCategories = eventCategories
        .where((cat) => !userTypicalCategories.contains(cat))
        .length;
  ...
  double _calculateConnectionOpportunity(
      List<QuantumEntityState> eventEntities) {
  ...
  double _calculateVibeExpansionPotential(
    ...
    for (final dimension in dimensions) {
      ...
      // Calculate variance
      final mean =
          eventValues.fold(0.0, (sum, val) => sum + val) / eventValues.length;
      final variance = eventValues.fold(
