...
class QuantumMatchingMetricsService {
  ...
  Future<Map<String, dynamic>> getMetricsSummary({
    ...
    try {
      ...
      if (serviceName != null) {
        filteredMetrics =
            filteredMetrics.where((m) => m.serviceName == serviceName).toList();
      }
  ...
  Future<Map<String, dynamic>> compareMethods({
    ...
    try {
      ...
      final quantumAvgCompat =
          quantumSummary['avgCompatibility'] as double? ?? 0.0;
      final classicalAvgCompat =
          classicalSummary['avgCompatibility'] as double? ?? 0.0;
      final compatibilityImprovement = quantumAvgCompat - classicalAvgCompat;
      final compatibilityImprovementPercent = classicalAvgCompat > 0
          ? (compatibilityImprovement / classicalAvgCompat) * 100
          : 0.0;

      final quantumAvgTime = quantumSummary['avgExecutionTimeMs'] as int? ?? 0;
      final classicalAvgTime =
          classicalSummary['avgExecutionTimeMs'] as int? ?? 0;
      final timeDifference = quantumAvgTime - classicalAvgTime;
      final timeDifferencePercent = classicalAvgTime > 0
      ...
      return {
        ...
        'comparison': {
          ...
          'compatibilityImprovementPercent': compatibilityImprovementPercent,
