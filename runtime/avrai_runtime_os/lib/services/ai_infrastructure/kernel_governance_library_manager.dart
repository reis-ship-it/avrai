...
class KernelGovernanceLibraryManager implements KernelJsonLibraryManager {
  static const String _logName = 'KernelGovernanceLibraryManager';
      ...
      if (Platform.isIOS) {
        try {
          _kernelLib = DynamicLibrary.open(
            'AVRAIGovernanceKernel.framework/AVRAIGovernanceKernel',
          );
        } catch (_) {
        ...
        if (_isFlutterTest) {
          ...
          _kernelLib = existingPath == null
              ? DynamicLibrary.open('libavrai_governance_kernel.dylib')
              : DynamicLibrary.open(existingPath);
        }
      } else if (Platform.isAndroid || Platform.isLinux) {
        _kernelLib = DynamicLibrary.open('libavrai_governance_kernel.so');
      } else if (Platform.isWindows) {
        _kernelLib = DynamicLibrary.open('avrai_governance_kernel.dll');
      } else {
        throw UnsupportedError(
          'Unsupported platform for governance kernel library.',
        );
      }
      developer.log('Governance kernel library loaded', name: _logName);
      return _kernelLib!;
    } catch (error, stackTrace) {
      developer.log(
        'Failed to load governance kernel library: $error',
        name: _logName,
        error: error,
