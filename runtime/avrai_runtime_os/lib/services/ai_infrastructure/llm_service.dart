...
class LLMService {
  ...
  Future<List<ChatMessage>> _augmentMessagesForLocalIfPossible(
    ...
    try {
      ...
      final systemPrompt = _augmentLocalSystemPromptWithTiming(
        prompt.trim(),
        context,
      );
  ...
  String _augmentLocalSystemPromptWithTiming(
    String basePrompt,
    LLMContext? context,
  ) {
    final conversationPreferences = context?.conversationPreferences;
    if (conversationPreferences == null || conversationPreferences.isEmpty) {
      return basePrompt;
    }
    final timingSummary =
        conversationPreferences['governed_knowledge_timing_summary']
            ?.toString()
            .trim();
    if (timingSummary == null || timingSummary.isEmpty) {
      return basePrompt;
    }

    final freshnessNote = switch (_conversationKnowledgeFreshness(
      conversationPreferences,
    )) {
      _ConversationKnowledgeFreshness.fresh =>
        'Treat the local governed context as current enough for grounded locality-aware guidance.',
      _ConversationKnowledgeFreshness.stale =>
        'Treat the local governed context cautiously because it may be stale relative to the user’s present state.',
      _ConversationKnowledgeFreshness.presentUnknownAge =>
        'Preserve the local governed timing context, but do not overstate how current it is.',
      _ConversationKnowledgeFreshness.none => null,
    };

    final additions = <String>[
      'Local governed context timing: $timingSummary.',
      if (freshnessNote != null) freshnessNote,
    ];
    return '$basePrompt\n\n${additions.join(' ')}';
  }
