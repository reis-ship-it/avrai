...
class Ai2AiChatEventIntakeService {
    ...
    try {
      ...
      if (_isExchangeReceiptArtifact(baseMetadata)) {
        ...
        return Ai2AiChatEventIntakeResult(
          ...
          metadata: baseMetadata,
          skipReason: observed
              ? 'exchange_receipt_observed'
              : 'exchange_receipt_ignored',
        );
      }
      ...
      await _stageUpwardLearningBestEffort(
        localUserId: localUserId,
        localAgentId: await _resolveActorAgentId(
          localUserId,
          explicitAgentId: localAgentId,
        ),
        senderUserId: senderUserId,
  ...
  Future<void> _stageUpwardLearningBestEffort({
    ...
    required Map<String, dynamic> learningMetadata,
  }) async {
    ...
    try {
      final sourceKind = learningMetadata['intent']?.toString() == 'correct'
      ...
      final airGapArtifact = const UpwardAirGapService().issueArtifact(
        ...
        pseudonymousActorRef:
            learningMetadata['pseudonymous_actor_ref']?.toString(),
      );
      await service.stageAi2AiIntake(
        ...
        boundaryMetadata: learningMetadata,
        airGapArtifact: airGapArtifact,
      );
    } catch (error, stackTrace) {
      developer.log(
        'Failed to stage governed upward learning intake for AI2AI event',
        name: _logName,
        error: error,
        stackTrace: stackTrace,
      );
  ...
  Future<bool> _observeExchangeReceipt({
    ...
    final observation =
        await governanceBindingService.buildAi2AiExchangeObservation(
      observationId:
          'ai2ai-exchange-receipt-observation-$exchangeId-$receiptStage',
  ...
  bool _isReceiptCorrelationValid({
    ...
    final expectedSenderUserId = metadata['ai2ai_sender_user_id']?.toString();
