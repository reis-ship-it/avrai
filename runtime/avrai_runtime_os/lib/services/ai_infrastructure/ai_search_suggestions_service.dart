...
class AISearchSuggestionsService {
  ...
  Future<List<SearchSuggestion>> generateSuggestions({
    ...
    try {
      // #region agent log
      developer.log(
          'Generating suggestions: query="$query", hasLocation=${userLocation != null}, recentSpots=${recentSpots?.length ?? 0}, trends=${communityTrends != null ? communityTrends.length : 0}',
          name: _logName);
      // #endregion
      ...
      if (normalizedQuery.isNotEmpty) {
        ...
        // #region agent log
        developer.log('Query completions: ${completions.length} suggestions',
            name: _logName);
        // #endregion
      }
      ...
      if (userLocation != null) {
        ...
        // #region agent log
        developer.log(
            'Contextual suggestions: ${contextual.length} suggestions',
            name: _logName);
        // #endregion
      }
      ...
      // #region agent log
      developer.log(
          'Personalized suggestions: ${personalized.length} suggestions',
          name: _logName);
      // #endregion
      ...
      if (communityTrends != null) {
        ...
        // #region agent log
        developer.log(
            'Community trend suggestions: ${trends.length} suggestions',
            name: _logName);
        // #endregion
      }
      ...
      if (query.isEmpty || query.length < 3) {
        ...
        // #region agent log
        developer.log('Discovery suggestions: ${discovery.length} suggestions',
            name: _logName);
        // #endregion
      }
      ...
      // #region agent log
      developer.log('NLP suggestions: ${nlp.length} suggestions',
          name: _logName);
      // #endregion
      ...
      // #region agent log
      developer.log(
          'Generated ${uniqueSuggestions.length} unique suggestions (from ${suggestions.length} total) for: "$query"',
          name: _logName);
      // #endregion
  ...
  void learnFromSearch({
    ...
    try {
      // #region agent log
      developer.log(
          'Learning from search: query="$query", results=${results.length}, selectedSpot=$selectedSpotId',
          name: _logName);
      // #endregion
      ...
      // #region agent log
      developer.log(
          'Category preferences updated: ${categoryCounts.length} unique categories, total preferences: ${_categoryPreferences.length}',
          name: _logName);
      // #endregion
      ...
      // #region agent log
      developer.log(
          'Learned from search: "$query" (${results.length} results), recentSearches=${_recentSearches.length}, timestamps: $beforeCleanup -> $afterCleanup',
          name: _logName);
      // #endregion
    } catch (e) {
      // #region agent log
  ...
  Map<String, dynamic> getSearchPatterns() {
    ...
    developer.log(
        'Retrieved search patterns: ${recentSearchesList.length} recent, ${topCategoriesList.length} top categories, ${patterns['total_searches']} total searches',
        name: _logName);
    // #endregion
  ...
  Future<List<SearchSuggestion>> _generateContextualSuggestions(
    ...
    for (final entry in sortedCategories.take(3)) {
      if (entry.key.contains(query) || query.isEmpty) {
        suggestions.add(SearchSuggestion(
          ...
          confidence: 0.6 +
              (entry.value / 20.0)
                  .clamp(0.0, 0.3), // Higher confidence for frequent categories
  ...
  List<SearchSuggestion> _generateCommunityTrendSuggestions(
  ...
  List<SearchSuggestion> _deduplicateAndRank(
  ...
  void _trackSuggestionGeneration(String query, int count) {
    ...
    developer.log(
        'Generated $count suggestions for query length: ${query.length}',
        name: _logName);
  }
  ...
  Map<String, int> _getSearchFrequencyPatterns() {
    ...
    // #region agent log
    developer.log(
        'Search frequency patterns: ${patterns.length} periods tracked, total searches: ${_searchTimestamps.length}',
        name: _logName);
    // #endregion
...
class SearchSuggestion {
  ...
  @override
  String toString() =>
      'SearchSuggestion(text: $text, type: $type, confidence: $confidence)';
}
