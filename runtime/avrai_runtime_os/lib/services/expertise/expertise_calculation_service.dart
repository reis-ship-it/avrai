...
class ExpertiseCalculationService {
  ...
  final String _agentRunId =
      'expertise_calc_${DateTime.now().microsecondsSinceEpoch}';
  void _agentLog(
    String hypothesisId,
    String location,
    String message,
  ...
  Future<ExpertiseCalculationResult> calculateExpertise({
    ...
    try {
      ...
      if (_dynamicThresholdService != null &&
          locality != null &&
          locality.isNotEmpty) {
        try {
          effectiveThresholds =
              await _dynamicThresholdService.calculateLocalThreshold(
            locality: locality,
            category: category,
  ...
  ThresholdValues _getEffectiveRequirements({
    ...
    final phaseMultiplier =
        platformPhase.getCategoryMultiplier(requirements.category);
  ...
  Future<PartnershipExpertiseBoost> calculatePartnershipBoost({
    ...
    try {
      ...
      return await _partnershipProfileService.getPartnershipExpertiseBoost(
          userId, category);
    } catch (e) {
      _logger.error(
  ...
  Future<ExpertiseLevel?> calculateExpertiseFromExpansion({
    ...
    try {
      ...
      _logger.error('Error calculating expertise from expansion',
