...
class MentorshipService {
  ...
  Future<MentorshipRelationship> requestMentorship({
    ...
    try {
      _logger.info('Mentorship request: ${mentee.id} -> ${mentor.id}',
          tag: _logName);
      ...
      final existing =
          await _getMentorshipRelationship(mentee.id, mentor.id, category);
      if (existing != null) {
      ...
      _logger.info('Mentorship request created: ${relationship.id}',
          tag: _logName);
      return relationship;
    } catch (e) {
  ...
  Future<List<MentorshipRelationship>> getMentors(UnifiedUser user) async {
    ...
    return relationships
        .where((r) =>
            r.mentee.id == user.id && r.status == MentorshipStatus.active)
        .toList();
  }
  ...
  Future<List<MentorshipRelationship>> getMentees(UnifiedUser user) async {
    ...
    return relationships
        .where((r) =>
            r.mentor.id == user.id && r.status == MentorshipStatus.active)
        .toList();
  }
