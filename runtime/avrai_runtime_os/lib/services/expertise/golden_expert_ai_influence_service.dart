...
class GoldenExpertAIInfluenceService {
  ...
  static const double _residencyMultiplier =
      0.01; // 1/100 for years to weight conversion
  ...
  double calculateInfluenceWeight(LocalExpertise? localExpertise) {
    try {
      ...
      final residencyYears =
          _getResidencyYears(localExpertise.continuousResidency);

      // Calculate weight: base + (residencyYears / 100)
      final calculatedWeight =
          _baseWeight + (residencyYears * _residencyMultiplier);
    ...
    try {
      ...
      if (weightedData.containsKey('influenceScore')) {
        weightedData['influenceScore'] =
            ((weightedData['influenceScore'] as num?)?.toDouble() ?? 0.0) *
                weight;
      }
