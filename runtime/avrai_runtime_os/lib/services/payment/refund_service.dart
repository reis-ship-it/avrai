...
class RefundService {
  ...
  Future<List<RefundDistribution>> processRefund({
    ...
    try {
      _logger.info(
          'Processing refund: payment=$paymentId, amount=\$${amount.toStringAsFixed(2)}',
          tag: _logName);
      ...
      if (!payment.isSuccessful) {
        throw Exception(
            'Payment must be successful to refund. Current status: ${payment.status}');
      }

      if (amount > payment.amount) {
        throw Exception(
            'Refund amount (\$${amount.toStringAsFixed(2)}) cannot exceed payment amount (\$${payment.amount.toStringAsFixed(2)})');
      }
      ...
      if (payment.stripePaymentIntentId != null) {
        try {
          ...
          stripeRefundId = await _stripeService.processRefund(
            ...
            reason: cancellationId != null
                ? 'Cancellation: $cancellationId'
                : 'Refund',
          );
          _logger.info('Stripe refund processed: $stripeRefundId',
              tag: _logName);
        } catch (e) {
        ...
        _logger.warn(
            'Payment has no Stripe payment intent ID, skipping Stripe refund',
            tag: _logName);
      }
      ...
      _logger.info(
          'Refund processed: payment=$paymentId, amount=\$${amount.toStringAsFixed(2)}, stripeRefund=$stripeRefundId',
          tag: _logName);
  ...
  Future<List<RefundDistribution>> processBatchRefunds({
    ...
    try {
      _logger.info(
          'Processing batch refunds: ${payments.length} payments, cancellation=$cancellationId',
          tag: _logName);
      ...
      for (final payment in payments) {
        try {
          final refundAmount = fullRefund
              ? payment.amount
              : payment.amount * 0.90; // 90% if not full refund
          ...
          _logger.error('Failed to refund payment ${payment.id}',
      ...
      _logger.info(
          'Batch refunds processed: ${allDistributions.length} distributions created',
          tag: _logName);
  ...
  Future<List<RefundDistribution>> getRefundDistributions(
      String cancellationId) async {
    try {
      return _refundDistributions[cancellationId] ?? [];
    } catch (e) {
      _logger.error('Error getting refund distributions',
          error: e, tag: _logName);
      return [];
    }
  }
