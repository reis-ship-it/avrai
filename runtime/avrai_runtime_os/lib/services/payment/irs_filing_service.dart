...
class IRSFilingService {
    ...
    try {
      ...
      if (_apiEndpoint == null || _apiKey == null) {
        ...
        return IRSFilingResult(
          ...
          message:
              'IRS filing API not configured. Configure API endpoint and key.',
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final confirmationNumber =
            response.data['confirmationNumber'] as String?;
        ...
        throw Exception(
            'IRS filing failed: ${response.statusCode} - ${response.data}');
      }
    } catch (e) {
      _logger.error('Failed to file with IRS',
          error: e, tag: 'IRSFilingService');
      rethrow;
    }
  }
  ...
  Future<IRSFilingStatus> checkFilingStatus(String confirmationNumber) async {
    try {
      ...
      _logger.error('Failed to check filing status',
          error: e, tag: 'IRSFilingService');
      return IRSFilingStatus.unknown;
    }
  }
