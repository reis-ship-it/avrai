...
class PayoutService {
  ...
  Future<Payout> schedulePayout({
    ...
    try {
      _logger.info(
          'Scheduling payout: party=$partyId, amount=\$${amount.toStringAsFixed(2)}, date=$scheduledDate',
          tag: _logName);
  ...
  Future<EarningsReport> trackEarnings({
    ...
    try {
      ...
      if (startDate != null) {
        payouts =
            payouts.where((p) => p.scheduledDate.isAfter(startDate)).toList();
      }
      if (endDate != null) {
        payouts =
            payouts.where((p) => p.scheduledDate.isBefore(endDate)).toList();
      }
      ...
      _logger.info(
          'Earnings report: total=\$${totalEarnings.toStringAsFixed(2)}, paid=\$${totalPaid.toStringAsFixed(2)}, pending=\$${totalPending.toStringAsFixed(2)}',
          tag: _logName);
      return report;
    } catch (e) {
  ...
  Future<Payout> updatePayoutStatus({
    ...
    try {
      _logger.info('Updating payout status: $payoutId -> ${status.name}',
          tag: _logName);
  ...
  Future<List<Payout>> getPayoutsForParty(String partyId) async {
    try {
      final allPayouts = await _getAllPayouts();
      return allPayouts.where((p) => p.partyId == partyId).toList()
...
class Payout {
  ...
  factory Payout.fromJson(Map<String, dynamic> json) {
    return Payout(
      ...
      status: PayoutStatusExtension.fromString(
