...
class BhamReplayPopulationProfileService {
    ...
    for (final observation in observations) {
      ...
      switch (observation.subjectIdentity.entityType) {
        case 'population_cohort':
          if (observation.normalizedFields['series_key'] == 'population_total') {
            final value =
                (observation.normalizedFields['metric_value'] as num?)?.toInt() ??
                    0;
  ...
  Map<String, int> _allocateActorTargets({
    ...
    for (final locality in localities) {
      final proportional = totalWeight == 0
          ...
          : ((weights[locality]! / totalWeight) * minimumWeightedActors).round();
  ...
  _ActorBuildResult _buildActorsForLocality({
    ...
    for (var configIndex = 0; configIndex < configs.length; configIndex++) {
      ...
      final representedPopulationTotal =
          math.max(1, (effectivePopulation * (config['share'] as double)).round());
  ...
  List<Map<String, dynamic>> _normalizeShares(List<Map<String, dynamic>> configs) {
