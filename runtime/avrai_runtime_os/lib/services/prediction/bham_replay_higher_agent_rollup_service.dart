...
class BhamReplayHigherAgentRollupService {
  ...
  ReplayHigherAgentRollupBatch buildRollups({
    ...
    return ReplayHigherAgentRollupBatch(
      ...
      metadata: <String, dynamic>{
        ...
        'cityCode': _cityCode(environment),
        'citySlug': _citySlug(environment),
        'cityDisplayName': _cityDisplayName(environment),
        'cityPackManifestRef': environment.metadata['cityPackManifestRef'],
        'cityPackId': environment.metadata['cityPackId'],
        'cityPackStructuralRef': environment.metadata['cityPackStructuralRef'],
        'campaignDefaultsRef': environment.metadata['campaignDefaultsRef'],
        'localityExpectationProfileRef':
            environment.metadata['localityExpectationProfileRef'],
        'worldHealthProfileRef': environment.metadata['worldHealthProfileRef'],
      },
    );
  }
  ...
  List<ReplayHigherAgentRollup> _buildPersonalRollups(
    ...
    return personalActors.map((actor) {
      final candidateNodes =
          byLocality[actor.localityAnchor] ?? const <ReplayVirtualWorldNode>[];
      final preferredNodes = candidateNodes
          .where(
            (node) =>
                actor.preferredEntityTypes
                    .contains(node.subjectIdentity.entityType) ||
          ...
          .toList(growable: false);
      final selectedNodes =
          (preferredNodes.isNotEmpty ? preferredNodes : candidateNodes)
              .take(8)
              .toList(growable: false);
      return _buildRollup(
  ...
  List<ReplayHigherAgentRollup> _buildLocalityRollups(
    ...
    return localities.map((locality) {
      ...
      return _buildRollup(
        ...
        canonicalName: locality == 'unanchored'
            ? 'Unanchored ${_cityDisplayName(environment)}'
            : locality,
  ...
  ReplayHigherAgentRollup _buildCityRollup(
    ...
    return _buildRollup(
      ...
      agentId: 'city-agent:${_citySlug(environment)}',
      canonicalName: '${_cityDisplayName(environment)} City Agent',
  ...
  String _cityCode(ReplayVirtualWorldEnvironment environment) {
    final value = environment.metadata['cityCode']?.toString().trim();
    if (value != null && value.isNotEmpty) {
      return value;
    }
    return _citySlug(environment);
  }

  String _citySlug(ReplayVirtualWorldEnvironment environment) {
    final value = environment.metadata['citySlug']?.toString().trim();
    if (value != null && value.isNotEmpty) {
      return value;
    }
    final environmentId = environment.environmentId.trim();
    final prefix = environmentId.split('-').first.trim().toLowerCase();
    return prefix.isEmpty ? 'replay' : prefix;
  }

  String _cityDisplayName(ReplayVirtualWorldEnvironment environment) {
    final value = environment.metadata['cityDisplayName']?.toString().trim();
    if (value != null && value.isNotEmpty) {
      return value;
    }
    final slug = _citySlug(environment);
    return slug
        .split('_')
