...
class BhamReplayExecutionService {
  ...
  Map<String, dynamic> _buildPlanMetadata({
    ...
    metadata.putIfAbsent('citySlug', () => _deriveCitySlug(pack));
    metadata.putIfAbsent('cityCode', () => metadata['citySlug'] ?? 'replay');
    metadata.putIfAbsent(
      'cityDisplayName',
      () => _deriveCityDisplayName(pack, metadata['citySlug']?.toString()),
    );
    metadata.putIfAbsent('executionSourceId', () => 'replay_execution');
    metadata.putIfAbsent(
      'consolidatedReplaySourceId',
      () => 'replay_consolidated_source_pack',
    );
    metadata.putIfAbsent(
      'environmentKind',
    ...
    metadata.putIfAbsent(
      ...
      () => _deriveCityPackStructuralRef(pack, metadata),
    );
    return metadata;
  }

  String? _deriveCityPackStructuralRef(
    ReplaySourcePack pack,
    Map<String, dynamic> metadata,
  ) {
    final explicit = (pack.metadata['cityPackStructuralRef'] ??
    ...
    final cityPackId = (pack.metadata['cityPackId'] ?? metadata['cityPackId'])
        ?.toString()
        .trim();
    if (cityPackId != null && cityPackId.isNotEmpty) {
      return 'city_pack:$cityPackId';
    }
    final manifestRef = (pack.metadata['cityPackManifestRef'] ??
            metadata['cityPackManifestRef'])
        ?.toString()
        .trim();
    if (manifestRef != null && manifestRef.isNotEmpty) {
      return manifestRef;
    }
    final citySlug = metadata['citySlug']?.toString().trim();
    if (citySlug != null && citySlug.isNotEmpty) {
      return 'city_pack:$citySlug:${pack.replayYear}';
    }
    return null;
  }

  String _deriveCitySlug(ReplaySourcePack pack) {
    final raw = pack.metadata['citySlug'] ??
        pack.metadata['cityCode'] ??
        pack.metadata['cityPackId'];
    final normalized = raw?.toString().trim().toLowerCase();
    if (normalized != null && normalized.isNotEmpty) {
      return normalized.replaceAll(RegExp(r'[^a-z0-9]+'), '_');
    }
    final packPrefix = pack.packId.split('-').first.trim().toLowerCase();
    return packPrefix.isEmpty ? 'replay' : packPrefix;
  }

  String _deriveCityDisplayName(ReplaySourcePack pack, String? citySlug) {
    final explicit =
        pack.metadata['cityDisplayName'] ?? pack.metadata['cityName'];
    final explicitString = explicit?.toString().trim();
    if (explicitString != null && explicitString.isNotEmpty) {
      return explicitString;
    }
    final slug = (citySlug ?? 'replay').replaceAll('_', ' ').trim();
    if (slug.isEmpty) {
      return 'Replay';
    }
    return slug
  ...
  Future<BhamReplayExecutionResult> executePlan({
    ...
    final executionSource =
        plan.metadata['executionSourceId']?.toString() ?? 'replay_execution';
    final consolidatedReplaySource =
        plan.metadata['consolidatedReplaySourceId']?.toString() ??
            'replay_consolidated_source_pack';
    ...
    for (final entry in plan.entries) {
      ...
      final event = RuntimeTemporalEvent(
        ...
        provenance: TemporalProvenance(
          authority: TemporalAuthority.historicalImport,
          source: consolidatedReplaySource,
        ),
