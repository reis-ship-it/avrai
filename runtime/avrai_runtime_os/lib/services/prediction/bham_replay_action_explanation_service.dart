...
class BhamReplayActionExplanationService {
    ...
    for (final action in behaviorPass.actions) {
      ...
      for (final nodeId in action.targetNodeIds) {
        supportingSourceRefs
            .addAll(nodeById[nodeId]?.sourceRefs ?? const <String>[]);
      }
      ...
      explanations.add(
        ReplayActionExplanation(
          ...
          localityAnchor:
              action.localityAnchor ?? actor?.localityAnchor ?? 'unanchored',
  ...
  List<String> _kernelLanesFor(ReplayHigherAgentActionType actionType) {
    return switch (actionType) {
      ReplayHigherAgentActionType.personalPlanDailyCircuit => const <String>[
          'when',
          'where',
          'who',
          'how'
      ...
      ReplayHigherAgentActionType.stabilizeLocalityTruth => const <String>[
          'where',
          'why',
          'governance'
        ],
      ReplayHigherAgentActionType.escalateLocalityReview => const <String>[
          'where',
          'why',
          'governance'
        ],
      ...
      ReplayHigherAgentActionType.aggregateCitySignal => const <String>[
          'higher_agent_truth',
          'what',
          'why'
        ],
      ReplayHigherAgentActionType.routeCityGuidanceDownward => const <String>[
          'higher_agent_truth',
          'how',
          'who'
        ],
      ReplayHigherAgentActionType.retainAsReplayPriorOnly => const <String>[
          'governance',
          'forecast',
          'why'
        ],
      ReplayHigherAgentActionType.auditContradictionSurface => const <String>[
          'governance',
          'why',
          'forecast'
        ],
  ...
  String _explanationFor({
    ...
    return switch (action.actionType) {
      ReplayHigherAgentActionType.personalPlanDailyCircuit =>
        'Representative personal agent `${action.agentId}` plans a $cityDisplayName-local circuit in `$locality` using replay-only place truth ($venueCount venues, $communityCount communities).',
      ReplayHigherAgentActionType.personalJoinCommunityPattern =>
      ...
      ReplayHigherAgentActionType.aggregateCitySignal =>
        '$cityDisplayName city agent aggregates locality signals into one replay-year city view before sending bounded guidance back down.',
      ReplayHigherAgentActionType.routeCityGuidanceDownward =>
        '$cityDisplayName city agent routes city-level guidance back to local and personal agents without direct human-facing speech.',
