...
class BhamReplayHigherAgentBehaviorService {
  ...
  ReplayHigherAgentBehaviorPass buildBehaviorPass({
    ...
    final actorsById = <String, ReplayActorProfile>{
      for (final actor in populationProfile?.actors ?? const <ReplayActorProfile>[])
        actor.actorId: actor,
    };
    ...
    if (rollup.level == ReplayHigherAgentLevel.personal) {
      actions.add(
        _buildAction(
          ...
          reason:
              'Representative personal agent should plan a replay-only Birmingham circuit using bounded locality and place truth.',
          guidance: <String>[
      ...
      actions.add(
        _buildAction(
          ...
          reason:
              'City agent should aggregate locality signals into one Birmingham replay-city view.',
          guidance: const <String>[
      ...
      actions.add(
        _buildAction(
          ...
          reason:
              'City agent should return bounded Birmingham-wide guidance back to local and personal replay agents.',
          guidance: const <String>[
