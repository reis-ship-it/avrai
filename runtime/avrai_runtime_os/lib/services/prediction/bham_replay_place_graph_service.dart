...
class BhamReplayPlaceGraphService {
  ...
  ReplayPlaceGraph buildGraph({
    ...
    for (final observation in observations) {
      ...
      observationsById.putIfAbsent(subjectId, () => <ReplayNormalizedObservation>[]);
    ...
    for (final subjectId in sortedIds) {
      ...
      final rows = observationsById[subjectId] ?? const <ReplayNormalizedObservation>[];
      final sample = rows.first;
      final sourceRefs = (sourceRefsById[subjectId] ?? <String>{}).toList()..sort();
      ...
      final category = _categoryFor(identity.entityType, sample.normalizedFields);
      final capacityBand = _capacityBand(category, sample.normalizedFields);
      final recurrenceAffinity =
          _recurrenceAffinity(identity.entityType, category, associatedEventIds.length);
      ...
      final node = ReplayPlaceGraphNode(
        ...
        venueCategory: _isVenueLike(identity.entityType, category)
            ? category
            : null,
      ...
      if (_isVenueLike(identity.entityType, category)) {
        ...
        venueCategoryCounts[category] = (venueCategoryCounts[category] ?? 0) + 1;
      }

      final organizationType = _organizationTypeFor(identity.entityType, sample.normalizedFields);
      final organizationId = _organizationIdFor(sample.normalizedFields, locality) ??
          (_shouldMaterializeOrganization(identity.entityType, category)
              ? 'organization:${_slugify('$locality-$category-${identity.canonicalName}')}' : null);
      final organizationName = _organizationNameFor(sample.normalizedFields, identity.canonicalName);
      if (organizationId != null) {
        organizationProfiles.add(
          ReplayOrganizationProfile(
            ...
            metadata: <String, dynamic>{
              'inferred': _organizationIdFor(sample.normalizedFields, locality) == null,
            },
          ),
        );
      ...
      if (identity.entityType == 'community') {
        final communityCategory = sample.normalizedFields['category']?.toString() ??
        ...
        communityProfiles.add(
          ReplayCommunityProfile(
            ...
            organizationIds: organizationId == null ? const <String>[] : <String>[organizationId],
      ...
      final directClubCategory = _clubCategoryFor(identity.entityType, category);
      if (directClubCategory != null) {
        final clubIdentity = identity.entityType == 'club'
            ...
            : ReplayEntityIdentity(
                ...
                entityType: 'club',
                canonicalName: '${identity.canonicalName} ${_clubDisplaySuffix(directClubCategory)}',
                localityAnchor: locality,
              );
        clubProfiles.add(
          ReplayClubProfile(
        ...
        eventProfiles.add(
          ReplayEventProfile(
            ...
            startsAtIso: sample.normalizedFields['starts_at']?.toString() ??
                sample.replayEnvelope.eventStartAt?.referenceTime.toUtc().toIso8601String() ??
                sample.replayEnvelope.observedAt.referenceTime.toUtc().toIso8601String(),
      ...
      if (_shouldGenerateRecurringEvents(identity.entityType, category, recurrenceAffinity)) {
        ...
        for (final generated in generatedEvents) {
          ...
          final eventCategory =
              generated.metadata['eventCategory']?.toString() ?? 'community_event';
          eventCategoryCounts[eventCategory] =
              (eventCategoryCounts[eventCategory] ?? 0) + 1;
          inferredNodes.add(
            ReplayPlaceGraphNode(
    ...
    final finalOrganizationProfiles = _dedupeOrganizations(organizationProfiles);
    ...
    return ReplayPlaceGraph(
      ...
      metadata: <String, dynamic>{
        ...
        'graphKind': 'canonical_bham_truth_year_place_graph',
    ...
    for (final community in communities) {
      ...
      inferred.add(
        ReplayVenueProfile(
          ...
          recurrenceAffinity: community.eventIds.length >= 3 ? 'high' : 'medium',
          demandPressureBand:
    ...
    for (final community in communities) {
      ...
      inferred.add(
        ReplayClubProfile(
          ...
          hostOrganizationId:
              community.organizationIds.isEmpty ? null : community.organizationIds.first,
          venueIds: venues
              .where((venue) => venue.localityAnchor == community.localityAnchor)
              .take(3)
    ...
    for (final organization in organizations) {
      ...
      inferred.add(
        ReplayClubProfile(
          ...
          venueIds: venues
              .where((venue) => venue.localityAnchor == organization.localityAnchor)
              .take(3)
              .map((venue) => venue.identity.normalizedEntityId)
              .toList(growable: false),
          eventIds: eventIds,
  ...
  String _eventCategoryFor(Map<String, dynamic> fields) {
    ...
    if (title.contains('tour') || title.contains('concert') || title.contains('celebration')) {
      return 'performance';
    }
  ...
  String? _venueCategoryForOrganization(ReplayOrganizationProfile organization) {
