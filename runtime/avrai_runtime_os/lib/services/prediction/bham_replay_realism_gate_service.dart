  ...
  ReplayRealismGateReport buildReport({
    ...
    records.add(
      ReplayRealismGateRecord(
        ...
        rationale: populationPass
            ? 'Dense Birmingham population, locality coverage, and AVRAI-agent split are present.'
    ...
    records.add(
      ReplayRealismGateRecord(
        ...
        rationale: placePass
            ? 'Canonical Birmingham place graph is dense enough for city movement.'
    ...
    final kernelPass = kernelReport.activeKernelCount == kernelReport.requiredKernelCount &&
        kernelReport.records.every(
          (record) =>
              record.status == 'active' &&
              record.evidenceCount >= (kernelMinimums[record.kernelId] ?? 1),
        );
    records.add(
      ReplayRealismGateRecord(
    ...
    final higherAgentPass =
        (rollupBatch.rollupCountsByLevel['personal'] ?? 0) > 0 &&
            ...
            dailyBehaviorBatch.actions.length >= populationProfile.modeledActorCount;
    records.add(
      ReplayRealismGateRecord(
    ...
    final explainabilityPass =
        actionExplanations.length >= 200 &&
        dailyBehaviorBatch.actions.any((action) => action.kernelLanes.isNotEmpty);
    records.add(
      ReplayRealismGateRecord(
    ...
    records.add(
      ReplayRealismGateRecord(
        ...
        rationale: calibrationReport.passed
            ? 'Replay metrics are calibrated against Birmingham truth-year targets.'
    ...
    final actorKernelPass = actorKernelCoverageReport != null &&
        actorKernelCoverageReport.actorCount == populationProfile.actors.length &&
