...
class CallingScoreDataCollector {
  ...
  Future<String?> logCallingScoreCalculation({
    ...
    if (!_enabled) {
      developer.log('Calling score data collection is disabled',
          name: _logName);
      return null;
    }
    ...
    try {
      ...
      for (final dimension in userVibe.anonymizedDimensions.keys) {
        userVibeDimensions[dimension] =
            userVibe.anonymizedDimensions[dimension] ?? 0.0;
      }
      ...
      for (final dimension in spotVibe.vibeDimensions.keys) {
        spotVibeDimensions[dimension] =
            spotVibe.vibeDimensions[dimension] ?? 0.0;
      }
  ...
  Future<void> linkOutcomeToCallingScore({
    ...
    try {
      ...
      await _supabase.from('calling_score_training_data').update({
        'outcome_type': outcome.outcomeType
            .toString()
            .split('.')
            .last, // 'positive', 'negative', 'neutral'
  ...
  Future<List<Map<String, dynamic>>> getTrainingData({
    ...
    try {
      ...
      if (includeOutcomesOnly) {
        filteredRecords =
            filteredRecords.where((r) => r['outcome_type'] != null).toList();
      }

      if (minOutcomeScore != null) {
        filteredRecords = filteredRecords
            .where((r) =>
                r['outcome_score'] != null &&
  ...
  Future<Map<String, dynamic>> getTrainingDataStats() async {
    try {
      ...
      for (final record in recordsWithOutcomes) {
        ...
        if (outcomeType != null &&
            outcomeDistribution.containsKey(outcomeType)) {
