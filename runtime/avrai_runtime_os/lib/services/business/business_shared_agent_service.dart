...
class BusinessSharedAgentService {
  ...
  Future<String> initializeSharedAgent(String businessId) async {
    try {
      _logger.info('Initializing shared agent for business: $businessId',
          tag: _logName);
      ...
      if (business.sharedAgentId != null) {
        _logger.info('Shared agent already exists: ${business.sharedAgentId}',
            tag: _logName);
  ...
  Future<PersonalityProfile> feedMemberLearning({
    ...
    try {
      ...
      final insight = pl.AI2AILearningInsight(
        ...
        dimensionInsights:
            aggregatedLearning['dimensionInsights'] as Map<String, double>? ??
                {},
        learningQuality:
            aggregatedLearning['confidenceScore'] as double? ?? 0.5,
  ...
  Future<PersonalityProfile> getSharedAgentProfile(String businessId) async {
    try {
      ...
      return await _personalityLearning
          .initializePersonality('business_$businessId');
    } catch (e) {
      _logger.error('Error getting shared agent profile',
  ...
  Future<Map<String, dynamic>> _aggregateMemberLearning({
    ...
    try {
      ...
      final members =
          await _memberService.getBusinessMembers(businessId: businessId);
      ...
      final confidenceScore =
          (newMemberLearning['confidenceScore'] as num? ?? 0.5).toDouble() *
              memberWeight;
      ...
      _logger.error('Error aggregating member learning',
          error: e, tag: _logName);
      return {
  ...
  Future<PersonalityProfile> getBusinessPersonalityForMatching(
  ...
  Future<String> resetSharedAgent(String businessId) async {
    try {
      _logger.warn('Resetting shared agent for business: $businessId',
          tag: _logName);
