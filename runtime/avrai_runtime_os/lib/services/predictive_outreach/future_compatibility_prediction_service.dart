...
class FutureCompatibilityPredictionService {
  ...
  Future<CompatibilityTrajectory> predictFutureCompatibility({
    ...
    try {
      ...
      final peakEntry =
          trajectory.entries.reduce((a, b) => a.value > b.value ? a : b);
        ...
        try {
          ...
          if (futureCompat > currentCompat && futureCompat >= 0.7) {
            final improvementRate =
                (futureCompat - currentCompat) / lookahead.inDays;
  ...
  DateTime? _calculateOptimalOutreachTime(
    ...
    return DateTime.now()
        .add(Duration(days: daysToTarget.round().clamp(0, lookahead.inDays)));
  }
