...
class TemporalQuantumCompatibilityService {
  ...
  Future<TemporalQuantumCompatibility> calculateTemporalCompatibility({
    ...
    try {
      ...
      final profileA =
          await _personalityLearning.getCurrentPersonality(userAId);
      final profileB =
          await _personalityLearning.getCurrentPersonality(userBId);
      ...
      final peakEntry = trajectory.entries
          .reduce((a, b) => a.value.combined > b.value.combined ? a : b);
      final peakTime = peakEntry.key;
      final peakCompatibility = peakEntry.value.combined;

      // 8. Calculate average
      final average =
          trajectory.values.map((s) => s.combined).reduce((a, b) => a + b) /
              trajectory.length;
