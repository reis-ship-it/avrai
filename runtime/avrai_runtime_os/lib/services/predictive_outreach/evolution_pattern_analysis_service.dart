...
class EvolutionPatternAnalysisService {
  ...
  Future<OutreachTimingRecommendation> calculateOptimalTiming({
    ...
    try {
      ...
      // 1. Analyze evolution patterns for both users
      final userPatterns =
          await _stringService.analyzeEvolutionPatterns(userId);
      final targetPatterns =
          await _stringService.analyzeEvolutionPatterns(targetId);
    ...
    for (final userTrend in userPatterns.trends) {
      for (final targetTrend in targetPatterns.trends) {
        if (_areTrendsConverging(userTrend, targetTrend)) {
          ...
          final strength =
              _calculateConvergenceStrength(userTrend, targetTrend);
  ...
  DateTime _findBestTiming(
    ...
    if (convergencePeriods.isNotEmpty) {
      final bestConvergence = convergencePeriods.reduce(
          (a, b) => a.convergenceStrength > b.convergenceStrength ? a : b);
    ...
    if (cycleAlignments.isNotEmpty) {
      final bestAlignment = cycleAlignments
          .reduce((a, b) => a.alignmentStrength > b.alignmentStrength ? a : b);
  ...
  String _generateTimingReasoning(
    ...
    if (convergencePeriods.isNotEmpty) {
      final best = convergencePeriods.reduce(
          (a, b) => a.convergenceStrength > b.convergenceStrength ? a : b);
