...
class OutreachHistoryService {
  ...
  Future<bool> canSendOutreach({
    ...
    try {
      ...
      final cooldownDate =
          DateTime.now().subtract(Duration(days: cooldownDays));
  ...
  Future<bool> recordOutreach({
    ...
    try {
      await _supabaseService.client.from('outreach_history').insert({
  ...
  Future<bool> updateOutreachResponse({
    ...
    try {
      await _supabaseService.client
          ...
          .eq('outreach_type', outreachType)
          .isFilter(
              'responded_at', null) // Only update if not already responded
          .limit(1);
