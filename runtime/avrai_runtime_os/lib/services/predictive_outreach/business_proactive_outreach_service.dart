...
class BusinessProactiveOutreachService {
  ...
  Future<void> processBusinessEventOutreach({
    ...
    try {
      ...
      final businessProfile =
          await _personalityLearning.getCurrentPersonality(businessId);
  ...
  Future<void> processBusinessExpertOutreach({
    ...
    try {
      ...
      for (final expert in expertMatches) {
        try {
          final expertProfile =
              await _personalityLearning.getCurrentPersonality(expert.expertId);
          ...
          if (outreachScore >= 0.75) {
            await _ai2aiCommunication.sendOutreachMessage(
              ...
              payload: {
                ...
                'compatibility_score': outreachScore,
                'reasoning':
                    'High compatibility for business-expert partnership',
              },
            );
          }
        } catch (e) {
  ...
  Future<void> processBusinessBusinessOutreach({
    ...
    try {
      ...
      final businessMatches =
          await _businessBusinessOutreach.discoverBusinesses(
        businessId: businessId,
        minCompatibilityScore: 0.7,
        limit: 20,
      );
      ...
      for (final match in businessMatches) {
        try {
          final targetBusinessProfile = await _personalityLearning
              .getCurrentPersonality(match.businessId);
  ...
  /// Find users for event
  Future<List<EventCandidate>> _findUsersForEvent(dynamic event,
      {double minCompatibility = 0.7}) async {
