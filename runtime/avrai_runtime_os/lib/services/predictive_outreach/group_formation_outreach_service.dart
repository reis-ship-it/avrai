...
class GroupFormationOutreachService {
  ...
  Future<void> formGroupWithOutreach({
    ...
    try {
      ...
      final finalCandidates =
          selectedCandidates.take(targetGroupSize - seedKnots.length).toList();
  ...
  Future<void> formOnboardingGroupWithOutreach({
    ...
    try {
      ...
      final compatibleProfiles =
          await _knotCommunityService.createOnboardingKnotGroup(
  ...
  Future<void> _sendGroupFormationOutreach({
    ...
    try {
      ...
      final seedProfile =
          await _personalityLearning.getCurrentPersonality(seedUserIds.first);
      if (seedProfile == null) {
        developer.log(
