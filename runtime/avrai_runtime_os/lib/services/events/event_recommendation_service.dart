...
class EventRecommendationService {
  ...
  EventRecommendationService({
    ...
    UserGovernedLearningUsageService? userGovernedLearningUsageService,
              ...
              userGovernedLearningProjectionService:
                  userGovernedLearningProjectionService ??
                      (GetIt.I.isRegistered<
                              UserGovernedLearningProjectionService>()
                          ? GetIt.I<UserGovernedLearningProjectionService>()
                          : null),
              userGovernedLearningUsageService:
                  userGovernedLearningUsageService ??
                      (GetIt.I.isRegistered<UserGovernedLearningUsageService>()
                          ? GetIt.I<UserGovernedLearningUsageService>()
                          : null),
            ),
        _telemetryService =
        ...
        _userGovernedLearningProjectionService =
            userGovernedLearningProjectionService ??
                (GetIt.I.isRegistered<UserGovernedLearningProjectionService>()
                    ? GetIt.I<UserGovernedLearningProjectionService>()
                    : null),
        _userGovernedLearningAdoptionService =
            userGovernedLearningAdoptionService ??
                (GetIt.I.isRegistered<UserGovernedLearningAdoptionService>()
                    ? GetIt.I<UserGovernedLearningAdoptionService>()
                    : null),
        _userGovernedLearningChatObservationService =
            userGovernedLearningChatObservationService ??
                (GetIt.I.isRegistered<
  ...
  Future<List<EventRecommendation>> getPersonalizedRecommendations({
    ...
    try {
      ...
      final influencesByEventId = <String, _GovernedLearningEventInfluence>{};

      for (final event in allEvents) {
        final governedLearningInfluence = _calculateGovernedLearningInfluence(
          event: event,
          visibleRecords: governedLearningRecords,
        );
        final relevanceScore = await _calculateRelevanceScore(
          ...
          category: category,
          location: location,
          governedLearningBoost: governedLearningInfluence.totalScoreDelta,
        );
        if (governedLearningInfluence.hasInfluence) {
          influencesByEventId[event.id] = governedLearningInfluence;
        }
      ...
      await _recordGovernedLearningUsageReceipts(
        user: user,
        recommendations: output,
        influencesByEventId: influencesByEventId,
  ...
  Future<List<EventRecommendation>> getRecommendationsForScope({
    ...
    try {
      ...
      await _recordGovernedLearningUsageReceipts(
        ...
        recommendations: output,
        influencesByEventId: influencesByEventId,
        decisionFamily: 'event_scope_recommendation',
        surface: 'events_scope_$scope',
      );
      await _dispatchUserRuntimeLearningSignal(
        userId: user.id,
        scopeHint: scope,
  ...
  Future<double> _calculateRelevanceScore({
    ...
    String? category,
    String? location,
    double governedLearningBoost = 0.0,
  }) async {
    ...
    fallbackScore =
        (fallbackScore + _calculateGovernedConsumerBoost(event)).clamp(
      0.0,
      1.0,
    );
    fallbackScore = (fallbackScore + governedLearningBoost).clamp(0.0, 1.0);
  ...
  double _calculateGovernedConsumerBoost(ExpertiseEvent event) {
    final service = _governedDomainConsumerStateService ??
        _tryResolveGovernedDomainConsumerStateService();
    if (service == null) {
      return 0.0;
    }
    final cityCode = event.cityCode;
    if (cityCode == null || cityCode.isEmpty) {
      return 0.0;
    }
    var boost = 0.0;
    final eventState =
        service.latestLiveStateFor(cityCode: cityCode, domainId: 'event');
    if (eventState != null) {
      boost += _scaledGovernedStateWeight(eventState, maxWeight: 0.04);
    }
    if (event.localityCode != null && event.localityCode!.isNotEmpty) {
      final localityState =
          service.latestLiveStateFor(cityCode: cityCode, domainId: 'locality');
      if (localityState != null) {
        boost += _scaledGovernedStateWeight(localityState, maxWeight: 0.025);
      }
    }
    final placeState = service.latestLiveStateFor(
      cityCode: cityCode,
      domainId: 'place',
    );
    if (placeState != null) {
      boost += _scaledGovernedStateWeight(placeState, maxWeight: 0.015);
    }
    final communityState = service.latestLiveStateFor(
      cityCode: cityCode,
      domainId: 'community',
    );
    if (communityState != null) {
      boost += _scaledGovernedStateWeight(communityState, maxWeight: 0.015);
    }
    final venueState = service.latestLiveStateFor(
      cityCode: cityCode,
      domainId: 'venue',
    );
    if (venueState != null) {
      boost += _scaledGovernedStateWeight(venueState, maxWeight: 0.02);
    }
    return boost.clamp(0.0, 0.11);
  }

  double _scaledGovernedStateWeight(
    GovernedDomainConsumerState state, {
    required double maxWeight,
  }) {
    final confidence = (state.averageConfidence ?? 0.65).clamp(0.0, 1.0);
    final requestWeight = state.requestCount <= 0
        ? 0.5
        : (state.requestCount / 4.0).clamp(0.5, 1.0);
    final temporalWeight = state.temporalFreshnessWeight();
    return maxWeight * confidence * requestWeight * temporalWeight;
  }

  GovernedDomainConsumerStateService?
      _tryResolveGovernedDomainConsumerStateService() {
    if (!GetIt.I.isRegistered<GovernedDomainConsumerStateService>()) {
      return null;
    }
    return GetIt.I<GovernedDomainConsumerStateService>();
  }
  ...
  Future<List<UserVisibleGovernedLearningRecord>>
    ...
    try {
      ...
      _logger.warn(
        'Failed to load governed learning records for event recommendations: $error',
        tag: _logName,
      );
      return const <UserVisibleGovernedLearningRecord>[];
    }
  ...
  _GovernedLearningEventInfluence _calculateGovernedLearningInfluence({
    required ExpertiseEvent event,
    required List<UserVisibleGovernedLearningRecord> visibleRecords,
  }) {
    if (visibleRecords.isEmpty) {
      return const _GovernedLearningEventInfluence.none();
    }
    final candidates = <_GovernedLearningEventInfluenceCandidate>[];
    for (final record in visibleRecords) {
      if (record.futureSignalUseBlocked) {
        continue;
      }
      final rawScore = _scoreGovernedLearningRecordForEvent(
        record: record,
        event: event,
      );
      if (rawScore <= 0.0) {
      ...
      candidates.add(
        _GovernedLearningEventInfluenceCandidate(
          record: record,
          rawScore: rawScore,
        ),
      );
    ...
    final normalizedCandidates = <_GovernedLearningEventInfluenceCandidate>[];
    ...
    return _GovernedLearningEventInfluence(
      totalScoreDelta: totalScoreDelta,
      candidates: normalizedCandidates,
    );
  }

  double _scoreGovernedLearningRecordForEvent({
    required UserVisibleGovernedLearningRecord record,
    required ExpertiseEvent event,
  }) {
    final eventTitle = _normalizeGovernedText(event.title);
    final eventCategory = _normalizeGovernedText(event.category);
    final eventDescription = _normalizeGovernedText(event.description);
    final eventType = _normalizeGovernedText(event.getEventTypeDisplayName());
    final eventLocation = _normalizeGovernedText(event.location ?? '');
    final eventContext = <String>[
      eventTitle,
      eventCategory,
      eventDescription,
      eventType,
      eventLocation,
      _normalizeGovernedText(event.cityCode ?? ''),
      _normalizeGovernedText(event.localityCode ?? ''),
    ].join(' ');
    ...
    for (final entity in record.referencedEntities) {
      ...
      if (eventTitle.contains(normalizedEntity)) {
        score += 0.08;
      } else if (eventContext.contains(normalizedEntity)) {
        score += 0.05;
      }
    }
    ...
    for (final domain in record.domainHints) {
      ...
      if (eventCategory.contains(normalizedDomain)) {
        score += 0.04;
      } else if (eventContext.contains(normalizedDomain)) {
        score += 0.025;
      }
    }
    ...
    if (record.sourceProvider == 'recommendation_feedback_intake' &&
        record.referencedEntities.any(
          (entity) => _normalizeGovernedText(entity) == eventTitle,
        )) {
      score += 0.02;
    }

    return score.clamp(0.0, 0.12);
  }

  Future<void> _recordGovernedLearningUsageReceipts({
    required UnifiedUser user,
    required List<EventRecommendation> recommendations,
    required Map<String, _GovernedLearningEventInfluence> influencesByEventId,
    ...
    if (usageService == null ||
        ...
        influencesByEventId.isEmpty) {
      return;
    }
    final decisionTimestamp = DateTime.now().toUtc();
    final receipts = <GovernedLearningUsageReceipt>[];
    for (final recommendation in recommendations) {
      final influence = influencesByEventId[recommendation.event.id];
      if (influence == null || !influence.hasInfluence) {
        continue;
      }
      final domainId = _normalizeGovernedText(
        recommendation.event.category.isEmpty
            ? 'event'
            : recommendation.event.category,
      );
      final domainLabel = _humanizeGovernedLabel(
        recommendation.event.category.isEmpty
            ? 'event'
            : recommendation.event.category,
      );
      final decisionId =
          '$decisionFamily:${user.id}:${recommendation.event.id}:${decisionTimestamp.microsecondsSinceEpoch}';
      for (final candidate in influence.candidates) {
        receipts.add(
          GovernedLearningUsageReceipt(
            ...
            domainId: domainId.isEmpty ? 'event' : domainId,
            domainLabel: domainLabel,
            targetEntityId: recommendation.event.id,
            targetEntityType: 'event',
            targetEntityTitle: recommendation.event.title,
            ...
            influenceReason:
                'Governed learning about ${candidate.record.safeSummary} helped boost this event recommendation.',
            surface: surface,
          ),
        );
      }
    ...
    try {
      ...
      _logger.warn(
        'Failed to record governed learning usage receipts: $error',
        tag: _logName,
      );
    }
    await _recordFirstSurfacedAdoptionReceipts(
      user: user,
      receipts: receipts,
      surface: surface,
    );
  ...
  Future<void> _recordFirstSurfacedAdoptionReceipts({
    required UnifiedUser user,
    ...
    final existing = await adoptionService.listReceiptsForUser(
      ownerUserId: user.id,
      limit: 500,
    );
    final existingSurfacedKeys = existing
        .where(
          (entry) =>
              entry.status ==
                  GovernedLearningAdoptionStatus.firstSurfacedOnSurface &&
              (entry.surface?.trim().isNotEmpty ?? false),
        )
        .map((entry) => '${entry.envelopeId}:${entry.surface}')
        .toSet();
    final pendingReceipts = <GovernedLearningAdoptionReceipt>[];
    final seenPendingKeys = <String>{};
    for (final receipt in receipts) {
      final surfacedKey = '${receipt.envelopeId}:$surface';
      if (existingSurfacedKeys.contains(surfacedKey) ||
          !seenPendingKeys.add(surfacedKey)) {
        continue;
      }
      pendingReceipts.add(
        GovernedLearningAdoptionReceipt(
          id: '${receipt.envelopeId}:$surface:first_surfaced_on_surface',
          ownerUserId: user.id,
          envelopeId: receipt.envelopeId,
          sourceId: receipt.sourceId,
          status: GovernedLearningAdoptionStatus.firstSurfacedOnSurface,
          recordedAtUtc: receipt.usedAtUtc,
          surface: surface,
          decisionFamily: receipt.decisionFamily,
          domainId: receipt.domainId,
          domainLabel: receipt.domainLabel,
          targetEntityId: receipt.targetEntityId,
          targetEntityType: receipt.targetEntityType,
          targetEntityTitle: receipt.targetEntityTitle,
          reason:
              'This governed learning record first surfaced on ${_describeSurfacedLane(surface)}.',
        ),
      );
    }
    ...
    try {
      await adoptionService.recordReceipts(pendingReceipts);
      final observationService = _userGovernedLearningChatObservationService;
      if (observationService != null) {
        for (final adoptionReceipt in pendingReceipts) {
      ...
      _logger.warn(
        'Failed to record governed learning adoption receipts: $error',
        tag: _logName,
      );
    }
  }
  ...
  String _describeSurfacedLane(String surface) {
    switch (_normalizeGovernedText(surface)) {
  ...
  List<String> _extractGovernedLearningTerms(String value) {
    ...
    return _normalizeGovernedText(value)
        .split(' ')
        .where((term) => term.length >= 4 && !stopWords.contains(term))
        .toList(growable: false);
  }

  String _humanizeGovernedLabel(String raw) {
    return raw
        .replaceAll('_', ' ')
        .replaceAllMapped(
          RegExp(r'\b\w'),
...
class _GovernedLearningEventInfluence {
  const _GovernedLearningEventInfluence({
    required this.totalScoreDelta,
    required this.candidates,
  });

  const _GovernedLearningEventInfluence.none()
      : totalScoreDelta = 0.0,
        candidates = const <_GovernedLearningEventInfluenceCandidate>[];

  final double totalScoreDelta;
  final List<_GovernedLearningEventInfluenceCandidate> candidates;

  bool get hasInfluence => totalScoreDelta > 0.0 && candidates.isNotEmpty;
}

class _GovernedLearningEventInfluenceCandidate {
  const _GovernedLearningEventInfluenceCandidate({
  ...
  _GovernedLearningEventInfluenceCandidate copyWith({
    double? normalizedScore,
  }) {
    return _GovernedLearningEventInfluenceCandidate(
      record: record,
      rawScore: rawScore,
