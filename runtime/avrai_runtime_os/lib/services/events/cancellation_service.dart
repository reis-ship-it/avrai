...
class CancellationService {
  ...
  Future<Cancellation> attendeeCancelTicket({
    ...
    try {
      _logger.info(
          'Processing attendee cancellation: event=$eventId, attendee=$attendeeId',
          tag: _logName);
      ...
      if (payment == null) {
        throw Exception(
            'Payment not found for attendee $attendeeId and event $eventId');
      }
      ...
      if (refundAmount > 0) {
        try {
          ...
          _logger.info(
              'Attendee cancellation processed: ${cancellation.id}, refund: \$${refundAmount.toStringAsFixed(2)}',
              tag: _logName);

          // Step 7: Notify host (spot reopened)
          await _notifyHostOfCancellation(
              event.host.id, eventId, cancellation.id);
        ...
        _logger.info('No refund available (day of event): ${cancellation.id}',
            tag: _logName);
      ...
      _logger.error('Error processing attendee cancellation',
  ...
  Future<Cancellation> hostCancelEvent({
    ...
    try {
      _logger.info('Processing host cancellation: event=$eventId, host=$hostId',
          tag: _logName);
      ...
      if (totalRefunds > 0) {
        try {
          ...
          _logger.info(
              'Host cancellation processed: ${cancellation.id}, total refunds: \$${totalRefunds.toStringAsFixed(2)}',
              tag: _logName);
          ...
          _logger.error('Batch refund processing failed',
              error: e, tag: _logName);
      ...
      _logger.error('Error processing host cancellation',
          error: e, tag: _logName);
      rethrow;
    }
  }
  ...
  Future<Cancellation> emergencyCancelEvent({
    ...
    try {
      _logger.info(
          'Processing emergency cancellation: event=$eventId, weather=$weatherRelated',
          tag: _logName);
      ...
      final cancellation = Cancellation(
        ...
        initiator: weatherRelated
            ? CancellationInitiator.weather
            : CancellationInitiator.platform,
        reason: reason,
        refundStatus: RefundStatus.pending,
        paymentIds: payments.map((p) => p.id).toList(),
        createdAt: DateTime.now(),
      ...
      if (totalRefunds > 0) {
        try {
          ...
          _logger.info(
              'Emergency cancellation processed: ${cancellation.id}, total refunds: \$${totalRefunds.toStringAsFixed(2)}',
              tag: _logName);
      ...
      _logger.error('Error processing emergency cancellation',
          error: e, tag: _logName);
      rethrow;
    }
  }
  ...
  Future<List<Cancellation>> getCancellationsForEvent(String eventId) async {
    try {
      return _cancellations.values.where((c) => c.eventId == eventId).toList();
    } catch (e) {
      _logger.error('Error getting cancellations for event',
          error: e, tag: _logName);
      return [];
    }
  }
  ...
  Future<Payment?> _getPaymentForAttendee(
    ...
    try {
      ...
      _logger.error('Error getting payment for attendee',
          error: e, tag: _logName);
      return null;
    }
  }
  ...
  Future<List<Payment>> _getAllPaymentsForEvent(String eventId) async {
    try {
      ...
      _logger.error('Error getting payments for event',
          error: e, tag: _logName);
      return [];
    }
  }
  ...
  Future<void> _notifyHostOfCancellation(
      String hostId, String eventId, String cancellationId) async {
    // In production, send notification to host
    _logger.info(
        'Notifying host $hostId of cancellation $cancellationId for event $eventId',
        tag: _logName);
    // TODO: Implement notification system
  }

  Future<void> _notifyAttendeesOfCancellation(
      List<Payment> payments, String reason) async {
    // In production, send notifications to all attendees
    _logger.info(
        'Notifying ${payments.length} attendees of cancellation: $reason',
        tag: _logName);
    // TODO: Implement notification system
  }
  ...
  Future<void> _applyHostPenalty(String hostId, double penaltyAmount) async {
    ...
    _logger.info(
        'Applying host penalty: host=$hostId, amount=\$${penaltyAmount.toStringAsFixed(2)}',
        tag: _logName);
