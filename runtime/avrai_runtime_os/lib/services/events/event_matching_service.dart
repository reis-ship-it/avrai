...
class EventMatchingService {
  ...
  static const String _quantumEventMatchingFlag =
          ...
          try {
            ...
            if (expertEvents.isNotEmpty) {
              ...
              if (quantumResult != null) {
                ...
                final hybridScore =
                    0.6 * quantumResult.compatibility + 0.4 * classicalScore;
  ...
  Future<double> _getExternalSocialScore(UnifiedUser expert) async {
    ...
    try {
      ...
      final score =
          (math.log(maxFollowers.toDouble() + 1.0) / denom).clamp(0.0, 1.0);
      return score;
    } catch (e) {
      _logger.warn('Failed to compute external social score: $e',
          tag: _logName);
      return 0.0;
    }
  }
  ...
  int? _extractFollowerCount(Map<String, dynamic> profileData) {
    ...
    final candidates = <dynamic>[
      ...
      (profileData['profile'] is Map
          ? (profileData['profile'] as Map)['followers_count']
          : null),
      (profileData['profile'] is Map
          ? (profileData['profile'] as Map)['followersCount']
          : null),
      (profileData['profile'] is Map
          ? (profileData['profile'] as Map)['connectionsCount']
          : null),
    ];
    ...
    try {
      ...
      try {
        ...
        final respects = await client
            .from('user_respects')
            .select(
                'id, created_at, list_id, spot_lists!inner(created_by, category)')
