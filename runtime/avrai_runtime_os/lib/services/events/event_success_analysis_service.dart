...
class EventSuccessAnalysisService {
  ...
  Future<EventSuccessMetrics> analyzeEventSuccess(String eventId) async {
    try {
      ...
      final partnerRatings =
          await _feedbackService.getPartnerRatingsForEvent(eventId);
      ...
      final qualityMetrics =
          _calculateQualityMetrics(feedbacks, partnerRatings);
      ...
      final metrics = EventSuccessMetrics(
        ...
        revenueVsProjected:
            (financialMetrics['revenueVsProjected'] as double?) ?? 0.0,
        ...
        attendeesWhoWouldReturn:
            qualityMetrics['attendeesWhoWouldReturn'] as int,
        attendeesWhoWouldRecommend:
            qualityMetrics['attendeesWhoWouldRecommend'] as int,
        partnerSatisfaction:
            qualityMetrics['partnerSatisfaction'] as Map<String, double>,
        partnersWouldCollaborateAgain:
            qualityMetrics['partnersWouldCollaborateAgain'] as bool,
      ...
      _logger.info(
          'Event success analyzed: event=$eventId, level=${successLevel.name}',
          tag: _logName);
      return metrics;
    } catch (e) {
  ...
  Map<String, dynamic> _calculateAttendanceMetrics(
    ...
    final feedbackResponseRate =
        feedbacks.length / ticketsSold.clamp(1, double.infinity);
    final estimatedAttendance =
        (ticketsSold * feedbackResponseRate.clamp(0.8, 1.0)).round();
    final attendanceRate =
        ticketsSold > 0 ? estimatedAttendance / ticketsSold : 0.0;
  ...
  Future<Map<String, dynamic>> _calculateFinancialMetrics(
    ...
    final profitMargin =
        grossRevenue > 0 ? (netRevenue / grossRevenue) * 100 : null;
  ...
  Map<String, dynamic> _calculateQualityMetrics(
    ...
    final averageRating =
        feedbacks.map((f) => f.overallRating).reduce((a, b) => a + b) /
            feedbacks.length;
    ...
    final promoters =
        feedbacks.where((f) => f.wouldRecommend && f.overallRating >= 4).length;
    final detractors = feedbacks
        .where((f) => !f.wouldRecommend || f.overallRating <= 2)
        .length;
    ...
    final feedbackResponseRate = totalResponses > 0
        ? totalResponses / totalResponses.clamp(1, double.infinity)
        : 0.0;

    // Count attendees who would return/recommend
    final attendeesWhoWouldReturn =
        feedbacks.where((f) => f.wouldAttendAgain).length;
    final attendeesWhoWouldRecommend =
        feedbacks.where((f) => f.wouldRecommend).length;
    ...
    final partnersWouldCollaborateAgain = partnerRatings.isNotEmpty &&
        partnerRatings.where((r) => r.wouldPartnerAgain >= 4).length /
  ...
  Future<void> _updateHostReputation(
      String hostId, EventSuccessMetrics metrics) async {
    // In production, update host's reputation score
    _logger.info(
        'Updating host reputation: host=$hostId, successLevel=${metrics.successLevel.name}',
        tag: _logName);
