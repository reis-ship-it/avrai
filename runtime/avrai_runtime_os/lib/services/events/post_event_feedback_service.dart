...
class PostEventFeedbackService {
  ...
  final PostEventFeedbackPromptPlannerService _promptPlannerService;
  ...
  PostEventFeedbackService({
    ...
    PostEventFeedbackPromptPlannerService? promptPlannerService,
  ...
  Future<EventFeedback> submitFeedback({
    ...
    try {
      ...
      final event = await _eventService.getEventById(eventId);
      await _stageEventFeedbackUpwardBestEffort(
        feedback: feedback,
        event: event,
      );
      await _planEventFeedbackFollowUpBestEffort(
        feedback: feedback,
        event: event,
      );
  ...
  Future<PartnerRating> submitPartnerRating({
    ...
    try {
      ...
      final event = await _eventService.getEventById(eventId);
      await _stagePartnerRatingUpwardBestEffort(
        rating: rating,
        event: event,
      );
  ...
  Future<void> _stageEventFeedbackUpwardBestEffort({
    ...
    try {
      final airGapArtifact = const UpwardAirGapService().issueArtifact(
        ...
        sanitizedPayload: <String, dynamic>{
          ...
          if (event != null) 'eventCategory': event.category,
          if (event?.localityCode?.trim().isNotEmpty ?? false)
            'localityCode': event!.localityCode!,
        },
      );
      await service.stageEventFeedbackIntake(
        feedback: feedback,
      ...
      _logger.warning(
        'Event feedback upward learning staging skipped: $e',
        tag: _logName,
      );
    }
  }

  Future<void> _planEventFeedbackFollowUpBestEffort({
    required EventFeedback feedback,
    required ExpertiseEvent? event,
  }) async {
    try {
      ...
      _logger.warning(
        'Event feedback follow-up planning skipped: $e',
        tag: _logName,
      );
    }
  }

  Future<void> _stagePartnerRatingUpwardBestEffort({
    ...
    try {
      ...
      await service.stagePartnerRatingIntake(
        rating: rating,
        airGapArtifact: airGapArtifact,
        event: event,
      );
      ...
      _logger.warning(
        'Partner rating upward learning staging skipped: $e',
        tag: _logName,
      );
    }
  }
