...
class ABTestingService {
  ...
  Future<T?> getParameterValue<T>({
    ...
    final variant =
        await getVariant(experimentId: experimentId, userId: userId);
...
class ABExperiment {
  ...
  factory ABExperiment.fromJson(Map<String, dynamic> json) {
    return ABExperiment(
      ...
      trafficPercentage:
          (json['trafficPercentage'] as num?)?.toDouble() ?? 100.0,
...
class ListExperiments {
  ...
  static final compatibilityThreshold = ABExperiment(
    ...
    description:
        'Test different minimum compatibility scores for list inclusion',
