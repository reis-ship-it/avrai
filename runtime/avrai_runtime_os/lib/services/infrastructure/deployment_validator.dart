...
class DeploymentValidator {
  ...
  final AppLogger _logger =
      const AppLogger(defaultTag: 'Deployment', minimumLevel: LogLevel.info);
  ...
  Future<DeploymentReadinessScore> calculateReadinessScore(
    ...
    try {
      ...
      final performanceScore =
          await _validatePerformance(issues, recommendations);
      ...
      final complianceScore =
          await _validateOurGutsCompliance(issues, recommendations);
      ...
      _logger.error('Error calculating readiness score',
      ...
      return DeploymentReadinessScore(
        ...
        privacyScore: 0.0,
        criticalIssues: [
          DeploymentIssue.critical('Error during validation: $e')
        ],
  ...
  Future<bool> checkPrivacyCompliance() async {
    try {
      final privacyScore =
          await _validatePrivacy(<DeploymentIssue>[], <String>[]);
      return privacyScore >= _minPrivacyScore;
    } catch (e) {
      _logger.error('Error checking privacy compliance',
          error: e, tag: _logName);
      return false;
    }
  }
  ...
  Future<bool> checkPerformanceMetrics() async {
    try {
      final performanceScore =
          await _validatePerformance(<DeploymentIssue>[], <String>[]);
      return performanceScore >= _minPerformanceScore;
    } catch (e) {
      _logger.error('Error checking performance metrics',
          error: e, tag: _logName);
      return false;
    }
  }
  ...
  Future<double> _validatePerformance(
    ...
    try {
      ...
      if (memoryMB > 300) {
        issues.add(DeploymentIssue.critical(
            'Memory usage too high: ${memoryMB.toStringAsFixed(1)}MB'));
        score -= 0.2;
      } else if (memoryMB > 200) {
        issues.add(DeploymentIssue.warning(
            'Memory usage high: ${memoryMB.toStringAsFixed(1)}MB'));
      ...
      final report =
          await _performanceMonitor.generateReport(const Duration(hours: 1));

      if (report.averageMemoryUsageMB > 250) {
        issues.add(DeploymentIssue.warning(
            'Average memory usage exceeds recommended threshold'));
        score -= 0.1;
      }

      if (report.alerts.isNotEmpty) {
        final criticalAlerts =
            report.alerts.where((a) => a.severity == 'Critical').length;
        if (criticalAlerts > 0) {
          issues.add(DeploymentIssue.critical(
              '$criticalAlerts critical performance alerts'));
          score -= 0.15;
        }
      }
  ...
  Future<double> _validateSecurity(
    ...
    try {
      ...
      final encryptionResult =
          await _securityValidator.validateDataEncryption();
      if (!encryptionResult.isCompliant) {
        issues
            .add(DeploymentIssue.critical('Data encryption validation failed'));
        score -= 0.3;
      }

      // Validate authentication
      final authResult =
          await _securityValidator.validateAuthenticationSecurity();
      ...
      final privacyResult =
          await _securityValidator.validatePrivacyProtection();
      if (!privacyResult.isCompliant) {
        issues.add(
            DeploymentIssue.critical('Privacy protection validation failed'));
        score -= 0.2;
      }
  ...
  Future<double> _validatePrivacy(
    ...
    try {
      ...
      try {
        ...
        issues.add(DeploymentIssue.critical(
            'Privacy protection validation failed: $e'));
        score -= 0.3;
      }
  ...
  Future<double> _validateOurGutsCompliance(
    ...
    try {
      ...
      if (privacyScore < 0.95) {
        issues.add(DeploymentIssue.critical(
            'OUR_GUTS.md: Privacy compliance insufficient'));
        score -= 0.2;
      }
      ...
      _logger.error('Error validating OUR_GUTS compliance',
          error: e, tag: _logName);
      return 0.0;
    }
  }
