...
import 'package:avrai_core/models/misc/verification_result.dart';
...
class IdentityVerificationService {
  ...
  Future<bool> requiresVerification(String userId) async {
    try {
      _logger.info('Checking if user needs verification: user=$userId',
          tag: _logName);

      if (_taxComplianceService == null) {
        _logger.warn(
            'TaxComplianceService not available, cannot determine verification requirement',
            tag: _logName);
        return false;
      }
      ...
      const monthlyEarnings =
          0.0; // await _taxComplianceService!.calculateEarningsForYear(userId, currentYear);
      ...
      if (monthlyEarnings >= _monthlyEarningsThreshold) {
        _logger.info(
            'User meets monthly earnings threshold: user=$userId, earnings=\$${monthlyEarnings.toStringAsFixed(2)}',
            tag: _logName);
        return true;
      }
      ...
      if (monthlyEarnings >= _lifetimeEarningsThreshold) {
        _logger.info(
            'User meets lifetime earnings threshold: user=$userId, earnings=\$${monthlyEarnings.toStringAsFixed(2)}',
            tag: _logName);
        return true;
      }

      _logger.info('User does not need verification: user=$userId',
          tag: _logName);
      return false;
    } catch (e) {
      _logger.error('Error checking verification requirement',
  ...
  Future<VerificationSession> initiateVerification(String userId) async {
    try {
      ...
      final verificationUrl =
          'https://verify.stripe.com/identity/$stripeSessionId';
      ...
      _logger.info('Verification session created: session=${session.id}',
          tag: _logName);
  ...
  Future<VerificationResult> checkVerificationStatus(String sessionId) async {
    try {
      _logger.info('Checking verification status: session=$sessionId',
          tag: _logName);
      ...
      final result = VerificationResult(
        ...
        failureReason:
            status == VerificationStatus.failed ? 'Verification failed' : null,
      );
      ...
      _logger.info(
          'Verification status checked: session=$sessionId, status=${status.name}',
          tag: _logName);
