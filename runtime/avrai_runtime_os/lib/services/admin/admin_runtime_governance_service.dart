...
class AdminRuntimeGovernanceService {
  ...
  final Ai2AiTransportRetentionTelemetryStore?
      _ai2aiTransportRetentionTelemetryStore;
  ...
  Stream<Ai2AiTransportRetentionSnapshot>
      watchAi2AiTransportRetentionDiagnostics() {
    _accessControl.requireAuthorization();
    return Stream<void>.periodic(
      const Duration(seconds: 3),
      (_) {},
    ).asyncMap((_) => getAi2AiTransportRetentionDiagnosticsSnapshot());
  }

  Stream<ArtifactLifecycleVisibilitySnapshot>
      watchArtifactLifecycleVisibilityDiagnostics() {
    _accessControl.requireAuthorization();
    return Stream<void>.periodic(
      const Duration(seconds: 3),
      (_) {},
    ).asyncMap((_) => getArtifactLifecycleVisibilityDiagnosticsSnapshot());
  }
  ...
  Future<Ai2AiTransportRetentionSnapshot>
      getAi2AiTransportRetentionDiagnosticsSnapshot() async {
    _accessControl.requireAuthorization();
    return _ai2aiTransportRetentionTelemetryStore?.snapshot(
          capturedAtUtc: DateTime.now().toUtc(),
        ) ??
  ...
  Future<ArtifactLifecycleVisibilitySnapshot>
      getArtifactLifecycleVisibilityDiagnosticsSnapshot() async {
    _accessControl.requireAuthorization();
    final capturedAtUtc = DateTime.now().toUtc();
    return ArtifactLifecycleVisibilitySnapshot(
      capturedAtUtc: capturedAtUtc,
      replayUsesDedicatedProject: ReplayStorageConfig.usesDedicatedProject,
      replaySharesProjectWithApp: ReplayStorageConfig.sharesProjectWithApp,
      entries: <ArtifactLifecycleVisibilityEntry>[
        _entryFromMetadata(
          label: 'Governed upward review',
          metadata: const ArtifactLifecycleMetadata(
          ...
          summary:
              'Review/source descriptors remain durable and bounded after air-gap validation.',
        ),
        _entryFromMetadata(
          label: 'Governed upward job',
          metadata: const ArtifactLifecycleMetadata(
          ...
          summary:
              'Kernel-graph job lineage persists so review and rollback stay auditable.',
        ),
        _entryFromMetadata(
          label: 'Replay training manifest',
          metadata: const ArtifactLifecycleMetadata(
            artifactClass: ArtifactLifecycleClass.canonical,
            artifactState: ArtifactLifecycleState.accepted,
            retentionPolicy: ArtifactRetentionPolicy(
          ...
          summary:
              'Canonical replay-to-training manifest survives as the durable training record.',
        ),
        _entryFromMetadata(
          ...
          metadata: const ArtifactLifecycleMetadata(
            ...
            retentionPolicy: ArtifactRetentionPolicy(
              ...
              ttlDays: ReplayStorageConfig.stagingRetentionTtlDays,
            ),
          ...
          summary:
              'Managed local staging root expires and is deleted after successful replay upload.',
        ),
        _entryFromMetadata(
          label: 'Replay partition manifest',
          metadata: const ArtifactLifecycleMetadata(
            artifactClass: ArtifactLifecycleClass.staging,
            artifactState: ArtifactLifecycleState.accepted,
            retentionPolicy: ArtifactRetentionPolicy(
              mode: ArtifactRetentionMode.ttlDelete,
              ttlDays: ReplayStorageConfig.partitionRetentionTtlDays,
            ),
          ...
          cleanupAfterSuccessfulUpload:
              ReplayStorageConfig.cleanupStagingOnSuccessfulUpload,
          summary:
              'Partition chunks are derived upload artifacts, not canonical retained data.',
        ),
        _entryFromMetadata(
          label: 'Replay upload manifest',
          metadata: const ArtifactLifecycleMetadata(
          ...
          summary:
              'Upload/index manifest persists as archive lineage after staging deletion.',
        ),
        _entryFromAi2AiPolicy(
          label: 'AI2AI wormhole queue',
          familyId: 'ai2ai_wormhole_queue',
          artifactClass: ArtifactLifecycleClass.temporary,
          artifactState: ArtifactLifecycleState.candidate,
          policy: Ai2AiRetentionConfig.wormholeQueue,
          summary:
              'Ephemeral delivery queue; not training-eligible and retained only for bounded delivery/retry.',
        ),
        _entryFromAi2AiPolicy(
          ...
          artifactClass: ArtifactLifecycleClass.temporary,
          artifactState: ArtifactLifecycleState.accepted,
          policy: Ai2AiRetentionConfig.dmTransportBlob,
          summary:
              'Persist local-first, then consume the per-device transport blob.',
        ),
        _entryFromAi2AiPolicy(
          label: 'Community transport blob',
          familyId: 'ai2ai_community_transport_blob',
          artifactClass: ArtifactLifecycleClass.temporary,
          artifactState: ArtifactLifecycleState.accepted,
          policy: Ai2AiRetentionConfig.communityTransportBlob,
          summary:
              'Persist local-first, then consume the per-recipient community transport blob.',
        ),
        _entryFromAi2AiPolicy(
          label: 'Local delivered AI2AI history',
          familyId: 'ai2ai_local_delivered_history',
          artifactClass: ArtifactLifecycleClass.temporary,
          artifactState: ArtifactLifecycleState.accepted,
          policy: Ai2AiRetentionConfig.localDeliveredHistory,
          summary:
              'Bounded local UX history only; not a durable learning substrate.',
        ),
        _entryFromAi2AiPolicy(
          label: 'Locality mesh hot cache',
          familyId: 'locality_mesh_hot_cache',
          artifactClass: ArtifactLifecycleClass.temporary,
          artifactState: ArtifactLifecycleState.accepted,
          policy: Ai2AiRetentionConfig.localityMeshHotCache,
          deleteWhenSuperseded: true,
          summary:
              'Same-key updates supersede old cache state and expired entries are cleaned up.',
        ),
      ],
    );
  }

  ArtifactLifecycleVisibilityEntry _entryFromMetadata({
    required String label,
    required ArtifactLifecycleMetadata metadata,
    required String summary,
    bool cleanupAfterSuccessfulUpload = false,
  }) {
    return ArtifactLifecycleVisibilityEntry(
      label: label,
      familyId: metadata.artifactFamily ?? label.toLowerCase(),
      artifactClass: metadata.artifactClass.toWireValue(),
      artifactState: metadata.artifactState.toWireValue(),
      retentionMode: metadata.retentionPolicy.mode.toWireValue(),
      ttlSeconds: metadata.retentionPolicy.ttlDays == null
          ? null
          : Duration(days: metadata.retentionPolicy.ttlDays!).inSeconds,
      deleteWhenSuperseded: metadata.retentionPolicy.deleteWhenSuperseded,
      cleanupAfterSuccessfulUpload: cleanupAfterSuccessfulUpload,
      containsRawPersonalPayload: metadata.containsRawPersonalPayload,
      containsMessageContent: metadata.containsMessageContent,
      trainingEligible: _isTrainingEligibleLifecycle(metadata),
      sourceScope: metadata.sourceScope,
      summary: summary,
    );
  }

  bool _isTrainingEligibleLifecycle(ArtifactLifecycleMetadata metadata) {
    if (metadata.artifactClass != ArtifactLifecycleClass.canonical) {
      return false;
    }
    if (metadata.artifactState != ArtifactLifecycleState.accepted) {
      return false;
    }
    if (metadata.containsRawPersonalPayload ||
        metadata.containsMessageContent) {
      return false;
    }
    return true;
  }

  ArtifactLifecycleVisibilityEntry _entryFromAi2AiPolicy({
    required String label,
    required String familyId,
    required ArtifactLifecycleClass artifactClass,
    required ArtifactLifecycleState artifactState,
    required Ai2AiRetentionPolicy policy,
    required String summary,
    bool deleteWhenSuperseded = false,
  }) {
    return ArtifactLifecycleVisibilityEntry(
      label: label,
      familyId: familyId,
      ...
      retentionMode: _mapAi2AiRetentionMode(policy.mode),
      ttlSeconds: policy.ttl?.inSeconds,
      cleanupGraceSeconds: policy.cleanupGrace?.inSeconds,
      deleteWhenSuperseded: deleteWhenSuperseded,
      deleteOnConsume: policy.deleteOnConsume,
      cleanupAfterSuccessfulUpload: false,
      containsRawPersonalPayload: false,
      containsMessageContent: true,
      trainingEligible: policy.trainingEligible,
      sourceScope: 'ai2ai_runtime',
      summary: summary,
    );
  }

  String _mapAi2AiRetentionMode(Ai2AiRetentionMode mode) {
    return switch (mode) {
      Ai2AiRetentionMode.ephemeralQueue => 'ttl_delete',
      Ai2AiRetentionMode.consumeOnRead => 'user_controlled',
      Ai2AiRetentionMode.localHistoryTtl => 'ttl_delete',
      Ai2AiRetentionMode.ttlDelete => 'ttl_delete',
    };
  }
...
class ArtifactLifecycleVisibilitySnapshot {
  final DateTime capturedAtUtc;
  final List<ArtifactLifecycleVisibilityEntry> entries;
  final bool replayUsesDedicatedProject;
  final bool replaySharesProjectWithApp;

  ArtifactLifecycleVisibilitySnapshot({
    required this.capturedAtUtc,
    required this.entries,
    required this.replayUsesDedicatedProject,
    required this.replaySharesProjectWithApp,
  });

  int get canonicalEntryCount =>
      entries.where((entry) => entry.artifactClass == 'canonical').length;

  int get stagingEntryCount =>
      entries.where((entry) => entry.artifactClass == 'staging').length;

  int get archiveEntryCount =>
      entries.where((entry) => entry.artifactClass == 'archive').length;

  int get temporaryEntryCount =>
      entries.where((entry) => entry.artifactClass == 'temporary').length;
}

class ArtifactLifecycleVisibilityEntry {
  final String label;
  ...
  final int? cleanupGraceSeconds;
  final bool deleteWhenSuperseded;
  final bool deleteOnConsume;
  final bool cleanupAfterSuccessfulUpload;
  ...
  ArtifactLifecycleVisibilityEntry({
    ...
    this.ttlSeconds,
    this.cleanupGraceSeconds,
    required this.deleteWhenSuperseded,
    this.deleteOnConsume = false,
    required this.cleanupAfterSuccessfulUpload,
    required this.containsRawPersonalPayload,
    required this.containsMessageContent,
