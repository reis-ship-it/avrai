...
class RoleManagementServiceImpl implements RoleManagementService {
  ...
  Future<UserRoleAssignment> assignRole({
    ...
    try {
      developer.log(
          'Assigning role ${role.name} to user $userId for list $listId',
          name: _logName);
  ...
  Future<UserRoleAssignment> revokeRole({
    ...
    try {
      developer.log('Revoking role for user $userId from list $listId',
          name: _logName);
  ...
  Future<void> transferOwnership({
    ...
    try {
      developer.log('Transferring ownership of list $listId to $newCuratorId',
          name: _logName);
  ...
  Future<List<UserRoleAssignment>> _loadAllRoleAssignments() async {
    try {
      ...
      return data
          .map((json) =>
              UserRoleAssignment.fromJson(json as Map<String, dynamic>))
          .toList();
    } catch (e) {
