...
class ForecastKernelAdminService {
  ...
  Future<ForecastKernelAdminSnapshot> getForecastSnapshot({
    ...
    final snapshot = ForecastKernelAdminSnapshot(
      generatedAt: now,
  ...
  Future<void> _stageAssistantObservation(
    ...
    if (intakeService == null || snapshot.issuedCount == 0) {
      return;
    }
    final fingerprint = jsonEncode(<String, dynamic>{
      'issuedCount': snapshot.issuedCount,
      'resolvedCount': snapshot.resolvedCount,
      'averageForecastStrength':
          snapshot.averageForecastStrength.toStringAsFixed(4),
      'averageChangeRisk': snapshot.averageChangeRisk.toStringAsFixed(4),
      'recentForecastIds': snapshot.recentForecasts
          .take(5)
          .map((row) => row.forecastId)
          .toList(growable: false),
    });
    ...
    final dominantBandEntry =
        snapshot.bandCounts.entries.fold<MapEntry<String, int>?>(
      null,
      (best, entry) => best == null || entry.value > best.value ? entry : best,
    );
    final summary =
        'Forecast kernel observed ${snapshot.issuedCount} issued forecasts '
        'with average strength ${snapshot.averageForecastStrength.toStringAsFixed(2)} '
        'and change risk ${snapshot.averageChangeRisk.toStringAsFixed(2)} '
        'in the current window.';
    final boundedMetadata = <String, dynamic>{
      'surface': 'forecast_kernel_admin',
      'windowStartUtc': snapshot.windowStart.toIso8601String(),
      'windowEndUtc': snapshot.windowEnd.toIso8601String(),
      'issuedCount': snapshot.issuedCount,
      'resolvedCount': snapshot.resolvedCount,
      'averageOutcomeProbability': snapshot.averageOutcomeProbability,
      'averageForecastStrength': snapshot.averageForecastStrength,
      'averageActionability': snapshot.averageActionability,
      'averageSupportQuality': snapshot.averageSupportQuality,
      'averageCalibrationGap': snapshot.averageCalibrationGap,
      'averageChangeRisk': snapshot.averageChangeRisk,
      'averageSkillLowerConfidenceBound':
          snapshot.averageSkillLowerConfidenceBound,
      'bandCounts': snapshot.bandCounts,
      'dispositionCounts': snapshot.dispositionCounts,
      'recentForecastBands': snapshot.recentForecasts
          .take(5)
    ...
    final airGapArtifact = const UpwardAirGapService().issueArtifact(
      ...
      pseudonymousActorRef: 'assistant:forecast_kernel',
      sanitizedPayload: <String, dynamic>{
        'observationKind': 'forecast_kernel_snapshot',
        'summary': summary,
    ...
    await intakeService.stageSupervisorAssistantObservationIntake(
      observerId: 'assistant_forecast_kernel',
      ...
      upwardReferencedEntities: snapshot.recentForecasts
          .take(5)
          .expand(
            (row) =>
                <String>[row.forecastId, row.subjectId, row.forecastFamilyId],
          )
          .toSet()
          .toList(growable: false),
      upwardQuestions: <String>[
        if (snapshot.averageForecastStrength < 0.5)
          'Why is recent forecast strength staying below the preferred threshold?',
        if ((dominantBandEntry?.key ?? '').isNotEmpty)
          'Should ${dominantBandEntry!.key} remain the dominant forecast posture for this window?',
      ],
      upwardSignalTags: <String>[
        'surface:forecast_kernel_admin',
        if (dominantBandEntry != null) 'dominant_band:${dominantBandEntry.key}',
        'issued_count:${snapshot.issuedCount}',
      ],
      boundedMetadata: boundedMetadata,
    );
  }
