...
class WorldOrientationService {
  // Singleton instance
  static final WorldOrientationService _instance =
      WorldOrientationService._internal();
  factory WorldOrientationService() => _instance;
  WorldOrientationService._internal();

  // Stream controllers
  final StreamController<Quaternion> _orientationController =
      StreamController<Quaternion>.broadcast();
  ...
  static const Duration _maxOutputInterval =
      Duration(milliseconds: 33); // ~30Hz
  ...
  Future<void> start() async {
    ...
    try {
      ...
      developer.log('Error starting WorldOrientationService: $e',
          name: 'WorldOrientationService');
    }
  }
  ...
  void _computeOrientation() {
    ...
    if (_lastOrientation != null) {
      _lastOrientation =
          _slerp(_lastOrientation!, targetOrientation, _smoothingFactor);
    } else {
