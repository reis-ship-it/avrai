...
class AutoDeviceLinkService {
  ...
  Future<List<RegisteredDevice>> checkForExistingDevices(String userId) async {
    try {
      ...
      final activeDevices =
          devices.where((d) => d.status == DeviceStatus.active).toList();
  ...
  Future<SharedLinkSecret> approveLinkRequest(
    ...
    final sharedSecret =
        _computeSharedSecret(keyPair.privateKey, theirPublicKey);
    ...
    developer.log('Approved link request: ${request.requestId}',
        name: _logName);
...
class SharedLinkSecret {
  ...
  Uint8List get encryptionKey =>
      Uint8List.fromList(sharedSecret.take(16).toList());

  /// Get IV (last 12 bytes)
  Uint8List get iv =>
      Uint8List.fromList(sharedSecret.skip(20).take(12).toList());
}
