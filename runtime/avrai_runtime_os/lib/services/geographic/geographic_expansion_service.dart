...
class GeographicExpansionService {
  ...
  Future<GeographicExpansion> trackEventExpansion({
    ...
    try {
      ...
      final isNewCity =
          city != null && !expansion.expandedCities.contains(city);
      final isNewState =
          state != null && !expansion.expandedStates.contains(state);
      final isNewNation =
          nation != null && !expansion.expandedNations.contains(nation);
      ...
      List<ExpansionEvent> updatedHistory =
          List.from(expansion.expansionHistory);
  ...
  Future<Map<String, double>> calculateLocalityCoverage({
    ...
    try {
      ...
      _logger.error('Error calculating locality coverage',
  ...
  Future<Map<String, double>> calculateStateCoverage({
    ...
    try {
      ...
      _logger.error('Error calculating state coverage',
          error: e, tag: _logName);
      rethrow;
    }
  }
  ...
  Future<Map<String, double>> calculateNationCoverage({
    ...
    try {
      ...
      _logger.error('Error calculating nation coverage',
          error: e, tag: _logName);
      rethrow;
    }
  }
  ...
  Future<double> calculateGlobalCoverage({
    ...
    try {
      ...
      _logger.error('Error calculating global coverage',
          error: e, tag: _logName);
      rethrow;
    }
  }
  ...
  Future<Map<String, double>> _recalculateLocalityCoverage({
    ...
    for (final locality in updatedLocalities) {
      ...
      final eventsInLocality =
          updatedEventHostingLocations[locality]?.length ?? 0;
  ...
  Future<Map<String, double>> _recalculateCityCoverage({
    ...
    for (final city in updatedCities) {
      ...
      final coverageValue =
          (localitiesWithCoverage / totalLocalitiesInCity).clamp(0.0, 1.0);
  ...
  Future<Map<String, double>> _recalculateStateCoverage({
    ...
    for (final state in updatedStates) {
      ...
      final coverageValue =
          (localitiesWithCoverage / totalLocalitiesInState).clamp(0.0, 1.0);
  ...
  Future<Map<String, double>> _recalculateNationCoverage({
    ...
    for (final nation in updatedNations) {
      ...
      final coverageValue =
          (localitiesWithCoverage / totalLocalitiesInNation).clamp(0.0, 1.0);
