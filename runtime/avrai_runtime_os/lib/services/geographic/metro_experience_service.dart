...
class MetroExperienceContext {
  ...
  final DateTime? governedKnowledgeOccurredAtUtc;
  final DateTime? governedKnowledgeCapturedAtUtc;
  final DateTime? governedKnowledgeIntegratedAtUtc;
  final DateTime? governedKnowledgeSyncedAtUtc;
  final String? governedKnowledgePhase;
  ...
  const MetroExperienceContext({
    ...
    this.governedKnowledgeOccurredAtUtc,
    this.governedKnowledgeCapturedAtUtc,
    this.governedKnowledgeIntegratedAtUtc,
    this.governedKnowledgeSyncedAtUtc,
    this.governedKnowledgePhase,
  });

  bool get hasMetroProfile => metroKey != 'default';

  bool get hasGovernedKnowledgeTiming =>
      governedKnowledgeOccurredAtUtc != null ||
      governedKnowledgeCapturedAtUtc != null ||
      governedKnowledgeIntegratedAtUtc != null ||
      governedKnowledgeSyncedAtUtc != null ||
      (governedKnowledgePhase?.trim().isNotEmpty ?? false);

  DateTime? get effectiveGovernedKnowledgeAtUtc =>
      governedKnowledgeCapturedAtUtc ??
      governedKnowledgeOccurredAtUtc ??
      governedKnowledgeIntegratedAtUtc ??
      governedKnowledgeSyncedAtUtc;

  String? get governedKnowledgeTimingSummary {
    if (!hasGovernedKnowledgeTiming) {
      return null;
    }
    final parts = <String>[];
    if (governedKnowledgePhase?.trim().isNotEmpty ?? false) {
      parts.add('phase ${governedKnowledgePhase!.trim()}');
    }
    if (effectiveGovernedKnowledgeAtUtc != null) {
      parts.add(
        'effective knowledge ${effectiveGovernedKnowledgeAtUtc!.toUtc().toIso8601String()}',
      );
    }
    if (governedKnowledgeSyncedAtUtc != null) {
      parts.add(
        'last sync ${governedKnowledgeSyncedAtUtc!.toUtc().toIso8601String()}',
      );
    }
  ...
  Map<String, dynamic> toPreferenceMap() {
    return {
      ...
      'governed_knowledge_occurred_at':
          governedKnowledgeOccurredAtUtc?.toUtc().toIso8601String(),
      'governed_knowledge_captured_at':
          governedKnowledgeCapturedAtUtc?.toUtc().toIso8601String(),
      'governed_knowledge_integrated_at':
          governedKnowledgeIntegratedAtUtc?.toUtc().toIso8601String(),
      'governed_knowledge_synced_at':
          governedKnowledgeSyncedAtUtc?.toUtc().toIso8601String(),
      'governed_knowledge_phase': governedKnowledgePhase,
      'governed_knowledge_timing_summary': governedKnowledgeTimingSummary,
    };
  }
}
...
class MetroExperienceService {
  ...
  Future<MetroExperienceContext> resolveBestEffort({
    ...
    final states = _resolveGovernedDomainConsumerStates(
      cityCode: cityCode,
      domainIds: const <String>{
        'locality',
        'event',
        'community',
        'place',
  ...
  List<GovernedDomainConsumerState> _resolveGovernedDomainConsumerStates({
    required String? cityCode,
    required Set<String> domainIds,
  }) {
    if (cityCode == null || cityCode.isEmpty) {
      return const <GovernedDomainConsumerState>[];
    }
    final service =
        _governedDomainConsumerStateService ?? _tryResolveStateService();
    if (service == null) {
      return const <GovernedDomainConsumerState>[];
    }
    final states = <GovernedDomainConsumerState>[];
    for (final domainId in domainIds) {
      final state =
          service.latestLiveStateFor(cityCode: cityCode, domainId: domainId);
      if (state != null) {
        states.add(state);
      }
    }
  ...
  GovernedDomainConsumerStateService? _tryResolveStateService() {
    final sl = GetIt.instance;
    if (!sl.isRegistered<GovernedDomainConsumerStateService>()) {
      return null;
    }
    return sl<GovernedDomainConsumerStateService>();
  }
  ...
  static MetroExperienceContext profileForMetroKey({
    ...
    String? cityCode,
    String? localityCode,
    List<GovernedDomainConsumerState> governedConsumerStates =
        const <GovernedDomainConsumerState>[],
  }) {
    late final MetroExperienceContext baseContext;
    switch (metroKey) {
      case 'nyc':
        baseContext = MetroExperienceContext(
          metroKey: 'nyc',
          displayName: 'NYC',
          cityCode: cityCode,
          localityCode: localityCode,
      ...
      case 'birmingham':
        ...
        break;
      default:
        baseContext = const MetroExperienceContext(
          metroKey: 'default',
    ...
    if (governedConsumerStates.isEmpty) {
      return baseContext;
    }
    return _applyGovernedConsumerStates(
      baseContext,
      governedConsumerStates,
    );
  }

  static MetroExperienceContext _applyGovernedConsumerStates(
    MetroExperienceContext context,
    List<GovernedDomainConsumerState> states,
  ) {
    final spotPriors = context.spotPriors.toList(growable: true);
    final eventPriors = context.eventPriors.toList(growable: true);
    final communityPriors = context.communityPriors.toList(growable: true);
    final promptSuggestions = context.promptSuggestions.toList(growable: true);
    final categoryKeywords = context.categoryKeywords.toList(growable: true);

    void addUnique(List<String> target, String value) {
    ...
    for (final state in states) {
      final cityLabel = context.displayName;
      switch (state.domainId) {
        case 'locality':
          addUnique(spotPriors, 'Governed locality priors for $cityLabel');
          addUnique(categoryKeywords, 'locality');
          break;
        case 'place':
          addUnique(spotPriors, 'Governed place intelligence for $cityLabel');
          addUnique(categoryKeywords, 'place');
          break;
        case 'venue':
          addUnique(spotPriors, 'Governed venue intelligence for $cityLabel');
          addUnique(categoryKeywords, 'venue');
          break;
        case 'event':
          addUnique(eventPriors, 'Governed event priors for $cityLabel');
          addUnique(categoryKeywords, 'event');
          break;
        case 'mobility':
          addUnique(eventPriors, 'Governed mobility guidance for $cityLabel');
          addUnique(categoryKeywords, 'mobility');
          break;
        case 'community':
          addUnique(
            communityPriors,
            'Governed community coordination for $cityLabel',
          );
          addUnique(categoryKeywords, 'community');
          break;
        case 'business':
          addUnique(
            communityPriors,
            'Governed business corridor signals for $cityLabel',
          );
          addUnique(categoryKeywords, 'business');
          break;
        case 'list':
          addUnique(
            communityPriors,
            'Governed list curation priorities for $cityLabel',
          );
          addUnique(categoryKeywords, 'list');
          break;
      }
      addUnique(
        promptSuggestions,
        'What changed in $cityLabel after the governed ${state.domainId} learning update?',
      );
    }

    DateTime? latestOf(Iterable<DateTime?> values) {
      DateTime? latest;
    ...
    final phaseState = states
        .where((state) => state.kernelExchangePhase?.trim().isNotEmpty ?? false)
        .fold<GovernedDomainConsumerState?>(
      null,
      (current, state) {
        if (current == null ||
            state.effectiveBehaviorAt.isAfter(current.effectiveBehaviorAt)) {
          return state;
        }
        return current;
      },
    ...
    return MetroExperienceContext(
      metroKey: context.metroKey,
      displayName: context.displayName,
      cityCode: context.cityCode,
      localityCode: context.localityCode,
      summary: context.summary,
      categoryKeywords: categoryKeywords,
      spotPriors: spotPriors,
      eventPriors: eventPriors,
      communityPriors: communityPriors,
      promptSuggestions: promptSuggestions,
      governedKnowledgeOccurredAtUtc:
          latestOf(states.map((state) => state.originOccurredAt)),
      governedKnowledgeCapturedAtUtc:
          latestOf(states.map((state) => state.localStateCapturedAt)),
      governedKnowledgeIntegratedAtUtc:
          latestOf(states.map((state) => state.learningIntegratedAt)),
      governedKnowledgeSyncedAtUtc:
          latestOf(states.map((state) => state.sourceLastSyncedAt)),
      governedKnowledgePhase: phaseState?.kernelExchangePhase,
    );
  }
