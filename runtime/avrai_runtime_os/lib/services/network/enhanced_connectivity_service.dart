...
class EnhancedConnectivityService {
  ...
  Future<bool> hasBasicConnectivity() async {
    try {
      ...
      // #region agent log
      developer.log(
          'Basic connectivity result: $hasConnectivity, types: ${result.map((r) => r.name).join(", ")}',
          name: _logName);
      // #endregion
  ...
  Future<bool> hasInternetAccess({bool forceRefresh = false}) async {
    // #region agent log
    developer.log('Checking internet access: forceRefresh=$forceRefresh',
        name: _logName);
    // #endregion
    ...
    if (!hasBasic) {
      // #region agent log
      developer.log('No basic connectivity, skipping internet check',
          name: _logName);
      // #endregion
      return false;
    }
    ...
    if (!forceRefresh && _lastPingResult != null && _lastPingTime != null) {
      ...
      if (age < _cacheTimeout) {
        // #region agent log
        developer.log(
            'Using cached ping result: $_lastPingResult (age: ${age.inSeconds}s, cacheTimeout: ${_cacheTimeout.inSeconds}s)',
            name: _logName);
        // #endregion
        ...
        // #region agent log
        developer.log(
            'Cache expired (age: ${age.inSeconds}s >= ${_cacheTimeout.inSeconds}s), performing new ping',
            name: _logName);
        // #endregion
      }
    }
    ...
    // #region agent log
    developer.log(
        'Internet access check complete: $pingResult, cached for ${_cacheTimeout.inSeconds}s',
        name: _logName);
    // #endregion
  ...
  Future<bool> _pingEndpoint() async {
    try {
      // #region agent log
      developer.log(
          'Pinging $_pingUrl with timeout: ${_pingTimeout.inSeconds}s',
          name: _logName);
      // #endregion

      final startTime = DateTime.now();
      final response =
          await _httpClient.head(Uri.parse(_pingUrl)).timeout(_pingTimeout);
      ...
      // #region agent log
      developer.log(
          'Ping result: $success (status: ${response.statusCode}, duration: ${duration.inMilliseconds}ms)',
          name: _logName);
      // #endregion
  ...
  Stream<bool> get connectivityStream {
    return _connectivity.onConnectivityChanged.map((result) {
      ...
      // #region agent log
      developer.log(
          'Connectivity changed: $hasConnectivity, types: ${result.map((r) => r.name).join(", ")}',
          name: _logName);
      // #endregion
  ...
  Stream<bool> internetAccessStream(
      {Duration checkInterval = const Duration(seconds: 30)}) async* {
    // #region agent log
    developer.log(
        'Starting internet access stream: checkInterval=${checkInterval.inSeconds}s',
        name: _logName);
    // #endregion
    ...
    // #region agent log
    developer.log('Initial internet access check: $initialResult',
        name: _logName);
    // #endregion
    yield initialResult;
    ...
    await for (final _ in Stream.periodic(checkInterval)) {
      ...
      // #region agent log
      developer.log('Periodic internet access check: $periodicResult',
          name: _logName);
      // #endregion
  ...
  Future<ConnectivityStatus> getDetailedStatus() async {
    ...
    // #region agent log
    developer.log(
        'Detailed status: ${status.statusString}, type: ${status.connectionTypeString}, fullyOnline: ${status.isFullyOnline}',
        name: _logName);
    // #endregion
  ...
  void clearCache() {
    // #region agent log
    developer.log(
        'Clearing ping cache (lastResult: $_lastPingResult, lastTime: $_lastPingTime)',
        name: _logName);
    // #endregion
  ...
  void _ensureBackOnlineMonitor() {
    ...
    _backOnlineSubscription = connectivityStream.listen((isOnline) {
      if (isOnline && !_previouslyOnline) {
        developer.log('Device back online – firing onBackOnline',
            name: _logName);
