...
class MultiDeviceMessageService {
  ...
  static const String _localInboxBox = 'spots_ai';
  static const String _localInboxPrefix = 'multi_device_inbox_v1:';
  ...
  Future<List<RegisteredDevice>> _getRecipientDevices(String userId) async {
    try {
      ...
      final devices =
          await _deviceRegistrationService.loadDevicesFromCloud(userId);
  ...
  Future<MultiDeviceSendResult> _sendToSingleDevice({
    ...
    final encrypted =
        await _encryptionService.encrypt(message, recipientUserId);
  ...
  Future<DecryptedMessage?> fetchAndDecryptForDevice({
    ...
    try {
      ...
      if (blob != null) {
        ...
        final persisted = await _persistDeliveredMessage(
          message: message,
        ...
        unawaited(_consumeDmTransportBlobBestEffort(
          ...
          recipientDeviceId: currentDevice.deviceIdString,
        ));
        return message;
      }
  ...
  Future<bool> _persistDeliveredMessage({
    required DecryptedMessage message,
    required String deviceId,
  }) async {
    try {
      await _storageService.setObject(
        _localMessageKey(message.messageId, deviceId),
        <String, Object?>{
          'message_id': message.messageId,
          'sender_id': message.senderId,
          'recipient_id': message.recipientId,
          'content': message.content,
          'timestamp_utc': message.timestamp.toUtc().toIso8601String(),
          'device_id': deviceId,
      ...
      developer.log(
        'Failed to persist multi-device message locally: $e',
        name: _logName,
        error: e,
        stackTrace: st,
      );
  ...
  Future<void> _consumeDmTransportBlobBestEffort({
    ...
    try {
      ...
      developer.log(
        'Failed to consume multi-device DM transport blob: $e',
        name: _logName,
        error: e,
        stackTrace: st,
      );
  ...
  String _localMessageKey(String messageId, String deviceId) {
    return '$_localInboxPrefix$deviceId:$messageId';
  }
}
