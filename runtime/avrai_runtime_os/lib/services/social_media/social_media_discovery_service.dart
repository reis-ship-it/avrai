...
class SocialMediaDiscoveryService {
  ...
  Future<List<FriendSuggestion>> findFriendsWhoUseSpots({
    ...
    try {
      _logger.info(
          '🔍 Finding friends who use SPOTS for agent: ${agentId.substring(0, 10)}...',
          tag: _logName);
      ...
      for (final connection in connections) {
        try {
          ...
          for (final friend in friends) {
            final friendId =
                friend['id'] as String? ?? friend['user_id'] as String?;
            if (friendId != null) {
          ...
          _logger.warn(
              'Failed to fetch friends from ${connection.platform}: $e',
              tag: _logName);
      ...
      final suggestions =
          await _queryFriendMatches(agentId, hashedFriendIds.keys.toList());

      // Cache suggestions
      await _cacheFriendSuggestions(agentId, suggestions);

      _logger.info('✅ Found ${suggestions.length} friend suggestion(s)',
          tag: _logName);
      return suggestions;
    } catch (e, stackTrace) {
      _logger.error('❌ Failed to find friends',
  ...
  Future<List<FriendSuggestion>> getCachedFriendSuggestions(
  ...
  Future<bool> sendFriendConnectionRequest({
    ...
    try {
      ...
      _logger.error('❌ Failed to send connection request',
          error: e, stackTrace: stackTrace, tag: _logName);
      return false;
    }
  }
  ...
  Future<bool> acceptFriendConnectionRequest({
    ...
    try {
      ...
      _logger.error('❌ Failed to accept connection request',
          error: e, stackTrace: stackTrace, tag: _logName);
      return false;
    }
  }
  ...
  Future<void> _storeHashedFriends(
      String agentId, Map<String, String> hashedFriends) async {
    try {
  ...
  Future<List<FriendSuggestion>> _queryFriendMatches(
    ...
    try {
      if (_supabaseService == null) {
        _logger.warn(
            'Supabase service not available, returning empty suggestions',
            tag: _logName);
        return [];
      }
  ...
  Future<void> _cacheFriendSuggestions(
      String agentId, List<FriendSuggestion> suggestions) async {
    try {
      final key = '$_friendSuggestionsKeyPrefix$agentId';
      await _storageService.setObject(key, {
  ...
  Future<void> _storeConnectionRequest(
      String agentId, String friendAgentId) async {
    try {
  ...
  Future<void> _storeAcceptedConnection(
      String agentId, String friendAgentId) async {
    try {
      final key = 'connected_friends_$agentId';
