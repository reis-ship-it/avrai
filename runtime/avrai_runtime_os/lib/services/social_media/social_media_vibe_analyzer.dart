...
class SocialMediaVibeAnalyzer {
      ...
      switch (platform.toLowerCase()) {
        case 'google':
          insights.addAll(await analyzeGoogleProfileForVibe(
            ...
            savedPlaces:
                profileData['savedPlaces'] as List<Map<String, dynamic>>? ?? [],
            reviews:
                profileData['reviews'] as List<Map<String, dynamic>>? ?? [],
  ...
  Future<Map<String, double>> analyzeInstagramProfileForVibe({
    ...
    final bio =
        ((profile['bio'] as String?) ?? (profileData['bio'] as String?) ?? '')
            .toLowerCase();
    ...
    if (interests.contains('travel') || interests.contains('adventure')) {
      insights['exploration_eagerness'] =
          (insights['exploration_eagerness'] ?? 0.0) + 0.15;
      insights['location_adventurousness'] =
          (insights['location_adventurousness'] ?? 0.0) + 0.12;
      insights['temporal_flexibility'] =
          (insights['temporal_flexibility'] ?? 0.0) + 0.10;
    }
    ...
    if (interests.contains('food') || interests.contains('culinary')) {
      insights['curation_tendency'] =
          (insights['curation_tendency'] ?? 0.0) + 0.12;
      insights['authenticity_preference'] =
          (insights['authenticity_preference'] ?? 0.0) + 0.10;
    }
    ...
    if (communities.length > 20) {
      insights['community_orientation'] =
          (insights['community_orientation'] ?? 0.0) + 0.12;
      insights['trust_network_reliance'] =
          (insights['trust_network_reliance'] ?? 0.0) + 0.10;
    }

    // Analyze follows count → social discovery style
    if (follows.length > 500) {
      insights['social_discovery_style'] =
          (insights['social_discovery_style'] ?? 0.0) + 0.15;
    } else if (follows.length > 200) {
      insights['social_discovery_style'] =
          (insights['social_discovery_style'] ?? 0.0) + 0.10;
    }

    // Analyze post captions for keywords
    for (final post in posts) {
      final caption = (post['caption'] as String? ?? '').toLowerCase();
      if (caption.contains('authentic') ||
          caption.contains('local') ||
          caption.contains('hidden gem')) {
        insights['authenticity_preference'] =
            (insights['authenticity_preference'] ?? 0.0) + 0.05;
      }
      if (caption.contains('explore') ||
          caption.contains('discover') ||
          caption.contains('adventure')) {
        insights['exploration_eagerness'] =
            (insights['exploration_eagerness'] ?? 0.0) + 0.05;
      }
    }
