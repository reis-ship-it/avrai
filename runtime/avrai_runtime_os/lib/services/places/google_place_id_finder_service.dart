...
@Deprecated(
    'Use GooglePlaceIdFinderServiceNew instead. This uses the legacy Places API.')
class GooglePlaceIdFinderService {
  ...
  Future<String?> findPlaceId(Spot spot) async {
    try {
      ...
      if (textPlaceId != null) {
        developer.log('Found Place ID via text search: $textPlaceId',
            name: _logName);
        return textPlaceId;
      }
  ...
  Future<String?> _findPlaceIdByTextSearch(Spot spot) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (distance > _maxDistanceMeters * 2) {
          return null; // More lenient for text search
        }
