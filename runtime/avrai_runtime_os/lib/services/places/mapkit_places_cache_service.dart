...
class MapKitPlacesCacheService {
  ...
  Future<List<Spot>> searchCachedPlaces(String query) async {
    try {
      ...
      _logger.error('Error searching cached Apple places',
    ...
    try {
      ...
      for (final key in allKeys) {
        ...
        if (spotLat != null && spotLon != null) {
          final distance =
              _calculateDistance(latitude, longitude, spotLat, spotLon);
      ...
      _logger.error('Error getting nearby cached Apple places',
          error: e, tag: _logName);
      return [];
    }
  }
  ...
  Future<Spot?> getCachedPlace(String placeId) async {
    try {
      ...
      _logger.error('Error getting cached Apple place',
          error: e, tag: _logName);
      return null;
    }
  }
  ...
  Future<void> clearExpiredCache() async {
    try {
      ...
      developer.log('Cleared $cleared expired Apple place entries',
          name: _logName);
    } catch (e) {
      _logger.error('Error clearing expired Apple cache',
          error: e, tag: _logName);
    }
  }
