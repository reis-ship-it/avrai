...
class GooglePlacesCacheService {
  ...
  static const Duration _placeExpiry =
      Duration(days: 7); // Cache places for 7 days
  static const Duration _detailsExpiry =
      Duration(days: 30); // Cache details for 30 days

  /// Cache a Google Place Spot
  Future<void> cachePlace(Spot spot) async {
    try {
      if (!spot.hasGooglePlaceId) {
        developer.log('Spot does not have Google Place ID, skipping cache',
            name: _logName);
        return;
      }
      ...
      developer.log('Cached Google Place: ${spot.googlePlaceId}',
          name: _logName);
    } catch (e) {
  ...
  Future<void> cachePlaces(List<Spot> spots) async {
    try {
      final placesToCache =
          spots.where((spot) => spot.hasGooglePlaceId).toList();
      ...
      developer.log('Cached ${placesToCache.length} Google Places',
          name: _logName);
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> getCachedPlaceDetails(String placeId) async {
    try {
      ...
      _logger.error('Error getting cached place details',
  ...
  Future<List<Spot>> searchCachedPlaces(String query) async {
    try {
      ...
      developer.log('Found ${results.length} cached places for query: $query',
          name: _logName);
      return results;
    } catch (e) {
  ...
  Future<List<Spot>> getCachedPlacesNearby({
    ...
    try {
      ...
      for (final key in _box.getKeys()) {
        ...
        final distance =
            _calculateDistance(latitude, longitude, spotLat, spotLng);
      ...
      developer.log('Found ${results.length} cached places nearby',
          name: _logName);
      return results;
    } catch (e) {
  ...
  Future<void> clearExpiredCache() async {
    try {
      ...
      developer.log(
          'Cleared $expiredPlaces expired places and $expiredDetails expired details',
          name: _logName);
    } catch (e) {
