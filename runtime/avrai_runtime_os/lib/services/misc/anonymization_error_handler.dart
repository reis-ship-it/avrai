...
class AnonymizationErrorHandler {
  ...
  static String _handleForbiddenDataError(String technicalMessage) {
    ...
    if (technicalMessage.contains('email') ||
        technicalMessage.contains('email_address')) {
      violations.add('email address');
    }
    if (technicalMessage.contains('phone') ||
        technicalMessage.contains('phone_number')) {
      violations.add('phone number');
    }
    if (technicalMessage.contains('name') ||
        technicalMessage.contains('display_name')) {
      violations.add('name');
    }
    if (technicalMessage.contains('address') ||
        technicalMessage.contains('street_address')) {
      violations.add('address');
    }
    if (technicalMessage.contains('userid') ||
        technicalMessage.contains('user_id')) {
      violations.add('user ID');
    }
    if (technicalMessage.contains('ssn') ||
        technicalMessage.contains('social_security')) {
      violations.add('social security number');
    }
    if (technicalMessage.contains('credit_card') ||
        technicalMessage.contains('card_number')) {
  ...
  static List<String> getActionableGuidance(
    ...
    if (message.contains('forbidden') || message.contains('suspicious')) {
      guidance.add(
          'Remove any personal information (name, email, phone, address) from your message');
      guidance.add('Use only anonymous identifiers and non-personal data');
      guidance
          .add('Check your privacy settings to ensure proper anonymization');
    }
