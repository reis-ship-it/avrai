...
import 'package:avrai_core/models/user/user_agreement.dart';
...
class LegalDocumentService {
  ...
  Future<UserAgreement> acceptTermsOfService({
    ...
    try {
      _logger.info('User accepting Terms of Service: user=$userId',
          tag: _logName);
      ...
      _logger.error('Error accepting Terms of Service',
  ...
  Future<UserAgreement> acceptPrivacyPolicy({
    ...
    try {
      _logger.info('User accepting Privacy Policy: user=$userId',
          tag: _logName);
  ...
  Future<UserAgreement> acceptEventWaiver({
    ...
    try {
      _logger.info('User accepting event waiver: user=$userId, event=$eventId',
          tag: _logName);
  ...
  Future<void> _saveAgreement(UserAgreement agreement) async {
    final agreements =
        _readLocalAgreements(agreement.userId).toList(growable: true);
  ...
  Future<void> _writeLocalAgreements(
      String userId, List<UserAgreement> agreements) async {
  ...
  Future<void> _syncFromLedger(String userId) async {
    ...
    try {
      ...
      await ingest(
          ...
          eventType: 'privacy_policy_accepted');
      await ingest(
          domain: LedgerDomainV0.expertise, eventType: 'event_waiver_accepted');

      final merged = byId.values.toList(growable: false);
      await _writeLocalAgreements(userId, merged);
      await _storage.userStorage
          .write(_lastSyncMsKey(userId), DateTime.now().millisecondsSinceEpoch);
    } catch (e) {
