...
class CommunityChatService {
    ...
    try {
      await for (final msg in realtime.subscribeToMessages(inboxChannel)) {
        try {
          ...
          final storeResult = await _storeReceivedGroupMessage(
          ...
          if (storeResult.shouldConsumeTransport) {
            unawaited(_consumeCommunityTransportBlobBestEffort(
              messageId: blob.messageId,
  ...
  Future<_StoredCommunityMessageResult> _storeReceivedGroupMessage({
    required String messageId,
    required String communityId,
    required String senderUserId,
    required String plaintext,
    ...
    try {
      ...
      if (alreadyExists) {
        return const _StoredCommunityMessageResult(
          status: _StoredCommunityMessageStatus.duplicate,
        );
      }
      ...
      await box.write(key, existing);
      return _StoredCommunityMessageResult(
        status: _StoredCommunityMessageStatus.stored,
        message: chatMessage,
      );
    } catch (e, st) {
      developer.log(
      ...
      return const _StoredCommunityMessageResult(
        status: _StoredCommunityMessageStatus.failed,
      );
    }
  }

  Future<void> _consumeCommunityTransportBlobBestEffort({
    required String messageId,
    ...
    try {
      await messageStore.consumeMessageBlob(
        messageId: messageId,
        recipientUserId: recipientUserId,
      );
    } catch (e, st) {
      developer.log(
        'Failed to consume community transport blob after local persistence: $e',
        name: _logName,
        error: e,
        stackTrace: st,
      );
...
enum _StoredCommunityMessageStatus {
  stored,
  duplicate,
  failed,
}

class _StoredCommunityMessageResult {
  final _StoredCommunityMessageStatus status;
  final CommunityChatMessage? message;

  const _StoredCommunityMessageResult({
    required this.status,
    this.message,
  });

  bool get shouldConsumeTransport =>
      status != _StoredCommunityMessageStatus.failed;
}
