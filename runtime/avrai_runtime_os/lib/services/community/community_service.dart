...
class CommunityService implements CommunityReader {
  ...
  Future<Community> createCommunity({
    ...
    await _saveCommunity(community);
    _invalidateCommunityCaches(community.id);
    await _stageCommunityCoordinationBestEffort(
      community: community,
      action: 'create',
  ...
  Future<void> addMember(Community community, String userId) async {
    try {
      ...
      try {
        ...
        if (agentIdService != null && personalityLearning != null) {
          ...
          final contribs = _repository != null
              ? await _repository.getVibeContributions(
                  communityId: community.id)
      ...
      await _createCommunityFollowUpPlanBestEffort(
        community: updated,
        action: 'add_member',
        actorUserId: userId,
        affectedRef: userId,
  ...
  Future<void> _stageCommunityCoordinationBestEffort({
    ...
    try {
      ...
      final airGapArtifact = const UpwardAirGapService().issueArtifact(
        ...
        sanitizedPayload: <String, dynamic>{
          ...
          if (community.localityCode?.trim().isNotEmpty ?? false)
      ...
      _logger.warning(
        'Failed to stage community coordination upward learning for ${community.id}: ${e.toString()}',
        tag: _logName,
      );
    }
  }

  Future<void> _createCommunityFollowUpPlanBestEffort({
    required Community community,
    required String action,
    required String actorUserId,
    String? affectedRef,
    ...
    if (planner == null || actorUserId.trim().isEmpty) {
      return;
    }
    try {
      ...
      _logger.warning(
        'Failed to create community follow-up plan for ${community.id}: ${e.toString()}',
        tag: _logName,
      );
    }
  }
