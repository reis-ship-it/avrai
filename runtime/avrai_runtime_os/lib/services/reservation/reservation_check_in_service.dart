...
class ReservationCheckInService {
  ...
  Future<CheckInResult> checkInViaNFC({
    ...
    try {
      ...
      await _stageReservationCheckInBehaviorBestEffort(
        reservation: updatedReservation,
        spot: spot,
        occurredAtUtc: DateTime.now().toUtc(),
        confidenceScore: confidenceScore,
        validationLayers: <String, bool>{
          'geohash': inProximity,
          'wifi': wifiValid,
  ...
  Future<void> _stageReservationCheckInBehaviorBestEffort({
    required Reservation reservation,
    required Spot spot,
    ...
    final ownerUserId =
        reservation.metadata['userId']?.toString().trim().isNotEmpty == true
            ? reservation.metadata['userId'].toString().trim()
            : null;
    if (ownerUserId == null || ownerUserId.isEmpty) {
      return;
    }
    final localityStableKey = spot.localityCode?.trim().isNotEmpty == true
        ? spot.localityCode!.trim()
        : reservation.metadata['localityCode']?.toString().trim().isNotEmpty ==
                true
    ...
    final structuredSignals = <String, dynamic>{
      ...
      'checkInSource': 'reservation_check_in',
      if (spot.cityCode?.trim().isNotEmpty == true)
        'cityCode': spot.cityCode!.trim(),
      if (localityStableKey.isNotEmpty) 'localityStableKey': localityStableKey,
    };
    ...
    try {
      ...
      if (service != null) {
        final airGapArtifact = const UpwardAirGapService().issueArtifact(
          ...
          sanitizedPayload: <String, dynamic>{
            'sourceKind': 'locality_observation_intake',
            'sensorSource': 'reservation_check_in',
            'reservationId': reservation.id,
            'spotId': spot.id,
        ...
        await service.stageLocalityObservationIntake(
          ...
          sourceKind: 'reservation_check_in',
          localityStableKey: localityStableKey,
          ...
          temporalContext: <String, dynamic>{
            'checkedInAtUtc': occurredAtUtc.toIso8601String(),
            'reservationTimeUtc':
                reservation.reservationTime.toUtc().toIso8601String(),
          },
          socialContext: 'reservation_check_in',
          activityContext: 'reservation_check_in',
          lineageRef:
              'reservation_check_in:${reservation.id}:${occurredAtUtc.microsecondsSinceEpoch}',
        );
      }
      ...
      // Best-effort only. Reservation check-in success should not fail if
      // bounded locality staging is unavailable.
    }
  }
