...
class AutomaticCheckInService {
  ...
  Future<AutomaticCheckIn> checkOut({
    ...
    try {
      ...
      if (visit != null) {
        ...
        await _stageUpwardVisitLearningBestEffort(
          updatedVisit,
          source: switch (activeCheckIn.triggerType) {
            CheckInTriggerType.geofence => 'geofence',
            CheckInTriggerType.bluetooth => 'bluetooth',
            CheckInTriggerType.unknown => 'unknown',
          },
        );
        await _createVisitFollowUpPlanBestEffort(
          updatedVisit,
          source: switch (activeCheckIn.triggerType) {
            CheckInTriggerType.geofence => 'geofence',
            CheckInTriggerType.bluetooth => 'bluetooth',
  ...
  Future<void> _stageUpwardVisitLearningBestEffort(
    Visit visit, {
    ...
    try {
      ...
      _logger.warning(
        'Upward visit learning staging skipped: $e',
        tag: _logName,
      );
    }
  }

  Future<void> _createVisitFollowUpPlanBestEffort(
    Visit visit, {
    required String source,
  }) async {
    ...
    try {
      await planner.createVisitPlan(
        visit: visit,
        source: source,
      );
    } catch (e) {
      _logger.warning(
        'Visit follow-up plan staging skipped: $e',
        tag: _logName,
      );
    }
  }
