...
class SpotVibeMatchingService {
  ...
  Future<double> _calculateLegacySpotUserCompatibility({
    ...
    try {
      ...
      final baseCompatibility =
          (vibeCompatibility * 0.85 + knotBonus * 0.15).clamp(0.0, 1.0);
      final governedVenueBoost = _calculateGovernedVenueBoost(spot);
      final adjustedCompatibility =
          (baseCompatibility + governedVenueBoost).clamp(0.0, 1.0);
      ...
      if (locationEntanglementEnabled &&
        ...
        final enhancedCompatibility =
          ...
          personalityCompatibility: adjustedCompatibility,
        ...
        developer.log(
          ...
          'venue=${(governedVenueBoost * 100).toStringAsFixed(1)}%, '
      ...
      developer.log(
        ...
        'knot=${(knotBonus * 100).toStringAsFixed(1)}%, '
        'venue=${(governedVenueBoost * 100).toStringAsFixed(1)}%, '
        'total=${(adjustedCompatibility * 100).toStringAsFixed(1)}%',
        name: _logName,
      );
    ...
    try {
      ...
      for (final spot in candidateSpots) {
        ...
        final compatibility = (spotVibe.calculateVibeCompatibility(userVibe) +
                _calculateGovernedVenueBoost(spot))
            .clamp(0.0, 1.0);
  ...
  double _calculateGovernedVenueBoost(Spot spot) {
    final service = _governedDomainConsumerStateService;
    final cityCode = spot.cityCode;
    if (service == null || cityCode == null || cityCode.isEmpty) {
      return 0.0;
    }
    final venueState =
        service.latestLiveStateFor(cityCode: cityCode, domainId: 'venue');
    if (venueState == null) {
      return 0.0;
    }
    return _scaledGovernedStateWeight(venueState, maxWeight: 0.05)
        .clamp(0.0, 0.05);
  }
