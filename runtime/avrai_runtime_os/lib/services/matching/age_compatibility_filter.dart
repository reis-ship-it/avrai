...
import 'package:avrai_core/models/spots/spot.dart' as app_spot;
...
class AgeCompatibilityFilter {
  ...
  double calculateAgeCompatibility(
    ...
    if (age1 == null || age2 == null) {
      developer.log(
          'Age unknown for one or both users, returning neutral compatibility',
          name: _logName);
      return 0.5;
    }
    ...
    if (allowOverride) {
      developer.log(
          'Age override allowed (e.g., parent-child scenario), returning full compatibility',
          name: _logName);
      return 1.0;
    }
    ...
    developer.log(
        'Cross-age connection: $group1 and $group2, compatibility: $compatibility',
        name: _logName);
  ...
  bool isSpotAgeAppropriate(spots_core.Spot spot, UnifiedUser user) {
    ...
    if (userAge == null) {
      developer.log(
          'User age unknown, allowing spot access (may need age verification)',
          name: _logName);
      return true;
    }
    ...
    if (spot.isAgeRestricted) {
      ...
      if (_requiresAge21(spot)) {
        if (userAge < spotRestriction21) {
          developer.log('Spot requires 21+, user is $userAge, blocking access',
              name: _logName);
          return false;
        }
      }
      // Check if spot requires 18+
        ...
        if (userAge < spotRestriction18) {
          developer.log('Spot requires 18+, user is $userAge, blocking access',
              name: _logName);
          return false;
        }
      }
    ...
    if (userAge <= childMaxAge) {
      if (_isAdultOnlySpot(spot)) {
        developer.log(
            'User is child ($userAge), spot is adult-only, blocking access',
            name: _logName);
        return false;
      }
    }
  ...
  bool _requiresAge18(spots_core.Spot spot) {
    ...
    if (metadata.containsKey('ageRestriction')) {
      final restriction = metadata['ageRestriction'];
      if (restriction is int &&
          restriction >= spotRestriction18 &&
          restriction < spotRestriction21) {
        return true;
      }
  ...
  double applyAgeMultiplier(
    ...
    final ageCompatibility =
        calculateAgeCompatibility(user1, user2, allowOverride: allowOverride);
    ...
    final adjustedCompatibility =
        (baseCompatibility * 0.7) + (ageCompatibility * 0.3);
  ...
  double calculateLearningFilter(
    ...
    Object?
        behaviorContext, // Optional: what behavior triggered this learning (String or Map)
  ) {
  ...
  bool shouldAllowAgeOverride(
    ...
    UnifiedUser user2,
    List<String>
        recentActions, // Action history that might indicate parent-child relationship
  ) {
    ...
    if (ageDifference >= 18) {
      ...
      final hasChildActions = recentActions.any((action) => childKeywords
          .any((keyword) => action.toLowerCase().contains(keyword)));

      if (hasChildActions) {
        developer.log(
            'Age override allowed: potential parent-child relationship with child-appropriate actions',
            name: _logName);
        return true;
      }
    }
  ...
  List<spots_core.Spot> filterSpotsByAge(
      List<spots_core.Spot> spots, UnifiedUser user) {
  ...
  List<app_spot.Spot> filterAppSpotsByAge(
      List<app_spot.Spot> spots, UnifiedUser user) {
  ...
  bool _isAppSpotAgeAppropriate(app_spot.Spot spot, UnifiedUser user) {
    ...
    if (userAge == null) {
      developer.log('User age unknown, allowing spot access (app spot)',
          name: _logName);
      return true;
    }
    ...
    if (isAgeRestricted) {
      if (_requiresAge21FromMetadataAndCategory(spot.metadata, spot.category)) {
        if (userAge < spotRestriction21) {
          developer.log(
              'App spot requires 21+, user is $userAge, blocking access',
              name: _logName);
          return false;
        }
        ...
        if (userAge < spotRestriction18) {
          developer.log(
              'App spot requires 18+, user is $userAge, blocking access',
              name: _logName);
          return false;
        }
      }
    ...
    if (userAge <= childMaxAge) {
      if (_isAdultOnlyAppSpot(spot)) {
        developer.log(
            'User is child ($userAge), app spot is adult-only, blocking access',
            name: _logName);
        return false;
      }
    }
  ...
  bool _isAppSpotAgeRestricted(app_spot.Spot spot) {
    final metadata = spot.metadata;
    final restriction = metadata['ageRestriction'];
    final ageRestrictedFlag =
        metadata['isAgeRestricted'] ?? metadata['ageRestricted'];
  ...
  bool _requiresAge21FromMetadataAndCategory(
      Map<String, dynamic> metadata, String category) {
    if (metadata.containsKey('ageRestriction')) {
      final restriction = metadata['ageRestriction'];
