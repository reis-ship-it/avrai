...
const _defaultPackPath =
    '../../work/docs/plans/beta_launch/AVRAI_BHAM_BETA_LAUNCH_PACK/Bham simulation/08_BHAM_REPLAY_SOURCE_PACK_2023_SEED.json';
const _defaultMarkdownOutPath =
    '../../work/docs/plans/beta_launch/AVRAI_BHAM_BETA_LAUNCH_PACK/Bham simulation/08_BHAM_REPLAY_NORMALIZED_OBSERVATIONS_SEED_2023.md';
const _defaultJsonOutPath =
    '../../work/docs/plans/beta_launch/AVRAI_BHAM_BETA_LAUNCH_PACK/Bham simulation/08_BHAM_REPLAY_NORMALIZED_OBSERVATIONS_SEED_2023.json';
...
Future<void> main(List<String> args) async {
  ...
  final result = await service.ingestManifest(
    manifest: manifest,
    rawRecordsBySourceName: const BhamReplaySourcePackService()
  ...
  stdout.writeln(
    'Ingested BHAM replay source pack ${pack.packId} for ${pack.replayYear}.',
  );
...
ReplaySourcePack _loadPack(String path) {
  final file = File(path);
  if (!file.existsSync()) {
    stderr.writeln('Missing replay source pack: $path');
    exit(1);
  }
  final raw = file.readAsStringSync().trim();
  if (raw.isEmpty) {
    stderr.writeln('Replay source pack is empty: $path');
    exit(1);
  }
  try {
    return const BhamReplaySourcePackService().parsePackJson(raw);
  } on FormatException catch (error) {
    stderr.writeln('Invalid replay source pack: ${error.message}');
    exit(1);
  }
}
...
String _buildMarkdown({
  ...
  final buffer = StringBuffer()
    ..writeln('# BHAM Replay Normalized Observations')
    ..writeln()
