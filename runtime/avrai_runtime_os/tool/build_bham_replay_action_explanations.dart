...
const _defaultMarkdownOutPath =
    'work/docs/plans/beta_launch/AVRAI_BHAM_BETA_LAUNCH_PACK/Bham simulation/55_BHAM_REPLAY_ACTION_EXPLANATIONS_2023.md';
...
Future<void> main(List<String> args) async {
  ...
  File(markdownOut)
    ...
    ..writeAsStringSync(_buildMarkdown(explanations));
  File(jsonOut)
    ..createSync(recursive: true)
    ..writeAsStringSync(
      '${const JsonEncoder.withIndent('  ').convert(explanations.map((entry) => entry.toJson()).toList())}\n',
    );

  stdout.writeln(
    'Wrote BHAM replay action explanations for ${explanations.length} actions.',
  );
}

String _buildMarkdown(List<ReplayActionExplanation> explanations) {
  final buffer = StringBuffer()
    ..writeln('# BHAM Replay Action Explanations')
    ..writeln()
    ..writeln('- Explanations: `${explanations.length}`')
    ..writeln()
    ..writeln('## Sample Explanations')
    ..writeln();

  for (final explanation in explanations.take(40)) {
    buffer
      ..writeln(
        '- `${explanation.actionId}` `${explanation.localityAnchor}` `${explanation.kernelLanes.join(', ')}`',
      )
