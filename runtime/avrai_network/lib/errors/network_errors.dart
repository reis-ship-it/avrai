...
class NoInternetException extends NetworkException {
  NoInternetException()
    : super('No internet connection available', 'NO_INTERNET');
}
...
class TimeoutException extends NetworkException {
  ...
  TimeoutException(this.timeout)
    : super('Request timed out after ${timeout.inSeconds} seconds', 'TIMEOUT');
}
...
class AuthorizationException extends NetworkException {
  const AuthorizationException([String? message])
    : super(message ?? 'Insufficient permissions', 'AUTHORIZATION_ERROR');
}
...
class ParseException extends NetworkException {
  ...
  const ParseException(this.originalResponse, [String? message])
    : super(message ?? 'Failed to parse response', 'PARSE_ERROR');
}
...
class ValidationException extends NetworkException {
  ...
  const ValidationException(this.validationErrors, [String? message])
    : super(
...
class ConflictException extends NetworkException {
  ...
  const ConflictException(this.conflictReason)
    : super('Conflict: $conflictReason', 'CONFLICT');
}
...
class ServiceUnavailableException extends NetworkException {
  ...
  const ServiceUnavailableException([this.retryAfter])
    : super('Service temporarily unavailable', 'SERVICE_UNAVAILABLE');
}
...
class UnknownNetworkException extends NetworkException {
  ...
  const UnknownNetworkException(this.originalError)
    : super('Unknown network error: $originalError', 'UNKNOWN_ERROR');
}
