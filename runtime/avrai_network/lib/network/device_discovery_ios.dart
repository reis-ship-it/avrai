...
class IOSDeviceDiscovery extends DeviceDiscoveryPlatform {
  ...
  Future<bool> requestPermissions() async {
    try {
      developer.log(
        'Requesting iOS device discovery permissions',
        name: _logName,
      );
      ...
      _hasPermissions =
          true; // Multipeer Connectivity handles permissions internally

      developer.log(
        'iOS permissions configured via Info.plist',
        name: _logName,
      );
  ...
  Future<List<DiscoveredDevice>> scanForDevices({
    ...
    if (!isSupported()) {
      developer.log(
        'iOS discovery not supported on this platform',
        name: _logName,
      );
      return [];
    }
  ...
  Future<Map<String, dynamic>> getDeviceInfo() async {
    try {
      ...
      return {
        ...
        'note':
            'Device info collection requires platform-specific implementation',
      };
    } catch (e) {
          ...
          if (_isSpotsService(serviceInfo)) {
            ...
            devices.add(
              DiscoveredDevice(
                ...
                metadata: {'service_type': serviceType, 'domain': domain},
              ),
            );
          }
        } catch (e) {
      ...
      try {
        sub = FlutterBluePlus.scanResults.listen((results) {
          for (final scanResult in results) {
            ...
            final personalityData = _extractPersonalityFromAdvertisement(
              scanResult,
            );
            ...
            devices.add(
              DiscoveredDevice(
                ...
                metadata: {
                  ...
                  'service_data': scanResult.advertisementData.serviceData
                      .toString(),
  ...
  AnonymizedVibeData? _extractPersonalityFromService(NsdServiceInfo service) {
    try {
      ...
      developer.log(
        'TXT record extraction requires service resolution - not implemented yet',
        name: _logName,
      );
      return null;
    } catch (e) {
      developer.log(
        'Error extracting personality from service: $e',
        name: _logName,
      );
      return null;
    }
  ...
  bool _isSpotsDevice(ScanResult scanResult) {
    ...
    if (serviceUuids.any(
      (g) => _normalizeUuid(g.toString()) == spotsServiceUuid,
    )) {
      return true;
    }
  ...
  String _normalizeUuid(String raw) {
    return raw.toLowerCase().replaceAll('{', '').replaceAll('}', '').trim();
  }

  /// Extract personality data from BLE advertisement
  AnonymizedVibeData? _extractPersonalityFromAdvertisement(
    ScanResult scanResult,
  ) {
    try {
      final manufacturerData = scanResult.advertisementData.manufacturerData;
