...
class ReplayProtection {
  ...
  _NonceWindow _getOrCreateWindow(String peerAgentId) {
    return _nonceWindows.putIfAbsent(peerAgentId, () => _NonceWindow());
  }
  ...
  Map<String, dynamic> getStats() {
    return {
      ...
      'totalNonces': _nonceWindows.values.fold(
        0,
        (sum, window) => sum + window.seenNonces.length,
      ),
    };
  }
}
...
class _NonceWindow {
  ...
  void advanceWindow(int newCounter) {
    final newBase = (newCounter - ReplayProtection.windowSize + 1).clamp(
      0,
      newCounter,
    );
  ...
  void addNonce(String nonceKey, int counter) {
    ...
    if (_seenNonces.length > ReplayProtection.windowSize * 2) {
      ...
      final toRemove = sorted.take(
        _seenNonces.length - ReplayProtection.windowSize,
      );
