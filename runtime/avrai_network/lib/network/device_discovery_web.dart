...
class WebDeviceDiscovery extends DeviceDiscoveryPlatform {
  ...
  Future<bool> requestPermissions() async {
    try {
      developer.log(
        'Requesting Web device discovery permissions',
        name: _logName,
      );
  ...
  Future<List<DiscoveredDevice>> scanForDevices({
    ...
    if (!isSupported()) {
      developer.log(
        'Web discovery not supported on this platform',
        name: _logName,
      );
      return [];
    }
    ...
    try {
      ...
      if (_signalingConfig.isConfigured()) {
        try {
          ...
          developer.log(
            'Found ${webrtcDevices.length} WebRTC devices',
            name: _logName,
          );
        } catch (e) {
      ...
      if (_signalingConfig.isConfigured()) {
        try {
          ...
          developer.log(
            'Found ${websocketDevices.length} WebSocket devices',
            name: _logName,
          );
        } catch (e) {
  ...
  Future<List<DiscoveredDevice>> _scanNetworkServiceDiscovery() async {
    ...
    try {
      ...
      developer.log(
        'mDNS discovery not available on Web platform - using WebRTC/WebSocket instead',
        name: _logName,
      );
                ...
                if (peerData['spots_enabled'] == true) {
                  ...
                  devices.add(
                    DiscoveredDevice(
                      ...
                      deviceName:
                          peerData['device_name'] as String? ?? 'SPOTS Device',
  ...
  AnonymizedVibeData? _extractPersonalityFromPeer(
    ...
    try {
      ...
      developer.log(
        'Error extracting personality from peer: $e',
        name: _logName,
      );
      return null;
    }
