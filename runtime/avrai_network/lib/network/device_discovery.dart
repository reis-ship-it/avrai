...
class DeviceDiscoveryService {
  ...
  Future<void> _performScan({required Duration scanWindow}) async {
    try {
      if (_platform == null) {
        developer.log(
          'No platform discovery implementation available',
          name: _logName,
        );
        return;
      }
      ...
      final spotsDevices = devices
          .where((device) => device.isSpotsEnabled)
          .toList();
      ...
      developer.log(
        'Discovered ${spotsDevices.length} SPOTS devices',
        name: _logName,
      );
    } catch (e) {
  ...
  void _cleanupStaleDevices(Duration timeout) {
    ...
    if (staleDevices.isNotEmpty) {
      developer.log(
        'Removed ${staleDevices.length} stale devices',
        name: _logName,
      );
    }
  }
  ...
  Future<AnonymizedVibeData?> extractPersonalityData(
