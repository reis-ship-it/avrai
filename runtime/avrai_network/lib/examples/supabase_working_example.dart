...
class SupabaseWorkingExample {
  ...
  static Future<String?> getFileUrl(String path) async {
    ...
    try {
      final response = _client!.storage.from('files').getPublicUrl(path);
...
class SupabaseExampleUsage {
  static Future<void> runExample() async {
    ...
    final createdUser = await SupabaseWorkingExample.createDocument(
      'users',
      userData,
    );
    if (createdUser != null) {
      ...
      // Delete the user
      final deleted = await SupabaseWorkingExample.deleteDocument(
        'users',
        createdUser['id'],
      );
      if (deleted) {
    ...
    final uploadedPath = await SupabaseWorkingExample.uploadFile(
      'test.txt',
      fileData,
    );
