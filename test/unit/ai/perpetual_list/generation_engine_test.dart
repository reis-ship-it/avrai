  ...
  group('GenerationEngine', () {
    ...
    group('combined score calculation', () {
      test('should calculate combined score correctly', () {
        ...
        final combinedScore = possibilityScore * GenerationEngine.possibilityWeight +
      ...
      test('should filter candidates below threshold', () {
        ...
        final filtered = candidates
            .where((c) => c.combinedScore >= GenerationEngine.minCompatibilityThreshold)
            .toList();
    ...
    group('list grouping', () {
      ...
      test('should respect list count limit', () {
        ...
        final sortedCategories = byCategory.entries.toList()
          ..sort((a, b) => b.value.first.combinedScore.compareTo(a.value.first.combinedScore));
    ...
    group('novelty scoring', () {
      ...
      double calculateNovelty(bool visited, bool recentlySuggested, double categoryFamiliarity) {
        double novelty = 1.0;
    ...
    group('category classification', () {
      test('should classify high energy categories', () {
        final highEnergyCategories = ['nightclub', 'gym', 'sports', 'bar', 'club'];

        for (final category in highEnergyCategories) {
          final isHighEnergy = ['nightclub', 'gym', 'sports', 'bar', 'club']
              .any((c) => category.contains(c));
          expect(isHighEnergy, isTrue, reason: '$category should be high energy');
        }
      });

      test('should classify low energy categories', () {
        final lowEnergyCategories = ['spa', 'library', 'meditation', 'yoga', 'tea'];
      ...
      test('should classify solo categories', () {
        final soloCategories = ['library', 'bookstore', 'coffee', 'cafe', 'museum'];
    ...
    group('time slot category matching', () {
      test('should match morning categories', () {
        expect(_doesCategoryMatchTimeSlot('coffee shop', TimeSlot.morning), isTrue);
        expect(_doesCategoryMatchTimeSlot('brunch spot', TimeSlot.morning), isTrue);
      });

      test('should match evening categories', () {
        expect(_doesCategoryMatchTimeSlot('fine dining restaurant', TimeSlot.evening), isTrue);
        expect(_doesCategoryMatchTimeSlot('bar and grill', TimeSlot.evening), isTrue);
      });

      test('should match night categories', () {
        expect(_doesCategoryMatchTimeSlot('nightclub', TimeSlot.night), isTrue);
        expect(_doesCategoryMatchTimeSlot('late night bar', TimeSlot.night), isTrue);
      });
    });
...
bool _doesCategoryMatchTimeSlot(String category, TimeSlot slot) {
  switch (slot) {
    ...
    case TimeSlot.morning:
      return ['cafe', 'coffee', 'breakfast', 'brunch'].any((c) => category.contains(c));
    case TimeSlot.afternoon:
      return ['restaurant', 'museum', 'shopping', 'lunch'].any((c) => category.contains(c));
    case TimeSlot.evening:
      return ['restaurant', 'dinner', 'bar'].any((c) => category.contains(c));
    case TimeSlot.night:
      return ['bar', 'nightclub', 'club', 'late'].any((c) => category.contains(c));
