  ...
  group('LocationPatternAnalyzer', () {
    group('VisitPattern model', () {
      ...
      test('should calculate time slot from hour', () {
        ...
        expect(getTimeSlotFromDateTime(DateTime(2024, 1, 1, 6, 0)), equals(TimeSlot.earlyMorning));

        // Morning: 8-12 PM
        expect(getTimeSlotFromDateTime(DateTime(2024, 1, 1, 9, 0)), equals(TimeSlot.morning));

        // Afternoon: 12-5 PM
        expect(getTimeSlotFromDateTime(DateTime(2024, 1, 1, 14, 0)), equals(TimeSlot.afternoon));

        // Evening: 5-9 PM
        expect(getTimeSlotFromDateTime(DateTime(2024, 1, 1, 19, 0)), equals(TimeSlot.evening));

        // Night: 9 PM - 12 AM
        expect(getTimeSlotFromDateTime(DateTime(2024, 1, 1, 22, 0)), equals(TimeSlot.night));

        // Late night: 12-5 AM
        expect(getTimeSlotFromDateTime(DateTime(2024, 1, 1, 2, 0)), equals(TimeSlot.lateNight));
      });

      test('should calculate day of week from datetime', () {
        // Monday = 1 in DateTime
        expect(getDayOfWeekFromDateTime(DateTime(2024, 1, 1)), equals(DayOfWeek.monday));

        // Saturday = 6 in DateTime
        expect(getDayOfWeekFromDateTime(DateTime(2024, 1, 6)), equals(DayOfWeek.saturday));

        // Sunday = 7 in DateTime
        expect(getDayOfWeekFromDateTime(DateTime(2024, 1, 7)), equals(DayOfWeek.sunday));
      });
    ...
    group('DayOfWeek enum', () {
      ...
      test('should identify weekend days', () {
        ...
        final weekdayPattern = _createVisitPattern(dayOfWeek: DayOfWeek.wednesday);
    ...
    group('pattern analysis', () {
      ...
      test('should calculate average dwell time by category', () {
        final patterns = [
          _createVisitPattern(category: 'cafe', dwellTime: const Duration(minutes: 30)),
          _createVisitPattern(category: 'cafe', dwellTime: const Duration(minutes: 60)),
          _createVisitPattern(category: 'cafe', dwellTime: const Duration(minutes: 45)),
        ];

        final totalMinutes = patterns
    ...
    group('TimingPreferences model', () {
      test('should store category preferred times', () {
        ...
        expect(preferences.categoryPreferredTimes['cafe'], equals(TimeSlot.morning));
    ...
    group('lookback window', () {
      test('should filter patterns by lookback period', () {
        ...
        final patterns = [
          ...
          _createVisitPattern(timestamp: now.subtract(const Duration(days: 15))),
          _createVisitPattern(timestamp: now.subtract(const Duration(days: 35))),
        ];
        ...
        final recentPatterns = patterns
            .where((p) => p.atomicTimestamp.isAfter(cutoff))
            .toList();
