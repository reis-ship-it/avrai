...
| 1.4.16 | **Discoverability non-gate feedback channel.** Add explicit user controls for "show everything in this area/category" and log outcomes to ensure personalized ranking never becomes hard suppression outside safety/legal constraints | COMPLETE (2026-02-25: added discoverability feedback lane in `ContinuousLearningSystem` with explicit `discoverability_show_everything_area/category` overrides, per-agent override state tracking, and `discoverability_guardrail_violation` episodic telemetry when personalization suppresses overridden discoverability outside allowed legal/safety/privacy/consent reasons; covered by `continuous_learning_system_discoverability_feedback_test`) |
