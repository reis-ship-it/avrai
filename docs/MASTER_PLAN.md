...
| 1.4.12 | **"Bad day" detection and dampening.** If a user gives 3+ negative signals in a single session (e.g., rapid-fire dismissals, multiple 1-star ratings), detect potential mood-driven feedback. Options: (a) downweight session signals by 0.5x (possible bad mood, not genuine dislike), OR (b) treat as genuine (the model really misfired). Resolution: track whether the user's NEXT session returns to normal patterns. If yes: apply 0.5x retroactive dampening to the negative session. If the user continues the negative pattern across 2+ sessions: treat as genuine taste shift and keep full weight | COMPLETE (2026-02-24: `ContinuousLearningSystem` now tracks per-session negative-signal counts, emits `bad_day_session_detected` after 3+ negatives, and validates behavior in the next session; normal next-session behavior emits `bad_day_retroactive_dampening` with factor `0.5`, while continued negativity emits `bad_day_dampening_withheld` to preserve full weight as genuine shift; covered by `continuous_learning_system_bad_day_dampening_test` plus regression with `continuous_learning_system_negative_confidence_decay_test`) |
