...
| 1.4.15 | **Source utility feedback loop.** Track whether recommendations influenced by a research source family (internal telemetry, external paper, third-party dataset) actually improve outcomes. De-rank source families that repeatedly fail downstream validation | COMPLETE (2026-02-25: added source-family utility tracking in `ContinuousLearningSystem` that scores downstream positive/negative outcomes per source family, computes reliability + weight multiplier, emits `source_utility_feedback` tuples for each scored outcome, and emits `source_family_deranked` tuples when repeated failures exceed threshold; covered by `continuous_learning_system_source_utility_feedback_test`) |
