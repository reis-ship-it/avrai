...
| 1.1E.17 | Add `KernelLifecyclePolicy` contract: upgrade/downgrade protocol, compatibility matrix, mandatory rollback TTL by kernel family, and signed change windows | Extends 1.1E.12, 10.9.13 -- COMPLETE (2026-02-20: added deterministic `KernelLifecyclePolicy` contract with upgrade/downgrade direction, signed change-window enforcement, compatibility-matrix checks against runtime kernel versions, mandatory rollback TTL, and JSON contract coverage in `kernel_lifecycle_policy_test`) |
