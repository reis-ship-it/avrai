...
| 1.1E.7 | Add privacy boundaries: journals remain local plaintext only if protected by on-device encryption at rest; federated sync shares DP-safe summaries only (no raw journal text) | Extends Phase 2, 8.1 -- COMPLETE (2026-02-20: added `JournalPrivacyBoundary` with explicit local-storage encryption-at-rest gate and DP-safe federated summary transformation that strips raw journal text fields/metadata, enforcing no-raw-text export via policy validation; covered by `journal_privacy_boundary_test`) |
