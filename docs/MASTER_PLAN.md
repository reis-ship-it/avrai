...
| 1.1D.4 | Implement structured retrieval lane for hard constraints: date/time, geo bounds, open-now, platform availability, age/safety constraints | New -- COMPLETE (2026-02-20: added `StructuredRetrievalLane` with deterministic constraint filtering for time window, geo radius, open-now, platform, age floor, and safety gating plus unified ranking traces; covered by `structured_retrieval_lane_test`) |
...
| 1.1D.6 | Add retrieval observability: log per-lane candidate sets, score contributions, selected top-K, latency budget, and final user action outcome | Extends `AIImprovementTrackingService` -- COMPLETE (2026-02-20: extended `AIImprovementTrackingService` with retrieval observability persistence/query APIs and added `RetrievalObservabilityLogger` adapter to log lane candidate sets, per-item score contributions, selected top-k, latency budget/actual latency, and final user outcomes; covered by `ai_improvement_tracking_service_test` + `retrieval_observability_logger_test`) |
...
| 1.1D.8 | Build retrieval evaluation set for local events/places/platforms (golden queries + expected relevance tiers). Include edge cases: sparse areas, ambiguous places, multilingual names | New -- COMPLETE (2026-02-20: added `local_retrieval_evaluation_set_v1.json` golden query corpus with relevance tiers and required sparse/ambiguous/multilingual edge cases, plus `RetrievalEvaluationSet` loader and coverage tests in `retrieval_evaluation_set_test`) |
...
| 1.1D.10 | Add `SearchLearningAdapter`: learns better ranking/query-rewrite parameters from outcomes without changing core production logic directly | New -- COMPLETE (2026-02-20: added proposal-only `SearchLearningAdapter` that derives bounded ranking/query-rewrite parameter deltas from `search_retrieval` episodic tuples and downstream outcomes, without directly mutating production retrieval logic; covered by `search_learning_adapter_test`) |
...
| 1.1E.2 | Define `HistoryJournal` schema (chronological decision/event trace: hypothesis, experiment, rollout, rollback, outcome) | New -- COMPLETE (2026-02-20: added deterministic `HistoryJournalEntry`/`HistoryJournalSnapshot` schema with bounded `HistoryEntryType` lifecycle (`hypothesis`, `experiment`, `rollout`, `rollback`, `outcome`), validation helpers, and JSON round-trip coverage in `history_journal_schema_test`) |
...
| 1.1E.4 | Implement memory-window consolidation for journals: summarize older entries, keep critical facts and failure signatures verbatim | Extends 1.1C -- COMPLETE (2026-02-20: added `JournalWindowConsolidationService` in `memory/consolidation/journal_window_consolidation_service.dart` to deterministically summarize stale non-critical entries while preserving critical facts and failure-signature history verbatim; covered by `journal_window_consolidation_service_test`) |
| 1.1E.5 | Wire autonomous research lane (Phase 7.9) to write every experiment contract and outcome summary into `HistoryJournal` | Feeds 7.9 -- COMPLETE (2026-02-20: added `AutonomousResearchHistoryJournalAdapter` and `HistoryJournalSink` boundary to persist deterministic `experiment` + `outcome` entries (contract metadata + adoption verdict) for every autonomous research experiment; covered by `autonomous_research_history_journal_adapter_test`) |
| 1.1E.6 | Wire model lifecycle (Phase 7.7) to log promotion rationale and rollback triggers in deterministic form | Feeds 7.7 -- COMPLETE (2026-02-20: added `ModelLifecycleHistoryJournalAdapter` to emit deterministic lifecycle `HistoryJournal` entries for promotion rationale and rollback trigger events with structured metadata; covered by `model_lifecycle_history_journal_adapter_test`) |
| 1.1E.7 | Add privacy boundaries: journals remain local plaintext only if protected by on-device encryption at rest; federated sync shares DP-safe summaries only (no raw journal text) | Extends Phase 2, 8.1 -- COMPLETE (2026-02-20: added `JournalPrivacyBoundary` contract to block local plaintext persistence without encryption-at-rest policy and enforce DP-safe federated summary payloads (no raw journal text fields); covered by `journal_privacy_boundary_test`) |
| 1.1E.8 | Add failure-signature index: known bad patterns from prior rollbacks are tagged and queryable to prevent repeat regressions | Feeds 7.7, 7.9 -- COMPLETE (2026-02-20: added `FailureSignatureIndex` with deterministic fingerprint upsert, occurrence tracking, recency ordering, and issue-class query APIs so rollback failure patterns are explicitly tagged and queryable before promotion decisions; covered by `failure_signature_index_test`) |
| 1.1E.9 | Add `EvidenceBundle` deterministic envelope for external and internal research claims: source URI, method class, recency, dataset fingerprint, experiment contract ID, and adoption verdict | Extends 7.9, 9.2.6 -- COMPLETE (2026-02-20: added deterministic `EvidenceBundle` envelope with source URI, method class, observed-at recency, dataset fingerprint, experiment contract linkage, and adoption verdict semantics; covered by `evidence_bundle_test`) |
| 1.1E.10 | Add `ConvictionLedger` append-only journal: every confidence increase/decrease must record supporting evidence IDs, contradiction IDs, and delayed-outcome validation window IDs | Feeds 1.4, 4.1, 5.1, 5.2, 6.1 -- COMPLETE (2026-02-20: added append-only `ConvictionLedger` and `ConvictionLedgerEntry` schema with strict confidence-update evidence/contradiction/delayed-validation requirements plus deterministic JSON round-trip and query coverage in `conviction_ledger_test`) |
| 1.1E.11 | Add third-party research ingest receipts: consent scope, legal basis, DP tier, retention window, and allowed-use class before any claim can influence training/planning | Extends 2.1, 2.2, 9.2.6 -- COMPLETE (2026-02-20: added `ThirdPartyIngestReceipt` contract + policy gate requiring consent scope, legal basis, DP tier, retention window, and allowed-use class before downstream training/planning influence; covered by `third_party_ingest_receipt_test`) |
| 1.1E.12 | Add immutable `KernelRegistry` contract for core behavior kernels (`Purpose`, `Safety`, `Truth`, `Recovery`, `Learning`, `Exploration`, `Federation`, `Resource`, `HumanOverride`). Runtime reads only signed kernel manifests | Extends Hardcoded Invariants, 7.7.1 -- COMPLETE (2026-02-20: added signed-manifest `KernelRegistryContract` + `KernelManifest` validation/snapshot APIs that reject unsigned or invalid-window manifests and expose runtime reads for only currently valid kernel manifests; covered by `kernel_registry_contract_test`) |
| 1.1E.13 | Add `PurposeKernel` schema with explicit initial goals: understand human condition, test/prove convictions, learn from mistakes and fixes, improve user and agent happiness, discover new goals under safety/legal bounds | Feeds 6.1, 7.9, 8.9 -- COMPLETE (2026-02-20: added deterministic `PurposeKernelSchema` with explicit required initial-goal set and fail-closed goal parsing/validation (including duplicate and unknown-goal rejection); covered by `purpose_kernel_schema_test`) |
| 1.1E.14 | Add `FirstOccurrenceIssueLedger`: first-seen issue signature must create deterministic triage entry with severity, impact radius, and next action (`fix`, `experiment`, `escalate`) | Extends 1.1E.8, 10.9.12 -- COMPLETE (2026-02-20: added deterministic `FirstOccurrenceIssueLedger` with first-seen signature capture, severity/impact-based triage action policy, immutable first-occurrence preservation, and JSON-serializable entry contract; covered by `first_occurrence_issue_ledger_test`) |
| 1.1E.15 | Add `DwellBudgetPolicy` contract per issue class (max active attempts, max wall-clock dwell time, forced escalation path) to prevent infinite rumination loops | Extends 7.9, 10.9.12 -- COMPLETE (2026-02-20: added deterministic `DwellBudgetPolicy` + registry evaluation contract with per-issue-class attempt/time ceilings and forced escalation path decisions; covered by `dwell_budget_policy_test`) |
| 1.1E.16 | Add model-family namespace for deterministic memory and telemetry (`reality_model`, `universe_model`, `world_model`) so federated/meta-learning decisions remain traceable by family and locality | Extends 8.1, 10.9.5 -- COMPLETE (2026-02-20: added `ModelFamilyNamespaceTag` + registry contract to enforce family/locality-scoped deterministic memory and telemetry keys for `realityModel`, `universeModel`, and `worldModel`; covered by `model_family_namespace_test`) |
| 1.1E.17 | Add `KernelLifecyclePolicy` contract: upgrade/downgrade protocol, compatibility matrix, mandatory rollback TTL by kernel family, and signed change windows | Extends 1.1E.12, 10.9.13 -- COMPLETE (2026-02-20: added deterministic `KernelLifecyclePolicy` with upgrade/downgrade direction, compatibility-matrix checks, mandatory rollback TTL, and signed change-window enforcement; covered by `kernel_lifecycle_policy_test`) |
| 1.1E.18 | Add `KernelEmergencyFreeze` contract: deterministic freeze trigger classes, freeze scope (`single_kernel`, `family`, `global`), and required human-release path | Extends 1.1E.17, 10.9.13 -- COMPLETE (2026-02-20: added deterministic `KernelEmergencyFreeze` contract with explicit trigger classes, freeze scope validation (`singleKernel`, `family`, `global`), and mandatory human release-path requirements; covered by `kernel_emergency_freeze_test`) |
| 1.1E.19 | Add `DreamLedger` schema for speculative episodes: dream id, model family, assumptions, simulator version, hypothesis refs, predicted deltas, and falsification plan id | Extends 7.9, 5.2 -- COMPLETE (2026-02-20: added deterministic `DreamLedgerEntry`/`DreamLedger` schema with dream id, model family, assumptions, simulator version, hypothesis refs, predicted deltas, and falsification plan linkage; covered by `dream_ledger_test`) |
| 1.1E.20 | Add `BeliefTierContract` with immutable precedence (`dream < hypothesis < candidate_conviction < proven_conviction`) and confidence ceilings per tier; reject any write that violates monotonic promotion flow | Extends 1.1E.10, 10.9.21 -- COMPLETE (2026-02-20: added deterministic `BeliefTierContract` with immutable tier precedence, per-tier confidence ceilings, and write validation that rejects non-monotonic promotions or ceiling violations; covered by `belief_tier_contract_test`) |
...
| 1.1E.22 | Add `DreamFailureArchive` with anti-repeat suppression tags so invalidated dream paths are queryable and blocked from re-promotion without new contradiction-clearing evidence | Extends 1.1E.8, 7.7.11 -- COMPLETE (2026-02-20: added deterministic `DreamFailureArchive` and `DreamFailureRecord` contract with anti-repeat suppression tags, query APIs, and re-promotion blocking until contradiction-clearing evidence is recorded; covered by `dream_failure_archive_test`) |
...
| 1.2.4 | Wire `ContinuousLearningSystem.processUserInteraction` to write episodic tuples (not just dimension updates) | Existing service -- COMPLETE (2026-02-23: confirmed tuple write path in `ContinuousLearningSystem._recordEpisodicTuple(...)` is invoked from `processUserInteraction(...)` for interaction events, including zero-dimension-update events; added regression coverage in `continuous_learning_system_test` asserting tuple persistence and `has_dimension_updates=false` for unmapped observations) |
| 1.2.5 | Add `community_join` as an interaction type in `ContinuousLearningSystem` (currently missing) | Gap fix -- COMPLETE (2026-02-23: `ContinuousLearningSystem.processUserInteraction(...)` explicitly maps `community_join` into social-dynamics/community-evolution updates and episodic tuple writes; added focused coverage in `continuous_learning_system_phase1_community_join_test`) |
| 1.2.6 | Wire `EventAttendanceController` to record `(state, attend_event, next_state, checkin_confirmed)` tuples | Existing controller -- COMPLETE (2026-02-23: `EventAttendanceController.registerForEvent` now writes episodic tuples through `_recordAttendanceOutcomeTuple` with `actionType=attend_event` and taxonomy outcome `checkin_confirmed`; DI wiring added for `EpisodicMemoryStore`, covered by `event_attendance_controller_phase1_outcome_test`) |
...
| 1.2.27 | **Wearable/physiology outcome channel (consent-gated).** Capture optional physiological context tuples `(state, context_signal, physiology_features, downstream_outcome)` for temporal conditioning and drift interpretation. Zero-fill when consent absent | Extends 2.1.3, 5.1.12 -- COMPLETE (2026-02-23: added consent-gated physiology channel in `ContinuousLearningSystem` via `_recordPhysiologyOutcomeTupleIfApplicable`, writing `context_signal` tuples with `physiology_features` and `downstream_outcome`; consent false path zero-fills physiological features, consent true path preserves provided values; covered by `continuous_learning_system_phase1_physiology_test`) |
| 1.2.28 | **Volunteer action/outcome pipeline.** Add first-class volunteer actions across community/event/business lanes: `volunteer_signup`, `volunteer_attend`, `volunteer_retention`, `volunteer_dropoff` with positive-social weighting and delayed outcomes (30/90-day community impact) | Extends 6.1, 8.9, 9.2.6 -- COMPLETE (2026-02-24: `ContinuousLearningSystem` now treats `volunteer_signup/attend/retention/dropoff` as first-class interactions with lane tagging and positive-social weighting, while `OutcomeTaxonomy` maps volunteer retention/dropoff delayed windows for 30/90-day impact tracking; covered by `continuous_learning_system_phase1_volunteer_pipeline_test`) |
| 1.2.29 | **Nearby invite/install conversion telemetry.** Capture non-user discovery/install funnel tuples for BLE/Wi-Fi invite pathways: `nearby_discovered_non_member`, `invite_sent`, `install_started`, `install_completed`, `first_action_after_install` (privacy-safe, consent-bounded) | Extends 8.4, 10.1 -- COMPLETE (2026-02-24: added consent-bounded `nearby_invite_install_funnel` episodic tuple channel in `ContinuousLearningSystem` via `_recordNearbyInviteInstallTupleIfApplicable`, with privacy-minimal payload mode and sensitive-key stripping; expanded dimension updates + `OutcomeTaxonomy` event mapping for all five funnel stages; covered by `continuous_learning_system_phase1_nearby_invite_funnel_test`) |
...
| 1.4.2 | Define explicit feedback triggers: post-event (next app open after check-in), post-reservation (24hr after), post-community-join (7 days after) | COMPLETE (2026-02-24: added `ExplicitFeedbackTriggerPolicy` with typed trigger decisions for post-event-next-open, post-reservation+24h, and post-community-join+7d windows, including idempotent `already_prompted` gating; covered by `explicit_feedback_trigger_policy_test`) |
| 1.4.3 | Design minimal-friction feedback UI: 1-tap thumbs up/down as default, optional 1-5 star and free-text for users who want to elaborate | COMPLETE (2026-02-24: added reusable `MinimalFeedbackPrompt` widget with one-tap thumbs up/down default actions plus optional expanded slider (1-5) and free-text notes submission path; covered by `minimal_feedback_prompt_test`) |
| 1.4.4 | Implement feedback rate limiting: max 1 explicit feedback request per session, never back-to-back, respect `OutreachPreferencesService` settings | COMPLETE (2026-02-24: added `ExplicitFeedbackRateLimiter` with session cap (`1/session`), back-to-back cooldown enforcement by outreach frequency tier, and `OutreachPreferencesService`-compatible preference gating; covered by `explicit_feedback_rate_limiter_test`) |
| 1.4.5 | Implement negative signal detection: user sees recommendation → closes app → doesn't return for N days = implicit negative outcome | COMPLETE (2026-02-24: added `NegativeOutcomeSignalDetector` to derive `recommendation_post_view_abandonment` implicit negatives from recommendation-view + app-close + return-gap sequences, wired event into `OutcomeTaxonomy` and `ContinuousLearningSystem` dimension penalties, and covered by `negative_outcome_signal_detector_test` plus taxonomy regression tests) |
| 1.4.6 | **One-tap rejection on recommendation cards.** Every recommendation card (spot, event, list, community) gets a small "X" or swipe-left dismiss action. One tap, zero chat, zero text. Record as `(user_state, dismiss_entity, entity_features, explicit_rejection)` episodic tuple. This is orders of magnitude more informative than browse-and-leave (Phase 1.2.19): browse-and-leave is ambiguous ("didn't notice" vs "not interested"), but a tap on X is an explicit "not this." Weight: 5x stronger negative signal than browse-and-leave, 2x stronger than recommendation-shown-but-ignored | COMPLETE (2026-02-24: added `RecommendationDismissalRecorder` that writes `dismiss_entity` tuples with `explicit_rejection` outcomes and `signal_weight=5.0`; added one-tap dismiss affordance (`spot_card_dismiss_button`) to `SpotCard`; covered by `recommendation_dismissal_recorder_test` and updated `spot_card_test`) |
| 1.4.7 | **"Not for me" category dismiss.** After 3 dismissals of the same entity category (e.g., 3 nightclubs dismissed), offer a one-tap "Show fewer [category]?" option. If accepted, record as `(user_state, suppress_category, category_features, explicit_preference)`. This creates an extremely strong negative signal without any conversation. The energy function should learn to assign HIGH energy to that category for this user | COMPLETE (2026-02-24: extended `RecommendationDismissalRecorder` with category-dismiss counters and threshold-gated `suppress_category` tuple writes (`explicit_preference`, signal weight 10x) when user accepts the "show fewer" prompt; added taxonomy mapping and coverage in `recommendation_dismissal_recorder_test` + `outcome_taxonomy_search_test`) |
| 1.4.8 | **Implicit positive signals (no interaction required).** Define implicit positive signals that require zero user action: (a) Return visit to a spot within 30 days (Phase 1.2.18), (b) Opening the app within 1 hour of receiving a recommendation notification (interested enough to check), (c) Visiting a spot/event that was recommended even without tapping the recommendation (discovered organically but model was right), (d) Spending >60s on entity detail page (genuine interest). These are the chat-free equivalents of "the user told their agent they liked it" | COMPLETE (2026-02-24: added `ImplicitPositiveSignalDetector` for return-visit, notification-open latency, organic recommended-entity visit without tap, and long detail dwell signals; extended `OutcomeTaxonomy` and `ContinuousLearningSystem` with `recommendation_notification_opened`, `recommended_entity_visited_organically`, and `entity_detail_long_dwell`; covered by `implicit_positive_signal_detector_test` and taxonomy regression tests) |
| 1.4.9 | **Signal strength hierarchy.** Define explicit ordering of signal informativeness for training the energy function. Strongest to weakest: (1) Explicit rating (1-5 stars) -- 10x, (2) Return visit / repeat action -- 8x, (3) One-tap rejection / dismiss -- 5x, (4) Category suppress -- 10x negative, (5) Reservation/attendance -- 4x, (6) Save/bookmark -- 3x, (7) Notification opened -- 2x, (8) Detail page >60s dwell -- 1.5x, (9) Browse-and-leave -- 1x baseline, (10) Scroll-past-without-tap -- 0.5x (weakest, most ambiguous). These weights are initial values; the model should learn to adjust them from outcome data over time | COMPLETE (2026-02-24: added `FeedbackSignalStrengthHierarchy` as a single-source 10-signal weighting/ranking contract aligned to 1.4.9, wired implicit/explicit retrieval points to hierarchy-derived weights, and covered by `feedback_signal_strength_hierarchy_test` plus updated implicit and dismissal recorder tests) |
| 1.4.10 | **Negative outcome amplification (asymmetric loss).** Bad experiences are more damaging than good experiences are rewarding (loss aversion). The outcome pipeline must implement asymmetric weighting: negative outcomes get 2x the training weight of positive outcomes of the same signal strength. Example: if a user rates a spot 1 star (10x signal), that becomes 20x effective weight vs. a 5-star rating (10x). This teaches the energy function to AVOID bad recommendations more aggressively than it pursues good ones. The asymmetry factor (2x) is a starting value; learn the optimal ratio from outcome data over time. Wire into energy function training (Phase 4.1.7) | COMPLETE (2026-02-24: `OutcomeTaxonomy` now applies phase 1.4.10 asymmetric loss metadata (`base_signal_weight`, `effective_training_weight`, `negative_outcome_amplified`, 2x loss factor) across classified outcomes, including default signal-weight fallbacks for core feedback events; `ContinuousLearningSystem` now doubles negative dimension deltas for designated failure events during training updates; covered by `outcome_taxonomy_asymmetric_loss_test` and taxonomy regression suites) |
| 1.4.11 | **Negative outcome → confidence decay.** When a recommendation results in a negative outcome: (a) reduce confidence in the relevant entity category by 20% (multiplicative: `confidence *= 0.8`), (b) if 3 consecutive negative outcomes in the same category, trigger re-exploration for that category (Phase 6.2.9-6.2.10), (c) log a "model failure" event to episodic memory with context: `(user_state_at_recommendation, entity, predicted_outcome, actual_negative_outcome)`. These model failure tuples are the MOST valuable training data -- they represent exactly where the model was wrong. Weight them 3x in the next training cycle | COMPLETE (2026-02-24: `ContinuousLearningSystem` now applies per-category confidence decay (`*0.8`) on negative outcomes, tracks consecutive negative streaks and emits `trigger_reexploration` at 3-in-a-row, and writes `model_failure_event` episodic tuples including predicted-vs-actual context with `training_weight=3.0`; covered by `continuous_learning_system_negative_confidence_decay_test` plus regression checks for contrastive and bias tuple paths) |
...
| 1.4.14 | **Delayed conviction validation windows.** For strong confidence updates, require 7/30/90-day delayed-outcome checkpoints before finalizing conviction increase. Early positive signals can provisionally increase confidence, but missing delayed validation decays it | COMPLETE (2026-02-25: extended `ContinuousLearningSystem` with delayed-validation lifecycle for strong `conviction_confidence_update` events: provisional confidence increases are marked pending with required `7d/30d/90d` windows, checkpoint pass/fail events update `ConvictionLedger`, finalization occurs only after all windows pass, and missing-window expiry triggers confidence decay; covered by `continuous_learning_system_delayed_conviction_validation_test`) |
...
| 1.4.24 | **ContinuousLearningSystem modular decomposition plan (no behavior change).** Keep `ContinuousLearningSystem` as the stable facade API while extracting internal responsibilities into focused modules. Objective: reduce monolith risk, improve testability, and lower merge-conflict surface without changing runtime behavior. **Scope lock:** all interim additions delivered before and during this window (including 1.4.1-1.4.23 and onward until split completion) must be folded into the decomposed modules + DI wiring, with facade parity preserved | New |
| 1.4.25 | **Extract interaction ingest module.** Move payload/context normalization and event-envelope parsing from `ContinuousLearningSystem.processUserInteraction(...)` into `continuous_learning/ingest/interaction_ingest_service.dart` with typed `LearningEvent`/`LearningContext` outputs | Extends 1.4.24 |
| 1.4.26 | **Extract episodic outcome writer module.** Move tuple recording logic (`_recordEpisodicTuple` + correlation tuple writers) into `continuous_learning/episodic/episodic_outcome_writer.dart`; keep idempotency and metadata parity with existing tuple schemas | Extends 1.4.24, 1.2.x |
| 1.4.27 | **Extract signal resolution + policy gate modules.** Move spot outcome resolution, volunteer lane resolution, consent checks, and telemetry gating into `continuous_learning/signals/signal_resolution_service.dart` and `continuous_learning/governance/consent_and_policy_gate.dart` | Extends 1.4.24, 1.2.27, 1.2.29 |
| 1.4.28 | **Extract training update engine module.** Move `_calculateDimensionUpdates` and asymmetric-loss amplification routing into `continuous_learning/training/dimension_update_engine.dart`; maintain parity with 1.4.10 weighting semantics | Extends 1.4.10, 4.1.7 |
| 1.4.29 | **Facade parity + migration hardening.** Keep external API signatures stable, wire new modules through DI, and require module-level contract tests + end-to-end regression tests proving no behavior drift from pre-split baseline | Extends 1.4.24-1.4.28, 10.9.4 |
