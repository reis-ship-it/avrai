...
| 1.1E.2 | Define `HistoryJournal` schema (chronological decision/event trace: hypothesis, experiment, rollout, rollback, outcome) | New -- COMPLETE (2026-02-20: added `HistoryJournalEntry`/`HistoryJournalSnapshot` schema in `history_journal_schema.dart` with typed event lifecycle (`hypothesis/experiment/rollout/rollback/outcome`), contract linkage, actor metadata, validation, and JSON round-trip coverage in `history_journal_schema_test`) |
