...
class LocationTimingQuantumStateService {
  ...
  Future<EntityLocationQuantumState> createLocationQuantumState({
    ...
    try {
      ...
      final inferredLocationType =
          locationType ?? _inferLocationType(location.city, location.address);
  ...
  Future<EntityTimingQuantumState> createTimingQuantumStateWithPreferredDays({
    ...
    final normalizedDays = preferredDays
        .map((day) => day > 7 ? day - 1 : day)
        .toList();
  ...
  Future<EntityTimingQuantumState> createTimingQuantumStateFromDateTime({
    ...
    final timeOfDayPreference =
        (hour * 60 + minute) / (24 * 60); // Normalize to [0, 1]
    ...
    final dayOfWeekPreference = weekday >= 6
        ? 1.0
        : 0.0; // Saturday/Sunday = weekend
...
class LocationCompatibilityCalculator {
  ...
  static double calculateLocationCompatibility(
    ...
    innerProduct +=
        locationA.latitudeQuantumState * locationB.latitudeQuantumState;

    // Longitude contribution
    innerProduct +=
        locationA.longitudeQuantumState * locationB.longitudeQuantumState;

    // Location type contribution (match if same type)
    final typeMatch = locationA.locationType == locationB.locationType
        ? 1.0
        : 0.5;
...
class TimingCompatibilityCalculator {
  ...
  static double calculateTimeOverlapCompatibility({
    ...
    final timeCompatibility = math.exp(
      -timeDiff * timeDiff * 10.0,
    ); // Sharp peak
