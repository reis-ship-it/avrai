...
class DimensionalityReductionService {
  ...
  Future<List<double>> reduceWithPCA({
    ...
    try {
      ...
      final variances = originalVector
          .map((v) => (v - mean) * (v - mean))
          .toList();
      ...
      final selectedIndices =
          indexedVariances.take(targetDimensions).map((e) => e.key).toList()
            ..sort();
  ...
  SparseTensor toSparseTensor(List<double> vector, {double threshold = 0.001}) {
    ...
    return SparseTensor(dimension: vector.length, entries: sparseEntries);
  }
      ...
      for (int i = 0; i < subsystemADim; i++) {
        for (int j = 0; j < subsystemADim; j++) {
          ...
          for (int k = 0; k < subsystemBDim; k++) {
            ...
            if (indexI < densityMatrix.length &&
                indexJ < densityMatrix.length) {
  ...
  Future<List<double>> quantumInspiredApproximation({
    ...
    try {
      ...
      final selectedIndices =
          indexedMagnitudes.take(targetDimensions).map((e) => e.key).toList()
            ..sort();
  ...
  List<List<double>> _vectorToMatrix(List<double> vector, int rows, int cols) {
    final matrix = List.generate(
      rows,
  ...
  double _calculateNorm(List<double> vector) {
    return math.sqrt(vector.fold<double>(0.0, (sum, val) => sum + val * val));
  }
  ...
  Future<void> _cacheReducedString(String cacheKey, KnotString reduced) async {
    ...
    try {
      ...
      final predictionData = {
        ...
        'initial_knot_crossing_number':
            reduced.initialKnot.invariants.crossingNumber,
...
class SparseTensor {
  ...
  SparseTensor({required this.dimension, required this.entries});
