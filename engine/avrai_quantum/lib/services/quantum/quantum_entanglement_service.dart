...
class QuantumEntanglementService {
  ...
  Future<EntangledQuantumState> createEntangledState({
    ...
    try {
      ...
      final finalCoefficients =
          coefficients ?? _calculateDefaultCoefficients(normalizedStates);
  ...
  List<double> _calculateDefaultCoefficients(
    List<QuantumEntityState> entityStates,
  ) {
    final coefficients = <double>[];

    for (final state in entityStates) {
      final weight = QuantumEntityTypeMetadata.getDefaultWeight(
        state.entityType,
      );
  ...
  List<double> _tensorProductVectors(
    List<double> vectorA,
    List<double> vectorB,
  ) {
    final result = <double>[];
  ...
  List<double> _createEntangledVector(
    List<double> tensorProduct,
    List<double> coefficients,
  ) {
    ...
    final avgCoeff =
        coefficients.fold<double>(0.0, (sum, c) => sum + c) /
        coefficients.length;
  ...
  List<double> _normalizeVector(List<double> vector) {
    final norm = vector.fold<double>(0.0, (sum, value) => sum + value * value);
  ...
  Future<double> calculateFidelity(
    ...
    if (state1.entangledVector.length != state2.entangledVector.length) {
      throw ArgumentError(
        'Cannot calculate fidelity between states of different dimensions',
      );
    }
    ...
    developer.log('Fidelity: ${fidelity.toStringAsFixed(4)}', name: _logName);
  ...
  Future<double> calculateKnotCompatibilityBonus(
    ...
    try {
      ...
      for (final s in entityStates) {
        entityKnots.add(
          await _quantumStateKnotService.generateEntityKnot(state: s),
        );
      }
      ...
      if (entityKnots.length == 2) {
        ...
        if (engine != null) {
          ...
          if (stringService != null && targetTime != null) {
            try {
              ...
              final futureKnotA = await stringService.predictFutureKnot(
                agentIdA,
                targetTime,
              );
              final futureKnotB = await stringService.predictFutureKnot(
                agentIdB,
                targetTime,
              );
              ...
              if (futureKnotA != null && futureKnotB != null) {
                ...
                baseCompatibility =
                    (0.6 * currentKnotBonus + 0.4 * futureKnotBonus).clamp(
                      0.0,
                      1.0,
                    );
              } else {
        ...
        if (fabricService != null) {
          try {
            ...
            final fabricStability = await fabricService.measureFabricStability(
              fabric,
            );

            // Hybrid: 70% multi-entity weave, 30% fabric stability
            baseCompatibility =
                (0.7 * baseCompatibility + 0.3 * fabricStability).clamp(
                  0.0,
                  1.0,
                );
  ...
  Future<bool> shareEntanglementInsightsViaMesh({
    ...
    required dynamic
    messageType, // MessageType enum from main app (REQUIRED - dynamic to avoid circular dependency)
  }) async {
    ...
    try {
      ...
      final insightPayload = <String, dynamic>{
        ...
        'created_at': entangledState.tAtomic.serverTime
            .toUtc()
            .toIso8601String(),
