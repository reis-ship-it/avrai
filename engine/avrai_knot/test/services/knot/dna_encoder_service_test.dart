...
void main() {
  group('DnaEncoderService', () {
    ...
    PersonalityKnot createTestKnot({
      ...
      final truncatedNow = DateTime.fromMillisecondsSinceEpoch(now.millisecondsSinceEpoch, isUtc: true);
    ...
    test('should serialize and deserialize correctly (round-trip)', () {
      ...
      expect(restored.createdAt.millisecondsSinceEpoch, equals(original.createdAt.millisecondsSinceEpoch));
      expect(restored.lastUpdated.millisecondsSinceEpoch, equals(original.lastUpdated.millisecondsSinceEpoch));
      ...
      expect(restored.physics!.energy, closeTo(original.physics!.energy, 0.001));
      expect(restored.physics!.stability, closeTo(original.physics!.stability, 0.001));
      expect(restored.physics!.length, closeTo(original.physics!.length, 0.001));
      ...
      expect(restInv.hyperbolicVolume, closeTo(origInv.hyperbolicVolume!, 0.001));
      
      // Verify Polynomials
      expect(restInv.jonesPolynomial.length, equals(origInv.jonesPolynomial.length));
      for (int i = 0; i < restInv.jonesPolynomial.length; i++) {
        expect(restInv.jonesPolynomial[i], closeTo(origInv.jonesPolynomial[i], 0.001));
      }
      
      expect(restInv.alexanderPolynomial.length, equals(origInv.alexanderPolynomial.length));
      for (int i = 0; i < restInv.alexanderPolynomial.length; i++) {
        expect(restInv.alexanderPolynomial[i], closeTo(origInv.alexanderPolynomial[i], 0.001));
      }
      
      expect(restInv.homflyPolynomial!.length, equals(origInv.homflyPolynomial!.length));
      for (int i = 0; i < restInv.homflyPolynomial!.length; i++) {
        expect(restInv.homflyPolynomial![i], closeTo(origInv.homflyPolynomial![i], 0.001));
      }
    });
    ...
    test('should serialize and deserialize from hex string', () {
      ...
      // Assert
      expect(restored.agentId, equals(original.agentId));
      expect(restored.invariants.crossingNumber, equals(original.invariants.crossingNumber));
    });

    test('should compress knot strictly under 2KB (2048 bytes) for BLE compatibility', () {
      // Create a heavily populated knot with realistic sizes
      final now = DateTime.now();
      final heavyKnot = PersonalityKnot(
        agentId: 'a_very_long_agent_id_that_might_be_used_in_production_environments_1234567890',
        createdAt: now,
        lastUpdated: now,
        braidData: List.generate(50, (i) => i * 0.5), // realistic braid size for complex knots
        ...
        invariants: KnotInvariants(
          ...
          jonesPolynomial: List.generate(15, (i) => i.toDouble()), // typical polynomial degree ~ crossing number
    ...
    test('should throw FormatException for invalid hex string', () {
      expect(() => encoder.decodeFromHex('invalid_hex!'), throwsFormatException);
      expect(() => encoder.decodeFromHex('123'), throwsFormatException); // Odd length
    });
  });
}
