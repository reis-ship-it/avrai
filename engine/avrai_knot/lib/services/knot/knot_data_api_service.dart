...
class KnotDataAPI {
  ...
  Future<KnotPatternAnalysis> getKnotPatterns({
    ...
    developer.log('Getting knot patterns: type=$type', name: _logName);
  ...
  Future<List<PatternInsight>> _analyzeCompatibilityPatterns() async {
      ...
      for (int j = i + 1; j < sampleSize && j < allKnots.length; j++) {
        ...
        final crossingDiff =
            (knotA.invariants.crossingNumber - knotB.invariants.crossingNumber)
                .abs();
        final writheDiff = (knotA.invariants.writhe - knotB.invariants.writhe)
            .abs();
    ...
    if (compatibilityScores.isNotEmpty) {
      final avgCompatibility =
          compatibilityScores.reduce((a, b) => a + b) /
  ...
  Future<List<PatternInsight>> _analyzeEvolutionPatterns() async {
    ...
    final crossingNumbers = allKnots
        .map((k) => k.invariants.crossingNumber)
        .toList();
    final avgCrossings =
  ...
  Future<KnotPersonalityCorrelations> getCorrelations() async {
    developer.log('Getting knot correlations', name: _logName);
    ...
    try {
      ...
      final crossingNumbers = allKnots
          .map((k) => k.invariants.crossingNumber.toDouble())
          .toList();
      final writhes = allKnots
          .map((k) => k.invariants.writhe.toDouble())
          .toList();
      ...
      correlationMatrix['crossing_number'] = {
        ...
        'jones_poly_length': _calculatePearsonCorrelation(
          crossingNumbers,
          jonesPolyLengths,
        ),
        'alexander_poly_length': _calculatePearsonCorrelation(
          crossingNumbers,
          alexanderPolyLengths,
        ),
      };
      ...
      strongestCorrelations.sort(
        (a, b) => b.correlation.abs().compareTo(a.correlation.abs()),
      );
      ...
      return KnotPersonalityCorrelations(
        ...
        strongestCorrelations: strongestCorrelations
            .take(10)
            .toList(), // Top 10
  ...
  Stream<AnonymizedKnotData> streamKnotData({required StreamType type}) async* {
    developer.log('Starting knot data stream: type=$type', name: _logName);
