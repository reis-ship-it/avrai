...
class ProminenceCalculator {
  ...
  static const double _recentRelevanceTau =
      604800000.0; // 1 week in milliseconds

  final AtomicClockService _atomicClock;

  ProminenceCalculator({required AtomicClockService atomicClock})
  ...
  Future<ProminenceScore> calculateProminenceScore({
    ...
    final activityLevel = await _calculateNormalizedActivityLevel(
      entity,
      cluster,
    );
  ...
  Future<double> _calculateNormalizedActivityLevel(
    ...
    final engagementScore =
        entity.metadata['engagementScore'] as double? ?? 0.0;
    ...
    final activityScore =
        (0.6 * engagementScore) + (0.4 * normalizedInteractions);
    ...
    return maxActivity > 0.0
        ? (activityScore / maxActivity).clamp(0.0, 1.0)
        : 0.0;
  }
  ...
  Future<double> _calculateNormalizedStatusScore(
    ...
    final centralityScore =
        entity.metadata['centralityScore'] as double? ?? 0.0;
  ...
  Future<double> _calculateNormalizedTemporalRelevance(
    ...
    if (peakActivityTime != null) {
      try {
        ...
        developer.log('Error parsing peakActivityTime: $e', name: _logName);
      }
    }
    ...
    if (peakTime != null) {
      final timeDistance =
          (atomicTimestamp.serverTime.millisecondsSinceEpoch -
  ...
  Future<double> _calculateNormalizedConnectionStrength(
    ...
    final avgConnectionStrength =
        entity.metadata['avgConnectionStrength'] as double? ?? 0.0;
    ...
    final connectionScore =
        (0.5 * normalizedCount) + (0.5 * avgConnectionStrength);
