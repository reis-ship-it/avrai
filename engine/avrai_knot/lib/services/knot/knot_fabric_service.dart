...
class KnotFabricService {
  ...
  Future<FabricInvariants> calculateFabricInvariants(KnotFabric fabric) async {
    ...
    final alexanderPolynomial = await _calculateAlexanderPolynomial(
      fabric.braid,
    );
  ...
  Future<double> _calculateStability(KnotFabric fabric) async {
    ...
    if (clusters.isNotEmpty) {
      ...
      final avgClusterDensity =
          clusters.map((c) => c.density).reduce((a, b) => a + b) /
          clusters.length;
    ...
    // Combine factors
    final stability =
        (densityFactor * 0.4 + complexityFactor * 0.3 + cohesionFactor * 0.3)
            .clamp(0.0, 1.0);
    ...
    for (int i = 0; i < strandCount; i++) {
      ...
      for (int j = 0; j < strandCount; j++) {
        if (i != j) {
          ...
          final crossings = _countCrossingsBetweenStrands(fabric.braid, i, j);
  ...
  Future<Map<String, ClusterBoundary>> _determineClusterBoundaries(
    ...
    for (final cluster in clusters) {
      ...
      for (final strandIdx in cluster.strandIndices) {
        ...
        for (
          int otherIdx = 0;
          otherIdx < fabric.braid.strandCount;
          otherIdx++
        ) {
...
class KnotLinkNetwork {
  ...
  KnotLinkNetwork({required this.relationships, required this.networkId});
}
