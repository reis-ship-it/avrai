...
class KnotAudioService {
  ...
  Future<void> playBirthHarmony(PersonalityKnot knot) async {
    if (_isPlaying) {
      developer.log(
        'Audio already playing, skipping birth harmony',
        name: _logName,
      );
      return;
    }
  ...
  Future<List<double>> _generateBirthHarmonyAudio(PersonalityKnot knot) async {
    ...
    final baseFrequency =
        110.0 * math.pow(2, crossingNumber.clamp(0, 12) / 12.0);
  ...
  void _generateEmergencePhase(
    ...
    for (int i = startSample; i < endSample; i++) {
      ...
      final numHarmonics = (progress * 6 * (1 + harmonicRichness)).ceil().clamp(
        1,
        8,
      );
    ...
    for (int i = startSample; i < endSample; i++) {
      ...
      for (int c = 0; c < fullChordRatios.length; c++) {
        ...
        if (harmonicRichness > 0.2) {
          ...
          sample +=
              math.sin((i * harm2 * twoPi) / _sampleRate) *
              0.1 *
              harmonicRichness;
          sample +=
              math.sin((i * harm3 * twoPi) / _sampleRate) *
              0.05 *
              harmonicRichness;
        }
      }
      ...
      final vibrato =
          1.0 + math.sin(i * vibratoFreq * twoPi / _sampleRate) * vibratoDepth;
      sample *= vibrato;
