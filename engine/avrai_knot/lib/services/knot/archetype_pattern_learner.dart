...
class ArchetypePatternLearner {
  ...
  void recordPositiveEncounter(PersonalityKnot knot) {
    ...
    developer.log('Recorded positive encounter for archetype learning.', name: _logName);
  }
  ...
  List<LearnedArchetype> deriveArchetypes() {
    ...
    final derivedArchetype = LearnedArchetype(
      ...
      avgWrithe: avgWrithe,
      baseJonesPolynomial: _positiveEncounters.first.invariants.jonesPolynomial, // Simplified for Spike
      confidenceWeight: math.min(1.0, _positiveEncounters.length * 0.1), // Capped at 1.0
    );
  ...
  double calculateLocationAffinity(
    ...
    for (final archetype in userArchetypes) {
      ...
      final crossDist = (archetype.avgCrossingNumber - locationVibeKnot.crossingNumber).abs();
      ...
      final affinity = ((crossSim + writheSim) / 2.0) * archetype.confidenceWeight;
