...
class HierarchicalLayoutService {
  ...
  Future<Map<EntityKnot, RadialPosition>> _calculateRadialPositions({
    ...
    final normalizedStrengths = _normalizeConnectionStrengths(
      connectionStrengths,
    );

    // Sort by connection strength (descending)
    final sortedEntities = cluster.where((e) => e != center).toList()
      ..sort(
        (a, b) => normalizedStrengths[b]!.compareTo(normalizedStrengths[a]!),
      );
  ...
  Future<Map<EntityKnot, RadialPosition>> _applyQuantumPhaseAdjustments({
    ...
    for (final entry in positions.entries) {
      ...
      final adjustedAngle =
          position.theta + (quantumPhase * _quantumInfluenceWeight);

      adjustedPositions[entity] = RadialPosition.fromPolar(
        position.r,
        adjustedAngle,
      );
    }
