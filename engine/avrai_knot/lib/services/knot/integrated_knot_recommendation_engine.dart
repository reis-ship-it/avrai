...
class IntegratedKnotRecommendationEngine {
  ...
  Future<CompatibilityScore> calculateIntegratedCompatibility({
    ...
    try {
      ...
      final knotA =
          profileA.personalityKnot ?? await _knotService.generateKnot(profileA);
      final knotB =
          profileB.personalityKnot ?? await _knotService.generateKnot(profileB);
      ...
      final combined =
          (_quantumWeight * quantumCompatibility) +
  ...
  double _calculateQuantumCompatibility(
    ...
    return (dimensionSimilarity * 0.7 + archetypeSimilarity * 0.3).clamp(
      0.0,
      1.0,
    );
  }
  ...
  double _calculateDimensionSimilarity(
    ...
    final allKeys = <String>{...dimensionsA.keys, ...dimensionsB.keys};
  ...
  double _calculateKnotTopologicalCompatibility(
    ...
    return (invariantSimilarity * 0.7 + complexitySimilarity * 0.3).clamp(
      0.0,
      1.0,
    );
  }
  ...
  double _compareKnotInvariants(
    ...
    final arfSimilarity =
        invariantsA.arfInvariant != null && invariantsB.arfInvariant != null
    ...
    final homflySimilarity =
        invariantsA.homflyPolynomial != null &&
  ...
  List<String> _generateKnotInsights(
    ...
    if (crossingA == crossingB) {
      insights.add(
        'Both knots have the same complexity ($crossingA crossings)',
      );
    } else {
      insights.add('Knot complexity: $crossingA vs $crossingB crossings');
    }
  ...
  double _calculateRarityBonus(PersonalityKnot knotA, PersonalityKnot knotB) {
