...
class Worldsheet4DVisualizationService {
  ...
  Worldsheet4DVisualizationService({Knot3DConverterService? knot3dConverter})
  ...
  Fabric3DData _interpolateFabric3D(
    ...
    final maxStrands =
        before.strandPositions.length > after.strandPositions.length
    ...
    for (int i = 0; i < maxStrands; i++) {
      ...
      for (int j = 0; j < maxPoints; j++) {
        final beforePoint = j < beforeStrand.length
            ? beforeStrand[j]
            : Vector3.zero();
        final afterPoint = j < afterStrand.length
            ? afterStrand[j]
            : Vector3.zero();

        // Linear interpolation: lerp(a, b, t) = a + (b - a) * t
        final interpolatedPoint =
            beforePoint + (afterPoint - beforePoint) * factor;
    ...
    final interpolatedInvariants = FabricInvariantsData(
      stability:
          (before.invariants.stability * (1 - factor) +
          after.invariants.stability * factor),
      density:
          (before.invariants.density * (1 - factor) +
          after.invariants.density * factor),
      crossingNumber:
          ((before.invariants.crossingNumber * (1 - factor) +
                  after.invariants.crossingNumber * factor)
              .round()),
    );
