...
class TemporalStringIntegrationService {
  ...
  Future<KnotString?> createStringWithTemporalState({
    ...
    try {
      ...
      if (string == null) {
        developer.log('No string created for agentId', name: _logName);
        return null;
      }
  ...
  Future<PersonalityKnot?> interpolateStringAtTemporalState({
    ...
    try {
      ...
      if (knot == null) {
        developer.log('No knot found at time $t', name: _logName);
        return null;
      }
  ...
  double calculateTemporalStringCompatibility({
    ...
    developer.log('Calculating temporal-string compatibility', name: _logName);
    ...
    try {
      ...
      final timeDiff = (t1.serverTime.difference(t2.serverTime).inHours.abs())
          .toDouble();
      final temporalCompatibility = math.exp(
        -timeDiff / 24.0,
      ); // Decay over 24 hours
