...
class KnotPrivacyService {
  ...
  AnonymizedKnot generateAnonymizedKnot(PersonalityProfile profile) {
    developer.log('Generating anonymized knot for profile', name: _logName);
  ...
  PersonalityKnot _addPrivacyNoise(PersonalityKnot knot, double noiseLevel) {
    // Add noise to polynomial coefficients
    final noisyJones = knot.invariants.jonesPolynomial.map((coeff) {
      final noise =
          (coeff * noiseLevel) *
          (0.5 - 0.5); // Random between -noiseLevel and +noiseLevel
