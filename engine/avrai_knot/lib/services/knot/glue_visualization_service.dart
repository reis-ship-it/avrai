...
class GlueVisualizationService {
  ...
  Future<GlueMetrics> calculateGlueMetrics({
    ...
    final avgGlue = cluster.length > 1 ? totalGlue / (cluster.length - 1) : 0.0;
  ...
  Future<double> _calculateIntegratedCompatibility(
    ...
    final compatibility = await _compatibilityService
        .calculateIntegratedCompatibility(entityA: center, entityB: entity);
  ...
  Future<GlueVisualization> generateGlueVisualization({
    ...
    final thickness =
        _thicknessMin + (_thicknessMax - _thicknessMin) * glueStrength;
    ...
    final opacity = math
        .pow(glueStrength, _opacityExponent)
        .toDouble()
        .clamp(0.0, 1.0);
  ...
  int _calculateConnectionColor(double strength, ConnectionType type) {
    ...
    return _hsvToArgb(hue, saturation, value);
  }

  /// Convert HSV parameters to ARGB integer
...
enum ConnectionType { strong, moderate, weak, neutral }
