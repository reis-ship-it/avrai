...
class KnotCommunityService {
  ...
  Future<List<KnotCommunity>> findKnotTribe({
    ...
    try {
      ...
      knotCommunities.sort(
        (a, b) => b.knotSimilarity.compareTo(a.knotSimilarity),
      );
  ...
  Future<List<PersonalityProfile>> createOnboardingKnotGroup({
    ...
    try {
      ...
      final newUserKnot = await _personalityKnotService.generateKnot(
        newUserProfile,
      );
  ...
  Future<PersonalityKnot?> _calculateAverageCommunityKnot(
    Community community,
  ) async {
  ...
  List<String> _generateKnotInsights(PersonalityKnot knot) {
    ...
    if (complexity < 5) {
      ...
      insights.add(
        'Your personality has moderate complexity with several key connections',
      );
    } else {
      insights.add(
        'Your personality has rich complexity with many interconnected dimensions',
      );
    }
    ...
    if (writhe > 0) {
      insights.add(
        'Your personality dimensions tend to align in a positive direction',
      );
    } else if (writhe < 0) {
      insights.add(
        'Your personality dimensions show interesting counter-balances',
      );
    } else {
    ...
    insights.add(
      'Your dimensions form $complexity key topological connections',
    );
