...
class PersonalityKnotService {
  ...
  PersonalityKnot _generateKnotDartFallback(
    ...
    final numCrossings = braidData.length > 1
        ? ((braidData.length - 1) ~/ 2)
        : 0;
    ...
    final signature = entanglement.values
        .fold<double>(0.0, (sum, v) => sum + (v - 0.5))
        .round();

    // Determinant approximation
    final determinant = numCrossings > 0 ? (numCrossings * 2 + 1) : 1;
