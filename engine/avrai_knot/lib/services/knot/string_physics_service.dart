...
class StringPhysicsService {
  ...
  StringEvolutionRate calculateEvolutionRate({
    ...
    final relaxation =
        relaxationRate * _calculateEquilibriumDistance(currentKnot);
  ...
  double _calculateKnotVelocity(PersonalityKnot knot1, PersonalityKnot knot2) {
    ...
    final delta =
        (knot2.invariants.crossingNumber - knot1.invariants.crossingNumber)
            .abs()
            .toDouble();
    return delta;
  }
  ...
  PersonalityKnot predictFutureKnot({
    ...
    final newCrossingNumber =
        (currentKnot.invariants.crossingNumber + complexityChange.round())
            .clamp(3, 100); // Minimum: trefoil (3 crossings)

    // Evolve other invariants proportionally
    final complexityFactor =
        newCrossingNumber / currentKnot.invariants.crossingNumber.clamp(1, 100);

    final newWrithe = (currentKnot.invariants.writhe * complexityFactor)
        .round();
    final newSignature = (currentKnot.invariants.signature * complexityFactor)
        .round();
    final newBridgeNumber =
        (currentKnot.invariants.bridgeNumber * complexityFactor).round().clamp(
          1,
          100,
        );
  ...
  PersonalityKnot applyExternalForce({
    ...
    final newCrossingNumber =
        (knot.invariants.crossingNumber + complexityChange.round()).clamp(
          3,
          100,
        );
    ...
    final complexityFactor =
        newCrossingNumber / knot.invariants.crossingNumber.clamp(1, 100);
