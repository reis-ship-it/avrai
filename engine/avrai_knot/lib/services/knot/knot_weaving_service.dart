...
class KnotWeavingService {
  ...
  KnotWeavingService({required PersonalityKnotService personalityKnotService})
  ...
  Future<double> calculateWeavingCompatibility({
    ...
    try {
      ...
      final compatibility =
          (_topologicalWeight * topological) + (_quantumWeight * quantum);
  ...
  List<double> _createFriendshipBraid(
    List<double> braidA,
    List<double> braidB,
  ) {
  ...
  List<double> _createMentorshipBraid(
    List<double> braidA,
    List<double> braidB,
  ) {
    final strandsA = braidA.isNotEmpty ? braidA[0].toInt() : 8;
  ...
  List<double> _createCollaborativeBraid(
    List<double> braidA,
    List<double> braidB,
  ) {
    final strandsA = braidA.isNotEmpty ? braidA[0].toInt() : 8;
  ...
  List<double> _createProfessionalBraid(
    List<double> braidA,
    List<double> braidB,
  ) {
    final strandsA = braidA.isNotEmpty ? braidA[0].toInt() : 8;
    ...
    final maxPairs =
        (crossingsA.length > crossingsB.length
            ? crossingsA.length
            : crossingsB.length) ~/
        2;
  ...
  double _calculateStability(
    ...
    try {
      ...
      developer.log('Error calculating stability: $e', name: _logName);
      return 0.5; // Neutral fallback
    }
  }
