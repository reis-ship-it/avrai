...
Point cubicSplineInterpolate(List<Point> points, double t) {
  ...
  final h = List<double>.generate(n, (i) => points[i + 1].x - points[i].x);
...
Point bezierInterpolate(List<Point> controlPoints, double t) {
  ...
  for (int i = 0; i <= n; i++) {
    ...
    final bernstein = binomial * math.pow(1 - t, n - i) * math.pow(t, i);
...
Point hermiteInterpolate(Point p0, Point p1, Vector2 m0, Vector2 m1, double t) {
  // Clamp t to [0, 1]
  t = t.clamp(0.0, 1.0);
