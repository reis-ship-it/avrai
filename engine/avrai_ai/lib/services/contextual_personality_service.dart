...
class ContextualPersonalityService {
  ...
  Future<String> classifyChange({
    ...
    try {
      ...
      // #region agent log
      developer.log(
          'Change magnitude: ${changeMagnitude.toStringAsFixed(3)}, threshold: $_significantChangeThreshold',
          name: _logName);
      // #endregion
      ...
      if (changeMagnitude < _significantChangeThreshold) {
        // #region agent log
        developer.log(
            'Small change detected, routing to ${activeContext != null ? "context" : "core"}',
            name: _logName);
        // #endregion
      ...
      if (changeSource == 'ai2ai' && changeMagnitude > _velocityThreshold) {
        // #region agent log
        developer.log(
            'Large AI2AI change detected (magnitude: ${changeMagnitude.toStringAsFixed(3)} > $_velocityThreshold), resisting homogenization',
            name: _logName);
        // #endregion
        return 'resist';
      }
      ...
      if (changeSource == 'user_action') {
        ...
        if (activeContext != null && activeContext != 'general') {
          // #region agent log
          developer.log(
              'User action in context "$activeContext", routing to context',
              name: _logName);
          // #endregion
          return 'context';
        }
        ...
        // #region agent log
        developer.log(
            'User action in general context, routing to core (authentic behavior)',
            name: _logName);
        // #endregion
        return 'core';
      }
      ...
      if (currentProfile.isInTransition) {
        ...
        if (_isChangeConsistentWithTransition(proposedChanges, transition)) {
          // #region agent log
          developer.log(
              'Change consistent with active transition, routing to core (authentic transformation)',
              name: _logName);
          // #endregion
          ...
          // #region agent log
          developer.log(
              'Change inconsistent with active transition, routing to context',
              name: _logName);
          // #endregion
        }
      }
      ...
      // #region agent log
      developer.log('Default routing: $result (context: $activeContext)',
          name: _logName);
      // #endregion
      return result;
    } catch (e) {
  ...
  Future<TransitionMetrics?> detectTransition({
    ...
    try {
      ...
      if (recentChanges.length < 5) {
        // #region agent log
        developer.log(
            'Not enough data for transition detection: ${recentChanges.length} < 5',
            name: _logName);
        // #endregion
      ...
      if (authenticityScore >= _authenticTransitionThreshold &&
        ...
        return TransitionMetrics(
          transitionId:
              '${profile.agentId}_trans_${DateTime.now().millisecondsSinceEpoch}',
      ...
      // #region agent log
      developer.log(
          'No transition detected: authenticity=${authenticityScore.toStringAsFixed(3)} < $_authenticTransitionThreshold or consistency=${consistency.toStringAsFixed(3)} < $_consistencyThreshold',
          name: _logName);
      // #endregion
  ...
  Future<PersonalityProfile> completeTransition({
    ...
    final newPhase = LifePhase(
      phaseId:
          '${profile.agentId}_phase_${profile.evolutionTimeline.length + 1}',
      name: newPhaseName,
    ...
    final currentIndex =
        updatedTimeline.indexWhere((p) => p.phaseId == currentPhase.phaseId);
    if (currentIndex >= 0) {
    ...
    return PersonalityProfile(
      ...
      learningHistory: {
        ...
        'phase_transitions':
            (profile.learningHistory['phase_transitions'] as int? ?? 0) + 1,
      },
  ...
  List<String> _identifyTriggers(Map<String, double> changes) {
    ...
    changes.forEach((dimension, change) {
      if (change.abs() > _significantChangeThreshold) {
        triggers.add(
            '$dimension: ${change > 0 ? '+' : ''}${change.toStringAsFixed(2)}');
      }
    });
